function [x,y,typ]=INTGAINBLK(job,arg1,arg2)
// Copyright INRIA
  ...
  //** ----- Identification End -----------------------------
    ...
    while %t do
      [ok,gain,over,exprs]=getvalue('Set gain block parameters',..
			       ['Gain';..
				'Do on Overflow (0=Nothing 1=Saturate 2=Error)'],list('mat',[-1,-1],'vec',1),exprs)
      if ~ok then break,end
      if gain==[] then
	message('Gain must have at least one element');ok=%f;
      else
	if (over==0) then
    ...
    exprs=[strcat(sci2exp(gain));sci2exp(0)]
    gr_i=''
