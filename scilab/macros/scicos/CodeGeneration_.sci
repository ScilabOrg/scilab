...
function CodeGeneration_()
  ...
  x_message (["This is the code generator of B4_scicos";
              "That''s the exprimental Rev. 29_06_07_12_04";
              "";
...
//
//Copyright INRIA
function SCode=gen_loader(rdnom)
...
//              lccwin32 compiler
//
// Input : name     : name of the library
...
//
//Copyright INRIA
function T=gen_make_lccwin32(name,files,filestan,libs)
  ...
  T=["# generated by builder.sce: Please do not edit this file"
     ...
     "CC_LDFLAGS = "
     "CFLAGS = $(CC_OPTIONS) -I"""+WSCI+"\routines"""
...
//
//Copyright INRIA
function [Makename,txt]=gen_make(name,files,filestan,libs,Makename)

  //** generate Makefile for LCC compilator
  if with_lcc()==%T then
    txt=gen_make_lccwin32(name,files,filestan,libs)
    Makename = strsubst(Makename,'/','\')+'.lcc'

  //** generate Makefile for Crosoft compilator
  elseif getenv('WIN32','NO')=='OK' then
    txt=gen_make_win32(name,files,filestan,libs)
    select COMPILER;
      case 'VC++' then
        Makename = strsubst(Makename,'/','\')+'.mak'
    end
  ...
  //** unix case
    ...
    txt=gen_make_unix(name,files,filestan,libs)
...
//
//Copyright INRIA
function T=gen_make_unix(name,files,filestan,libs)
...
//
//Copyright INRIA
function T=gen_make_win32(name,files,filestan,libs)
...
//                   compile and link it with Scilab
//
// input : rdnom    : a prefix
//         archname : a vector of string of object files
//                    to include in the building process
...
//
//Copyright INRIA
function [ok]=buildnewblock(rdnom,archname,rpat)
  ...
  //** def make file name
  Makename=rpat+'/'+rdnom+'_Makefile';

  //** generate txt of makefile and get wright name
  //   of the Makefile file
  [Makename2,txt]=gen_make(rdnom,files,filestan,archname,Makename);
  ...
  [txt]=gen_loader(rdnom)

  //** write text of the loader in file
  ierr=execstr('mputl(txt,rpat+''/''+rdnom+''_loader.sce'')','errcatch')
  if ierr<>0 then
    x_message(['Can''t write '+rdnom+'_loader.sce';lasterror()])
    ok=%f
    return
  end
...
//
 ...
 //end
  ...
  if ok then
    ok=buildnewblock(rdnom,archname,rpat)
  end
