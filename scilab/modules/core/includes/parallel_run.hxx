...
namespace
{
    ...
    template< typename ParallelWrapper> struct scheduler
    {
            ...
            : w(wrapper), nb_process(nb_proc), dynamic(dyn), chunk_size(chunk_s), backup_res(wrapper.lhs)
        {
            ...
            *todo= 0;
            out_of_work= allocSharedMem<sem_t>();
            sem_init(out_of_work, 1, 0); 
            todo_protect= allocSharedMem<sem_t>();
    ...
    private:
        ...
        workshare_t getWork() {
            ...
            res.first= *todo;
            if (*todo != w.n) {
                res.second= std::min(w.n, *todo + chunk_size);// no need to handle static scheduling because the last initialWork then set todo=w.n
                *todo= res.second;
            }
            else {
                res.second= w.n;
