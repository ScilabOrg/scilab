...
function bOK = doDisplayWhosLine(_rhs, _opt, _name, _namedtype, _sel)
  ...
  if _rhs == 2 then
    if _opt == "-type" then
      if part(_namedtype, 1:length(_sel)) <> _sel then
        bOK = %f;
      end
    elseif _opt == "-name" then
      if part(convstr(_name,"l"), 1:length(_sel)) <> convstr(_sel,"l") then
        bOK = %f;
      end
    end
  else
...
//=============================================================================
  ...
  if %_rhs == 1 then
    tmp = gettext("%s: Wrong number of input arguments: %d or %d expected.\n")
    error(msprintf(tmp, "whos", 0, 2));
  end

  if %_rhs == 2 then
    %_nams($-1:$) = [];
    if %_opt <> "-type" & %_opt <> "-name" then
      tmp = gettext("%s: Wrong value for input argument #%d: ''%s'' or ''%s'' expected.\n")
      error(msprintf(tmp, "whos", 1, "-name", "-type"));
    end
  end
