...
Runner* Runner::m_RunMe = NULL;
bool Runner::m_bInterruptibleCommand = true;

void Runner::launch()
{
    // get the runner to execute
    Runner* runMe = Runner::getRunner();
    // set if the current comment is interruptible
    Runner::setInterruptibleCommand(runMe->isInterruptible());
    ...
    {
        std::cout << "ScilabException" << std::endl;
...
void Runner::setRunner(Runner* _RunMe)
{
    m_RunMe = _RunMe;
}

Runner* Runner::getRunner(void)
{
...
bool Runner::isRunnerAvailable(void)
{
    ThreadManagement::LockRunner();
...
void Runner::setInterruptibleCommand(bool _bInterruptibleCommand)
{
    ThreadManagement::LockRunner();
...
bool Runner::isInterruptibleCommand()
{
    ThreadManagement::LockRunner();
