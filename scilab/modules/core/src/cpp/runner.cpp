...
void *Runner::launch(void *args)
{
    ...
    ConfigVariable::getThread(me->getThreadKey())->setStatus(ThreadId::Done);
    //unregister thread
    ConfigVariable::deleteThread(me->getThreadKey());
    UnlockPrompt();
...
__threadSignal resume;
__threadSignalLock resumeLock;
...
void Runner::execAndWait(ast::Exp* _theProgram, ast::ExecVisitor *_visitor)
{
    try
    {
        ...
        //register thread
        ConfigVariable::setThread(threadKey, new ThreadId(threadId));
