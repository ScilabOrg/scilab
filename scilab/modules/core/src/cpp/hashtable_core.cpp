...
{
    table_t::const_iterator i = std::find_if(table.begin(), table.end(), equal_data(*scilab_funptr));
    if (i != table.end())
    {
...
static BOOL doEnterFunction(int *key, char *name, int *scilab_funptr)
{
    if (table.size() < MAXELEMENTFUNCTIONLIST)
    {
        ...
        table.insert(partition_point_n(table.begin(), table.size(), key_less_than(tmp.key)), tmp);
...
static BOOL doDeleteFunction(int *key, int *scilab_funptr)
{
    ...
    table_t::iterator i(std::
                        find_if(partition_point_n(table.begin(), table.size(), not_less_than_key(key)), table.end(), equal_data(*scilab_funptr)));
...
struct has_namefunction:std::unary_function < entry const &, bool >
{
    bool operator() (entry const &e)const
    {
        return e.namefunction[0] != '\0';
    }                           // <=> strlen(e.namefunction) > 0
};
