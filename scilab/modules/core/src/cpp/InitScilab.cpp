...
__threadSignalLock *pLaunchScilabLock;
...
static void processCommand(ScilabEngineInfo* _pSEI)
{
    ...
    if (_pSEI->iExecAst)
    {
        ...
        callOnPrompt();
        execAstTask((ast::Exp*)_pSEI->pExpTree, _pSEI->iSerialize != 0, _pSEI->iTimed != 0, _pSEI->iAstTimed != 0, _pSEI->iExecVerbose != 0, _pSEI->isInterruptible != 0);
    }
...
void* scilabReadAndExec(void* param)
{
    ...
    while (!ConfigVariable::getForceQuit())
    {
        ...
        do
        {
            ...
            {
                std::cout << "last command " << command << std::endl;
        ...
        __Lock(pExecDoneLock);
        __Wait(&ExecDone, pExecDoneLock);
        __UnLock(pExecDoneLock);
    }
...
static int interactiveMain(ScilabEngineInfo* _pSEI)
{
    ...
    __CreateThreadWithParams(&threadIdConsole, &threadKeyConsole, &scilabReadAndExec, _pSEI);
