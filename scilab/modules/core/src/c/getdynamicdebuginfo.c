...
char **getDynamicDebugInfo(int *sizeArray, int* _piKey)
{
	...
	*sizeArray = 0;
	...
	sciErr = getNamedVarType(_piKey, "SCI", &iType);
	if ((sciErr.iErr == 0) && (iType == 10))
	{
		...
		sciErr = readNamedMatrixOfString(_piKey, "SCI", &m, &n, &SCI_length, &SCI_value);
	...
	if ((sciErr.iErr == 0) && (iType == 10))
	{
		...
		sciErr = readNamedMatrixOfString(_piKey, "SCIHOME", &m, &n, &SCIHOME_length, &SCIHOME_value);
		if ( (sciErr.iErr == 0) && ((m == 1) && (n == 1)) )
		{
