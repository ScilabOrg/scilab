...
L1:
    ...
    if (C2F(iop).ddt == 4)
    {
        sprintf(tmp, " TOP    pt:%d rstk(pt):%d icall: %d niv: %d err:%d", Pt, Rstk[Pt], C2F(recu).icall, C2F(recu).niv, Err);
...
L12:
    ...
    else
    {
        if (Lct[4] / 2 % 2 == 1)
        {
            ...
            Lct[1] = 0;
            if (C2F(recu).paus == 0 && C2F(iop).rio == C2F(iop).rte && C2F(recu).macr == 0)
            {
                if (Pt != 0)
                {
                    Msgs(30, 0);
...
L15:
    ...
    if (C2F(iop).ddt == 4)
    {
        sprintf(tmp, " parse  pt:%d rstk(pt):%d top: %d niv: %d err:%d", Pt, r, Top, C2F(recu).niv, Err);
        C2F(basout) (&io, &C2F(iop).wte, tmp, (long)strlen(tmp));
    }
...
L18:
    ...
    C2F(getsym) ();
    if (C2F(com).sym == right || C2F(com).sym == rparen || C2F(com).sym == less || C2F(com).sym == great)
    {
        SciError(1);
        goto L98;
    }
...
L73:
    ...
    if (!((C2F(com).sym != semi && Lct[3] == 0) || (C2F(com).sym == semi && Lct[3] == 1)))
    {
        goto L76;
    }
L74:
...
L77:
    ...
    {                           /* there are breakpoints set */
        ...
        {                       /* loop on table of functions containing breakpoints */
...
L98:
    ...
    if (Pt != 0)
    {
        ...
        if (C2F(errgst).err1 != 0 && Rstk[Pt] == 808)
        {                       /* catched error in a try */
            goto L80;
        }
    }
