...
L10:                           /* Current opcode finished handle error, interruptions,... */
    ...
    if (lc - l0 == nc)
    {                           /* is current opcodes block (if, for, .. structure) finished ? */
        /* yes */
        r = Rstk[Pt] - 610;
        switch (r)
        {
...
L40:
    ...
    if (C2F(errgst).err1 <= 0)
    {
        ...
        {
            C2F(getsimat) ("run", &Top, &Top, &mm1, &nn1, &c__1, &c__1, &lr, &nlr, 3L);
...
L96:
    ...
    else if (Rstk[p] == 501 || Rstk[p] == 502 || Rstk[p] == 503)
    {
...
L121:
        ...
        if (Rstk[Pt] == 802 || Rstk[Pt] == 612 || (Rstk[Pt] == 805 && Ids[1 + Pt * nsiz] == iselect) || (Rstk[Pt] == 616 && Pstk[Pt] == 10))
            Top--;
        if (Rstk[Pt] != 503)
            goto L121;
