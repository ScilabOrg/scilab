...
static PrivateResult sci_existsOrIsdef(types::typed_list &in, const char *fname)
{
    ...
    if (!in[0]->isString() || in[0]->getAsString()->getSize() != 1)
    {
        Scierror(999,_("%s: Wrong type for input argument #%d: A single string expected.\n"), fname, 1);
        return FunctionFailed;
    }

    if (in.size() == 2 && (!in[1]->isString() || in[1]->getAsString()->getSize() != 1))
    {
        Scierror(999,_("%s: Wrong type for input argument #%d: A single string expected.\n"), fname, 2);
        return FunctionFailed;
    }
    ...
    if (in.size() == 2)
    {
        psScope = in[1]->getAsString()->getString(0,0);
    }
    else
    {
    ...
    {
    case All:
        pitReturn = symbol::Context::getInstance()->get(in[0]->getAsString()->getString(0,0));
        break;
    case Local:
        pitReturn = symbol::Context::getInstance()->getCurrentLevel(in[0]->getAsString()->getString(0,0));
        break;
    case NoLocal:
        pitReturn = symbol::Context::getInstance()->getAllButCurrentLevel(in[0]->getAsString()->getString(0,0));
        break;
    default :
