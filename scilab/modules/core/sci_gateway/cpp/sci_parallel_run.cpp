...
#include "parameters.cpp" /* directly include this file for now, waiting for proper implementation from module/parameters 
                           will then only #include "parameters.hpp" */
    ...
    */
    ...
    bool getConfigParameters(int config_arg_pos, int& nb_workers, bool& shared_memory, bool& dynamic_scheduling, int& chunk_size, char const*& prologue, char const*& epilogue){
        ...
        if(has_config_arg) {
            ...
            get_string_parameter(addr, "prologue", const_cast<char**>(&prologue), &found, const_cast<char*>(prologue), log, CHECK_NONE);
            get_string_parameter(addr, "epilogue", const_cast<char**>(&epilogue), &found, const_cast<char*>(epilogue), log, CHECK_NONE);
        }
    ...
    struct simple_wrapper{
        ...
        void operator()(int i)
        {
            ...
            { /* do nothing on empty name */
                ...
                {
                    double* tmpPtr;
                    SciErr err= allocMatrixOfDouble(pvApiCtx, ++Top, 1, 1, &tmpPtr);
