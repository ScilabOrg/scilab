...
/*---------------------------------------------------------------------------------*/
    void DrawableObject::displayChildren(void)
    {
        ...
        while (curSon != NULL)
        {
            ...
            if (!sciIsAutomaticallyRedrawn(curSon->pointobj) && curType != SCI_TEXT && curType != SCI_LABEL)
            {
...
/*---------------------------------------------------------------------------------*/
    void DrawableObject::pointScale(double xCoord, double yCoord, double zCoord, double *xScaled, double *yScaled, double *zScaled)
    {
        getSubwinDrawer(sciGetParentSubwin(m_pDrawed))->pointScale(xCoord, yCoord, zCoord, xScaled, yScaled, zScaled);
    }
/*------------------------------------------------------------------------------------------*/
    void DrawableObject::inversePointScale(double xScaled, double yScaled, double zScaled, double *xCoord, double *yCoord, double *zCoord)
    {
        getSubwinDrawer(sciGetParentSubwin(m_pDrawed))->inversePointScale(xScaled, yScaled, zScaled, xCoord, yCoord, zCoord);
    }
/*------------------------------------------------------------------------------------------*/
...
/*------------------------------------------------------------------------------------------*/
                                        ...
                                        double *xScaled, double *yScaled, double *zScaled)
    {
        getSubwinDrawer(sciGetParentSubwin(m_pDrawed))->directionScale(xCoord, yCoord, zCoord, startingPointX,
                                                                       startingPointY, startingPointZ, xScaled, yScaled, zScaled);
    }
/*------------------------------------------------------------------------------------------*/
    void DrawableObject::directionScale(double vectorX[], double vectorY[], double vectorZ[],
                                        double startingPointsX[], double startingPointsY[], double startingPointsZ[], double vectorLength)
    {
        getSubwinDrawer(sciGetParentSubwin(m_pDrawed))->directionScale(vectorX, vectorY, vectorZ,
                                                                       startingPointsX, startingPointsY, startingPointsZ, vectorLength);
    }
/*------------------------------------------------------------------------------------------*/
...
/*------------------------------------------------------------------------------------------*/
    void DrawableObject::translate(void)
    {
        ...
        if (m_aMoveTranslation[0] != 0.0 || m_aMoveTranslation[1] != 0.0 || m_aMoveTranslation[2] != 0.0)
        {
