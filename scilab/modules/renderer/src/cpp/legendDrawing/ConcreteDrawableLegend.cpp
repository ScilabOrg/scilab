...
namespace sciGraphics
{
    ...
    /*---------------------------------------------------------------------------------*/
      ConcreteDrawableLegend::ConcreteDrawableLegend(sciPointObj * pLegend):DrawableLegend(pLegend)
    {
        m_pNames = NULL;
        m_aLines = NULL;
        m_aBox = NULL;
    ...
    void ConcreteDrawableLegend::setTextParameters(void)
    {
        ...
        m_pNames = allocateText(sciGetParentSubwin(m_pDrawed), textMat, nbLegends, 1,
                                0.0, 0.0, TRUE, NULL, FALSE, &foreground, &background, FALSE, FALSE, FALSE, ALIGN_LEFT);
    ...
    void ConcreteDrawableLegend::setLinesParameters(void)
    {
        ...
        for (int i = 0; i < nbLegends; i++)
        {
            ...
            // same for lines
            if (sciGetPolylineStyle(legendedObject) == 4)
            {                   //arrowed polyline
            ...
            if (sciGetIsDisplayingLines(legendedObject) || sciGetPolylineStyle(legendedObject) == 3 || sciGetPolylineStyle(legendedObject) == 6)
            {
                                            ...
                                            const double lowerRightCorner[3], const double upperRightCorner[3])
    {
        ...
        for (int i = 0; i < nblegends; i++)
        {
            ...
            if (sciGetPolylineStyle(legendedObject) == 5 || sciGetPolylineStyle(legendedObject) == 6 || sciGetIsFilled(legendedObject))
            {
