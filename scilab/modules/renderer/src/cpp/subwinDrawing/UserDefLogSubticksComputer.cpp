...
/*------------------------------------------------------------------------------------------*/
    UserDefLogSubticksComputer::UserDefLogSubticksComputer(DrawableSubwin * subwin):UserDefinedSubticksComputer(subwin)
    {
...
/*------------------------------------------------------------------------------------------*/
    void UserDefLogSubticksComputer::getSubticksPosition(const double ticksPositions[], int nbTicks, double subTickspositions[])
    {
        /*    |              |              |    */
        ...
        for (int i = 0; i < nbTicks - 1; i++)
        {
            ...
            for (int j = 0; j < m_iNbSubticks; j++)
            {
                ...
                subTickspositions[j + m_iNbSubticks * i] = prevTick + (nextTick - prevTick) * defaultIntervalPosition;
            }
        }
    }
/*------------------------------------------------------------------------------------------*/
