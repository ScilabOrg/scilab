...
/*------------------------------------------------------------------------------------------*/
    AutoLogSubticksComputer::AutoLogSubticksComputer(DrawableSubwin * subwin):ComputeSubticksStrategy(subwin)
    {
...
/*------------------------------------------------------------------------------------------*/
    void AutoLogSubticksComputer::getSubticksPosition(const double ticksPositions[], int nbTicks, double subTickspositions[])
    {
        ...
        for (int i = 0; i < nbTicks - 1; i++)
        {
            ...
            for (int j = 0; j < nbSubtics; j++)
            {
                ...
                subTickspositions[j + nbSubtics * i] = prevTick + (nextTick - prevTick) * defaultIntervalPosition;
            }
        }
    }
/*------------------------------------------------------------------------------------------*/
