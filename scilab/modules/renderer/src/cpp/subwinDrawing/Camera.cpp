...
/*--------------------------------------------------------------------------*/
    ...
    {
        getCameraImp()->setAxesReverse(axesReverse[0] == TRUE, axesReverse[1] == TRUE, axesReverse[2] == TRUE);
    }
/*--------------------------------------------------------------------------*/
...
/*--------------------------------------------------------------------------*/
    void Camera::computeZoomAreaLines(const double areaPixCorners[4][2], double areaLines[4][2][3])
    {
        for (int i = 0; i < 4; i++)
        {
            ...
            pointScale(areaLines[i][0][0], areaLines[i][0][1], areaLines[i][0][2], &areaLines[i][0][0], &areaLines[i][0][1], &areaLines[i][0][2]);
            pointScale(areaLines[i][1][0], areaLines[i][1][1], areaLines[i][1][2], &areaLines[i][1][0], &areaLines[i][1][1], &areaLines[i][1][2]);
        }
    }
/*--------------------------------------------------------------------------*/
...
/*--------------------------------------------------------------------------*/
    void Camera::updateXCoordinate(const double intersections[][3],
                                   int nbIntersections, double oldXmin, double oldXmax, double &newXmin, double &newXmax)
    {
...
/*--------------------------------------------------------------------------*/
    void Camera::updateYCoordinate(const double intersections[][3],
                                   int nbIntersections, double oldYmin, double oldYmax, double &newYmin, double &newYmax)
    {
        for (int i = 0; i < nbIntersections; i++)
        {
...
/*--------------------------------------------------------------------------*/
    void Camera::updateZCoordinate(const double intersections[][3],
                                   int nbIntersections, double oldZmin, double oldZmax, double &newZmin, double &newZmax)
    {
        for (int i = 0; i < nbIntersections; i++)
        {
...
/*--------------------------------------------------------------------------*/
    int Camera::getProjectedXIntersections(const double intersections[4][3], double projectedIntersections[8][3],
                                           double planeXCoord, double yMin, double yMax, double zMin, double zMax)
    {
        ...
        nbIntersections = getProjectedIntersections2D(intersections2D, projectedIntersections2D, yMin, yMax, zMin, zMax);
...
/*--------------------------------------------------------------------------*/
    int Camera::getProjectedYIntersections(const double intersections[4][3], double projectedIntersections[8][3],
                                           double planeYCoord, double xMin, double xMax, double zMin, double zMax)
    {
        int nbIntersections = 0;
        double intersections2D[4][2];
        double projectedIntersections2D[8][2];
        ...
        // get intersections on the plane
        nbIntersections = getProjectedIntersections2D(intersections2D, projectedIntersections2D, xMin, xMax, zMin, zMax);
...
/*--------------------------------------------------------------------------*/
    int Camera::getProjectedZIntersections(const double intersections[4][3], double projectedIntersections[8][3],
                                           double planeZCoord, double xMin, double xMax, double yMin, double yMax)
    {
        int nbIntersections = 0;
        double intersections2D[4][2];
        double projectedIntersections2D[8][2];
        ...
        // get intersections on the plane
        nbIntersections = getProjectedIntersections2D(intersections2D, projectedIntersections2D, xMin, xMax, yMin, yMax);
                                            ...
                                            double xMin, double xMax, double yMin, double yMax)
    {
        ...
        for (int i = 0; i < 4; i++)
        {
            if (computeLineFillRectangleIntersections(intersections[i],
                                                      intersections[(i + 1) % 4], xMin, xMax, yMin, yMax, &(projectedIntersections[nbIntersections])))
            {
                // intersection found
                nbIntersections += 2;
            }
...
/*--------------------------------------------------------------------------*/
    bool Camera::computeLineFillRectangleIntersections(const double p1[2], const double p2[2],
                                                       double xMin, double xMax, double yMin, double yMax, double intersections[2][2])
    {
        // compute the number of intersections with the rectangle (just boundary not inside)
        int nbIntersectionWithRectangle = computeLineRectangleIntersections(p1, p2, xMin, xMax, yMin, yMax, intersections);
...
/*--------------------------------------------------------------------------*/
    bool Camera::isInsideRectangle(const double p[2], double xMin, double xMax, double yMin, double yMax)
    {
...
/*--------------------------------------------------------------------------*/
                                                  ...
                                                  double xMin, double xMax, double yMin, double yMax, double intersections[2][2])
    {
        ...
        if (p1[0] != p2[0])
        {
            if (checkXIntersection(p1, p2, intersections[nbIntersections], xMin, yMin, yMax))
            {
                nbIntersections++;
            }

            // same for x = xMax
            if (checkXIntersection(p1, p2, intersections[nbIntersections], xMax, yMin, yMax))
            {
                nbIntersections++;
            }
        ...
        if (p1[1] != p2[1])
        {
            if (checkYIntersection(p1, p2, intersections[nbIntersections], yMin, xMin, xMax))
            {
                nbIntersections++;
            }

            // same for x = xMax
            if (checkYIntersection(p1, p2, intersections[nbIntersections], yMax, xMin, xMax))
            {
                nbIntersections++;
            }
...
/*--------------------------------------------------------------------------*/
    bool Camera::checkXIntersection(const double p1[2], const double p2[2], double res[2], double rectXCoord, double yMin, double yMax)
    {
...
/*--------------------------------------------------------------------------*/
    bool Camera::checkYIntersection(const double p1[2], const double p2[2], double res[2], double rectYCoord, double xMin, double xMax)
    {
...
/*--------------------------------------------------------------------------*/
    bool Camera::isPointInQuadrangle(const double point[2], const double corners[4][2])
    {
        ...
        return (isPointInTriangle(point, corners[0], corners[1], corners[2]) || isPointInTriangle(point, corners[1], corners[2], corners[3]));
    }
/*--------------------------------------------------------------------------*/
