...
/*--------------------------------------------------------------------------*/
    CameraJoGL::CameraJoGL(Camera * camera):CameraBridge(), DrawableObjectJoGL(camera)
    {
...
/*--------------------------------------------------------------------------*/
    ...
    {
        getCameraJavaMapper()->setViewingArea(m_aViewingTranslation[0], m_aViewingTranslation[1], m_aViewingScale[0], m_aViewingScale[1]);
        ...
        getCameraJavaMapper()->setAxesRotationParameters(m_aBoxCenter[0], m_aBoxCenter[1], m_aBoxCenter[2], m_dAlpha, m_dTheta);
...
/*--------------------------------------------------------------------------*/
    void CameraJoGL::project(const double projMatrix[4][4], const double viewPort[4], const double sceneCoordinates[3], double pixCoordinates[3])
    {
...
/*--------------------------------------------------------------------------*/
    void CameraJoGL::unProject(double unProjMatrix[4][4], double viewPort[4], const double pixCoordinates[3], double sceneCoordinates[3])
    {
        double viewPix[3];
...
/*--------------------------------------------------------------------------*/
    void CameraJoGL::getPixelCoordinates(const double userCoords[3], double pixCoords[3])
    {
        ...
        m_pDrawer->pointScale(userCoords[0], userCoords[1], userCoords[2], &(finalUserCoord[0]), &(finalUserCoord[1]), (&finalUserCoord[2]));
...
/*--------------------------------------------------------------------------*/
    void CameraJoGL::get2dViewPixelCoordinates(const double userCoord[3], double pixCoord[3])
    {
        ...
        // convert user coordinates to log scale if needed
        m_pDrawer->pointScale(userCoord[0], userCoord[1], userCoord[2], &(finalUserCoord[0]), &(finalUserCoord[1]), (&finalUserCoord[2]));
...
/*--------------------------------------------------------------------------*/
    void CameraJoGL::getSceneCoordinates(const double pixCoords[3], double userCoords[3])
    {
        ...
        // convert user coordinates to log scale if needed
        m_pDrawer->inversePointScale(userCoords[0], userCoords[1], userCoords[2], &(userCoords[0]), &(userCoords[1]), &(userCoords[2]));
    }
/*--------------------------------------------------------------------------*/
...
/*--------------------------------------------------------------------------*/
    void CameraJoGL::get2dViewCoordinates(const int pixCoords[2], double userCoord2D[2])
    {
        ...
        // convert user coordinates to log scale if needed
        m_pDrawer->inversePointScale(sceneCoords[0], sceneCoords[1], sceneCoords[2], &(userCoord2D[0]), &(userCoord2D[1]), NULL);
    }
/*--------------------------------------------------------------------------*/
