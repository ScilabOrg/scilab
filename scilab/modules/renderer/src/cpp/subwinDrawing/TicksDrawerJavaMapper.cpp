...
/*--------------------------------------------------------------------------*/
    TicksDrawerJavaMapper::TicksDrawerJavaMapper(void):DrawableObjectJavaMapper()
    {
...
/*--------------------------------------------------------------------------*/
                                            ...
                                            double axisSegmentStart[3], double axisSegmentEnd[3], double ticksDirection[3])
    {
        return m_pJavaObject->drawTicks(axisSegmentStart, 3,
                                        axisSegmentEnd, 3,
                                        ticksDirection, 3,
                                        (double *)ticksPositions, nbTicks, (double *)subticksPositions, nbSubticks, ticksLabels, nbTicks);
    }
/*--------------------------------------------------------------------------*/
...
/*--------------------------------------------------------------------------*/
                                            ...
                                            double axisSegmentStart[3], double axisSegmentEnd[3], double ticksDirection[3])
    {
        return m_pJavaObject->drawTicks(axisSegmentStart, 3,
                                        ...
                                        (double *)subticksPositions, nbSubticks, ticksLabels, nbTicks, labelsExponents, nbTicks);
    }
/*--------------------------------------------------------------------------*/
    void TicksDrawerJavaMapper::setAxisParamerters(int lineStyle, float lineWidth, int lineColor,
                                                   int fontType, double fontSize, int fontColor, bool useFractionalMetrics)
    {
        m_pJavaObject->setAxisParameters(lineStyle, lineWidth, lineColor, fontType, fontSize, fontColor, useFractionalMetrics);
    }
/*--------------------------------------------------------------------------*/
