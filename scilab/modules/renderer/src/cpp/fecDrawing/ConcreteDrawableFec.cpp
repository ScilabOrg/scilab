...
/*---------------------------------------------------------------------------------*/
      ConcreteDrawableFec::ConcreteDrawableFec(sciPointObj * pFec):DrawableFec(pFec)
    {
...
/*---------------------------------------------------------------------------------*/
    DrawableObject::EDisplayStatus ConcreteDrawableFec::drawFec(void)
    {
        ...
        drawFec(xCoords, yCoords, pFEC_FEATURE(m_pDrawed)->pfun, nbNodes, firstPoints, secondPoints, thirdPoints, nbTriangles);
...
/*---------------------------------------------------------------------------------*/
                                      ...
                                      const int firstPoints[], const int secondPoints[], const int thirdPoints[], int nbTriangles)
    {
        ...
        for (; it != m_oDrawingStrategies.end(); it++)
        {
            (*it)->drawFec(xCoords, yCoords, values, nbNodes, firstPoints, secondPoints, thirdPoints, nbTriangles);
        }
    }
/*---------------------------------------------------------------------------------*/
...
/*---------------------------------------------------------------------------------*/
    void ConcreteDrawableFec::decomposeFec(double xCoords[], double yCoords[], int firstPoints[], int secondPoints[], int thirdPoints[])
    {
        int nbNodes = getNbNodes();
