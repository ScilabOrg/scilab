...
/*---------------------------------------------------------------------------------*/
    DrawableClippedObject::DrawableClippedObject(sciPointObj * drawed):DrawableObject(drawed)
    {
        ...
        case 0:                // clipgrf state, use axes bounds
            {
                ...
                setClipBox(clipBounds[0], clipBounds[1], clipBounds[2], clipBounds[3], clipBounds[4], clipBounds[5]);
        ...
        case 1:                // on state, use its own bounds
            {
                ...
                // xmin, ymin
                pointScale(clipBounds[0], clipBounds[1] - clipBounds[3], 0.0, &(scaledClipBounds[0]), &(scaledClipBounds[2]), &(scaledClipBounds[4]));
                // xmax, ymax
                pointScale(clipBounds[0] + clipBounds[2], clipBounds[1], 0.0, &(scaledClipBounds[1]), &(scaledClipBounds[3]), &(scaledClipBounds[5]));
                // beware in scilab y is upper left point
                setClipBox(scaledClipBounds[0], scaledClipBounds[1],
                           scaledClipBounds[2], scaledClipBounds[3], scaledClipBounds[4], scaledClipBounds[5]);
                m_bXClippingEnable = true;
                m_bYClippingEnable = true;
                m_bZClippingEnable = false;
            }
...
/*---------------------------------------------------------------------------------*/
    void DrawableClippedObject::setClipBox(double xMin, double xMax, double yMin, double yMax, double zMin, double zMax)
    {
