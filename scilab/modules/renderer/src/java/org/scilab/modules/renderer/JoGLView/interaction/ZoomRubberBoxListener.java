    ...
    private void performZoom(Point firstPoint, Point secondPoint) {
        ...
        if (axes.getCamera().getView().equals(Camera.ViewType.VIEW_2D)) {
            if (!firstPoint.equals(secondPoint)) {
                ...
                bounds[0] = (double) Math.nextAfter((float) Math.min(p1[0], p2[0]), -Double.MAX_VALUE);
                bounds[1] = (double) Math.nextAfter((float) Math.max(p1[0], p2[0]), Double.MAX_VALUE);

                bounds[2] = (double) Math.nextAfter((float) Math.min(p1[1], p2[1]), -Double.MAX_VALUE);
                bounds[3] = (double) Math.nextAfter((float) Math.max(p1[1], p2[1]), Double.MAX_VALUE);
