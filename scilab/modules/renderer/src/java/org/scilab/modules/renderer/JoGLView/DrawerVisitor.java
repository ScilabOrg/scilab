...
public class DrawerVisitor implements Visitor, Drawer, GraphicView {
    ...
    public void updateObject(Integer id, int property) {
         ...
         */
        if (id == figure.getIdentifier() && property == GraphicObjectProperties.__GO_CHILDREN__)
        {
    ...
    private boolean isFigureChild(Integer id) {
        ...
        Integer childType = (Integer) GraphicController.getController().getProperty(id, GraphicObjectProperties.__GO_TYPE__);
        boolean realJoGLChild = figure.getIdentifier() == parentFigureID && !(childType == GraphicObjectProperties.__GO_UICONTROL__ ||  childType == GraphicObjectProperties.__GO_UIMENU__);
        return realJoGLChild;
    }
