...
public class AxesRulerDrawer {
    ...
    public void computeRulers(Axes axes, AxesDrawer axesDrawer, ColorMap colorMap, DrawingTools drawingTools, Transformation transformation, Transformation canvasProjection) {
        ...
        // Draw Z ruler
        if (axes.getView() == ViewType.VIEW_3D && axes.getRotationAngles()[1] != 90.0) {
    ...
    private Vector3d computeXAxisPosition(double[] projectionMatrix, Double[] bounds, AxisLocation axisLocation) {
        double y, z;
        switch (axisLocation) {
            default:
            case BOTTOM:
    ...
    private Vector3d computeYAxisPosition(double[] matrix, Double[] bounds, AxisLocation axisLocation) {
        double x, z;
        switch (axisLocation) {
            default:
            case LEFT:
