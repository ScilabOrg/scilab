...
public class ArrowDrawer {
    ...
    public void drawArrows(String parentAxesId, String identifier, double arrowSize, double lineThickness, boolean isSegs,
        ...
        int firstVertexOffset = segmentIndices.getData().capacity();
        ...
        fillArrowVertexData(projection, vertices, firstVertexOffset,
            segmentIndices, numSegments, triangleIndices, arrowSize);
     ...
     * index buffer. Segment vertices are stored consecutively, (2*Nsegments) elements, as well as
     * arrow tip vertices, (3*Nsegments) elements, which are written after the former. The vertex buffer must
     ...
     * @param firstArrowVertex the index of the first arrow tip vertex in the vertex buffer.
     * @param segmentIndices the segments' indices (2*Nsegments elements).
     * @param numSegments the number of segments (Nsegments).
     * @param triangleIndices the arrow tips' triangle indices (3*Nsegments elements).
     * @param arrowSize the arrow size (in pixels).
     */
    private void fillArrowVertexData(Transformation projection, ElementsBuffer vertices, int firstArrowVertex,
        IndicesBuffer segmentIndices, int numSegments, IndicesBuffer triangleIndices, double arrowSize) {
        int i0;
        int i1;
     ...
     * It is computed depending on the Axes box's dimensions, displayed bounds, in order to obtain a constant size
     * in pixels, or else a size varying with the zoom level.
     * @param the average norm of a segment in window coordinates (used only when the size parameter is < 0).
