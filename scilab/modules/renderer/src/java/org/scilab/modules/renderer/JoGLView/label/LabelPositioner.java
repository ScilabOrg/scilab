 ...
 * The distance ratio and projected ticks direction are computed by the Ruler
 * drawer (used by AxesRulerDrawer), whereas the label position and the box coordinates
 * ticks direction are obtained from the AxesRuler drawer {@see AxesRulerDrawer}.
 * These four parameters must be set before computing the label's position values.
 *
 * To do:
...
public class LabelPositioner {
        ...
        /**
         * The label's position, which is its anchor point's position
        ...
        /**
         * Returns the position of the label's anchor point, depending on the
         * automatic position mode. If equal to true, the automatically
         * computed position is returned. If not, the user-specified position
         * is returned.
         * It additionally sets the anchorPoint member to the returned position.
        ...
        private Vector3d computeDisplacedPosition() {
                ...
                this.labelDisplacement = ticksDirection.times(distRatio);

                position = position.plus(this.labelDisplacement);

                return position;
        }

        /**
         * Returns the sprite anchor position depending on the automatic position mode.
         * It additionally sets the anchorPosition member.
