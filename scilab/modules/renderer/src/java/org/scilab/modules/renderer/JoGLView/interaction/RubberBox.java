...
public class RubberBox extends FigureInteraction implements PostRendered, MouseListener, MouseMotionListener, KeyListener {
     ...
     */
    private void setInfoMessage(String baseMessage, AbstractPointComputer pointComputer, boolean oneAxis) {
        if ((pointComputer != null) && (pointComputer.isValid())) {
            ...
            for (int i = 0; i < 3; i++) {
                if ((i != pointComputer.getFirstAxisIndex()) ^ oneAxis) {
                    message += AXES_NAMES[i] + " = " + df.format(data[i]) + comma;
                    comma = "";
                }
            }
            GraphicController.getController().setProperty(
    ...
    private boolean setPointD(Point point) {
        ...
        if (pointDComputer.isValid()) {
            this.pointDComputer = pointDComputer;
