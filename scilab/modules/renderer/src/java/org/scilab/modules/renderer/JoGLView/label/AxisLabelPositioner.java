...
public class AxisLabelPositioner extends LabelPositioner {
        ...
        /**
         * Determines and returns the rotation angle (in degrees) between the window coordinate system's axis
         * (+Y, -Y, +X, -X) most closely aligned to the projected ticks direction and the projected ticks direction itself.
         ...
         * The anchor position remains constant when the angle lies in the initial quadrant centered about
         * the ticks direction (from -45 to 45 degrees relative to it),
         *
         * The result is such that the anchor-to-opposite side vector always points away from
         * the interior of the Axes box. This avoids overlapping between the label's box and the axis
        ...
        protected SpriteAnchorPosition getAutoAnchorPosition() {
                ...
                /* Get the angle from the closest axis to the ticks direction. */
                double ticksDirectionAngle = computeTicksDirectionAngle();

                tmpRotationAngle = rotationAngle % 360.0;

                /*
                 * Subtract the angle from the closest window coordinate axis to the ticks direction
                 * from the rotation angle to take into account the associated axis' orientation.
                 */
         ...
         * @return the sprite anchor position.
         */
        protected SpriteAnchorPosition getUncorrectedAutoAnchorPosition() {
                if (projectedTicksDirection.getY() > Math.abs(projectedTicksDirection.getX())) {
