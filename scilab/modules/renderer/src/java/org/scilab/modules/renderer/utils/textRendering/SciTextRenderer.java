...
public class SciTextRenderer {
        ...
        private String rendererType = "none";
        ...
        public void draw3D(GL gl, String str, double x, double y, double z, double angle) {
            if (rendererType.equals("none") && str.length() > 0 && str.charAt(0) == '$') { /* LaTeX management */
                speRenderer.draw3D(str, (float) x, (float) y, (float) z, useFractionalMetrics ? scaleFactor : 1.0f);
                return;
            }

            if (str.length() > 0 && rendererType.equals("latex")) {
                String ltx = str;
                if (str.charAt(0) != '$') {
                    ltx = "$" + str + "$";
                }
                speRenderer.draw3D(ltx, (float) x, (float) y, (float) z, useFractionalMetrics ? scaleFactor : 1.0f);
                return;
            }

            if (str.length() > 0 && rendererType.equals("mathml") && str.charAt(0) == '<') {
                speRenderer.draw3D(str, (float) x, (float) y, (float) z, useFractionalMetrics ? scaleFactor : 1.0f);
                return;
            }
        ...
        public Rectangle2D getBounds(String str) {
                ...
                /* Set also for the MathML / LaTeX rendering */
                if (str.length() > 0 && ((rendererType.equals("mathml") && str.charAt(0) == '<') || ((rendererType.equals("latex") || rendererType.equals("none")) && str.charAt(0) == '$'))) {
