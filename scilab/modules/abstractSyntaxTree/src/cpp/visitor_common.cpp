...
types::InternalType* AddElementToVariable(types::InternalType* _poDest, types::InternalType* _poSource, int _iRows, int _iCols, int *_piRows, int *_piCols)
{
	...
	{//Just add the new value in the current item
		...
		case types::GenericType::RealDouble :
			...
			if(_poSource->getAs<types::Double>()->getSize() != 1)
			{
				poResult->getAs<types::Double>()->append(iCurRow, iCurCol, _poSource->getAs<types::Double>());
			}
			else
			{
				poResult->getAs<types::Double>()->setReal(iCurRow, iCurCol, _poSource->getAs<types::Double>()->getReal(0,0));
				poResult->getAs<types::Double>()->setImg(iCurRow, iCurCol, _poSource->getAs<types::Double>()->getImg(0,0));
			}
		...
		case types::GenericType::RealBool:
			...
			*_piRows = _poSource->getAsBool()->getRows();
			*_piCols = _poSource->getAsBool()->getCols();
			break;
		case types::GenericType::RealInt :
			...
			*_piCols = _poSource->getAsInt()->getCols();
			break;
		case types::GenericType::RealString :
            if(_poSource->getAsString()->getSize() != 1)
            {
                poResult->getAsString()->append(iCurRow, iCurCol, _poSource->getAsString());
            }
            else
            {
                poResult->getAsString()->setString(iCurRow, iCurCol, _poSource->getAsString()->getString(0,0));
            }

            *_piRows = _poSource->getAsString()->getRows();
			*_piCols = _poSource->getAsString()->getCols();
			break;
