...
int C2F(intnorm)(char *fname,unsigned long fname_len)
{
    ...
    {
        ret = 0;
        sciErr = createMatrixOfDouble(pvApiCtx, Rhs + 1, 1, 1, &ret); // A = [] => returning 0
    ...
    if (Rhs == 1)
    {
        ...
        sciErr = createMatrixOfDouble(pvApiCtx, Rhs + 1, 1, 1, &ret); // One argument => returning norm 2
        AssignOutputVariable(pvApiCtx, 1) = Rhs + 1;
        return 0;
    }
    else
    ...
    if (iType == sci_strings)
    {
        ...
        sciErr = createMatrixOfDouble(pvApiCtx, Rhs + 1, 1, 1, &ret); // flag is a string => returning the corresponding norm
        AssignOutputVariable(pvApiCtx, 1) = Rhs + 1;
        return 0;
    }
    else
    {
        if (iType == sci_matrix)
        {
            ...
            if (*pflagVal != 1 && *pflagVal != 2) // flag must be = 1 or 2
            {
            ...
            sciErr = createMatrixOfDouble(pvApiCtx, Rhs + 1, 1, 1, &ret); // flag is an integer => returning the corresponding norm
            AssignOutputVariable(pvApiCtx, 1) = Rhs + 1;
            return 0;
        }
    else
