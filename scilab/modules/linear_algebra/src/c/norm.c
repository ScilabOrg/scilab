...
extern double C2F(zlange) (char *norm, int *m, int *n, doublecomplex *A, int *lda, double *work);
extern int     C2F(zgesdd) (char *job, int *m, int *n, doublecomplex *A, int *lda,
...
double normPC (doublecomplex *A, int iRows, int iCols, double p)
{
    ...
    for (i = 0; i < iRows; ++i)
    {
        modulus = sqrt(pow(A[i].r, 2) + pow(A[i].i, 2)); // Retrieving A[i] modulus
        ret += pow(modulus, p); // modulus^p
    }
    return pow(ret, 1. / p);
}
