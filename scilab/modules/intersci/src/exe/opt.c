...
void WriteOptArg(FILE * f, VARPTR var)
{
    ...
    case VALUE:
        ...
        case MATRIX:
            ...
            AddDeclare1(DEC_DATA, "%s dat%d[]= %s", SGetCDec(var->for_type), var->stack_position, ldata);
            Fprintf(f, indent, "m%d = 1;n%d = %s;\n", var->stack_position, var->stack_position, lsize);
            (*(CRERHSTAB[var->type].fonc)) (f, var);
            AddDeclare1(DEC_INT, "un=1");
            Fprintf(f, indent, "C2F(%scopy)(&m%d,dat%d,&un,%s(l%d),&un);\n",
                    SGetForTypeAbrev(var), var->stack_position, var->stack_position, SGetForTypeStack(var), var->stack_position);

            break;
        case STRING:
            Fprintf(f, indent, "m%d = %d\n", var->stack_position, strlen(var->opt_name));
            (*(CRERHSTAB[var->type].fonc)) (f, var);
...
void OptMATRIX(FILE * f, VARPTR var)
{
    ...
    Fprintf(f, indent++, "if ( opts[%d].position == -1 ){\n", opt_posi);
    Fprintf(f, indent, "iopos++ ; opts[%d].position = iopos;\n", opt_posi);
    AddDeclare1(DEC_DATA, "%s xdat%d[]= %s, *dat%d = xdat%d", SGetCDec(var->for_type), opt_posi, data, opt_posi, opt_posi);
