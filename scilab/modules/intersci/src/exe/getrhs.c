...
void GetIMATRIX(FILE * f, VARPTR var, int flag)
{
    ...
    if (var->list_el == 0)
    {
      /** A scilab matrix argument **/
        sprintf(str1, "%d", i1);
        Fprintf(f, indent, C, str2, SGetForTypeAbrev(var), i1, i1, i1, i1, i1);
        /* Adding the calling sequence in the for_names  */
        ChangeForName2(var, "%s(lr%s),%s(lc%s),&it%s", SGetForTypeStack(var), str1, SGetForTypeStack(var), str1, str1);
    }
    else
    {
      /** A scilab matrix argument inside a list **/
        ...
        Fprintf(f, indent, LC, str2, var->list_el, SGetForTypeAbrev(var), str1, str1, str1, str1, str1, str1, str1);
        /* Adding the calling sequence in the for_names  */
        ChangeForName2(var, "%s(lr%s),%s(lc%s),&it%s", SGetForTypeStack(var), str1, SGetForTypeStack(var), str1, str1);
    }
...
void Check(FILE * f, VARPTR var, int nel)
{
    ...
    if (isdigit(var1->name[0]) != 0)
    {
        ...
        if (strcmp(var1->for_name[0], var1->name) != 0)
        {
            if (var->list_el == 0)
            {
                Fprintf(f, indent, "CheckOneDim(%d,%d,%s,%s);\n", var->stack_position, nel + 1, var1->for_name[0], var1->name);
            }
            else
            {
                Fprintf(f, indent, "CheckListOneDim(%s,%d,%d,%s,%s);\n", str2, var->list_el, nel + 1, var1->for_name[0], var1->name);
            }
        }
    }
}
