...
void CheckSTRINGMAT(f, var, flag)
     FILE *f;
     VARPTR var;
     int flag;
{
...
void CheckDIMFOREXT(f, var, flag)
     FILE *f;
     VARPTR var;
     int flag;
{
        ...
        if (isdigit(var1->name[0]) != 0)
        {
            ...
            if (strcmp(var1->for_name[0], var1->name) != 0)
            {
                Fprintf(f, indent, "CheckOneDim(opts[%d].position,%d,%s,%s);\n",
                        var->stack_position - basfun->NewMaxOpt + 1, nel + 1, var1->for_name[0], var1->name);
            }
        }
    }
}
