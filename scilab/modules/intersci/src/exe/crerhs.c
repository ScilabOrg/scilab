     ...
     VARPTR var;
{
    ...
    else
    {
        ...
        if (strncmp(str1, "&istk(", 6) == 0 || strncmp(str1, "&cstk(", 6) == 0 || strncmp(str1, "&rstk(", 6) == 0 || strncmp(str1, "&stk(", 5) == 0)
            lstr1 = str1 + 1;
        else
            lstr1 = str1;
        if (strncmp(str2, "&istk(", 6) == 0 || strncmp(str2, "&cstk(", 6) == 0 || strncmp(str2, "&rstk(", 6) == 0 || strncmp(str2, "&stk(", 5) == 0)
            lstr2 = str2 + 1;
        else
            lstr2 = str2;

        Fprintf(f, indent, "CreateVar(%d,\"%s\",%s,%s,&l%d);/* named: %s */\n",
                var->stack_position, SGetForTypeAbrev(var), lstr1, lstr2, var->stack_position, var->name);
        AddDeclare1(DEC_INT, "l%d", var->stack_position);
        ChangeForName2(var, "%s(l%d)", SGetForTypeStack(var), var->stack_position);
    }
}
     ...
     VARPTR var;
{
  ...
  /** peut-etre un pb si str1 contient des stk istk .. XXXXXX **/
    if (strncmp(lstr1, "stk", 3) == 0 || strncmp(lstr1, "istk", 4) == 0 || strncmp(lstr1, "rstk", 4) == 0 || strncmp(lstr1, "cstk", 4) == 0)
        sprintf(str2, "(mn%d=*%s,&mn%d)", var->stack_position, lstr1, var->stack_position);
    else
        sprintf(str2, "(mn%d=%s,&mn%d)", var->stack_position, lstr1, var->stack_position);
    strcpy(str1, "(un=1,&un)");
    CreCommon(f, var);
     ...
     VARPTR var;
{
    ...
    else
    {
        Fprintf(f, indent, "CreateCVar(%d,\"%s\",&%s,&%s,&%s,&lr%d,&lc%d);\n",
                var->stack_position, SGetForTypeAbrev(var), str3, str1, str2, var->stack_position, var->stack_position);
        ...
        ChangeForName2(var, "%s(lr%d),%s(lc%d),&it%d",
                       SGetForTypeStack(var), var->stack_position, SGetForTypeStack(var), var->stack_position, var->stack_position);
    }
}
     ...
     VARPTR var;
{
    ...
    else
    {
        ...
        AddDeclare1(DEC_INT, "lr%d", var->stack_position);
        ChangeForName2(var, "stk(lr%d)", var->stack_position);
    }
}
    ...
    if (variables[var->el[iel] - 1]->nfor_name == 0)
    {
        ...
        if (isdigit(lstr[0]) == 0)
        {
            if (variables[var->el[iel] - 1]->is_sciarg == 1)
            {
                ...
                Fprintf(f, indent, "%s= (int) *stk(lr%d));\n", sdim, variables[var->el[iel] - 1]->stack_position);
...
void GetDim(char *lstr, IVAR ivar)
{
    ...
    if (strncmp(s, "stk", 3) == 0 || strncmp(s, "istk", 4) == 0
        || strncmp(s, "sstk", 4) == 0 || strncmp(s, "stk", 3) == 0 || strncmp(s, "cstk", 4) == 0)
    {
        strcpy(lstr, s);
        return;
    }
