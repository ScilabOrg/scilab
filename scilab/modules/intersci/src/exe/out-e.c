    ...
    if (insidelist != 0)
    {
      /** var is inside a list **/
        ...
        Fprintf(f, indent, "CreateListVarFrom(%d,%d,\"%s\",&%s,&%s,&lrs,&%s);\n",
                vout->stack_position, nel, SGetForTypeAbrev(var), (str1[0] == '&') ? str1 + 1 : str1, (str2[0] == '&') ? str2 + 1 : str2, str3);
    }
    else
    {
        ...
        if (var->list_el != 0)
        {
            ...
            Fprintf(f, indent, "CreateVarFrom(%d,\"%s\",&%s,&%s,&lrs,&%s);\n",
                    icre, SGetForTypeAbrev(var), (str1[0] == '&') ? str1 + 1 : str1, (str2[0] == '&') ? str2 + 1 : str2, str3);
...
void OutLIST(FILE * f, VARPTR var, int insidelist, int nel)
{
    ...
    else
    {
      ...
      /** var is a list output ==> backconversion of its arguments **/
        ...
        {
            var1 = variables[k];
            if ((var1->list_el != 0) && (strcmp(var1->list_name, var->name) == 0) && var1->present)
            {
...
void OutLISTarg(FILE * f, VARPTR var, VARPTR var1, int insidelist, int nel)
{
    ...
    case VECTOR:
        if (var1->for_type != DOUBLE)
        {
            ...
            Fprintf(f, indent, "ConvertData(\"%s\",%s*%s,l%de%d);\n", SGetForTypeAbrev(var1), str1, str2, var1->stack_position, var1->list_el);
        }
        break;
    case COLUMN:
        strcpy(str2, "un");
    ...
    if (insidelist != 0)
    {
      /** var is inside a list **/
        ...
        Fprintf(f, indent, "CreateListCVar(%d,%d,\"%s\",%s,%s,%s,&lrs,&lcs,%s,%s);\n",
                vout->stack_position, nel, SGetForTypeAbrev(var), str3, str1, str2, str4, str5);
    }
    else
    {
        /* var is not outputed inside a list but var can be a list
...
void OutSTRINGMAT(FILE * f, VARPTR var, int insidelist, int nel)
{
    ...
    if (insidelist != 0)
    {
        ...
        Fprintf(f, indent, "CreateListVarFromPtr(%d,%d,\"S\",&%s,&%s,Str%d);\n",
                vout->stack_position, nel, (str1[0] == '&') ? str1 + 1 : str1, (str2[0] == '&') ? str2 + 1 : str2, var->stack_position);
    }
    else
    {
        Fprintf(f, indent, "CreateVarFromPtr(%d, \"S\",&%s,&%s,Str%d);\n",
                icre, (str1[0] == '&') ? str1 + 1 : str1, (str2[0] == '&') ? str2 + 1 : str2, var->stack_position);
