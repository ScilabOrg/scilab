...
C     .  ipar(2)=0 then collpnt is set to max( max(m(j))+1, 5-max(m(j)) ).
 ...
 12       continue
      ...
      if(istk(ilipar+1).gt.7 .or. istk(ilipar).lt.iMaxm) then
C     .  bvode: ipar(2) : max(m(j)) <= collpnt <= 7
C   /!\     /!\     /!\     /!\     /!\     /!\
C       in colnew.f line 581 we ckeck
C       IF ( K .LT. 0 .OR. K .GT. 7 ) RETURN where K is ipar(2)
C       what is the good condition ???
C   /!\     /!\     /!\     /!\     /!\     /!\
...
C   /!\     /!\     /!\     /!\     /!\
C     How to compute nreq and nmax ???
C   /!\     /!\     /!\     /!\     /!\
      nsizef = 4 + 3*iMaxm +
     $        (5+istk(ilipar+1)*ncomp)*(istk(ilipar+1)*ncomp+iMaxm) +
...
c               nsizef=4 + 3*M + (5+collpnt*N)*(collpnt*N+M) + (2*M-nrec)*2*M
      ...
      nsizei = 3 + istk(ilipar+1)*ncomp + iMaxm
C   /!\     /!\     /!\     /!\     /!\
C     How to compute nmax ???
C   /!\     /!\     /!\     /!\     /!\
