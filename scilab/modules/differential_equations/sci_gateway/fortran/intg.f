...
c              case (6)
     ...
     &            'occurs at some points of the integration interval.')
                  return
               case (4)
                  call msgstxt('Warning: Convergence problem, round-'//
     &            'off error detected. Try using bigger tolerances.')
                  return
               case (5)
...
c              case (6)
               case default
                  call msgstxt('Warning: Invalid input, absolute '//
     &            'tolerance <= 0 and relative tolerance < 2.e-14.')
                  return
            end select
        endif
      ...
      if(lhs.ge.2) then
         ...
         if (lhs.eq.3) then
            top=top+1
c           here, find a way to return ier in third argument. stk(...) = ier doesn't work'
            return
         endif
         return
      endif
