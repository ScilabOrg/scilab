...
c
      ...
      character*(nlgh+1) namer,namej,names,namep
      common /dassln/ namer,namej,names,namep
      external bresd,bjacd,bsurfd,bpsold
      external setfresd,setfjacd,setfsurfd,setfpsold
      common/ierode/iero
c     
...
c     [,psol]][,hotdata])
      ...
      lw = lstk(topw)
      l0 = lstk(top+1-rhs)
      if (.not.checkrhs(fname,6,12)) return
...
c     --------------------------------------
 ...
 10   hotstart=.false.
      if(rhs.eq.12-iskip) then
         hotstart=.true.
c     
c     Checking variable hotdata (number 12)
c     --------------------------------------
         il12 = iadr(lstk(top-rhs+12-iskip))
         if (istk(il12) .ne. 1) then
            err = 12-iskip
            call error(53)
            return
         endif
         n12 = istk(il12+1)*istk(il12+2)
         lhot = sadr(il12+4)
      elseif(rhs.ne.11-iskip) then
         call error(39)
         return
      endif
...
c         liw = base + LENIWP
      ...
      if(.not.hotstart) then
         ...
         if(lrw+liw.gt.n12+200) then
c           + 200 until I figure out what that test is for
            err=12-iskip
            call error(89)
            return
         endif
...
c           iwork(27) = LENWP
c           iwork(28) = LENIWP
            istk(iadr(liwork)+26) = 30
            istk(iadr(liwork)+27) = 30
      endif
