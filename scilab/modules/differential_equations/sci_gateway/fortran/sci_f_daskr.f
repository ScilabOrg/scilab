...
c     create header of wp
      if (.not.cremat(fname,topw,0,1,neq*neq,lr,lc)) return
      iheaderwp =iadr(lstk(topw))
      topw=topw+1
c     create header of wp and iwp
      if (.not.cremat(fname,topw,0,2,neq*neq,lr,lc)) return
      iheaderiwp =iadr(lstk(topw))
      topw=topw+1
...
c     header used to create wp
      istk(ilext+19)=iheaderwp
c     header used to create iwp
      istk(ilext+20)=iheaderiwp
c
c     function pjac
      istk(ilext+21)=kpjac
