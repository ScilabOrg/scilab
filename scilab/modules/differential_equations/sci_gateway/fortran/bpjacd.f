...
c     Test if the variable on the stack has same type and size as the theoretical iwp
      if (istk(il).ne.1.or.n.ne.liwp) then
         call error(98)
         return
      endif
      lx=sadr(il+hsize)
      print *,'iwp before copy:'
      do 900 i=1,2*neq*neq
         print *,' iwp(',i,')',iwp(i)
900   continue
c      call unsfdcopyint(liwp,stk(lx),1,iwp,1)
c      memmov(iwp, istk(lx), (2*neq*neq * sizeof(int)));
      do 901 i=1,2*neq*neq
         iwp(i) = stk(lx+i-1)
901   continue
      print *,'iwp after copy:'
      do 902 i=1,2*neq*neq
         print *,' iwp(',i,')',iwp(i)
902   continue
      top = top-1
      if(err.gt.0.or.err1.gt.0) return
