...
//extern int C2F(getexternal) (char *fname, int *topk, int *lw, char *namex, int *typex, void (*setfun) (char *, int *), unsigned long fname_len, unsigned long name_len);
...
int sci_daskr(char *fname, unsigned long fname_len)
{
    ...
    if (nbInputArg == 7) // daskr (x0, t0, t, res, ng, g, jroot)
    {
        ...
        {
            Scierror(999, _("First argument should be a column vector or a matrix of two column vectors (initial conditions).\n"));
            return 0;
        }
        ...
        else // iCols = 2, user provided y0 and yp0
        {
            pyp0 = (R *) MALLOC(iRows * sizeof(R));
            pyp0 = pInitCond + iRows;
        }
        ...
        if (!isDoubleType(pvApiCtx, piAddr) || !isScalar(pvApiCtx, piAddr))
        {
            Scierror(999, _("Second argument should be a real (start time).\n"));
            return 0;
        }
        ...
        if (!isDoubleType(pvApiCtx, piAddr) || !isScalar(pvApiCtx, piAddr))
        {
            Scierror(999, _("Third argument should be a real (end time).\n"));
            return 0;
        }
        iflag = getScalarDouble(pvApiCtx, piAddr, ptEnd);
        if (iflag)
        {
            Scierror(999, _("%s: Can not read input argument #%d.\n"), fname, 3);
            return iflag;
        ...
        top       = Top;
        resIndex  = 4;
        iflag = C2F(getexternal) (fname, &top, &resIndex, resName, &resType, resFn, fname_len, resLen);
        if (!iflag)
        {
            Scierror(999, _("%s: Can not read input argument #%d.\n"), fname, 4);
            return iflag;
        ...
        {
            Scierror(999, _("Fourth argument should be a function (residual).\n"));
            return 0;
        }
        ...
        {
            Scierror(999, _("Fifth argument should be a real (end time).\n"));
            return 0;
        }
        ...
        if (!isPointerType(pvApiCtx, piAddr))
        {
            Scierror(999, _("Sixth argument should be a function (g=f(t,y)).\n"));
            return 0;
        }
        ...
        if (!isIntegerType(pvApiCtx, piAddr) || isScalar(pvApiCtx, piAddr))
        {
            Scierror(999, _("Seventh argument should be a integer tab (jroot).\n"));
            return 0;
        }
        jroot = CALLOC(ng, sizeof(int));
        sciErr = getMatrixOfInteger32(pvApiCtx, piAddr, &iRows, &iCols, &jroot);
        if (sciErr.iErr)
        {
            printError(&sciErr, 0);
            Scierror(999, _("%s: Can not read input argument #%d.\n"), fname, 7);
    ...
    if (nbInputArg == 8)
    {
        ...
        if (!isDoubleType(pvApiCtx, piAddr) || !isScalar(pvApiCtx, piAddr))
        {
            Scierror(999, _("Eighth argument should be a real (start time).\n"));
            return 0;
        }
        iflag = getScalarDouble(pvApiCtx, piAddr, abstol);
        if (iflag)
        {
            Scierror(999, _("%s: Can not read input argument #%d.\n"), fname, 8);
            return iflag;
    ...
    if (nbInputArg == 9)
    {
        ...
        if (!isDoubleType(pvApiCtx, piAddr) || !isScalar(pvApiCtx, piAddr))
        {
            Scierror(999, _("Ninth argument should be a real (start time).\n"));
            return 0;
        }
        iflag = getScalarDouble(pvApiCtx, piAddr, reltol);
        if (iflag)
        {
            Scierror(999, _("%s: Can not read input argument #%d.\n"), fname, 9);
            return iflag;
    ...
    if (nbInputArg == 10)
    {
        ...
        if (!isDoubleType(pvApiCtx, piAddr) || !isScalar(pvApiCtx, piAddr))
        {
            Scierror(999, _("Tenth argument should be a real (start time).\n"));
            return 0;
        }
        iflag = getScalarDouble(pvApiCtx, piAddr, reltol);
        if (iflag)
