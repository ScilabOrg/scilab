...
version = getversion("scilab");
 ...
 y=ode(y0,t0,t,f);
 assert_checkequal( size(y) <> [1 32] , %f);
clear y;
clear t;
...
//for i=1:12, assert_checkequal( poly(res(:,i), "s", "coeff") - res6(i) <> 0 , %f); end
//res7 = ode(yy, t, tout, rtol, atol, list('fex2', 3.0d+7, -0.04, 1.0d+4));
//for i=1:12, assert_checkequal( poly(res(:,i), "s", "coeff") - res7(i) <> 0 , %f); end
