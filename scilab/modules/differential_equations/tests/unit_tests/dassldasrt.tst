...
assert_checkfalse(norm(yy2-yy0,1)>1E-5);
// fortran routines, with jacobian
yy3=dassl([y0,y0d],t0,t,'dres1','djac1',info);  //=yy1
assert_checkfalse(norm(yy3-yy1,1)>1E-5);
...
assert_checkfalse(norm(pp*x1-x1,1)>1.d-5);
//x(4) goes through 1 at  t=1.5409711;
t=1.5409711;ww=dassl([x0,x0d],t0,t,g,info);
assert_checkfalse(abs(ww(5)-1)>0.001);
...
assert_checkfalse(norm(x2-ww(2:nx+1,1),1)>0.0001);
...
assert_checkfalse(norm(x-xn,1)>0.00001);
...
assert_checkfalse(abs(nn(1)-2.47)>0.001);
y0=yy(2,2);y0d=yy(3,2);t0=nn(1);t=[3,4,5,6];
[yy,nn]=dasrt([y0,y0d],t0,t,atol,rtol,'res1',ng,'gr1',info);
assert_checkfalse(abs(nn(1)-2.5)>0.001);
y0=yy(2,1);y0d=yy(3,1);t0=nn(1);t=[3,4,5,6];
[yy,nn]=dasrt([y0,y0d],t0,t,atol,rtol,'res1',ng,'gr1',info);
assert_checkfalse(abs(nn(1)-2.53)>0.001);
...
assert_checkfalse(abs(nn(1)-81.163512)>0.001);
...
assert_checkfalse(abs(nn(1)-162.57763)>0.004);
