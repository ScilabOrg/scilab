...
C   021105  Changed yprime argument in DRCHEK_DDASKR calls to YPRIME.
C   021217  Modified error return for zeros found too close together.
C   021217  Added root direction output in JROOT.
C   040518  Changed adjustment to X2 in Subr. DROOTS_DDASKR.
...
C   071003  In DRCHEK_DDASKR, fixed bug in TEMP2 (HMINR) below 110.
C   110608  In DRCHEK_DDASKR, fixed bug in setting of T1 at 300.
C***CATEGORY NO.  I1A2
...
C                       the line  CALL XSETUN_DDASKR(L)  in your program.  ****
C
...
C           RTOL*abs(Y(i)) + ATOL (unless you supplied routine DDAWTS_DDASKR
...
C
C    SUBROUTINE DDAWTS_DDASKR (NEQ, IWT, RTOL, ATOL, Y, EWT, RPAR, IPAR)
C    DIMENSION RTOL(*), ATOL(*), Y(*), EWT(*), RPAR(*), IPAR(*)
C
C  A DDAWTS_DDASKR routine has been included with DDASKR which sets the
...
C  your own error weights by writing a subroutine DDAWTS_DDASKR to be 
...
C
C   DRCHEK_DDASKR does preliminary checking for roots, and serves as an
C          interface between Subroutine DDASKR and Subroutine DROOTS_DDASKR.
C   DROOTS_DDASKR finds the leftmost root of a set of functions.
...
C   DDAWTS_DDASKR sets error weight quantities.
C   DINVWT tests and inverts the error weights.
C   DDATRP_DDASKR performs interpolation to get an output solution.
C   DDWNRM computes the weighted root-mean-square norm of a vector.
C   D1MACH_DDASKR provides the unit roundoff of the computer.
C   XERRWD/XSETF_DDASKR/XSETUN_DDASKR/IXSAV is a package to handle error messages. 
...
C   DCNST0, DDAWTS_DDASKR, DINVWT, D1MACH_DDASKR, DDWNRM, DDASIC, DDATRP_DDASKR, DDSTP,
C   DRCHEK_DDASKR, XERRWD
C
...
C
      CALL DDAWTS_DDASKR(NEQ,INFO(2),RTOL,ATOL,Y,RWORK(LWT),RPAR,IPAR)
...
C
      ...
      CALL DRCHEK_DDASKR(1,RT,NRT,NEQ,T,TOUT,Y,YPRIME,RWORK(LPHI),
...
C
      CALL DRCHEK_DDASKR(2,RT,NRT,NEQ,TN,TOUT,Y,YPRIME,RWORK(LPHI),
     *   RWORK(LPSI),IWORK(LKOLD),RWORK(LR0),RWORK(LR1),
     *   RWORK(LRX),JROOT,IRT,RWORK(LROUND),INFO(3),
     *   RWORK,IWORK,RPAR,IPAR)
      IF (IRT .LT. 0) GO TO 731
...
410   CONTINUE
      ...
      CALL DDATRP_DDASKR(TN,TOUT,Y,YPRIME,NEQ,IWORK(LKOLD),
...
420   IF((TN-T)*H .LE. 0.0D0) GO TO 490
      ...
      CALL DDATRP_DDASKR(TN,TN,Y,YPRIME,NEQ,IWORK(LKOLD),
     *  RWORK(LPHI),RWORK(LPSI))
      T = TN
      IDID = 1
      DONE = .TRUE.
...
C
     ...
     *   (ABS(TN)+ABS(H)))GO TO 460
      CALL DDATRP_DDASKR(TN,TSTOP,Y,YPRIME,NEQ,IWORK(LKOLD),
     *  RWORK(LPHI),RWORK(LPSI))
      IDID=2
      T=TSTOP
      DONE = .TRUE.
...
C
510   CALL DDAWTS_DDASKR(NEQ,INFO(2),RTOL,ATOL,RWORK(LPHI),RWORK(LWT),
     *            RPAR,IPAR)
      CALL DINVWT(NEQ,RWORK(LWT),IER)
      IF (IER .NE. 0) THEN
        IDID = -3
...
C     Check for a zero of R near TN.
C
      CALL DRCHEK_DDASKR(3,RT,NRT,NEQ,TN,TOUT,Y,YPRIME,RWORK(LPHI),
     *   RWORK(LPSI),IWORK(LKOLD),RWORK(LR0),RWORK(LR1),
     *   RWORK(LRX),JROOT,IRT,RWORK(LROUND),INFO(3),
     *   RWORK,IWORK,RPAR,IPAR)
...
C------END OF SUBROUTINE DDASKR-----------------------------------------
      END
      SUBROUTINE DRCHEK_DDASKR (JOB, RT, NRT, NEQ, TN, TOUT, Y, YP, PHI, PSI,
     *   KOLD, R0, R1, RX, JROOT, IRT, UROUND, INFO3, RWORK, IWORK,
     *   RPAR, IPAR)
C
C***BEGIN PROLOGUE  DRCHEK_DDASKR
C***REFER TO DDASKR
C***ROUTINES CALLED  DDATRP_DDASKR, DROOTS_DDASKR, DCOPY, RT
C***REVISION HISTORY  (YYMMDD)
C   020815  DATE WRITTEN   
...
C input flag JOB.  It calls subroutine DROOTS_DDASKR to locate the root
C as precisely as possible.
C
C In addition to variables described previously, DRCHEK_DDASKR
C uses the following for communication..
C JOB    = integer flag indicating type of call..
C          JOB = 1 means the problem is being initialized, and DRCHEK_DDASKR
C                  is to look for a root at or very near the initial T.
C          JOB = 2 means a continuation call to the solver was just
C                  made, and DRCHEK_DDASKR is to check for a root in the
C                  relevant part of the step last taken.
C          JOB = 3 means a successful step was just taken, and DRCHEK_DDASKR
...
C          T0 is updated by DRCHEK_DDASKR, whether a root is found or not.
...
C Evaluate R at initial T (= RWORK(LT0)); check for zero values.--------
 100  CONTINUE
      CALL DDATRP_DDASKR(TN,RWORK(LT0),Y,YP,NEQ,KOLD,PHI,PSI)
...
C R has a zero at T0.  Look at R at T0+ = T0 + (small increment). ------
 ...
 220    Y(I) = Y(I) + TEMP2*PHI(I,2)
      GO TO 240
 230  CALL DDATRP_DDASKR (TN, RWORK(LT0), Y, YP, NEQ, KOLD, PHI, PSI)
...
C Set T1 to TN or TOUT, whichever comes first, and get R at T1. --------
 ...
 330  CALL DDATRP_DDASKR (TN, T1, Y, YP, NEQ, KOLD, PHI, PSI)
      CALL RT (NEQ, T1, Y, YP, NRT, R1, RPAR, IPAR)
      IWORK(LNRTE) = IWORK(LNRTE) + 1
C Call DROOTS_DDASKR to search for root in interval from T0 to T1. ------------
      JFLAG = 0
 350  CONTINUE
      CALL DROOTS_DDASKR (NRT, HMINR, JFLAG, RWORK(LT0),T1, R0,R1,RX, X, JROOT)
      IF (JFLAG .GT. 1) GO TO 360
      CALL DDATRP_DDASKR (TN, X, Y, YP, NEQ, KOLD, PHI, PSI)
...
C
 390  CONTINUE
      RETURN
C---------------------- END OF SUBROUTINE DRCHEK_DDASKR -----------------
      END
      SUBROUTINE DROOTS_DDASKR (NRT, HMIN, JFLAG, X0, X1, R0, R1, RX, X, JROOT)
C
C***BEGIN PROLOGUE  DROOTS_DDASKR
C***REFER TO DRCHEK_DDASKR
...
C are communicated through the call sequence of DROOTS_DDASKR.
...
C          JFLAG = 1 means DROOTS_DDASKR needs a value of R(x).  Set RX = R(X)
C                    and call DROOTS_DDASKR again.
...
C No sign changes in this interval.  Set X = X1, return JFLAG = 4. -----
 420  CALL DCOPY (NRT, R1, 1, RX, 1)
      ...
      JFLAG = 4
      RETURN
C----------------------- END OF SUBROUTINE DROOTSDDASKR ----------------------
      END
...
C
675   CONTINUE
      CALL DDATRPDDASKR(X,X,Y,YPRIME,NEQ,K,PHI,PSI)
      JSTART = 1
      IF (IDID .GE. 0) IDID = -7
      RETURN
C
...
C----------------------- END OF SUBROUTINE DCNST0 ----------------------
      END
      SUBROUTINE DDAWTS_DDASKR(NEQ,IWT,RTOL,ATOL,Y,WT,RPAR,IPAR)
C
C***BEGIN PROLOGUE  DDAWTS_DDASKR
C***REFER TO  DDASPK
C***ROUTINES CALLED  (NONE)
C***DATE WRITTEN   890101   (YYMMDD)
C***REVISION DATE  900926   (YYMMDD)
C***END PROLOGUE  DDAWTS_DDASKR
C-----------------------------------------------------------------------
...
20         CONTINUE
      RETURN
C
C------END OF SUBROUTINE DDAWTS_DDASKR----------------------------------
      END
...
C------END OF SUBROUTINE DINVWT-----------------------------------------
      END
      SUBROUTINE DDATRP_DDASKR(X,XOUT,YOUT,YPOUT,NEQ,KOLD,PHI,PSI)
C
C***BEGIN PROLOGUE  DDATRP_DDASKR
C***REFER TO  DDASPK
C***ROUTINES CALLED  (NONE)
C***DATE WRITTEN   890101   (YYMMDD)
C***REVISION DATE  900926   (YYMMDD)
C***END PROLOGUE  DDATRP_DDASKR
C
C-----------------------------------------------------------------------
C     The methods in subroutine DDSTP use polynomials
C     to approximate the solution.  DDATRP_DDASKR approximates the
...
C     DDSTP, so DDATRP_DDASKR cannot be used alone.
C
...
30       CONTINUE
      RETURN
C
C------END OF SUBROUTINE DDATRP_DDASKR---------------------------------
      END
