...
C
      ...
      WRITE (BUFERR,260) NCOMP
      CALL BASOUT(io,IOUT,BUFERR)
      WRITE (BUFERR,261) (M(IP), IP=1,NCOMP)
      CALL BASOUT(io,IOUT,BUFERR)
      GO TO 70
   60 WRITE (BUFERR,270) NCOMP
      CALL BASOUT(io,IOUT,BUFERR)
      WRITE (BUFERR,271) (M(IP), IP=1, NCOMP)
      CALL BASOUT(io,IOUT,BUFERR)
   70 WRITE (BUFERR,280) (ZETA(IP), IP=1,MSTAR)
      CALL BASOUT(io,IOUT,BUFERR)
      IF ( NFXPNT .GT. 0 ) THEN
        WRITE (BUFERR,340) NFXPNT, (FIXPNT(IP), IP=1,NFXPNT)
        CALL BASOUT(io,IOUT,BUFERR)
      ENDIF
      WRITE (BUFERR,290) K
      CALL BASOUT(io,IOUT,BUFERR)
      WRITE (BUFERR,300) (LTOL(IP), IP=1,NTOL)
      CALL BASOUT(io,IOUT,BUFERR)
      WRITE (BUFERR,310) (TOL(IP), IP=1,NTOL)
      CALL BASOUT(io,IOUT,BUFERR)
      IF (IGUESS .GE. 2) THEN
       WRITE (BUFERR,320)
       CALL BASOUT(io,IOUT,BUFERR)
...
C
  ...
  270      CONTINUE
           ...
           IF (IPRINT .LT. 0)  THEN
            WRITE (BUFERR,520) ITER, RELAX, ANORM,
     1           ANFIX, RNOLD, RNORM
            CALL BASOUT(io,IOUT,BUFERR)
           ENDIF
           GO TO 290
  280      IF (IPRINT .LT. 0) THEN
            WRITE (BUFERR,550) RELAX, ANORM, ANFIX,
     1           RNOLD, RNORM
            CALL BASOUT(io,IOUT,BUFERR)
           ENDIF
  290      ICOR = 0
...
C
  400      IF ( IPRINT .GE. 0 )                     GO TO 420
           DO 410 J = 1, MSTAR
             WRITE(BUFERR,610) J
             CALL BASOUT(io,IOUT,BUFERR)
  410      WRITE(BUFERR,620) (Z(LJ), LJ = J, NZ, MSTAR)
           CALL BASOUT(io,IOUT,BUFERR)
C
...
C
  430      IF ( IPRINT .LT. 1 )  THEN
            WRITE (BUFERR,570) ITER
            CALL BASOUT(io,IOUT,BUFERR)
           ENDIF
           GO TO 450
  440      IF( IPRINT .LT. 1 )  THEN
            WRITE(BUFERR,580) RELAX, RELMIN
            CALL BASOUT(io,IOUT,BUFERR)
           ENDIF
  450      IFLAG = -2
           NOCONV = NOCONV + 1
...
C
      NOLDP1 = NOLD + 1
      IF (IPRINT .LT. 1)  THEN
       WRITE(BUFERR,360) NOLD, (XIOLD(I), I=1,NOLDP1)
       CALL BASOUT(io,IOUT,BUFERR)
      ENDIF
...
C
      ...
      IF ( IPRINT .LT. 0 )  THEN
       WRITE(BUFERR,350) DEGEQU, NACCUM
       CALL BASOUT(io,IOUT,BUFERR)
      ENDIF
C
...
C
  ...
  320 CONTINUE
      NP1 = N + 1
      IF ( IPRINT .LT. 1 )  THEN
       WRITE(BUFERR,340) N, (XI(I),I=1,NP1)
       CALL BASOUT(io,IOUT,BUFERR)
      ENDIF
      NZ   = MSTAR * (N + 1)
      NDMZ = KD * N
...
C
   ...
   60 CONTINUE
      ...
      DO 70 J = 1,NCOMP
           MJ = LJ - 1 + M(J)
           WRITE(BUFERR,120) J, (ERREST(L), L= LJ, MJ)
           CALL BASOUT(io,IOUT,BUFERR)
           LJ = MJ + 1
   70 CONTINUE
      RETURN
...
C
   30 CONTINUE
     ...
     1                                              GO TO 40
      IF (IPRINT .LT. 1)  THEN
       WRITE(BUFERR,900) X, XI(1), XI(N+1)
       CALL BASOUT(io,IOUT,BUFERR)
      ENDIF
      IF ( X .LT. XI(1) )  X = XI(1)
