*DECK D1MACH_DDASKR
      DOUBLE PRECISION FUNCTION D1MACH_DDASKR (IDUMMY)
C***BEGIN PROLOGUE  D1MACH_DDASKR
C***PURPOSE  Compute the unit roundoff of the machine.
C***CATEGORY  R1
C***TYPE      DOUBLE PRECISION (R1MACH-S, D1MACH_DDASKR-D)
C***KEYWORDS  MACHINE CONSTANTS
C***AUTHOR  Hindmarsh, Alan C., (LLNL)
C***DESCRIPTION
C *Usage:
C        DOUBLE PRECISION  A, D1MACH_DDASKR
C        A = D1MACH_DDASKR(idummy)  [The argument is ignored.]
C
...
C     number u such that  1.0 + u .ne. 1.0.  This is computed by D1MACH_DDASKR
...
C***END PROLOGUE  D1MACH_DDASKR
C
      INTEGER IDUMMY
      DOUBLE PRECISION U, COMP
C***FIRST EXECUTABLE STATEMENT  D1MACH_DDASKR
      U = 1.0D0
 10   U = U*0.5D0
      CALL DUMSUM(1.0D0, U, COMP)
      IF (COMP .NE. 1.0D0) GO TO 10
      D1MACH_DDASKR = U*2.0D0
      RETURN
C----------------------- End of Function D1MACH_DDASKR ------------------
      END
      SUBROUTINE DUMSUM(A,B,C)
C     Routine to force normal storing of A + B, for D1MACH_DDASKR.
...
C***DESCRIPTION
C
C  Subroutines XERRWD, XSETF_DDASKR, XSETUN_DDASKR, and the function routine IXSAV,
...
C----------------------- End of Subroutine XERRWD ----------------------
      END
*DECK XSETF_DDASKR
      SUBROUTINE XSETF_DDASKR (MFLAG)
C***BEGIN PROLOGUE  XSETF_DDASKR
C***PURPOSE  Reset the error print control flag.
C***LIBRARY   MATHLIB
C***CATEGORY  R3A
C***TYPE      ALL (XSETF_DDASKR-A)
C***KEYWORDS  ERROR CONTROL
C***AUTHOR  Hindmarsh, Alan C., (LLNL)
C***DESCRIPTION
C
C   XSETF_DDASKR sets the error print control flag to MFLAG:
...
C***END PROLOGUE  XSETF_DDASKR
C
C Subroutines called by XSETF_DDASKR.. None
C Function routine called by XSETF_DDASKR.. IXSAV
C-----------------------------------------------------------------------
C**End
      INTEGER MFLAG, JUNK, IXSAV
C
C***FIRST EXECUTABLE STATEMENT  XSETF_DDASKR
      IF (MFLAG .EQ. 0 .OR. MFLAG .EQ. 1) JUNK = IXSAV (2,MFLAG,.TRUE.)
      RETURN
C----------------------- End of Subroutine XSETF_DDASKR ----------------
      END
*DECK XSETUN_DDASKR
      SUBROUTINE XSETUN_DDASKR (LUN)
C***BEGIN PROLOGUE  XSETUN_DDASKR
C***PURPOSE  Reset the logical unit number for error messages.
C***LIBRARY   MATHLIB
C***CATEGORY  R3B
C***TYPE      ALL (XSETUN_DDASKR-A)
C***KEYWORDS  ERROR CONTROL
C***DESCRIPTION
C
C   XSETUN_DDASKR sets the logical unit number for error messages to LUN.
C
C***AUTHOR  Hindmarsh, Alan C., (LLNL)
C***SEE ALSO  XERMSG, XERRWD, XERRWV
C***REFERENCES  (NONE)
...
C   930407  Corrected SEE ALSO section. (FNF)
C   930922  Made user-callable, and other cosmetic changes. (FNF)
C***END PROLOGUE  XSETUN_DDASKR
C
C Subroutines called by XSETUN_DDASKR.. None
C Function routine called by XSETUN_DDASKR.. IXSAV
C-----------------------------------------------------------------------
C**End
      INTEGER LUN, JUNK, IXSAV
C
C***FIRST EXECUTABLE STATEMENT  XSETUN_DDASKR
      IF (LUN .GT. 0) JUNK = IXSAV (1,LUN,.TRUE.)
      RETURN
C----------------------- End of Subroutine XSETUN_DDASKR ----------------
      END
*DECK IXSAV
