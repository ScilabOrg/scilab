...
//create false panel on the left to occupy the 3/4 of the screen (figure plot)
   empty_frame = uicontrol(fig_handle, "style", "frame", "constraints", c);
    //plot_frame = uicontrol(fig_handle,...
    //"style", "frame",...
    //"backgroundcolor", [1,1,1],...
    //"constraints", c,...
    //"tag", "plot_frame");

    c = createConstraints("gridbag",[1,2,1,1],[1,0.25], "both", "lower");
...
function create_param_zone(param_frame, param_list)
    //Set the layout for the param frame to a grid of the correct size
    //2 columns
    l = createLayoutOptions("grid",[ceil(size(param_list)/2), 2] );
    //   set(param_frame,"layout", "grid");
    param_frame.layout_options = l;
    param_frame.layout = "grid";

    //Creates the text and slider for each parameter
    for i= [1:size(param_list)]
        s = create_slider(param_frame, param_list(i));
    end
endfunction

//Creates a slider with the parameter defined
function slider_frame = create_slider(parent_frame, param)
    slider_frame = uicontrol(parent_frame,...
    "style", "frame",...
    "backgroundcolor", [1,1,1],...
    "layout", "gridbag");

    //Text on the left: name of the param
    c = createConstraints("gridbag",[1,1,1,1], [1/3,0],"both","right",[0,0]);

    name_p = uicontrol(slider_frame, ...
    "Style", "text", ...
    "String", param.name + " ", ...
    "BackgroundColor", [1 1 1], ...
    ...
    c = createConstraints("gridbag",[2,1,1,1], [1/3,0],"both","center");
    slider_p = uicontrol(slider_frame, ...
    "Style", "slider", ...
    "Value", param.default, ...
    "Min", param.range(1), ...
    "Max", param.range(2), ...
    ...
    c = createConstraints("gridbag",[3,1,1,1], [1/6,0],"both","center",[0,0],[100,0]);
    value_p = uicontrol(slider_frame, ...
    "Style", "text", ...
    ...
    c = createConstraints("gridbag",[4,1,1,1], [1/6,0],"both","left",[0,0],[100,0]);
    unite_p = uicontrol(slider_frame, ...
    "Style", "text", ...
    "String", param.unit, ...
    "BackgroundColor", [1 1 1], ...
    "constraints", c, ...
...
function testGui()
    axes_h = 600;
    axes_w = 800;
    f = createMainWindow([axes_h,axes_w], "testing Gui", "testGui");
    set(f, "immediate_drawing", "on");
    [plot_frame, param_frame, button_frame] = create_gui(f);
    ...
    param_list = list(y, slope, speed, gravity);
