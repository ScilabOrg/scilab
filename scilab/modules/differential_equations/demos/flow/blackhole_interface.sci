...
function exeblackhole()
    ...
    my_figure_handle = figure("tag", "main_figure", "layout", "border", "layout_options", createLayoutOptions("border"));
    clf(my_figure_handle,"reset");


    //adding the top frame for the plot
    c_gen = createConstraints("border","center", [600,600]);


    plot_frame = uicontrol(my_figure_handle,...
    "style", "frame",...
...
function draw_blackhole()
    ...
    my_handle          = findobj("tag","main_figure");
    my_plot_handle        = findobj("tag", "plot_zone");

    my_axe             = newaxes(my_plot_handle);
...
function draw_initial_point(g_r,g_t,g_V,g_Vdir,create)
    ...
    my_figure_handle = findobj("tag", "main_figure");
...
function clear_simu()
    ...
    fin=my_figure_handle.user_data
    if fin then
        my_figure_handle = findobj("tag","plot_zone");
        curAxe = gca();
        traj_handle=curAxe.children(1).children(5);
...
function createSliderFrame(parent_handler, tag_name, txt, txt_unit);
//Creates the components each sliders and gives them their tag
    c_slider = createConstraints("gridbag", [1,1,1,1], [0.1,1], "both", "center",[20,20], [50, 20]);

    c_slider.grid = [1,1,1,1];
    c_slider.weight = [0.3,1];
    text = uicontrol(parent_handler                            , ...
    "Style"             , "text"                                 , ...
    "String"            , txt                                  , ...
    "BackgroundColor"   , [1 1 1]                                , ...
    "tag"               , "text_" + tag_name,...                          
    "constraints", c_slider);

    c_slider.grid = [2,1,1,1];
    c_slider.weight = [1,1];
    slider = uicontrol(parent_handler                            , ...
    "Style"             , "slider"                               , ...
    "Value"             , 0                                 , ...
    "Min"               , 0                                      , ...
    "Max"               , 100                                      , ...
    "callback"          , "change_" + tag_name + "()"                           , ...
    "tag"               , "slider_" + tag_name,...                         
    "constraints", c_slider);

    c_slider.grid = [3,1,1,1];
    c_slider.weight = [0.3,1];
    value = uicontrol(parent_handler                             , ...
    "Style"             , "text"                                 , ...
    "String"            , string(0)                            , ...
    "BackgroundColor"   , [1 1 1]                                , ...
    "tag"               , "value_" + tag_name,...                          
    ...
    c_slider.grid = [4,1,1,1];
    c_slider.weight = [0.1,1];
    unite = uicontrol(parent_handler                             , ...
    "Style"             , "text"                                 , ...
    "String"            , txt_unit                                , ...
    "BackgroundColor"   , [1 1 1]                                , ...
    "tag"               , "unite_" + tag_name,...                          
