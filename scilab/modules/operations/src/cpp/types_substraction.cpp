...
int SubstractSparseToSparse(Sparse* sp1, Sparse* sp2, Sparse** pSpRes)
{
     return (*pSpRes=sp1->substract(*sp2)) ? 0 : 1;
}
int SubstractDoubleToSparse(Double* d, Sparse* sp, Double** pDRes)
{
    Double tmp(sp->rows_get(), sp->cols_get(), sp->isComplex()); /* TODO ensure destructor is ok */
    sp->fillDouble(tmp);
    return SubstractDoubleToDouble(&tmp, d, pDRes);
}
int SubstractSparseToDouble(Sparse* sp, Double* d, Double** pDRes)
{
    Double tmp(sp->rows_get(), sp->cols_get(), sp->isComplex()); /* TODO ensure destructor is ok */
    sp->fillDouble(tmp);
    return SubstractDoubleToDouble(d, &tmp, pDRes);
}
