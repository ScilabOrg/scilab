 ...
 */
 ...
 {
     return (*pSpRes=sp1->add(*sp2)) ? 0 : 1;
 }
 int AddSparseToDouble(Sparse* sp, Double* d, Double** pDRes)
 {
     Double tmp(sp->rows_get(), sp->cols_get(), sp->isComplex()); /* TODO ensure destructor is ok */
     sp->fillDouble(tmp);
     return AddDoubleToDouble(&tmp, d, pDRes);
 }
 int AddDoubleToSparse(Double* d, Sparse* sp, Double** pDRes)
 {/* uses commutativity */
