...
int AddDoubleToPoly(MatrixPoly *_pPoly, Double *_pDouble, MatrixPoly ** _pPolyOut)
{
	...
	{//cas balaise
		...
		{
			piRank[i] = _pPoly->getPoly(0)->getRank();
		}

		(*_pPolyOut) = new MatrixPoly(_pPoly->getVariableName(), _pDouble->getRows(), _pDouble->getCols(), piRank);
		...
		{
			Poly *pInPoly			= _pPoly->getPoly(0);
			Poly *pOutPoly		= (*_pPolyOut)->getPoly(i);
...
int AddPolyToPoly(MatrixPoly* _pPoly1, MatrixPoly* _pPoly2, MatrixPoly ** _pPolyOut)
{
	...
	if(_pPoly1->getRows() == _pPoly2->getRows() && _pPoly1->getCols() == _pPoly2->getCols())
	{
		...
		(*_pPolyOut) = new MatrixPoly(_pPoly2->getVariableName(), _pPoly1->getRows(), _pPoly1->getCols(), pRank);
		...
		if(bComplex1 == false && bComplex2 == false)
		{
			for(int i = 0 ; i < _pPoly1->getSize() ; i++)
			{
				iAddRealPolyToRealPoly(
					_pPoly1->getPoly(i)->getCoef()->getReal(), pRank1[i],
					_pPoly2->getPoly(i)->getCoef()->getReal(), pRank2[i],
					(*_pPolyOut)->getPoly(i)->getCoef()->getReal(), pRank[i]);
			}
		}
		else if(bComplex1 == false && bComplex2 == true)
		{
			for(int i = 0 ; i < _pPoly1->getSize() ; i++)
			{
				iAddRealPolyToComplexPoly(
					_pPoly1->getPoly(i)->getCoef()->getReal(), pRank1[i],
					_pPoly2->getPoly(i)->getCoef()->getReal(), _pPoly2->getPoly(i)->getCoef()->getImg(), pRank2[i],
					(*_pPolyOut)->getPoly(i)->getCoef()->getReal(), (*_pPolyOut)->getPoly(i)->getCoef()->getImg(), pRank[i]);
			}
		}
		else if(bComplex1 == true && bComplex2 == false)
		{
			for(int i = 0 ; i < _pPoly1->getSize() ; i++)
			{
				iAddRealPolyToComplexPoly(
					_pPoly2->getPoly(i)->getCoef()->getReal(), pRank2[i],
					_pPoly1->getPoly(i)->getCoef()->getReal(), _pPoly1->getPoly(i)->getCoef()->getImg(), pRank1[i],
					(*_pPolyOut)->getPoly(i)->getCoef()->getReal(), (*_pPolyOut)->getPoly(i)->getCoef()->getImg(), pRank[i]);
			}
		}
		else if(bComplex1 == true && bComplex2 == true)
	...
	{//size(p1) == 1
		...
		(*_pPolyOut) = new MatrixPoly(_pPoly2->getVariableName(), _pPoly2->getRows(), _pPoly2->getCols(), pRank);
		if(_pPoly1->isComplex() || _pPoly2->isComplex())
		{
			(*_pPolyOut)->setComplex(true);
		}
		...
		//Result P1(0) + P2(i)
		Double *pCoef1	= _pPoly1->getPoly(0)->getCoef();
		double *p1R			= pCoef1->getReal();
		double *p1I			= pCoef1->getImg();
		for(int i = 0 ; i < _pPoly2->getSize() ; i++)
		{
			Double *pCoef2	= _pPoly2->getPoly(i)->getCoef();
			double *p2R			= pCoef2->getReal();
			double *p2I			= pCoef2->getImg();

			Double *pCoefR	= (*_pPolyOut)->getPoly(i)->getCoef();
	...
	{//size(p2) == 1
		...
		(*_pPolyOut) = new MatrixPoly(_pPoly1->getVariableName(), _pPoly1->getRows(), _pPoly1->getCols(), pRank);
		if(_pPoly1->isComplex() || _pPoly2->isComplex())
		{
			(*_pPolyOut)->setComplex(true);
		}
		...
		//Result P1(i) + P2(0)
		Double *pCoef2	= _pPoly2->getPoly(0)->getCoef();
		double *p2R			= pCoef2->getReal();
		double *p2I			= pCoef2->getImg();

		for(int i = 0 ; i < _pPoly1->getSize() ; i++)
		{
			Double *pCoef1	= _pPoly1->getPoly(i)->getCoef();
			double *p1R			= pCoef1->getReal();
			double *p1I			= pCoef1->getImg();
...
int AddStringToString(String *_pString1, String *_pString2, String **_pStringOut)
{
	...
	{//concat pL with each element of pR
		...
		int iCommonLen	=	(int)wcslen(_pString1->getString(0,0));

		for(int i = 0 ; i < _pString2->getRows() ; i++)
		{
			for(int j = 0 ; j < _pString2->getCols() ; j++)
			{
				int iLen	= (int)wcslen(_pString2->getString(i, j));
				wchar_t* psz = new wchar_t[iLen + iCommonLen + 1];
				memset(psz, 0x00, (iLen + iCommonLen + 1) * sizeof(wchar_t));
				memcpy(psz, _pString1->getString(0,0), iCommonLen * sizeof(wchar_t));
				memcpy(psz + iCommonLen, _pString2->getString(i, j), iLen * sizeof(wchar_t));
				(*_pStringOut)->setString(i, j, psz);
				delete[] psz;
			}
		}
	}
	...
	{//concat each element of pL with pR
		...
		int iCommonLen	=	(int)wcslen(_pString2->getString(0,0));

		for(int i = 0 ; i < _pString1->getRows() ; i++)
		{
			for(int j = 0 ; j < _pString1->getCols() ; j++)
			{
				int iLen = (int)wcslen(_pString1->getString(i, j));
				wchar_t* psz = new wchar_t[iLen + iCommonLen + 1];
				memset(psz, 0x00, (iLen + iCommonLen + 1) * sizeof(wchar_t));

				memcpy(psz, _pString1->getString(i, j), iLen * sizeof(wchar_t));
				memcpy(psz + iLen, _pString2->getString(0,0), iCommonLen * sizeof(wchar_t));
	...
	{//concat pL with pR element wise
		...
		for(int i = 0 ; i < _pString1->getRows() ; i++)
		{
			for(int j = 0 ; j < _pString1->getCols() ; j++)
			{
				int iLenL	= (int)wcslen(_pString1->getString(i, j));
				int iLenR	= (int)wcslen(_pString2->getString(i, j));
				wchar_t* psz = new wchar_t[iLenL + iLenR + 1];
				memset(psz, 0x00, (iLenL + iLenR + 1) * sizeof(wchar_t));

				memcpy(psz          , _pString1->getString(i, j), iLenL * sizeof(wchar_t));
				memcpy(psz + iLenL  , _pString2->getString(i, j), iLenR * sizeof(wchar_t));
