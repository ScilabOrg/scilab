...
int DividePolyByDouble(MatrixPoly* _pPoly, Double* _pDouble, MatrixPoly** _pPolyOut)
{
    ...
    piRank = new int[1];
    piRank[0]	= _pPoly->getPoly(0)->getRank();
    }
    else */if(bScalar2)
    {
    ...
    else if(bScalar1)
    {
        ...
        pTemp = new MatrixPoly(_pPoly->getVariableName(), iRowResult, iRowResult, piRank);
        if(bComplex1 || bComplex2)
        {
            pTemp->setComplex(true);
        }

        Double *pdblData = _pPoly->getPoly(0)->getCoef();
        for(int i = 0 ; i < iRowResult ; i++)
        {
    ...
    */
    ...
    else if(bScalar1)
    {
        ...
        for(int i = 0 ; i < pTemp->getPoly(0)->getRank() ; i++)
        {
