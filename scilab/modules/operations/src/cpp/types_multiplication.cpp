...
int MultiplyDoubleByPoly(Double* _pDouble, MatrixPoly* _pPoly, MatrixPoly** _pPolyOut)
{
    ...
    if(bScalar1)
    {
        ...
        {
            piRank[i] = _pPoly->getPoly(i)->getRank();
        }
    }
    else if (bScalar2)
    {
    ...
    (*_pPolyOut) = new MatrixPoly(_pPoly->getVariableName(), iRowResult, iColResult, piRank);
    delete[] piRank;
    ...
    else if(_pPoly->getCols() == _pDouble->getRows())
    {
        ...
        {//for each col of _pDouble
            ...
            {//for each rows of _pDouble / cols of _pPoly
                ...
                {
                    Double *pPolyCoef = _pPoly->getPoly(iRow1, iRow2)->getCoef();
                    ...
                    AddDoubleToDouble(TimeDouble, (*_pPolyOut)->getPoly(iRow1, iCol2)->getCoef(), &pAddDouble);
...
int MultiplyPolyByPoly(MatrixPoly* _pPoly1, MatrixPoly* _pPoly2, MatrixPoly** _pPolyOut)
{
    ...
    if(_pPoly1->getSize() == 1 && _pPoly2->getSize() == 1)
    {
        ...
        piRank = new int[1];
        piRank[0] = _pPoly1->getPoly(0)->getRank() + _pPoly2->getPoly(0)->getRank() - 1;
    }
    else if(_pPoly1->getSize() == 1)
    {
        ...
        for(int i = 0 ; i < iRowResult * iColResult ; i++)
        {
            piRank[i] = _pPoly1->getPoly(0)->getRank() + _pPoly2->getPoly(i)->getRank() - 1;
        }
    }
    else if (_pPoly2->getSize() == 1)
    {
        ...
        for(int i = 0 ; i < iRowResult * iColResult ; i++)
        {
            piRank[i] = _pPoly2->getPoly(0)->getRank() * _pPoly1->getPoly(i)->getRank();
        }
    }
    ...
    (*_pPolyOut) = new MatrixPoly(_pPoly1->getVariableName(), iRowResult, iColResult, piRank);
    ...
    {//poly1(0) * poly2(0)
        if(bComplex1 == false && bComplex2 == false)
        {
            Poly *pPoly1		= _pPoly1->getPoly(0);
            Poly *pPoly2		= _pPoly2->getPoly(0);
            Poly *pPolyOut	= (*_pPolyOut)->getPoly(0);
    ...
    {//poly1(0) * poly2(n)
        ...
        if(bComplex1 == false && bComplex2 == false)
        {
            ...
            {
                Poly *pPoly2		= _pPoly2->getPoly(iPoly);
                Poly *pPolyOut	= (*_pPolyOut)->getPoly(iPoly);
    ...
    {//poly1(n) * poly2(0)
        ...
        if(bComplex1 == false && bComplex2 == false)
        {
            ...
            {
                Poly *pPoly1		= _pPoly1->getPoly(iPoly);
                Poly *pPolyOut	= (*_pPolyOut)->getPoly(iPoly);
    ...
    {// matrix by matrix
        ...
        {
            double *pReal	= NULL;
            Poly *pTemp		= new Poly(&pReal, (*_pPolyOut)->getMaxRank());

            for(int iRow = 0 ; iRow < _pPoly1->getRows() ; iRow++)
            {
                for(int iCol = 0 ; iCol < _pPoly2->getCols() ; iCol++)
                {
                    Poly *pResult = (*_pPolyOut)->getPoly(iRow, iCol);
                    pResult->getCoef()->setZeros();

                    for(int iCommon = 0 ; iCommon < _pPoly1->getCols() ; iCommon++)
                    {
                        Poly *pL			= _pPoly1->getPoly(iRow, iCommon);
                        Poly *pR			= _pPoly2->getPoly(iCommon, iCol);
        ...
        else if(bComplex1 == false && bComplex2 == true)
        {
            double *pReal	= NULL;
            double *pImg	= NULL;
            Poly *pTemp		= new Poly(&pReal, &pImg, (*_pPolyOut)->getMaxRank());
