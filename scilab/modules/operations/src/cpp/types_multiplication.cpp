...
int MultiplyDoubleBySparse(Double* d, Sparse* sp, Double** pDRes)
{
    Double tmp(sp->rows_get(), sp->cols_get(), sp->isComplex()); /* TODO ensure destructor is ok */
    sp->fillDouble(tmp);
    return MultiplyDoubleByDouble(d, &tmp, pDRes);
}
int MultiplySparseByDouble(Sparse* sp, Double* d, Double** pDRes)
{
    Double tmp(sp->rows_get(), sp->cols_get(), sp->isComplex()); /* TODO ensure destructor is ok */
    sp->fillDouble(tmp);
    return MultiplyDoubleByDouble(&tmp, d, pDRes);
}
