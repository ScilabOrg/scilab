...
wchar_t* findChars(wchar_t*, BOOL*); // Use to find what is inside the [] when the format is %[...].
   ...
   // wchar_t* wcsData            = _wcsData;
    ...
    while(i < (int)wcslen(_wcsFormat))
    {
        ...
        else switch(_wcsFormat[i])
        {
            ...
            case L'c' :
            {
                if(_iIterrator == 0 && !bStar) // Create and initialize the container only the first time.
                {
            ...
            case L's' :
            {
                ...
                if(wcsData != NULL && wcsData[0] != L'\0')
                {
                    ...
                    wcsRes = wcstok(wcsTemp, seps, &wcsTemp); // the seps[] found is replaced by the '\0' char.
            ...
            case L'o' : base += 8; // 8 = 8 :D
            {
                ...
                if(wcsData != NULL && wcsData[0] != L'\0')
                {
                    ...
                    BOOL bOut = FALSE;
                    while(!iswdigit(wcsData[0]) && !bOut)
                    {
                        switch(wcsData[0])
                        {
                            case L'-' : bNegatif = TRUE;
                            case L'+' :
                            case L' ' :
                            case L'\n':
                            case L'\t': wcsData++;
            ...
            case L'f' :
            {
                ...
                {
                    double dSingleData;
                    while(wcsData != NULL && !iswdigit(wcsData[0]))
                    {
                        switch(wcsData[0])
                        {
                            case L'-' : bNegatif = TRUE;
