        ...
        else switch(_wcsFormat[i])
        {
            ...
            case L'o' : base += 8; // 8 = 8 :D
            {
                if(_iIterrator == 0 && !bStar)
                {
                    switch(wcsLLH)
                    {
                        case L'h' : _pITOut->push_back(new types::Int16(dims,dimsArray));
                        break;
                        case L'l' :
                        case L'L' : _pITOut->push_back(new types::Int64(dims,dimsArray));
                        break;
                        default :  _pITOut->push_back(new types::Int32(dims,dimsArray));
                    }
                }
