...
#include "PATH_MAX.h"
/*--------------------------------------------------------------------------*/
static unsigned isDirSeparator(const char c);
...
char * get_full_path(char * _FullPath, const char * _Path, size_t _SizeInBytes)
{
    ...
    int lenFullPath = 0;
    int haveFileSep = ((lenPath > 1) && ((_Path[lenPath - 1] == '/') || (_Path[lenPath-1] == '\\')));
    int addFileSep = 0;
    ...
    addFileSep = ((lenFullPath > 1) && (!(_FullPath[lenFullPath-1] == '/') && !(_FullPath[lenFullPath-1] == '\\')) && haveFileSep);
    if (addFileSep)
    {
