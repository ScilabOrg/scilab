...
#ifdef _MSC_VER
            FILEINFO_ARRAY = fileinfo_WindowsW(pwstExp, _piErr);
#else
            FILEINFO_ARRAY = fileinfo_OthersW(pwstExp, _piErr);
#endif
...
double * fileinfoW(wchar_t* _pwstFilename, int *_piErr)
{
...
//	}
//
//	expandedpath = expandPathVariableW(_pwstFilename);
//	if (expandedpath)
//	{
//#ifdef _MSC_VER
//		FILEINFO_ARRAY = fileinfo_WindowsW(expandedpath, _piErr);
//#else
//		FILEINFO_ARRAY = fileinfo_OthersW(expandedpath, _piErr);
//#endif
//		FREE(expandedpath);
//		expandedpath = NULL;
//	}
...
double * filesinfo(char **filenames, int dim_filenames, int *ierrs)
{
	...
	if (dim_filenames > 0)
	{
		...
		for (i = 0; i < dim_filenames; i++)
		{
			int k = 0;
			int ierr = 0;
			double * FILEINFO_ARRAY = NULL;//fileinfo(filenames[i],&ierr);
			if (FILEINFO_ARRAY == NULL)
			{
...
/*--------------------------------------------------------------------------*/
#else
static double* fileinfo_OthersW(wchar_t* _pstFilename, int* _piErr)
{
	struct stat buf;
	double *FILEINFO_ARRAY = NULL;
	int result = 0;
