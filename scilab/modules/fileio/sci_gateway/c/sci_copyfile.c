...
int sci_copyfile(char *fname,unsigned long fname_len)
{
    ...
    if (getAllocatedSingleWideString(pvApiCtx, piAddressVarOne, &pStVarOne)) return 0;
    if (getAllocatedSingleWideString(pvApiCtx, piAddressVarTwo, &pStVarTwo)) 
    {
        if (pStVarOne) {freeAllocatedSingleWideString(pStVarOne); pStVarOne = NULL;}
        return 0;
    }

    if (pStVarOne == NULL)
    {
        if (pStVarTwo) {freeAllocatedSingleWideString(pStVarTwo); pStVarTwo = NULL;}
...
#define BUFFER_SIZE 1024
    ...
    {
        if (sciError) {FREE(sciError); sciError = NULL;}
        return 0;
    }
