...
Function::ReturnValue sci_pathconvert(typed_list &in, int _iRetCount, typed_list &out)
{
    ...
    if(in.size() > 3)
    {
        ...
        wchar_t* pwstType = in[3]->getAsString()->getString(0);
    ...
    if(in.size() > 2)
    {
        if(in[2]->isBool() == false || in[2]->getAsBool()->getSize() != 1)
        {
    ...
    if(in.size() > 1)
    {
        if(in[1]->isBool() == false || in[1]->getAsBool()->getSize() != 1)
        {
            ScierrorW(999, _W("%ls: Wrong type for input argument #%d: A boolean expected.\n"), L"pathconvert", 2);
            return Function::Error;
        }
    ...
    wchar_t** pStr  = pOut->getString();


    for(int i = 0 ; i < pS->getSize() ; i++)
    {
        pStr[i] = pathconvertW(pS->getString(i), iPathTrail, iPathExpand, PType);
    }
