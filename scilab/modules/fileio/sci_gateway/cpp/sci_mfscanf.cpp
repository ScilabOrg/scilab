...
Function::ReturnValue sci_mfscanf(types::typed_list &in, int _iRetCount, types::typed_list &out)
{
...
// get data
    if(bIsStd == FALSE)
    {
        ...
        if(iErr)
        {
            switch(iErr)
            {
                ...
                return types::Function::Error;
                case MGETL_ERROR :
                    ScierrorW(999, _W("%ls: Error in %ls function.\n"), L"mfscanf", L"mgetl");
                return types::Function::Error;
            }
        }
    }
    else
    {
        ...
        {
            char toto[1024];
            std::cin.getline(toto, 1024);
            wcsRead[i] = (wchar_t*)MALLOC(1024 * sizeof(wchar_t));
            wcsRead[i] = to_wide_string(toto);
        }
    }
    ...
    {
        // perform scanf  
        int error = scilab_sscanf(wcsFormat, wcsRead[i], i, iLinesRead, pIT);
        FREE(wcsRead[i]);
        if(error)
        {
            FREE(wcsRead);
