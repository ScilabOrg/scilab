...
// get data
    ...
    {
        // perform scanf
        iCountDataRead = scilab_sscanf(wcsFormat, pStrRead->get(i), i, iNiter, pIT);
        if(iCountDataRead < -1) // -1 = EOL
        {
    ...
    if(_iRetCount > 1)
    {
        ...
        {
            ScierrorW(77, _W("%ls: Wrong number of output argument(s): %d expected.\n"), L"msscanf", sizeOfVector+1);
            return types::Function::Error;
        }
