...
Function::ReturnValue sci_mgetl(typed_list &in, int _iRetCount, typed_list &out)
{
    ...
    if(in[0]->getType() == InternalType::RealDouble && in[0]->getAs<Double>()->getSize() == 1)
    {
        iFileID = static_cast<int>(in[0]->getAs<Double>()->getReal()[0]);
    }
    else if(in[0]->getType() == InternalType::RealString && in[0]->getAsString()->getSize() == 1)
    {
        wchar_t *expandedFileName = expandPathVariableW(in[0]->getAsString()->getString(0));
