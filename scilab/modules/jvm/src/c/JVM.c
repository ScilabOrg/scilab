...
BOOL finishJVM(void)
{
    ...
    if (!IsFromJava()) { /* Do not do that when Scilab is called from javasci */
        JNIEnv * curEnv = NULL;
        (*jvm_SCILAB)->AttachCurrentThread(jvm_SCILAB, (void **)&curEnv, NULL);
        jclass cls = (*curEnv)->FindClass(curEnv, "java/lang/System");
        
        jmethodID exitID = (*curEnv)->GetStaticMethodID(curEnv, cls, "exit", "(I)V" ) ;
        /* Kill the jvm in using System.exit(0) to be sure the shutdown hooks will be executed,
           it is required to save the docking configuration */
        (*curEnv)->CallStaticVoidMethod(curEnv, cls, exitID, 0);
    }
