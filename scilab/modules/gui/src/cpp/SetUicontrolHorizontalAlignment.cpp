...
int SetUicontrolHorizontalAlignment(sciPointObj * sciObj, size_t stackPointer, int valueType, int nbRow, int nbCol)
{
    ...
    if (valueType == sci_strings)
    {
        ...
        if (pUICONTROL_FEATURE(sciObj)->style != SCI_UIFRAME
            ...
            && pUICONTROL_FEATURE(sciObj)->style != SCI_POPUPMENU && pUICONTROL_FEATURE(sciObj)->style != SCI_LISTBOX)
        {
            CallScilabBridge::setWidgetHorizontalAlignment(getScilabJavaVM(), pUICONTROL_FEATURE(sciObj)->hashMapIndex, alignment);
        }
