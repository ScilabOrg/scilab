...
int SetUicontrolPosition(sciPointObj * sciObj, size_t stackPointer, int valueType, int nbRow, int nbCol)
{
    ...
    if (pUICONTROL_FEATURE(sciObj)->style == SCI_UIFRAME)   /* Frame style uicontrols */
    {
        ...
        if (parent != NULL && sciGetEntityType(parent) == SCI_UICONTROL)
        {
            ...
            returnValues = CallScilabBridge::getFramePosition(getScilabJavaVM(), pUICONTROL_FEATURE(parent)->hashMapIndex);
            xInt = returnValues[0] + xInt;
            yInt = returnValues[1] + yInt;
        }

        CallScilabBridge::setFramePosition(getScilabJavaVM(), pUICONTROL_FEATURE(sciObj)->hashMapIndex, xInt, yInt, widthInt, heightInt);
    }
    ...
    else                        /* All other uicontrol styles */
    {
        ...
        CallScilabBridge::setWidgetPosition(getScilabJavaVM(), pUICONTROL_FEATURE(sciObj)->hashMapIndex, xInt, yInt, widthInt, heightInt);
        ...
        if (pUICONTROL_FEATURE(sciObj)->style == SCI_SLIDER)
        {
            ...
            {
                CallScilabBridge::setSliderHorizontal(getScilabJavaVM(), pUICONTROL_FEATURE(sciObj)->hashMapIndex);
            }
            else                /* Vertical ScrollBar */
            {
                CallScilabBridge::setSliderVertical(getScilabJavaVM(), pUICONTROL_FEATURE(sciObj)->hashMapIndex);
            }
        }
    }
