...
int SetUicontrolString(sciPointObj * sciObj, size_t stackPointer, int valueType, int nbRow, int nbCol)
{
    ...
    if (sciGetEntityType(sciObj) == SCI_UICONTROL)
    {
        ...
        case SCI_UIFRAME:
            ...
            CallScilabBridge::setFrameText(getScilabJavaVM(), pUICONTROL_FEATURE(sciObj)->hashMapIndex, getStringMatrixFromStack(stackPointer)[0]);
        ...
        case SCI_LISTBOX:
            ...
            else
            {
                CallScilabBridge::setListBoxText(getScilabJavaVM(),
                                                 pUICONTROL_FEATURE(sciObj)->hashMapIndex, getStringMatrixFromStack(stackPointer), nbCol * nbRow);
                return SET_PROPERTY_SUCCEED;
            }
        case SCI_POPUPMENU:
            // String can be single value or vector of character
        ...
        default:
            ...
            // Send the label to Java
            CallScilabBridge::setWidgetText(getScilabJavaVM(), pUICONTROL_FEATURE(sciObj)->hashMapIndex, getStringMatrixFromStack(stackPointer)[0]);
            return SET_PROPERTY_SUCCEED;
        }
    }
    else
