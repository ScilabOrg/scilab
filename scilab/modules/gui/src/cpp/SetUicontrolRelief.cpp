...
int SetUicontrolRelief(sciPointObj * sciObj, size_t stackPointer, int valueType, int nbRow, int nbCol)
{
    ...
    if (valueType == sci_strings)
    {
        ...
        case SCI_UIFRAME:
            ...
            CallScilabBridge::setFrameRelief(getScilabJavaVM(), pUICONTROL_FEATURE(sciObj)->hashMapIndex, relief);
            return SET_PROPERTY_SUCCEED;
        default:               /* All Widgets */
            // Change the relief of the Java object
            CallScilabBridge::setWidgetRelief(getScilabJavaVM(), pUICONTROL_FEATURE(sciObj)->hashMapIndex, relief);
            return SET_PROPERTY_SUCCEED;
        }
    }
    else
