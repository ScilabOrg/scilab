...
int SetUiobjectCallback(sciPointObj * sciObj, size_t stackPointer, int valueType, int nbRow, int nbCol)
{
    ...
    if (sciGetEntityType(sciObj) == SCI_UIMENU)
    {
        ...
        CallScilabBridge::setWidgetCallback(getScilabJavaVM(), pUIMENU_FEATURE(sciObj)->hashMapIndex, cbString, cbType);
    ...
    else if (sciGetEntityType(sciObj) == SCI_UICONTROL)
    {
        ...
        {
            CallScilabBridge::setFrameCallback(getScilabJavaVM(), pUICONTROL_FEATURE(sciObj)->hashMapIndex, cbString, cbType);
        }
        else                    /* All other uicontrol styles */
        {
            CallScilabBridge::setWidgetCallback(getScilabJavaVM(), pUICONTROL_FEATURE(sciObj)->hashMapIndex, cbString, cbType);
        }
