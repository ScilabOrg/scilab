...
int SetUicontrolFontName(sciPointObj * sciObj, size_t stackPointer, int valueType, int nbRow, int nbCol)
{
    ...
    if (sciGetEntityType(sciObj) == SCI_UICONTROL)
    {
        ...
        {
            CallScilabBridge::setFrameFontName(getScilabJavaVM(), pUICONTROL_FEATURE(sciObj)->hashMapIndex, getStringFromStack(stackPointer));
        }
        else                    /* All other uicontrol styles */
        {
            CallScilabBridge::setWidgetFontName(getScilabJavaVM(), pUICONTROL_FEATURE(sciObj)->hashMapIndex, getStringFromStack(stackPointer));
        }
