...
public class SwingScilabFontChooser extends JDialog implements SimpleFontChooser {
	...
	public SwingScilabFontChooser(Font font, boolean boldItalicEnable) {
		...
		scinotesCompatibleCheckbox = new JCheckBox("SciNotes compatible", false);
		attributes.add(scinotesCompatibleCheckbox);
		scinotesCompatibleCheckbox.addItemListener(new ItemListener() {

			public void itemStateChanged(ItemEvent e) {
				setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
				if (scinotesCompatibleCheckbox.isSelected()) {
					((DefaultListModel) fontNameList.getModel()).removeAllElements();
					for (int fontIndex = 0; fontIndex < availableFontNames.length; fontIndex++) {
				...
				boldCheckbox.setEnabled(scinotesCompatibleCheckbox.isSelected());
				italicCheckbox.setEnabled(scinotesCompatibleCheckbox.isSelected());
				monospacedCheckbox.setEnabled(scinotesCompatibleCheckbox.isSelected());
				setCursor(Cursor.getPredefinedCursor(Cursor.DEFAULT_CURSOR));
			}
		...
		/* Manage boldItalicEnable option */
		boldCheckbox.setEnabled(boldItalicEnable);
		italicCheckbox.setEnabled(boldItalicEnable);
		monospacedCheckbox.setEnabled(boldItalicEnable);
		scinotesCompatibleCheckbox.setEnabled(!boldItalicEnable);
