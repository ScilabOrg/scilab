...
public class DataProperties extends BaseProperties {
    ...
    */
    public void dataPropertiesComponents() {
        ...
        Object[][] data = getData();
        boolean is3D = is3D(data);
        layout.addDataDialog(dataTableDialog, dataScroll, dataTable, append, delete, data, currentpolyline, is3D);
        ...
        cData.setText(data.length + "x" + (is3D==true ? "3" : "2"));
    ...
    */
    private void dataTableEvent(TableModelEvent evt) {
        if(dataTable.getSelectedColumn()==-1){
        } else {
