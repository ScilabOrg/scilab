...
public class LookAndFeelManager {
    ...
    private transient Font[] registered;
    private boolean ret;
    ...
    private void registerEmbeddedFonts() throws FontFormatException, IOException {
        ...
        String[] fontNames = { "DroidSans.ttf", "DroidSansMono.ttf", "DroidSans-Bold.ttf"};
        File[] possibleDirs = {
        ...
        // register fonts
        registered = new Font[fontNames.length];
        for (int i = 0; i < fontNames.length; i++) {
            File f  = new File(dir, fontNames[i]);
            if (f.canRead()) {
                registered[i] = Font.createFont(Font.TRUETYPE_FONT, f);
                ge.registerFont(registered[i]);
            }
        }
    ...
    private void initComponentDefaults(UIDefaults table) {
        if (registered == null || registered.length < 2) {
            return;
        }

        Font defaultFont = registered[0];
        Font defaultMonospacedFont = registered[1];
