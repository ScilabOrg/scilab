...
public class EntityPicker {
    ...
    */
    ...
    public AxesHandler.axisTo pickLabel(String figure, Integer[] pos) {
        ...
        double[] labelCoord, coord1, coord2;
        ...
        Double[] axesBounds = (Double[])GraphicController.getController().getProperty(axes, GraphicObjectProperties.__GO_AXES_BOUNDS__);
        Integer[] axesSize = (Integer[])GraphicController.getController().getProperty(figure, GraphicObjectProperties.__GO_AXES_SIZE__);
        Integer[] delta = { (int)( axesSize[0] * axesBounds[2] * 0.2), (int)(axesSize[1] * axesBounds[3] * 0.2)};
        for (Integer i = 0; i < 3; i++) {
            
            labelPosition = (Double[])GraphicController.getController().getProperty(label[i], GraphicObjectProperties.__GO_POSITION__);
            point[0] =  labelPosition[0];
            point[1] =  labelPosition[1];
            point[2] =  0.0;
            labelCoord = AxesDrawer.compute2dViewCoordinates(AxesHandler.getAxesFromUid(axes), point);
            point[0] = 1.0 * (pos[0] - delta[0] / 2);
            point[1] = 1.0 * (pos[1] - delta[1] / 2);
            point[2] = 1.0;
            coord1 = CallRenderer.get2dViewFromPixelCoordinates(axes, point);
            point[0] = 1.0 * (pos[0] + delta[0] / 2);
            point[1] = 1.0 * (pos[1] + delta[1] / 2);
            point[2] = 1.0;
            coord2 = CallRenderer.get2dViewFromPixelCoordinates(axes, point);
            if ( (labelCoord[0] >= coord1[0] && labelCoord[0] <= coord2[0]) || (labelCoord[0] <= coord1[0] && labelCoord[0] >= coord2[0])) {
                if((labelCoord[1] >= coord1[1] && labelCoord[1] <= coord2[1]) || (labelCoord[1] <= coord1[1] && labelCoord[1] >= coord2[1])) {
                    switch(i) {
                        case 0:
