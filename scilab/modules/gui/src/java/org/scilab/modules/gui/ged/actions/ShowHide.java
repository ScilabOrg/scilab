    ...
    */
    ...
    public static void axes(boolean hide) {
        try {
            ...
            org.scilab.modules.gui.ged.graphic_objects.axes.Font.getInstance().setVisibility(!hide);
    ...
    public static void checkAllButtons(String type) {
        try {
            if("axes".equals(type)) {
                boolean BP = org.scilab.modules.gui.ged.graphic_objects.axes.BaseProperties.getInstance().getStatus();
                boolean BX = org.scilab.modules.gui.ged.graphic_objects.axes.Box.getInstance().getStatus();
                boolean CM = org.scilab.modules.gui.ged.graphic_objects.axes.Camera.getInstance().getStatus();
                boolean FT = org.scilab.modules.gui.ged.graphic_objects.axes.Font.getInstance().getStatus();
                boolean LA = org.scilab.modules.gui.ged.graphic_objects.axes.Label.getInstance().getStatus();
                boolean PO = org.scilab.modules.gui.ged.graphic_objects.axes.Position.getInstance().getStatus();
                if (BP && BX && CM && FT && LA && PO)
                    setStatus(false);
                else if (!BP && !BX && !CM && !FT && !LA && !PO)
                    setStatus(true);
