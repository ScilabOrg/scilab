            ...
            if (VarType(1) != sci_handles)
            {
                if (VarType(1) == sci_matrix)
                {
                    ...
                    if (nbRow * nbCol == 1)
                    {
                        ...
                        if ((pParent == NULL) || (sciGetEntityType(pParent) != SCI_FIGURE)
                            && !((sciGetEntityType(pParent) == SCI_UICONTROL) && (pUICONTROL_FEATURE(pParent)->style == SCI_UIFRAME)))
                        {
        ...
        for (inputIndex = 1; inputIndex < NBPROPERTIES; inputIndex++)   /* Style has already been set */
        {
            if (propertiesValuesIndices[inputIndex] != NOT_FOUND)
            {
                if (inputIndex == 21 || inputIndex == 23)   /* User data settings */
                {
                    ...
                    nbCol = -1;
                    setStatus =
                        callSetProperty(sciGetPointerFromHandle(GraphicHandle), stkAdr, VarType(propertiesValuesIndices[inputIndex]), nbRow, nbCol,
                ...
                else            /* All other properties */
                {
                    ...
                    case sci_matrix:
                        ...
                        setStatus =
                            callSetProperty(sciGetPointerFromHandle(GraphicHandle), stkAdr, sci_matrix, nbRow, nbCol,
                                            (char *)propertiesNames[inputIndex]);
                        break;
                    case sci_strings:
                        if (inputIndex == 4)    /* Index for String property: Can be mon than one character string */
                        {
                            ...
                            setStatus =
                                callSetProperty(sciGetPointerFromHandle(GraphicHandle), (size_t) stkAdrForStrings, sci_strings, nbRow, nbCol,
                                                (char *)propertiesNames[inputIndex]);
                        ...
                        else
                        {
                            ...
                            setStatus =
                                callSetProperty(sciGetPointerFromHandle(GraphicHandle), stkAdr, sci_strings, nbRow, nbCol,
                                                (char *)propertiesNames[inputIndex]);
                        }
                        break;
                    case sci_handles:
                        ...
                        setStatus =
                            callSetProperty(sciGetPointerFromHandle(GraphicHandle), stkAdr, sci_handles, nbRow, nbCol,
                                            (char *)propertiesNames[inputIndex]);
                        break;
                    default:
