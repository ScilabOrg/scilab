...
function [] = bode(varargin)
    ...
    if rhs == 0 then
        ...
        bode([h1; h2], 0.01, 100, ["h1"; "h2"], "radasymp");
        return;
    end
    ...
    if type(varargin($)) == 10 then
        if or(varargin($) == ["rad" "asymp" "radasymp"]) then
            ...
            case "rad" then
                rad = %t;
            case "asymp" then
                // bode_asymp(ss, fmin, fmax) is not yet open to real 'ss'
                if and(typeof(varargin(1)) <> ["state-space" "rational"]) then
                    msg = _("%s: ""%s"" option is not applicable when first input argument is real => ignored.\n");
                    warning(msprintf(msg, "bode", "asymp"))
                else
                    asymp = %t;
                end
            else // "radasymp"
...
function [] = bode_asymp(ss, w_min, w_max)
    ...
    for elem=1:size(ss, "r") // Individually draw each asymptote of "ss" elements (row vector).
