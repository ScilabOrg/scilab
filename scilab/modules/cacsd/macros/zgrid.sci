...
// zgrid(Z,Wn [,"new"] [,colors])
  ...
  case 1 then
    zeta =  varargin(1)
    if type(zeta)|~isreal(zeta)<>1 then
      error(msprintf("%s: Wrong type for imput argument #%d : real floating point array expected\n"),"sgrid",1);
    end
    wn=defaultwn
    colors = defaultcolors
  case 2 then
  ...
  case 3 then
    ...
    colors = varargin(3);
    if type(colors)|~isreal(colors)<>1 then
      error(msprintf("%s: Wrong type for imput argument #%d : real floating point array expected\n"),"sgrid",3);
    end
