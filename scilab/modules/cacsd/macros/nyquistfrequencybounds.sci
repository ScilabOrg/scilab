...
//find frequencies that reaches the nyquist bounds on real an imag
//part
  ...
  if or(bounds(2,:)<bounds(1,:)) then
    error(msprintf(_("%s: Wrong value for input argument #%d: second row must be great than first one.\n"),fname,2))
  end
  ...
  if dom=='c' then
    c=2*%i*%pi
    function f=arg2freq(y),f=y/c,endfunction
    Hr=(H+horner(H,-%s))/2;
    Hi=(H-horner(H,-%s))/2;
  else
    c=2*%i*%pi*dom
    function f=arg2freq(y),f=log(y)/c,endfunction
    Hr=(H+horner(H,1/%z))/2;
    Hi=(H-horner(H,1/%z))/2;
  end
