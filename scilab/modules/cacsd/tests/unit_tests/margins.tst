...
if norm(f - f_exact)>1d-10  then pause,end
r=repfreq(h,f);
if ~isreal(r,1e-10)|real(r)>0 then pause,end
if abs(20*log10(1/abs(r))-g)>1d-10 then pause,end
...
[g,f]=g_margin(h);
f_exact=1/2
if abs(f-f_exact)>1d-10 then pause,end
r=repfreq(h,f);
if ~isreal(r,1e-10)|real(r)>0 then pause,end
if abs(20*log10(1/abs(r))-g)>1d-10 then pause,end
...
if norm(f-f_exact)>1d-10  then pause,end
r=repfreq(h,f);
if abs(p-(180+atan(imag(r),real(r))*180/%pi))>1d-10  then pause,end
