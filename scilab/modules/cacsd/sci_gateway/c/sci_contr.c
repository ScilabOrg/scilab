...
int intab01od(char *fname)
{
    ...
    CreateVar(Rhs + 1, MATRIX_OF_INTEGER_DATATYPE, (un = 1, &un), (un = 1, &un), &ptrNCONT);
    NCONT = Rhs + 1;
    CreateVar(Rhs + 2, MATRIX_OF_DOUBLE_DATATYPE, &N, &N, &ptrU);
    U = Rhs + 2;
    CreateVar(Rhs + 3, MATRIX_OF_INTEGER_DATATYPE, (un = 1, &un), &N, &ptrKSTAIR);
    KSTAIR = Rhs + 3;
    ...
    C2F(ab01od) ("A", JOBU, JOBV, &N, &M, stk(ptrA), &LDA,
                 ...
                 istk(ptrNCONT), &INDCON, istk(ptrKSTAIR), &theTOL, istk(ptrIWORK), stk(ptrDWORK), &LDWORK, &INFO);
    if (INFO != 0)
    {
