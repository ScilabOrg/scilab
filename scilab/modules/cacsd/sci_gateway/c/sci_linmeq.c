...
int sci_linmeq(char *fname, unsigned long fname_len)
{
    ...
    int lA = 0, lIMG = 0, lB = 0, lC = 0, lCC = 0, lDWORK = 0, lU = 0, lV = 0, lWR = 0, lWI = 0, lIWORK = 0, lSEP = 0;
    ...
    int MB = 0, NB = 0;
    int MC = 0, NC = 0;
    int ilTASK = 0, ilTRANS = 0, ilSCHUR = 0, ilFLAG = 0;
    ...
    //   task
    GetRhsVar(1, MATRIX_OF_INTEGER_DATATYPE, &Mt, &Nt, &ilTASK)
    if (Mt != 1 || Nt != 1)
    {
    ...
    if (Rhs >= IP)
    {
        GetRhsVar(IP, MATRIX_OF_INTEGER_DATATYPE, &Mt, &Nt, &ilTRANS);
    ...
    if (TASK == 1)
    {
        ...
        if (Rhs == IP + 1)
        {
            GetRhsVar(IP + 1, MATRIX_OF_INTEGER_DATATYPE, &Mt, &Nt, &ilSCHUR);
            if (Mt != 1 || Nt != 1)
            {
    ...
    GetRhsCVar(2, MATRIX_OF_DOUBLE_DATATYPE, &it, &MA, &NA, &lA, &lIMG);
    if (it)
    {
    ...
    if (TASK == 1)
    {
        GetRhsCVar(3, MATRIX_OF_DOUBLE_DATATYPE, &it, &MB, &NB, &lB, &lIMG);
        if (it)
        {
            Scierror(999, _("%s: Wrong type for input argument #%d: A real expected.\n"), fname, 3);
            return 1;
        ...
        GetRhsCVar(4, MATRIX_OF_DOUBLE_DATATYPE, &it, &MC, &NC, &lC, &lIMG);
        if (it)
        {
            Scierror(999, _("%s: Wrong type for input argument #%d: A real expected.\n"), fname, 4);
            return 1;
    ...
    {
        GetRhsCVar(3, MATRIX_OF_DOUBLE_DATATYPE, &it, &MC, &NC, &lC, &lIMG);
        if (it)
        {
            Scierror(999, _("%s: Wrong type for input argument #%d: A real expected.\n"), fname, 3);
            return 1;
    ...
    {
        //   flag
        GetRhsVar(IP, MATRIX_OF_INTEGER_DATATYPE, &Mt, &Nt, &ilFLAG);
        if (TASK == 1)
        {
            if (Mt * Nt > 3)
            {
    ...
    if (TASK == 1)
    {
        CreateVar(Nbvars + 1, MATRIX_OF_DOUBLE_DATATYPE, &NDWORK, &iOne, &lDWORK);
        if (NSCHUR == 2)
        {
            if (FLAG[1] == 1)
            {
                FACTA = 'S';
                LDU = 1;
                CreateVar(Nbvars + 1, MATRIX_OF_DOUBLE_DATATYPE, &LDU, &iOne, &lU);
            }
            else
            {
                FACTA = 'N';
                LDU = LDA;
                CreateVar(Nbvars + 1, MATRIX_OF_DOUBLE_DATATYPE, &LDU, &N, &lU);
            }
            ...
            if (FLAG[2] == 1)
            {
                FACTB = 'S';
                LDV = 1;
                CreateVar(Nbvars + 1, MATRIX_OF_DOUBLE_DATATYPE, &LDV, &iOne, &lV);
            }
            else
            {
                FACTB = 'N';
                LDV = LDB;
                CreateVar(Nbvars + 1, MATRIX_OF_DOUBLE_DATATYPE, &LDV, &M, &lV);
            }
        }
        else
        {
            SCHU = 'N';
            CreateVar(Nbvars + 1, MATRIX_OF_INTEGER_DATATYPE, &NIWORK, &iOne, &lIWORK);
            if (FLAG[1] == 1)
            {
                if (FLAG[2] == 1)
                {
            ...
            {
                LDU = LDB;
                CreateVar(Nbvars + 1, MATRIX_OF_DOUBLE_DATATYPE, &LDU, &M, &lU);
            }
        }
    }
    else if (TASK == 2)
    {
        ...
        CreateVar(Nbvars + 1, MATRIX_OF_DOUBLE_DATATYPE, &LDU, &N, &lU);
        CreateVar(Nbvars + 1, MATRIX_OF_DOUBLE_DATATYPE, &NDWORK, &iOne, &lDWORK);
        CreateVar(Nbvars + 1, MATRIX_OF_DOUBLE_DATATYPE, &N, &iOne, &lWI);
        CreateVar(Nbvars + 1, MATRIX_OF_DOUBLE_DATATYPE, &N, &iOne, &lWR);
        if (Lhs == 1)
        {
            CreateVar(Nbvars + 1, MATRIX_OF_INTEGER_DATATYPE, &iOne, &iOne, &lIWORK);
        }
        else
        {
            int NN = N * N;
            CreateVar(Nbvars + 1, MATRIX_OF_INTEGER_DATATYPE, &NN, &iOne, &lIWORK);
        }
    }
    else
    {
        ...
        else
        {
            ...
            else if (TRANS == 1)
            {
                ...
                else
                {
                    ...
                    {
                        int jm1 = j - 1;
                        C2F(dswap)(&jm1, stk(lA + (j - 1) * LDA), &iOne, stk(lA + j - 1), &LDA);
                    }

                    for (j = 1; j < M; j++)
                    {
                        int jm1 = j - 1;
                        C2F(dswap)(&jm1, stk(lB + (j - 1) * LDB), &iOne, stk(lB + j - 1), &LDB);
                    }
                }
            }
            else if (TRANS == 2)
            ...
            if (C2F(lsame)(&SCHU , "N"))
            {
                ...
                if (FLAG[0] == 0)
                {
                    C2F(sb04md)(&N, &M, stk(lA), &LDA, stk(lB), &LDB, stk(lC),
                                &LDC, stk(lU), &LDU, istk(lIWORK), stk(lDWORK), &NDWORK, &INFO);
                }
                else
                {
                    C2F(sb04qd)(&N, &M, stk(lA), &LDA, stk(lB), &LDB, stk(lC),
                                &LDC , stk(lU), &LDU, istk(lIWORK), stk(lDWORK), &NDWORK, &INFO);
                }
            }
            else if (C2F(lsame)(&SCHU , "S"))
            {
                if (FLAG[0] == 0)
                {
                    C2F(dtrsyl)(&TRANA, &TRANB, &ONE, &N, &M, stk(lA), &LDA,
                                stk(lB), &LDB, stk(lC), &LDC, &SCALE, &INFO);
                }
                else
                {
                    C2F(sb04py)(&TRANA, &TRANB, &ONE, &N, &M, stk(lA), &LDA,
            ...
            else
            {
                ...
                if (FLAG[0] == 0)
                {
                    C2F(sb04nd)(&SCHU, &ULA, &ULB, &N, &M, stk(lA), &LDA,
                                stk(lB), &LDB, stk(lC), &LDC, &TOL, istk(lIWORK),
                                stk(lDWORK), &NDWORK, &INFO);
                }
                else
                {
                    C2F(sb04rd)(&SCHU, &ULA, &ULB, &N, &M, stk(lA), &LDA,
                                stk(lB), &LDB, stk(lC), &LDC, &TOL, istk(lIWORK),
                                stk(lDWORK), &NDWORK, &INFO);
                }
            }
    ...
    else
    {
        ...
        {
            FACT = 'F';
            C2F(dlaset)("F", &N, &N, &ZERO, &ONE, stk(lU), &LDU);
        }
        ...
        if (TASK == 2)
        {
            ...
            {
                int NMC = N * MC;
                CreateVar(Nbvars + 1, MATRIX_OF_DOUBLE_DATATYPE, &LDC, &N, &lCC);

                C2F(dcopy)(&NMC, stk(lC), &iOne, stk(lCC), &iOne);
                C2F(sb03md)(&DICO, &JOB, &FACT, &TRANA, &N, stk(lA), &LDA,
                            stk(lU), &LDU, stk(lCC), &LDC, &SCALE, &SEP, &FERR, stk(lWR),
                            stk(lWI), istk(lIWORK), stk(lDWORK), &NDWORK, &INFO);
                C2F(dcopy)(&NMC, stk(lCC), &iOne, stk(lC), &iOne);
            }
            else
            {
                C2F(sb03md)(&DICO, &JOB, &FACT, &TRANA, &N, stk(lA), &LDA,
        ...
        {
            C2F(sb03od)(&DICO, &FACT, &TRANA, &N, &P, stk(lA), &LDA, stk(lU),
    ...
    if (INFO == 0 || PERTRB)
    {
        if (Lhs >= 1)
        {
            if (TASK == 3)
            {
                if (TRANS == 0 && P > N)
                {
                    C2F(dlacpy)("U", &N, &N, stk(lC), &LDC, stk(lC), &LDA);
                }

                if (N > 1)
                {
                    int Nm1 = N - 1;
                    C2F(dlaset)("L", &Nm1, &Nm1, &ZERO, &ZERO, stk(lC + 1), &LDA);
                }
            }
        }
        ...
        if (TASK == 2)
        {
            if (Lhs >= 2)
            {
                ...
                CreateVar(Nbvars + 1, MATRIX_OF_DOUBLE_DATATYPE, &iOne, &iOne, &lSEP);
                *stk(lSEP) = SEP;
                LhsVar(2) = Nbvars;
            }
        }
