        ...
        else
        {
            ...
            else if (TRANS == 1)
            {
                ...
                else
                {
                    ...
                    {
                        int jm1 = j - 1;
                        C2F(dswap)(&jm1, (lA + (j - 1) * LDA), &iOne, (lA + j - 1), &LDA);
                    }

                    for (j = 1; j < M; j++)
                    {
                        int jm1 = j - 1;
                        C2F(dswap)(&jm1, (lB + (j - 1) * LDB), &iOne, (lB + j - 1), &LDB);
                    }
                }
            }
            else if (TRANS == 2)
            ...
            else if (C2F(lsame)(&SCHU , "S"))
            {
                if (FLAG[0] == 0)
                {
                    C2F(dtrsyl)(&TRANA, &TRANB, &ONE, &N, &M, lA, &LDA,
                                lB, &LDB, lC, &LDC, &SCALE, &INFO);
                }
                else
                {
                    C2F(sb04py)(&TRANA, &TRANB, &ONE, &N, &M, lA, &LDA,
