...
types::Function::ReturnValue sci_arl2_ius(types::typed_list &in, int _iRetCount, types::typed_list &out)
{
    ...
    iRankDen = pPolyDen->get(0)->getRank();
    iRankDen--;
    types::Double* pDblCoef = pPolyDen->get(0)->getCoef();
    pdblDen = pDblCoef->get();
    ...
    if(bAll)
    {
        ...
        int iWorkSize = 34 + 34*iN + 7*iNg + iN*iNg + iNg*iNg*(iNg+2) + 4*(iN+1)*iMxsol;
    ...
    {
        // look for a solution
        double* pdblNum = new double[iN+1];
        int iWorkSize = 32 + 32*iN + 7*iNg + iN*iNg + iNg*iNg*(iNg+2);
        double* pdblWork = new double[iWorkSize];
        iWorkSize = 29 + iN*iN + 4*iN;
        int* piWork = new int[iWorkSize];

        C2F(arl2)(pdblY, &iVol1, pdblNum, pdblDen, &iRankDen, &iN, &dErrl2, pdblWork, piWork,
        ...
        types::Double* pDblTemp = pPolyDenOut->get(0)->getCoef();
        double* pdblDenOut = pPolyDenOut->get(0)->getCoef()->get();
        C2F(dcopy)(&iRank, pdblDen, &iOne, pdblDenOut, &iOne);
        out.push_back(pPolyDenOut);
