...
types::Function::ReturnValue sci_ldiv(types::typed_list &in, int _iRetCount, types::typed_list &out)
{
    ...
    for(int i = 0; i < iSize; i++)
    {
        double* temp1 = new double[piRank1[i]];
        double* temp2 = new double[piRank2[i]];
        C2F(dcopy)(piRank1 + i, pdblCoef1[i], &iOne, temp1, &iOne);
        C2F(dcopy)(piRank2 + i, pdblCoef2[i], &iOne, temp2, &iOne);
        C2F(dtild)(piRank1 + i, temp1, &iOne);
        C2F(dtild)(piRank2 + i, temp2, &iOne);
        C2F(expan)(temp2, piRank2 + i, temp1, piRank1 + i, pdblout, &iK);
        delete[] temp1;
        delete[] temp2;
    }
