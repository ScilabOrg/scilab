...
types::Function::ReturnValue sci_tzer(types::typed_list &in, int _iRetCount, types::typed_list &out)
{
    ...
    int iNwrk1  = max(iCols[1], iRows[2]);
    int iNwrk2  = max(iNwrk1, iCols[0] + 1);
    ...
    if(iRank == 0 && iNu > 0)
    {
        C2F(dset)(&iNu, &dZero, pdblR, &iOne);
        C2F(dset)(&iNu, &dZero, pdblI, &iOne);
        C2F(dset)(&iNu, &dZero, pdblWork2, &iOne);
    }

    /*** retrun output arguments ***/
    types::Double* pDblOut1 = new types::Double(iNu, min(iNu, 1), true);
    ...
    types::Double* pDblOut2 = new types::Double(iNu, min(iNu, 1));
