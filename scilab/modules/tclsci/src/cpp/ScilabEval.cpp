...
int TCL_EvalScilabCmd(ClientData clientData, Tcl_Interp * theinterp, int objc, CONST char ** argv)
{
    ...
    if (argv[1] != (char *)0)
    {
        ...
        if (strncmp(pstCommand, "flush", 5) == 0)
        {
            ...
            types::ThreadId* pThreadMe = ConfigVariable::getLastThread();
            pThreadMe->IncreaseRef();
            ConfigVariable::deleteThread(pThreadMe->getKey());
        ...
        else if ( (argv[2] != (char *)0) && (strncmp(argv[2], "sync", 4) == 0) )
        {
            ...
            StoreCommandWithFlags(pstCommand, 1/*is prioritary*/, isInterruptible, 0/*from console*/);

        }
        else
        {
            isInterruptible = ( (argv[2] != (char *)0) && (strncmp(argv[2], "seq", 3) == 0) ) ? 0 : 1;
            StoreCommandWithFlags(pstCommand, 0/*is prioritary*/, isInterruptible, 0/*from console*/);
            if (isInterruptible)
            {
