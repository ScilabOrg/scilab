...
void taucs_ccs_genmmd(taucs_ccs_matrix * m, int **perm, int **invperm)
{
    ...
    *invperm = NULL;
    ...
    F2C(genmmd) (&n, xadj, adjncy, invp, prm, &delta, dhead, qsize, llist, marker, &maxint, &nofsub);
...
static supernodal_factor_matrix *multifrontal_supernodal_create(void)
{
...
static supernodal_frontal_matrix *supernodal_frontal_create(int *firstcol_in_supernode, int sn_size, int n, int *rowind)
{
    ...
    if ((tmp->f1 == NULL && iDontCheckf1 == 0) || (tmp->f2 == NULL && iDontCheckf2 == 0) || (tmp->u == NULL && iDontChecku == 0))
    {
        FREE(tmp->u);
        FREE(tmp->f1);
        FREE(tmp->f2);
                                     ...
                                     int sn_size, taucs_ccs_matrix * A, supernodal_frontal_matrix * mtr, int *bitmap, supernodal_factor_matrix * snL)
{
    ...
    if (mtr->sn_size)
    {
        F2C(dpotrf) ("LOWER", &(mtr->sn_size), mtr->f1, &(mtr->sn_size), &INFO);
    }
    ...
    if (mtr->up_size && mtr->sn_size)
    {
        F2C(dtrsm) ("Right",
                    "Lower",
                    "Transpose", "No unit diagonal", &(mtr->up_size), &(mtr->sn_size), &done, mtr->f1, &(mtr->sn_size), mtr->f2, &(mtr->up_size));
    }
    ...
    if (mtr->up_size && mtr->sn_size)
    {
        F2C(dsyrk) ("Lower", "No Transpose", &(mtr->up_size), &(mtr->sn_size), &dminusone, mtr->f2, &(mtr->up_size), &done, mtr->u, &(mtr->up_size));
    }
...
static void recursive_postorder(int j, int first_child[], int next_child[], int postorder[], int ipostorder[], int *next)
{
    int c;
...
static void tree_first_descendant(int j, int isroot, int first_child[], int next_child[], int ipostorder[], int first_descendant[])
{
    int c;
    int fd = ipostorder[j];
...
int taucs_ccs_etree(taucs_ccs_matrix * A, int *parent, int *l_colcount, int *l_rowcount, int *l_nnz)
{
    ...
    if (l_colcount || l_rowcount || l_nnz)
    {
        ...
        tree_first_descendant(n, TRUE, first_child, next_child, ipostorder, first_descendant);
#endif
        ...
        for (p = 0; p < n; p++)
        {
            ...
            if (parent[jp] != n)
            {
                ...
                {
                    sciprint("jp %d parent %d (ipo_j %d ipo_parent %d\n\r", jp, parent[jp], ipostorder[jp], ipostorder[parent[jp]]);
                }
                               ...
                               int sn_next_child[], int rowind[], int column_to_sn_map[], int map[], int do_order, int ipostorder[])
{
    ...
    for (c = first_child[j]; c != -1; c = next_child[c])
    {
        recursive_symbolic_elimination(c, A, first_child, next_child, n_sn, sn_size, sn_up_size, sn_rowind, sn_first_child, sn_next_child, rowind,  /* temporary */
                                ...
                                int sn_next_child[], int rowind[], int column_to_sn_map[], int map[], int do_order, int ipostorder[])
{
    ...
    sn_znz.zeros = 0.0;
    sn_znz.nonzeros = (double)(((sn_up_size[sn] - sn_size[sn]) * sn_size[sn]) + (sn_size[sn] * (sn_size[sn] + 1)) / 2);

    if (sn_first_child[sn] == -1)
    {                           /* leaf */
        return sn_znz;
    }
    ...
    for (c_sn = sn_first_child[sn]; c_sn != -1; c_sn = sn_next_child[c_sn])
    {
        c_znz[i] = recursive_amalgamate_supernodes(c_sn, n_sn, sn_size, sn_up_size, sn_rowind, sn_first_child, sn_next_child, rowind,   /* temporary */
                                                   column_to_sn_map, map, do_order, ipostorder);
        assert(c_znz[i].zeros + c_znz[i].nonzeros ==
               (double)(((sn_up_size[c_sn] - sn_size[c_sn]) * sn_size[c_sn]) + (sn_size[c_sn] * (sn_size[c_sn] + 1)) / 2));
        i++;
    }
...

    {
        ...
        if ((new_sn_size < 16)
            ...
            || ((sn_size[sn] < 500) && (merged_znz.zeros < 0.10 * merged_znz.nonzeros)) || (merged_znz.zeros < 0.05 * merged_znz.nonzeros))
        {
            /*
    ...
    sn_rowind[sn] = (int *)REALLOC(sn_rowind[sn], new_sn_up_size * sizeof(int));
    for (ip = 0; ip < new_sn_up_size; ip++)
...
int taucs_ccs_symbolic_elimination(taucs_ccs_matrix * A, void *vL, int do_order)
{
    supernodal_factor_matrix *L = (supernodal_factor_matrix *) vL;
    int *first_child;
    int *next_child;
                                                                               ...
                                                                               taucs_ccs_matrix * A, supernodal_factor_matrix * snL, int *fail)
{
    ...
    if (!is_root)
    {
        if (multifrontal_supernodal_front_factor(sn, v, sn_size, A, my_matrix, bitmap, snL))
        {
            /* nonpositive pivot */
            *fail = TRUE;
...
void *taucs_ccs_factor_llt_mf(taucs_ccs_matrix * A)
{
    ...
    taucs_ccs_symbolic_elimination(A, L, FALSE /* don't sort row indices */ );
                                         ...
                                         int *up_blocks_ld, double *up_blocks[], double x[], double b[], double t[])
{
    ...
    if (!is_root)
    {
        ...
        flops = ((double)sn_size) * ((double)sn_size) + 2.0 * ((double)sn_size) * ((double)up_size);
        ...
        if (flops > BLAS_FLOPS_CUTOFF)
        {
            ...
            F2C(dtrsm) ("Left",
                        "Lower", "No Transpose", "No unit diagonal", &sn_size, &ione, &done, sn_blocks[sn], &(sn_blocks_ld[sn]), xdense, &sn_size);

            if ((up_size > 0) & (sn_size > 0))
            {
                F2C(dgemm) ("No Transpose", "No Transpose",
                            &up_size, &ione, &sn_size, &done, up_blocks[sn], &(up_blocks_ld[sn]), xdense, &sn_size, &dzero, bdense, &up_size);
            }
        ...
        if (flops > BLAS_FLOPS_CUTOFF)
        {
            ...
            if ((up_size > 0) & (sn_size > 0))
                F2C(dgemm) ("Transpose", "No Transpose",
                            &sn_size, &ione, &up_size, &dminusone, up_blocks[sn], &(up_blocks_ld[sn]), xdense, &up_size, &done, bdense, &sn_size);

            F2C(dtrsm) ("Left",
                        "Lower", "Transpose", "No unit diagonal", &sn_size, &ione, &done, sn_blocks[sn], &(sn_blocks_ld[sn]), bdense, &sn_size);
...
int taucs_supernodal_solve_llt(void *vL, double *x, double *b)
{
    supernodal_factor_matrix *L = (supernodal_factor_matrix *) vL;
    double *y;
    ...
    recursive_supernodal_solve_l(L->n_sn, TRUE, /* this is the root */
                                 ...
                                 L->sn_struct, L->sn_size, L->sn_up_size, L->sn_blocks_ld, L->sn_blocks, L->up_blocks_ld, L->up_blocks, y, x, t);

    recursive_supernodal_solve_lt(L->n_sn, TRUE,    /* this is the root */
                                  L->first_child, L->next_child,
                                  L->sn_struct, L->sn_size, L->sn_up_size, L->sn_blocks_ld, L->sn_blocks, L->up_blocks_ld, L->up_blocks, x, y, t);
...
taucs_ccs_matrix *taucs_supernodal_factor_to_ccs(void *vL)
{
    supernodal_factor_matrix *L = (supernodal_factor_matrix *) vL;
    taucs_ccs_matrix *C;
    int n, nnz;
