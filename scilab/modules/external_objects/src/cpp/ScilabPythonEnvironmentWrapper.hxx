...
class ScilabPythonEnvironmentWrapper : public ScilabAbstractEnvironmentWrapper
{
...
public:
    ...
    ScilabPythonEnvironmentWrapper(PythonVariablesScope & _scope, PythonOptionsHelper & _helper) : scope(_scope), helper(_helper) { }

    ~ScilabPythonEnvironmentWrapper() { }
...
private:
    ...
    inline void pyUnwrapMat(int id, const ScilabSingleTypeStackAllocator<T> & allocator) const
    {
        ...
        if (PyList_Check(obj))
        {
            ...
            if (row > 0)
            {
                PyObject * f = PyList_GetItem(obj, 0);
    ...
    inline int wrapData(T * x, int xSize, const bool isRef) const
    {
        if (helper.getUseNumpy())
        {
            ...
            else
            {
                ...
                {
                    throw ScilabPythonException(__LINE__, __FILE__, gettext("Cannot pass the data by reference"));
                }
    ...
    inline int wrapData(char ** x, int xSize, const bool isRef) const
    {
        if (helper.getUseNumpy())
        {
            ...
            {
                throw ScilabPythonException(__LINE__, __FILE__, gettext("Cannot pass Scilab strings by reference"));
            }
    ...
    inline int wrapData(double * re, double * im, int xSize, const bool isRef) const
    {
        if (helper.getUseNumpy())
        {
            ...
            {
                throw ScilabPythonException(__LINE__, __FILE__, gettext("Cannot pass Scilab Complex by reference"));
            }
...
#endif // __SCILABPYTHONENVIRONMENTWRAPPER_HXX__
