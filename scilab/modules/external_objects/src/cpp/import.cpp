...
int ScilabGateway::import(char * fname, const int envId, void * pvApiCtx)
{
    ...
    if (named)
    {
        ...
        for (int i = 0; i < rows * cols; i++)
        {
            ...
            if (isNamedVarExist(pvApiCtx, name[i].c_str()))
            {
                ...
                if (err.iErr || addr == 0 || !ScilabObjects::isExternalClass(addr, pvApiCtx) || ScilabObjects::getEnvironmentId(addr, pvApiCtx) != envId)
                {
                    freeAllocatedMatrixOfString(rows, cols, className);
                    delete[] name;
    ...
    for (int i = 0; i < rows * cols; i++)
    {
        try
        {
            ret = env.loadclass(className[i], static_cast<bool>(named), helper.getAllowReload());
        }
        ...
        if (named)
        {
            try
            {
                ScilabObjects::createNamedEnvironmentObject(EXTERNAL_CLASS, name[i].c_str(), ret, envId, pvApiCtx);
            }
