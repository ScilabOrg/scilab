...
int ScilabPythonEnvironment::loadclass(char * className, bool allowReload)
{
    ...
    else
    {
        ...
        // We put the name in Python namespace
        PyObject * _main_ = PyImport_AddModule("__main__");
        PyObject_SetAttrString(_main_, cn, pModule);
...
int ScilabPythonEnvironment::newinstance(int id, int * args, int argsSize)
{
    ...
    if (traceEnabled)
    {
        ...
        writeLog("newinstance", "Creation of a new object based on class with id %d with arguments: %s.", id, os.str().c_str());
    }
    ...
    int ret = scope.addObject(instance);
    writeLog("newinstance", "returned id %d.", ret);
