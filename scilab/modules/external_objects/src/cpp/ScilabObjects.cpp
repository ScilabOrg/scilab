...
const char * ScilabObjects::COPYINVOCATIONMACROTOSTACK = "copyInvocationMacroToStack";
...
void ScilabObjects::removeVar(int * addr, void * pvApiCtx)
{
    ...
    throw ScilabAbstractEnvironmentException(__LINE__, __FILE__, gettext("Invalid variable: an External Object or an External Class expected"));
}
...
int ScilabObjects::getMListType(int * mlist, void * pvApiCtx)
{
    ...
    if (!strcmp("_EObj", mtype))
    {
        type = EXTERNAL_OBJECT;
    }
    else if (!strcmp("_EClass", mtype))
    {
        type = EXTERNAL_CLASS;
    }
    else if (!strcmp("_EVoid", mtype))
    {
        type = EXTERNAL_VOID;
    }
