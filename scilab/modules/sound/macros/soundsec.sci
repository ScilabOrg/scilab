...
function [t]=soundsec (n,rate)
    ...
    if  typeof(n)=="constant" 
        if isreal(n)
            if ~isscalar(n)
                ...
                error(ERR_NON_SCALAR, ...
                msprintf(_("%s: Wrong size for input argument #%d: %d-by-%d matrix expected.\n"),"soundsec",1,1,1));
            end
        else ...//n is not real
            error(ERR_NON_REAL, ...
            msprintf(_("%s: Wrong type for argument %d: Real matrix expected.\n"),"soundsec",1));
        end
    ...
    if n<0
        error(ERR_NON_POSITIVE, ...
        msprintf(_("%s: Wrong value for input argument #%d: Must be in the interval [%s, oo).\n"),"soundsec",1,"0"));
    end
