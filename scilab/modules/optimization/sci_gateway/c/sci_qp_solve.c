...
extern int C2F(qpgen2) (double *dmat, double *dvec, int *fddmat, int *n,
                        ...
                        int *fdamat, int *q, int *meq, int *iact, int *nact, int *iter, double *work, int *ierr);
extern int C2F(qpgen1sci) (double *dmat, double *dvec, int *fddmat, int *n, double *sol, double *crval,
                           ...
                           double *bvec, int *q, int *meq, int *iact, int *nact, int *iter, double *work, int *ierr);
extern int C2F(spt) (int *m, int *n, int *nel, int *it, int *ptr,
                     double *A_R, double *A_I, int *mnel, int *A_icol, double *At_R, double *At_I, int *At_mnel, int *At_icol);
/*--------------------------------------------------------------------------*/
int sci_qp_solve(char *fname, unsigned long fname_len)
{
    ...
    if (!issparse)
    {
        ...
        C2F(qpgen2) (stk(Q), stk(p), &n, &n, stk(x), stk(crval), stk(C), stk(b), &n, &m, istk(me), istk(iact), &nact, istk(iter), work, &ierr);
    }
    else
    {
        ...
        C2F(spt) (&n, &m, &(Sp.nel), &(Sp.it), (int *)work, Sp.R, Sp.I, Sp.mnel, Sp.icol, R, I, ind, ind + m);

        C2F(qpgen1sci) (stk(Q), stk(p), &n, &n, stk(x), stk(crval),
                        ind, ind + m, R, stk(b), &m, istk(me), istk(iact), &nact, istk(iter), work, &ierr);
        FREE(work);
        work = NULL;
        FREE(R);
        R = NULL;
