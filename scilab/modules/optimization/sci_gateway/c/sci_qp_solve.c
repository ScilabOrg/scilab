...
int sci_qp_solve(char *fname, unsigned long fname_len)
{
    ...
    if ( nbis != n )
    {
        Scierror(999, _("%s: Wrong size for input argument #%d: %d column(s) expected for matrix C.\n"), fname, 3, n);
        return 0;
    }
    ...
    {
        Scierror(999, _("%s: Wrong value for input argument #%d: me must be a integer in the range 0 to %d.\n"), fname, 5, n);
        return 0;
    }
