...
extern int C2F(ql0001) (int *m, int *me, int *mmax, int *n, int *nmax, int *mnn,
                        ...
                        int *iprint, double *war, int *lwar, int *iwar, int *liwar, double *eps1);
/*--------------------------------------------------------------------------*/
int sci_qld(char *fname, unsigned long fname_len)
{
    ...
    {
        for (l = 0; l < m; l++)
            stk(C)[k * m + l] = -stk(C)[k * m + l];
    }

    iout = 0;
    ifail = 0;
    C2F(ql0001) (&m, istk(me), &mmax, &n, &n, &mnn, stk(Q), stk(p), stk(C),
                 stk(b), stk(lb), stk(ub), stk(x), stk(lambda), &iout, &ifail, &zero, stk(war), &lwar, istk(iwar), &n, &eps1);
