...
assert_checkerror(cmd,"optimsimplex_axes: The len vector is not consistent with the x0 point. Current shape of x0 is 1 x 2 while current shape of len is 1 x 3.");

// Test with a unconsistent x0
cmd = "newobj = optimsimplex_new ( ""axes"" , [1 2;3 4] , rosenbrock )";
assert_checkerror(cmd,"optimsimplex_axes: The x0 vector is expected to be a row matrix, but current shape is 2 x 2.");

// Test with a unconsistent len
cmd = "newobj = optimsimplex_new ( ""axes"" , [1 2] , rosenbrock , ""foo"" )";
assert_checkerror(cmd,"assert_typereal: Expected real variable for variable len at input #1, but got string instead.");
