...
refMsg = msprintf(_("%s: Wrong number of elements in input argument #%d: At least %d elements expected, but current number is 0.\n"), "derivative", 1, 2); 
assert_checkerror("derivative(list(),x)", refMsg);

assert_checkfalse(execstr("derivative(list(""foo""),x)", "errcatch") == 0);
refMsg = msprintf(_("%s: Wrong number of elements in input argument #%d: At least %d elements expected, but current number is 1.\n"), "derivative", 1, 2); 
assert_checkerror("derivative(list(""foo""),x)", refMsg);


assert_checkfalse(execstr("derivative(list(""foo"",""aa""),x)", "errcatch") == 0);
refMsg = msprintf(_("%s: Wrong type for element #%d in input argument #%d: A function is expected, but current type is string.\n"), "derivative", 1, 1); 
