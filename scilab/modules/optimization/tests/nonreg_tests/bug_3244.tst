...
assert_checkfalse(execstr("qp_solve(1, 1, [], 1, 1)", "errcatch") == 0);
refMsg = msprintf(_("%s: Wrong size for input argument #%d: n columns expected for matrix C."), "qp_solve", 3); 
assert_checkerror("qp_solve(1, 1, [], 1, 1)", refMsg);

assert_checkfalse(execstr("qp_solve(1, 2, 3, 4, 5)", "errcatch") == 0);
refMsg = msprintf(_("%s: Wrong value for input argument #%d: me must be a integer in the range 0 to rows number of matrix Q."), "qp_solve", 5); 
