...
static void cngrncb(int itype, int n, double *AP, double *B, double *CP, double *temp)
{
    ...
    if (itype == 1)
    {
        ...
        for (j = 0, pos = 0; j < n; pos += n - j, j++)
        {
            ...
            /* C(j:n,j) = B(j:n,:)*temp */
            lngth = n - j;
            F2C(dgemv) ("N", &lngth, &n, &dbl1, B + j, &n, temp, &int1, &dbl0, CP + pos, &int1);
    ...
    else
    {
        ...
        for (j = 0, pos = 0; j < n; pos += n - j, j++)
        {
            ...
            /* C(j:n,j) = B(:,j:n)'*temp */
            lngth = n - j;
            F2C(dgemv) ("T", &n, &lngth, &dbl1, B + j * n, &n, temp, &int1, &dbl0, CP + pos, &int1);
 ...
 */
{
    ...
    for (j = 0, pos = 0; j < L; j++)
        ...
        for (k = 0, lngth = blck_szs[j]; k < blck_szs[j]; pos += lngth, lngth -= 1, k++)
...
/*--------------------------------------------------------------------------*/
int C2F(spf) (int *m,           /* no of variables */
...
{
    return (sp(*m, *L, F, blck_szs, c, x, Z, ul, *nu, *abstol, *reltol, *tv, iters, work, *lwork, iwork, info));
}

/*--------------------------------------------------------------------------*/
int sp(int m,                   /* no of variables */
       int L,                   /* no of blocks in F */
       double *F,               /* F_i's in packed storage */
       int *blck_szs,           /* L-vector, dimensions of diagonal blocks */
       double *c,               /* m-vector */
 ...
 */
{
    int i, j, k, n, sz, up_sz, max_n, lngth, pos, pos2, pos3, pos4, ltemp, maxiters, info2, minlwork;
    double q, *rhs, *Fsc, *R, *X, rho, *dx, *sigx, *sigz, *dZ, *temp, scal,
        ...
        newl = 0.0, maxpossigx, minnegsigx, maxpossigz, minnegsigz, nrmc, nrmx, nrmz, nrmmax, rcond;
    int int2 = 2, int1 = 1;
    ...
    /* check lwork */
    minlwork = (m + 2) * sz + up_sz + 2 * n + Max(Max(m + sz, 3 * max_n + max_n * (max_n + 1)), 3 * m);
    if (lwork < minlwork)
    {
        if (getWarningMode())
    ...
    for (*iters = 0; *iters <= maxiters; (*iters)++)
    {
        ...
        for (i = 0, pos = 0, pos2 = 0, pos3 = 0, gap = 0.0; i < L;
             pos += blck_szs[i] * (blck_szs[i] + 1) / 2, pos2 += blck_szs[i] * blck_szs[i], pos3 += blck_szs[i], i++)
        {
            ...
            F2C(dspgv) (&int2, "V", "L", blck_szs + i, temp, X + pos, sigx + pos3, R + pos2, blck_szs + i, temp + lngth, &info2);
            if (info2)
            {
                if (getWarningMode())
        ...
        if (gap <= Max(abstol, MINABSTOL))
            *info = 2;
        else if ((ul[1] > 0.0 && gap <= reltol * ul[1]) || (ul[0] < 0.0 && gap <= reltol * (-ul[0])))
            *info = 3;
        ...
        for (j = 0, pos = 0; j < m; j++)
            for (i = 0, pos2 = 0; i < L; pos += blck_szs[i] * (blck_szs[i] + 1) / 2, pos2 += blck_szs[i] * blck_szs[i], i++)
            {
        ...
        rho = -q / gap;
        for (i = 0, pos = 0, pos3 = 0; i < L; pos += blck_szs[i] * (blck_szs[i] + 1) / 2, pos3 += blck_szs[i], i++)
            for (k = 0, pos4 = pos; k < blck_szs[i]; pos4 += blck_szs[i] - k, k++)
            {
                scal = sigx[pos3 + k];
        ...
        F2C(dgels) ("N", &sz, &m, &int1, Fsc, &sz, rhs, &sz, temp, &ltemp, &info2);
        if (info2)
        {
            if (getWarningMode())
        ...
        if (*iters == 0)
        {
            ...
            F2C(dtrcon) ("1", "U", "N", &m, Fsc, &sz, &rcond, temp, iwork, &info2);
            if (info2 < 0)
            {
                if (getWarningMode())
             ...
             pos += blck_szs[i] * (blck_szs[i] + 1) / 2, pos2 += blck_szs[i] * blck_szs[i], pos3 += blck_szs[i], i++)
        {
             ...
             * store in temp */
            F2C(dgemv) ("N", &lngth, &m, &dbl1, F + sz + pos, &sz, dx, &int1, &dbl0, temp, &int1);
            ...
            F2C(dspev) ("N", "L", blck_szs + i, temp, sigx + pos3, NULL, &int1, temp + 2 * lngth, &info2);
            if (info2)
            {
                if (getWarningMode())
            ...
            cngrncb(1, blck_szs[i], temp + lngth, R + pos2, dZ + pos, temp + 2 * lngth);
            ...
            F2C(dspgv) (&int1, "N", "L", blck_szs + i, temp, temp + lngth, sigz + pos3, NULL, &int1, temp + 2 * lngth, &info2);
            if (info2)
            {
                if (getWarningMode())
                    sciprint(_("Error in dspgv, info = %d.\n"), info2);
        ...
        if (newgap <= Max(abstol, MINABSTOL))
            *info = 2;
        else if ((newl > 0.0 && newgap <= reltol * newl) || (newu < 0.0 && newgap <= -reltol * newu))
            *info = 3;
        ...
        if (nrmx > SIGTOL * nrmmax)
            if (nrmz > SIGTOL * nrmmax) /* compute primal and dual steps */
                while (lambda_ls > 1e-4)
                {
                    ...
                    alphax += (lambda_ls > 0.25) ? dalphax / (1.0 + lambda_ls) : dalphax;
                    alphaz += (lambda_ls > 0.25) ? dalphaz / (1.0 + lambda_ls) : dalphaz;
