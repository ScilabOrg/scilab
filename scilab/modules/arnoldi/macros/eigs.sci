...
function [d, v] = eigs(varargin)
    ...
    if(rhs >= 1)
        if((typeof(varargin(1)) <> "constant")  & typeof(varargin(1)) <> "function" & (typeof(varargin(1)) <> "sparse") | varargin(1) == [])
            error(msprintf(gettext("%s: Wrong type for input argument(s) #%d: a full or sparse square matrix or a function expected"), "eigs", 1));
        end
    end
    ...
    if(rhs > 1 & typeof(varargin(1)) ==  "function")
        if(size(varargin(2)) <> 1)
            error(msprintf(gettext("%s: Wrong type for input argument(s) #%d: a positive integer expected if the first input argument is a function, "), "eigs",2));
        end
        a_real = 1;
        a_sym = 0;
        ...
        case 5
            ...
            case 1
                ...
                if(and(~isfield(opts, ["tol", "maxiter", "ncv", "resid", "cholB"])))
                    error(msprintf(gettext("%s: Wrong type for input argument(s): If A is a matrix, the fieldname does not exist. \nUse opts with tol, maxiter, ncv, resid, cholB"), "eigs"));
                end
                if(isfield(opts, "tol"))
                    tol = opts.tol;
                end
...
function [res_d, res_v] = speigs(A, B, nev, which, maxiter, tol, ncv, cholB, resid, info)
    ...
    if((typeof(B) <> "constant") & (typeof(B) <> "sparse"))
        error(msprintf(gettext("%s: Wrong type for input argument(s) #%d: a empty matrix or full or sparse square matrix expected. \n"), "speigs", 2));
    end
    [mB, nB] = size(B);
    ...
    if(typeof(nev) <> "constant")
        error(msprintf(gettext("%s: Wrong type for input argument(s) #%d: an integer expected. \n"), "speigs", 3));
    end
    ...
    if(size(nev, "*") <> 1)
        error(msprintf(gettext("%s: Wrong dimension for input argument(s) #%d: k must be 1 by 1 size. \n"), "speigs", 3));
    end

    if(nev <> floor(nev) | (nev<=0))
        error(msprintf(gettext("%s: Wrong type for input argument(s) #%d: k must be a positive integer. \n"), "speigs", 3));
    end

    if(Asym & Areal & Breal)
        if(nev >= nA)
            error(msprintf(gettext("%s: Wrong type for input argument(s) #%d: For real symmetric problems, k must be in the range 1 to N - 1. \n"), "speigs", 3));
        end
    else
        if(nev >= nA - 1)
            error(msprintf(gettext("%s: Wrong type for input argument(s) #%d: For real non symmetric or complex problems, k must be in the range 1 to N - 2. \n"), "speigs", 3));
        end
    end
    ...
    case 1 then
        if(typeof(which) <> "constant" | which == [] | isnan(which))
            error(msprintf(gettext("%s: Wrong type for input argument(s) #%d: a scalar expected. \n"), "speigs", 4));
        end
        sigma = which;
        which = 'LM';
    case 10 then
        ...
        if(strcmp(which,'LM') ~= 0 & strcmp(which,'SM') ~= 0  & strcmp(which,'LR') ~= 0 & strcmp(which,'SR') ~= 0 & strcmp(which,'LI') ~= 0 & strcmp(which,'SI') ~= 0 & strcmp(which,'LA') ~= 0 & strcmp(which,'SA') ~= 0 & strcmp(which,'BE') ~= 0)
            if(Areal & Breal & Asym)
                error(msprintf(gettext("%s: Wrong input argument #%d : Unrecognized sigma value.\n Sigma must be one of LM, SM, LA, SA or BE. \n"), "speigs", 4));
            else
        ...
        error(msprintf(gettext("%s: Wrong type for input argument(s) #%d: a real scalar or a string expected. \n"), "speigs", 4));
    end
    ...
    if(typeof(maxiter) <> "constant")
        error(msprintf(gettext("%s: Wrong type for input argument(s) #%d: opts.maxiter must be a scalar. \n"), "speigs", 5));
    end
    ...
    if((maxiter <> floor(maxiter)) | (maxiter <= 0) )
        error(msprintf(gettext("%s: Wrong type for input argument(s) #%d: opts.maxiter must be an integer positive value. \n"), "speigs", 5));
    end
    ...
    if(typeof(tol) <> "constant")
        error(msprintf(gettext("%s: Wrong type for input argument(s) #%d: opts.tol must be a real scalar. \n"), "speigs", 6));
    end
    ...
    if(size(tol, "*") <> 1)
        error(msprintf(gettext("%s: Wrong dimension for input argument(s) #%d: opts.tol must be 1 by 1 size. \n"), "speigs", 6));
    end
    ...
    if(typeof(ncv) <> "constant")
        error(msprintf(gettext("%s: Wrong type for input argument(s) #%d: opts.ncv must be a integer scalar. \n"), "speigs", 7));
    end
    ...
    if(size(ncv, "*") > 1 | ncv <> floor(ncv) | (ncv <> [] & ncv <= 0))
        error(msprintf(gettext("%s: Wrong dimension for input argument(s) #%d: opts.ncv must be a integer scalar. \n"), "speigs", 7));
    end

    if(ncv <> floor(ncv))
        error(msprintf(gettext("%s: Wrong type for input argument(s) #%d: opts.nev must be a integer scalar. \n"), "speigs", 7));
    end
    ...
    if(isempty(ncv))
        ...
        if(ncv <= nev | ncv > nA)
            if(Asym & Areal & Breal)
                error(msprintf(gettext("%s: Wrong type for input argument(s) #%d : For real symmetric problems, NCV must be NEV < NCV <= N. \n"), "speigs", 7));
            elseif(~Asym & Areal & Breal)
                error(msprintf(gettext("%s: Wrong type for input argument(s) #%d : For real non symmetric problems, NCV must be NEV+2 < NCV < N. \n"), "speigs", 7));
            else
                error(msprintf(gettext("%s: Wrong type for input argument(s) #%d : For complex problems, NCV must be NEV+1 < NCV <= N. \n"), "speigs", 7));
            end
        end
    end
    ...
    if(typeof(cholB) <> "constant")
        error(msprintf(gettext("%s: Wrong type for input argument(s) #%d: opts.cholB must be a integer scalar. \n"), "speigs", 8));
    end
    ...
    if(size(cholB, "*") <> 1 | cholB <> floor(cholB) | cholB > 1)
        error(msprintf(gettext("%s: Wrong dimension for input argument(s) #%d: opts.cholB must be between 0 and 1 . \n"), "speigs", 8));
    end
    ...
    if(typeof(resid) <> "constant")
        error(msprintf(gettext("%s: Wrong type for input argument(s) #%d: A real or complex matrix expected. \n"), "speigs", 9));
    end

    [mRESID, nRESID] = size(resid);
    if(mRESID * nRESID ~= nA)
        error(msprintf(gettext("%s: Wrong dimension for input argument(s) #%d: Start vector opts.resid must be N by 1. \n"), "speigs", 9));
    end

    if(Areal & Breal)
        //resid complexe ?
        if(~isreal(resid))
            error(msprintf(gettext("%s: Wrong type for input argument(s) #%d: Start vector opts.resid must be real for real problems. \n"), "speigs", 9));
        end
    else
        if(isreal(resid))
            error(msprintf(gettext("%s: Wrong type for input argument(s) #%d: Start vector opts.resid must be complex for complex problems. \n"), "speigs", 9));
        end
    end
    ...
    if(cholB)
        if(~and(triu(B) == B))
            error(msprintf(gettext("%s: Wrong type for input argument(s) #%d: B must be symmmetric or hermitian, definite, semi positive. \n"), "speigs", 2));
        end
        R = B;
        Rprime = R';
    end
    ...
    if(Areal)
        if(Asym)
            ...
            lworkl = 3 * ncv * (ncv + 2);
            v = zeros(nA, nA);  //expected nA * ncv but http:forge.scilab.org/index.php/p/arpack-ng/issues/800/
            workl = zeros(lworkl, 1);
            workd = zeros(3 * nA, 1);
            dr = zeros(nev+1, 1);
            di = zeros(nev+1, 1);
...
function [res_d, res_v] = feigs(A_fun, nA, B, nev, which, maxiter, tol, ncv, cholB, resid, info, a_real, a_sym)
    ...
    if(size(nA,'*') <> 1 | ~isreal(nA) | typeof(nA) <> "constant")
        error(msprintf(gettext("%s: Wrong type for input argument(s) #%d: n must be a positive integer. \n"), "feigs", 2));
    end
    ...
    if(ncv == nA & rvec & ~a_sym & Areal & Breal)
        error(msprintf(gettext("%s: Wrong type for input argument(s) #%d : For real non symmetric problems, NCV must be less than N. \n"), "feigs", 7));
    end
    ...
    if(ierr <> 0)
        if(ierr == 10)
            error(msprintf(gettext("%s: Wrong type for input argument(s) #%d: n does not match rows number of matrix A. \n"), "feigs", 2));
        end
