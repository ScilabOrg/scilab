function [d, v] = eigs(varargin)
    ...
    if (rhs >= 1) then
        if ((typeof(varargin(1)) <> "constant")  & (typeof(varargin(1)) <> "sparse") | varargin(1) == [])
            error(msprintf(gettext("%s: Wrong type for input argument(s) #%d: a full or sparse square matrice expected"), "eigs",1));
        end
...
function [res_d, res_v] = speigs(A, B, nev, which, maxiter, tol, ncv, cholB, resid, info)
    ...
    [mMAXITER, nMAXITER] = size(maxiter);
    if(mMAXITER * nMAXITER <> 1)
    ...
    [mNCV, nNCV] = size(ncv);
    if(mNCV * nNCV > 1 | ncv <> floor(ncv) | (ncv <> [] & ncv <= 0))
...
//    if(ncv == nA & rvec & ~Asym & Areal & Breal)
...
//    end
    ...
    [mCHOLB, nCHOLB] = size(cholB);
    if(mCHOLB * nCHOLB <> 1 | cholB <> floor(cholB) | cholB > 1)
