...
{ 
	...
	// BMAT
	if ((matB == NULL) || (IPARAM[6]==1))  // if B = [] or mode = 1 -> bmat = 'I' : standart eigenvalue problem
	{
		bmat = "I";
	}
	...
	if(!cholB[0] && IPARAM[6] == 1 && matB != NULL)
	{
		if (!Bcomplex)  // B is real
		{	
	...
	if (!Acomplex && !Bcomplex)		// A and B are not complex
	{
		if(IPARAM[6] == 3)	// if mode = 3
		{
			...
			if (matB == NULL) // if B = [] -> standart eigenvalue problem : A-sigma
			{
				for (i=0; i<N; i++)
				{
		...
		while(IDO != 99)
		{
			...
			if(IDO == -1 || IDO == 1 || IDO == 2)
			{
				...
				{
					if (matB == NULL) // B = [] -> standart eigenvalue problem
					{
						// OP = A*x
						l = IPNTR[1]-1;
						for (i=0; i<N; i++)
	...
	else // A or/and B complex
	{
		if(IPARAM[6] == 3)	// mode = 3
		{
			...
			if (matB == NULL)	// standart eigenvalue problem
			{
				for (i=0; i<N; i++)
				{
			...
			if (IDO == -1 || IDO == 1 || IDO == 2)
			{
				if (IPARAM[6] == 1)  // mode = 1
					...
					else
					{
						...
						{
							if (matB == NULL)	// B = [] -> matB is empty -> standart eigenvalue problem
							{
								if (IDO == 2)
								{
									// y = B*x where B = I so workd[ipntr[1]-1:ipntr[1]+N-1] = workd[ipntr[0]-1:ipntr[0]+N-1]
