         ...
         int* INFO, double* cholB, int* INFO_EUPD, doublecomplex* eigenvalue, doublecomplex* eigenvector)
{
    ...
    else
    {
        ...
        if (ncv == N && RVEC && !Asym && !Acomplex && !Bcomplex)
        {
            return -1;
        }
    }
    ...
    if (matB != 0)
    {
        if (cholB[0]) // Comparison between B and upper triangular matrix
        {
            ...
            else	// if B is complex
            {
                ...
                /*memset(RCprime, 0, N * N * sizeof(doublecomplex));
                for(i = 0 ; i < N ; i++)
                {
                	for(j = 0 ; j <= i ; j++)
                	{
                		RCprime[j + i * N].r = BC[j + i * N].r;
                		RCprime[j + i * N].i = BC[j + i * N].i;
                	}
                }*/
    ...
    if (!Acomplex && !Bcomplex)		// A and B are not complex
    {
        ...
        else	// DNEUPD
        {
            ...
            C2F(dneupd)(&RVEC, HOWMNY, SELECT, DR, DI, Z, &LDV, &SIGMAR, &SIGMAI, WORKEV, bmat, &N, which, &nev, tol, RESID, &ncv, V, &LDV, IPARAM, IPNTR, WORKD, WORKL, &LWORKL, &INFO_EUPD[0]);
