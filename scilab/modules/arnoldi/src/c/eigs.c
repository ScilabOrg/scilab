...
// dherk does a rank k symmetric update for hermitian matrices
extern int C2F(dherk) (char* uplo, char* trans, int* n, int* k, doublecomplex* alphac,
                       doublecomplex* Ac, int* lda, doublecomplex* betac, doublecomplex* B, int* ldb);
                    ...
                    if (IPARAM[6] == 3) // mode = 3
                    {
                        ...
                        else  // matB == 1 so B is not empty and bmat = 'G'-> generalized eigenvalue problem
                        {
                            if (IDO == 2 || IDO == -1)
                            {
                                ...
                                {
                                    memcpy(WORKD + IPNTR[1] - 1, WORKD + IPNTR[0] - 1, N * sizeof(doublecomplex));
    ...
    else // A or/and B complex
    {
        if (IPARAM[6] == 3)	// mode = 3
        {
            ...
            if (matB == 0)	// standard eigenvalue problem
            {
                for (i = 0 ; i < N ; i++)
                {
                    AMSBC[i + i * N].r = AMSBC[i + i * N].r - SIGMA.r;
                    AMSBC[i + i * N].i = AMSBC[i + i * N].i - SIGMA.i;
                }
            }
            else	// generalized eigenvalue problem
            {
                if (cholB[0])
                {
                    C2F(zherk) ("u", "c", &N, &N, mSIGMA, BC, &N, &iOne, AMSBC, &N);
                    /* zherk does a symmetric update so we need to correct for the antisymmetric part
                       maybe ztrmm is faster */
                    for (i = 0; i < N; i++)
                    {
                        for (j = 0; j < i; j++)
                        {
                            AMSBC[i + j * N].r = AMSBC[j + i * N].r + AC[i + j * N].r - AC[j + i * N].r;
                            AMSBC[i + j * N].i = AMSBC[j + i * N].i + AC[i + j * N].i + AC[j + i * N].i;
                        }
                    }
                }
                else
                    ...
                    if (IPARAM[6] == 3) // if mode = 3
                    {
                        ...
                        else  // matB == 1 so B is not empty and bmat = 'G'-> generalized eigenvalue problem
                        {
                            if (IDO == 2 || IDO == -1)
                            {
                                ...
                                {
                                    memcpy(WORKDC + IPNTR[1] - 1, WORKDC + IPNTR[0] - 1, N * sizeof(doublecomplex));
                                    C2F(dtrmv) ("u", "n", "n", &N, BC, &N, WORKDC + IPNTR[1] - 1, &iOne);
                                    C2F(dtrmv) ("u", "t", "n", &N, BC, &N, WORKDC + IPNTR[1] - 1, &iOne);
                                }
                                else	// workd[ipntr[1]-1:ipntr[1]+N-1] = B *workd[ipntr[0]-1:ipntr[0]+N-1]
                                {
