...
extern int C2F(dseupd) (int *rvec, char *howmny, int *select, double *d,
                        ...
                        unsigned long howmany_length, unsigned long bmat_length, unsigned long which_len);
/*--------------------------------------------------------------------------*/
int sci_dseupd(char *fname, unsigned long fname_len)
{
    ...
    C2F(dseupd) (istk(pRVEC), cstk(pHOWMANY), istk(pSELECT),
                 ...
                 istk(pNCV), stk(pV), &LDV, istk(pIPARAM), istk(pIPNTR), stk(pWORKD), stk(pWORKL), &LWORKL, istk(pINFO), 1L, 1L, 1L, 2L);
