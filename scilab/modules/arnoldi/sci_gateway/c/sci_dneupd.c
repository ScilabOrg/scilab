...
extern int C2F(dneupd) (int *rvec, char *howmny, int *select, double *dr,
                        ...
                        int *ncv, double *v, int *ldv, int *iparam, int *ipntr, double *workd, double *workl, int *lworkl, int *info);
/*--------------------------------------------------------------------------*/
int sci_dneupd(char *fname, unsigned long fname_len)
{
    ...
    C2F(dneupd) (istk(pRVEC), cstk(pHOWMANY), istk(pSELECT),
                 ...
                 istk(pNCV), stk(pV), &LDV, istk(pIPARAM), istk(pIPNTR), stk(pWORKD), stk(pWORKL), &LWORKL, istk(pINFO));
