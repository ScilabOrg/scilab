...
extern int C2F(znaupd) (int *ido, char *bmat, int *n, char *which,
                        ...
                        int *ipntr, doublecomplex * workd, doublecomplex * workl, int *lworkl, double *rwork, int *info);
/*--------------------------------------------------------------------------*/
int sci_znaupd(char *fname, unsigned long fname_len)
{
    ...
    C2F(znaupd) (istk(pIDO), cstk(pBMAT), istk(pN),
                 ...
                 istk(pIPARAM), istk(pIPNTR), zstk(pWORKD), zstk(pWORKL), &LWORKL, stk(pRWORK), istk(pINFO));
