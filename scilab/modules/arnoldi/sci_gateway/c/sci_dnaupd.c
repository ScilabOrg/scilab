...
extern int C2F(dnaupd) (int *ido, char *bmat, int *n, char *which, int *nev,
                        ...
                        double *workl, int *lworkl, int *info, unsigned long bmat_len, unsigned long which_len);
/*--------------------------------------------------------------------------*/
int sci_dnaupd(char *fname, unsigned long fname_len)
{
    ...
    C2F(dnaupd) (istk(pIDO), cstk(pBMAT), istk(pN),
                 ...
                 stk(pRESID), istk(pNCV), stk(pV), &LDV, istk(pIPARAM), istk(pIPNTR), stk(pWORKD), stk(pWORKL), &LWORKL, istk(pINFO), 1L, 2L);
