...
int sci_eigs(char *fname,unsigned long fname_len)
{
	...
	*****************************************/
	...
	//check type
	sciErr = getVarType(pvApiCtx, piAddressVarOne, &TypeVarOne);
	if(sciErr.iErr || TypeVarOne != sci_matrix)
	{
		printError(&sciErr, 0);
	...
	if(isVarMatrixType(pvApiCtx, piAddressVarOne))
    {
...
{	
		...
		else
		{
			...
			if(RowsThree*ColsThree!=1 || RowsThree!=ColsThree)
			{
	...
	*****************************************/
	...
	if(isVarMatrixType(pvApiCtx, piAddressVarEight))
	{
