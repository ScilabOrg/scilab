...
/*------------------------------------------------------------------------*/
int get_contraints_property(void* _pvCtx, int iObjUID)
{
    int iParent = 0;
    ...
    switch (iLayout)
    {
        ...
        case GRID :
        {
            ...
            getGraphicObjectProperty(iObjUID, __GO_UI_GRID_CONTRAINTS__, jni_double_vector, (void **)&pdblGrid);
            if (pdblGrid == NULL)
            {
                Scierror(999, _("'%s' property does not exist for this handle.\n"), "contraints");
        ...
        case BORDER :
        {
            ...
            getGraphicObjectProperty(iObjUID, __GO_UI_BORDER_CONTRAINTS__, jni_int, (void **)&piBorder);
            if (piBorder == NULL)
            {
                Scierror(999, _("'%s' property does not exist for this handle.\n"), "contraints");
                return SET_PROPERTY_ERROR;
            ...
            switch (iBorder)
            {
                ...
                case SOUTH :
                    return sciReturnString(_pvCtx, "south");
                case NORTH :
                    return sciReturnString(_pvCtx, "north");
                case WEST :
                    return sciReturnString(_pvCtx, "west");
                case EAST :
                    return sciReturnString(_pvCtx, "east");
            }
        }
        break;
    }
