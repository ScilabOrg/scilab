...
int set_links_property( sciPointObj * pobj, size_t stackPointer, int valueType, int nbRow, int nbCol )
{
    ...
    parentAxes = (char*) getGraphicObjectProperty(pobj->UID, __GO_PARENT_AXES__, jni_string);
    ...
    for (i=0; i<nblegends; i++)
    {
        ...
        type = (char*) getGraphicObjectProperty(polylineObject->UID, __GO_TYPE__, jni_string);
        ...
        polylineParentAxes = (char*) getGraphicObjectProperty(pobj->UID, __GO_PARENT_AXES__, jni_string);
