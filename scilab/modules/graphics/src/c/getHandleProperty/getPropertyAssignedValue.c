...
int tryGetBooleanValueFromStack(size_t stackPointer, int valueType, int nbRow, int nbCol, char *propertyName)
{
    if (valueType == sci_strings)
    {
        ...
        if (isStringParamEqual(stackPointer, "off"))
            return FALSE;
        if (isStringParamEqual(stackPointer, "1"))
            return TRUE;
        if (isStringParamEqual(stackPointer, "0"))
            return FALSE;
        if (isStringParamEqual(stackPointer, "T"))
            return TRUE;
        if (isStringParamEqual(stackPointer, "F"))
            return FALSE;
    ...
    {
        if (getDoubleFromStack(stackPointer) == 0)
            return FALSE;
        return TRUE;
    }
