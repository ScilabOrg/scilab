...
int set_position_property( sciPointObj * pobj, size_t stackPointer, int valueType, int nbRow, int nbCol )
{
    ...
    /* Type test required since a position set require a 4-element, 3-element, and 2-element vector
    ...
    else if (strcmp(type, __GO_LABEL__) == 0)
    {
        ...
        curPos = (double*) getGraphicObjectProperty(pobj->UID, __GO_POSITION__, jni_double_vector);

        posToSet[0] = values[0];
        posToSet[1] = values[1];
        posToSet[2] = curPos[2];

        status = setGraphicObjectProperty(pobj->UID, __GO_POSITION__, posToSet, jni_double_vector, 3);
