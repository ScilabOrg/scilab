...
/*------------------------------------------------------------------------*/
int set_contraints_property(void* _pvCtx, int iObjUID, void* _pvData, int valueType, int nbRow, int nbCol)
{
    int iParent = 0;
    ...
    switch (iLayout)
    {
        ...
        case GRID :
        {
            ...
            if (setGraphicObjectProperty(iObjUID, __GO_UI_GRID_CONTRAINTS__, _pvData, jni_double_vector, 6) == FALSE)
            {
                Scierror(999, _("'%s' property does not exist for this handle.\n"), "contraints");
                return SET_PROPERTY_ERROR;
            }
        ...
        case BORDER :
        {
            ...
            else if (stricmp((char*)_pvData, "south") == 0)
            {
                iBorder = SOUTH;
            }
            else if (stricmp((char*)_pvData, "north") == 0)
            {
                iBorder = NORTH;
            }
            else if (stricmp((char*)_pvData, "west") == 0)
            {
                iBorder = WEST;
            }
            else if (stricmp((char*)_pvData, "east") == 0)
            {
                iBorder = EAST;
            }
            else
            {
                Scierror(999, _("Wrong value for '%s' property: %s, %s or %s expected.\n"), "contraints", "'south', 'north'", "'west', 'east'", "'center'");
                return SET_PROPERTY_ERROR;
            }
            if (setGraphicObjectProperty(iObjUID, __GO_UI_BORDER_CONTRAINTS__, &iBorder, jni_int, 1) == FALSE)
            {
                Scierror(999, _("'%s' property does not exist for this handle.\n"), "contraints");
                return SET_PROPERTY_ERROR;
            }
