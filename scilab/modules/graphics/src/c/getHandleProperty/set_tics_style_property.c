...
int set_tics_style_property(sciPointObj * pobj, size_t stackPointer, int valueType, int nbRow, int nbCol)
{
    ...
    if (!isStringParamEqual(stackPointer, "v") && !isStringParamEqual(stackPointer, "r") && !isStringParamEqual(stackPointer, "i"))
    {
    ...
    if (pAXES_FEATURE(pobj)->str != NULL)
    {
        ...
        {
            sciprint(_
                     ("Warning: %s has been set by user ; you must first increase the size of the %s string vector before switching to the new %s mode.\n"),
