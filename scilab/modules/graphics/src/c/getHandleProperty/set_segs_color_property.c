...
int set_segs_color_property( sciPointObj * pobj, size_t stackPointer, int valueType, int nbRow, int nbCol )
{
    ...
    tmp = (int*) getGraphicObjectProperty(pobj->UID, __GO_NUMBER_ARROWS__, jni_int);
    ...
    if (nbRow*nbCol != 1 && nbRow*nbCol != nbSegs)
    {
    ...
    if ( nbRow * nbCol == 1 )
    {
        ...
        status = setGraphicObjectProperty(pobj->UID, __GO_SEGS_COLORS__, &value, jni_int_vector, 1);
    }
    else if ( nbRow * nbCol == nbSegs )
    {
        segsColors = (int*) MALLOC(nbSegs*sizeof(int));
        ...
        copyDoubleVectorToIntFromStack( stackPointer, segsColors, nbSegs);
        status = setGraphicObjectProperty(pobj->UID, __GO_SEGS_COLORS__, segsColors, jni_int_vector, nbSegs);
