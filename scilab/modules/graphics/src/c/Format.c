...
int ComputeC_format(sciPointObj * pobj, char *c_format)
{
    ...
    for (i = 0; i < (*nx); i++)
    {
        x[i] = pAXES_FEATURE(pobj)->vx[i];
    }

    ny[0] = pAXES_FEATURE(pobj)->ny;
    for (i = 0; i < (*ny); i++)
    {
        y[i] = pAXES_FEATURE(pobj)->vy[i];
    }
    ...
    case 'd':
        ...
        case 'r':
            ...
            break;
        case 'i':
            ChoixFormatE(c_format, (x[0] * exp10(x[2])), (x[1] * exp10(x[2])), ((x[1] * exp10(x[2])) - (x[0] * exp10(x[2]))) / x[3]);
    ...
    case 'l':
        ...
        case 'r':
            ...
            break;
        case 'i':
            ChoixFormatE(c_format, (y[0] * exp10(y[2])), (y[1] * exp10(y[2])), ((y[1] * exp10(y[2])) - (y[0] * exp10(y[2]))) / y[3]);
            break;              /* Adding F.Leray 06.05.04 */
        }
...
int ChooseGoodFormat(char *c_format, char logflag, double *_grads, int n_grads)
{
    ...
    if (logflag == 'l')
    {
        ChoixFormatE(c_format, exp10(_grads[0]), exp10(_grads[last_index]), ((exp10(_grads[last_index])) - (exp10(_grads[0]))) / (last_index));
    }
    else
    {
        ChoixFormatE(c_format, _grads[0], _grads[last_index], ((_grads[last_index]) - (_grads[0])) / (last_index)); /* Adding F.Leray 06.05.04 */
    }
