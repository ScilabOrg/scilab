...
static void look(ptr_level_f, int i, int j, int ib, int jb, int qq, double Cont, int style);

static int ffnd(ptr_level_f, int, int, int, int, int, int, int, int, int, int, double, int *);
...
static void look(ptr_level_f func, int i, int j, int ib, int jb, int qq, double Cont, int style)
{
    ...
    case 1:
        ...
        (*func) (0, Cont, x_cont(i), f_intercept(Cont, phi_cont(i, jm), y_cont(jm), phi_cont(i, j), y_cont(j)));
        break;
    case 2:
        ...
        /* Storing intersection point */
        (*func) (0, Cont, f_intercept(Cont, phi_cont(im, j), x_cont(im), phi_cont(i, j), x_cont(i)), y_cont(j));
        break;
    case 3:
        ...
        /* Storing intersection point */
        (*func) (0, Cont, x_cont(i), f_intercept(Cont, phi_cont(i, j), y_cont(j), phi_cont(i, jp), y_cont(jp)));
        break;
    case 4:
        ...
        /* Storing intersection point */
        (*func) (0, Cont, f_intercept(Cont, phi_cont(i, j), x_cont(i), phi_cont(ip, j), x_cont(ip)), y_cont(j));
        break;
    default:
        break;
    }
...
static int contourI(ptr_level_f func, double *x, double *y, double *z, double *zCont, int *N, int *style, int *err)
{
    ...
    /* Allocation */
    Gcont_size = 0;
                  /** initialize the array indices for storing contours **/
    ...
    /** all the boundary segments **/
        for (k = 1; k < n5; k++)
        {
            ...
            ib = xbd_cont[k - 1];
            jb = ybd_cont[k - 1];
            if (not_same_sign(phi_cont(i, j) - zCont[c], phi_cont(ib, jb) - zCont[c]))
                look(func, i, j, ib, jb, 1L, zCont[c], stylec);
        }
    /** inside segments **/
        for (i = 1; i < n1 - 1; i++)
            for (j = 1; j < n2 - 1; j++)
                if (not_same_sign(phi_cont(i, j) - zCont[c], phi_cont(i, j - 1) - zCont[c]))
...
static int ffnd(ptr_level_f func, int i1, int i2, int i3, int i4, int jj1, int jj2, int jj3, int jj4, int ent, int qq, double Cont, int *zds)
{
    ...
    *zds = 0;
    ...
    (*func) (1, Cont, f_intercept(0.0, phi1, x_cont(i1), phiav, xav), f_intercept(0.0, phi1, y_cont(jj1), phiav, yav));
    /*
    ...
    for (i = 0;; i++)
    {
        ...
        if (phiav != 0.0)
        {
            (*func) (1, Cont, f_intercept(0.0, phi2, x_cont(i2), phiav, xav), f_intercept(0.0, phi2, y_cont(jj2), phiav, yav));
        }
    ...
    (*func) (1, Cont, f_intercept(0.0, phi1, x_cont(i1), phi2, x_cont(i2)), f_intercept(0.0, phi1, y_cont(jj1), phi2, y_cont(jj2)));
