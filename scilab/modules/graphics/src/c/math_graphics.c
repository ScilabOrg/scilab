...
void rotate2Dim(double from[2], double center[2], double cosAngle, double sinAngle, double dest[2])
{
    double diff[2];
...
{
    return (areOnSameSideOfLine(point, a, b, c) && areOnSameSideOfLine(point, b, a, c) && areOnSameSideOfLine(point, c, a, b));
}

/*----------------------------------------------------------------------------*/
BOOL areOnSameSideOfLine(const double p1[2], const double p2[2], const double a[2], const double b[2])
{
...
void mat4DMult(const double mat4D[4][4], const double vect3D[3], double res[3])
{
    ...
    for (i = 0; i < 4; i++)
    {
        res4D[i] = vect3D[0] * mat4D[i][0] + vect3D[1] * mat4D[i][1] + vect3D[2] * mat4D[i][2] + mat4D[i][3];
    }
