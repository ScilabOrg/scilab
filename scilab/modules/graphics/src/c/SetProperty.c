 ...
 */
int sciSetColormap(sciPointObj * pobj, double *rgbmat, int m, int n)
{
...
void sciRecursiveUpdateBaW(sciPointObj * pobj, int old_m, int m)
{
    ...
    if ((sciGetEntityType(pobj) != SCI_AXES) && !sciIsAutomaticallyRedrawn(pobj))
    {
        if (old_m + 1 == sciGetForeground(pobj))
        {                       /* 0 => deals with Foreground */
        ...
        if (old_m + 1 == sciGetBackground(pobj))
    ...
    if ((sciGetEntityType(pobj) == SCI_TEXT) ||
        ...
        (sciGetEntityType(pobj) == SCI_SUBWIN) || (sciGetEntityType(pobj) == SCI_FIGURE) || (sciGetEntityType(pobj) == SCI_LABEL))
    {
        ...
        if (old_m + 1 == sciGetFontBackground(pobj))
        {                       /* 3 => deals with FontBackground */
            sciSetNumColors(pobj, m);
    ...
    if ((sciGetEntityType(pobj) == SCI_FIGURE) ||
        ...
        (sciGetEntityType(pobj) == SCI_SURFACE) || (sciGetEntityType(pobj) == SCI_AXES) || (sciGetEntityType(pobj) == SCI_SEGS))
    {
        if (old_m + 1 == sciGetMarkForeground(pobj))
        {                       /* 4 => deals with MarkForeground */
            sciSetNumColors(pobj, m);
        ...
        if (old_m + 1 == sciGetMarkBackground(pobj))
        {                       /* 5 => deals with MarkBackground */
            sciSetNumColors(pobj, m);
...
int sciInitBackground(sciPointObj * pobj, int colorindex)
{
    ...
    if (!sciCheckColorIndex(pobj, colorindex))
        return 0;
...
int sciInitMarkForeground(sciPointObj * pobj, int colorindex)
{
    ...
    if (sciGetGraphicContext(pobj) != NULL)
    {
        sciGetGraphicContext(pobj)->markforeground = Max(-1, Min(colorindex - 1, sciGetNumColors(pobj) + 1));
        return 0;
    }
...
int sciInitMarkBackground(sciPointObj * pobj, int colorindex)
{
    ...
    if (sciGetGraphicContext(pobj) != NULL)
    {
        sciGetGraphicContext(pobj)->markbackground = Max(-1, Min(colorindex - 1, sciGetNumColors(pobj) + 1));
        return 0;
    }
 ...
 */
int sciSetFontBackground(sciPointObj * pobj, int colorindex)
{
    colorindex = sciSetGoodIndex(pobj, colorindex); /* Adding F.Leray 31.03.04 */
...
int sciInitIsClipping(sciPointObj * pobj, int value)
{
    ...
    case SCI_SUBWIN:
        ...
        if (value > 0)
            pSUBWIN_FEATURE(pobj)->clip_region_set = 1;
        break;
    case SCI_ARC:
        pARC_FEATURE(pobj)->isclip = value;
        if (value > 0)
            pARC_FEATURE(pobj)->clip_region_set = 1;
        break;
    case SCI_POLYLINE:
        pPOLYLINE_FEATURE(pobj)->isclip = value;
        if (value > 0)
            pPOLYLINE_FEATURE(pobj)->clip_region_set = 1;
        break;
    case SCI_RECTANGLE:
        pRECTANGLE_FEATURE(pobj)->isclip = value;
        if (value > 0)
            pRECTANGLE_FEATURE(pobj)->clip_region_set = 1;
        break;
    case SCI_SEGS:
        pSEGS_FEATURE(pobj)->isclip = value;
        if (value > 0)
            pSEGS_FEATURE(pobj)->clip_region_set = 1;
        break;
    case SCI_TEXT:
        pTEXT_FEATURE(pobj)->isclip = value;
        if (value > 0)
            pTEXT_FEATURE(pobj)->clip_region_set = 1;
        break;
    case SCI_AXES:
        pAXES_FEATURE(pobj)->isclip = value;
        if (value > 0)
            pAXES_FEATURE(pobj)->clip_region_set = 1;
        break;
    case SCI_SURFACE:
        pSURFACE_FEATURE(pobj)->isclip = value;
        if (value > 0)
            pSURFACE_FEATURE(pobj)->clip_region_set = 1;
        break;
    case SCI_FEC:
        pFEC_FEATURE(pobj)->isclip = value;
        if (value > 0)
            pFEC_FEATURE(pobj)->clip_region_set = 1;
        break;
    case SCI_GRAYPLOT:
        pGRAYPLOT_FEATURE(pobj)->isclip = value;
        if (value > 0)
            pGRAYPLOT_FEATURE(pobj)->clip_region_set = 1;
        break;
    case SCI_LEGEND:
        pLEGEND_FEATURE(pobj)->isclip = value;
        if (value > 0)
        {
            pLEGEND_FEATURE(pobj)->clip_region_set = 1;
        }
        break;
    case SCI_AGREG:
    case SCI_FIGURE:
...
int sciSetClipping(sciPointObj * pobj, double pclip[4])
{
    ...
    case SCI_SUBWIN:
        for (i = 0; i < 4; i++)
        {
            pSUBWIN_FEATURE(pobj)->clip_region[i] = pclip[i];
        }
        break;
    case SCI_ARC:
        for (i = 0; i < 4; i++)
        {
            pARC_FEATURE(pobj)->clip_region[i] = pclip[i];
        }
        break;
    case SCI_POLYLINE:
        for (i = 0; i < 4; i++)
        {
            pPOLYLINE_FEATURE(pobj)->clip_region[i] = pclip[i];
        }
        break;
    case SCI_RECTANGLE:
        for (i = 0; i < 4; i++)
        {
            pRECTANGLE_FEATURE(pobj)->clip_region[i] = pclip[i];
        }
        break;
    case SCI_SEGS:
        for (i = 0; i < 4; i++)
        {
            pSEGS_FEATURE(pobj)->clip_region[i] = pclip[i];
        }
        break;
    case SCI_TEXT:
        for (i = 0; i < 4; i++)
        {
            pTEXT_FEATURE(pobj)->clip_region[i] = pclip[i];
        }
        break;
    case SCI_AXES:
        for (i = 0; i < 4; i++)
        {
            pAXES_FEATURE(pobj)->clip_region[i] = pclip[i];
        }
        break;
    case SCI_SURFACE:
        for (i = 0; i < 4; i++)
        {
            pSURFACE_FEATURE(pobj)->clip_region[i] = pclip[i];
        }
        break;
    case SCI_LEGEND:
        for (i = 0; i < 4; i++)
        {
            pLEGEND_FEATURE(pobj)->clip_region[i] = pclip[i];
        }
        break;
    case SCI_FEC:
        for (i = 0; i < 4; i++)
        {
            pFEC_FEATURE(pobj)->clip_region[i] = pclip[i];
        }
        break;
    case SCI_GRAYPLOT:
        for (i = 0; i < 4; i++)
        {
            pGRAYPLOT_FEATURE(pobj)->clip_region[i] = pclip[i];
        }
        break;
    case SCI_LABEL:
...
{
    if ((sciInitGraphicContext(sciGetCurrentFigure()) == -1) || (sciInitGraphicMode(sciGetCurrentFigure()) == -1) || (sciInitFontContext(sciGetCurrentFigure()) == -1)) /* Adding F.Leray 13.04.04 to have the completed init. */
    {
...
{
    if (newName != NULL && sciGetName(pobj) != NULL && strcmp(sciGetName(pobj), newName) == 0)
    {
        // nothing to do
        return 1;
    }
...
void sciSelectFirstSubwin(sciPointObj * parentFigure)
{
    ...
    if (figureSons != (sciSons *) NULL)
    {
        ...
        while ((figureSons->pnext != (sciSons *) NULL) && (sciGetEntityType(figureSons->pointobj) != SCI_SUBWIN))
        {
...
int sciSetPoint(sciPointObj * pthis, double *tab, int *numrow, int *numcol)
{
    ...
    case SCI_POLYLINE:
        ...
        if (*numrow != n1)      /* SS 30/1/02 */
        {
            ...
            pPOLYLINE_FEATURE(pthis)->pvx = NULL;
            FREE(pPOLYLINE_FEATURE(pthis)->pvy);
            pPOLYLINE_FEATURE(pthis)->pvy = NULL;
    ...
    case SCI_SEGS:
        if (pSEGS_FEATURE(pthis)->ptype <= 0)
        {
            ...
            else
            {
                if ((*numcol == 3) && (pSEGS_FEATURE(pthis)->vz == NULL))
                    if ((pSEGS_FEATURE(pthis)->vz = MALLOC(n1 * sizeof(double))) == NULL)
                        return -1;
        ...
        {                       /* Strange test object == Champ: e=gce(); e.data = e.data
            ...
            if (*numrow != n1)  /* SS 30/1/02 */
            {
                ...
                if ((pvfx = MALLOC((n1 * n1) * sizeof(double))) == NULL)
                    return -1;
    ...
    case SCI_SURFACE:          /* DJ.A 2003 */
        ...
        break;
    case SCI_GRAYPLOT:
        if (pGRAYPLOT_FEATURE(pthis)->type == 0)
        {                       /* gray plot */
            ...
            if (pGRAYPLOT_FEATURE(pthis)->ny != ny || pGRAYPLOT_FEATURE(pthis)->nx != nx)
            {
                ...
                pGRAYPLOT_FEATURE(pthis)->pvecx = pvecx;
                FREE(pGRAYPLOT_FEATURE(pthis)->pvecy);
                pGRAYPLOT_FEATURE(pthis)->pvecy = pvecy;
                FREE(pGRAYPLOT_FEATURE(pthis)->pvecz);
                pGRAYPLOT_FEATURE(pthis)->pvecz = pvecz;
            }
            for (i = 0; i < nx; i++)
    ...
    case SCI_FEC:
        {
            ...
            if (pFEC_FEATURE(pthis)->Nnode != Nnode)
            {
                ...
                pFEC_FEATURE(pthis)->pvecx = pvecx;
                FREE(pFEC_FEATURE(pthis)->pvecy);
                pFEC_FEATURE(pthis)->pvecy = pvecy;
                FREE(pFEC_FEATURE(pthis)->pfun);
                pFEC_FEATURE(pthis)->pfun = pfun;
            }
            for (i = 0; i < Nnode; i++)
            {
...
int sciInitIs3d(sciPointObj * pObj, BOOL is3d)
{
    ...
    case SCI_SUBWIN:
        if (is3d)
        {
            ...
            Obj_RedrawNewAngle(pObj, pSUBWIN_FEATURE(pObj)->alpha_kp, pSUBWIN_FEATURE(pObj)->theta_kp);
        }
        else
        {
            /* switch to 2d */
...
{
    if (!sciCheckColorIndex(pObj, newColor))
    {
        return -1;
    }
...
{
    if (!sciCheckColorIndex(pObj, xStyle) || !sciCheckColorIndex(pObj, yStyle) || !sciCheckColorIndex(pObj, zStyle))
    {
        return -1;
    }
...
int sciInitIsEventHandlerEnable(sciPointObj * pObj, BOOL enable)
{
    if (sciGetEntityType(pObj) == SCI_FIGURE)
    {
         ...
         ** raise a Warning.
         */
        if (ppFigure != NULL && ppFigure->eventHandler != NULL && (strcmp(ppFigure->eventHandler, "") != 0 || enable == FALSE))
        {
...
int sciSetLogFlags(sciPointObj * pObj, char logFlags[3])
{
    ...
    if (logFlags[0] == curLogFlags[0] && logFlags[1] == curLogFlags[1] && logFlags[2] == curLogFlags[2])
    {
        // nothing to do
        return 1;
    }
