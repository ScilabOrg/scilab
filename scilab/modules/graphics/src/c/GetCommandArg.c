...
int get_rect_arg(char *fname, int pos, rhs_opts opts[], double **rect)
{
    ...
    else if ((kopt = FindOpt("rect", opts)))
    {                           /* named argument: rect=value */
...
int get_zminmax_arg(char *fname, int pos, rhs_opts opts[], double **zminmax)
{
    ...
    else if ((kopt = FindOpt("zminmax", opts)))
    {                           /* named argument: rect=value */
        GetRhsVar(kopt, MATRIX_OF_DOUBLE_DATATYPE, &m, &n, &l);
        if (m * n != 2)
        {
        ...
        if (VarType(pos))
        {
            ...
            else
            {
                if (((*cstk(l) != 'g') && (*cstk(l) != 'e') && (*cstk(l) != 'o')) ||
                    (*cstk(l + 1) != 'l' && *cstk(l + 1) != 'n') || (*cstk(l + 2) != 'l' && *cstk(l + 2) != 'n'))
                {
                    Err = pos;
                    SciError(116);
                    return 0;
...
int get_optional_double_arg(char *fname, int pos, char *name, double **value, int sz, rhs_opts opts[])
{
    ...
    if (pos < first_opt)
    {
        ...
        {
            GetRhsVar(pos, MATRIX_OF_DOUBLE_DATATYPE, &m, &n, &l);
            CheckLength(pos, m * n, sz) * value = stk(l);
        }
    }
    else if ((kopt = FindOpt(name, opts)))
    {
        GetRhsVar(kopt, MATRIX_OF_DOUBLE_DATATYPE, &m, &n, &l);
        CheckLength(kopt, m * n, sz) * value = stk(l);
    }
    return 1;
}
...
int get_optional_int_arg(char *fname, int pos, char *name, int **value, int sz, rhs_opts opts[])
{
    ...
    if (pos < first_opt)
    {
        ...
        {
            GetRhsVar(pos, MATRIX_OF_INTEGER_DATATYPE, &m, &n, &l);
            CheckLength(pos, m * n, sz) * value = istk(l);
        }
    }
    else if ((kopt = FindOpt(name, opts)))
    {
        GetRhsVar(kopt, MATRIX_OF_INTEGER_DATATYPE, &m, &n, &l);
        CheckLength(kopt, m * n, sz) * value = istk(l);
    }
    return 1;
}
