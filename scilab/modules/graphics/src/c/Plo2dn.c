            ...
            BOOL flagNax, int lstr1, int lstr2)
{
    ...
    if (flagNax == TRUE)
    {
        ...
        if (logFlags[0] == 0 && logFlags[1] == 0)
        {
			...
			getGraphicObjectProperty(psubwinUID, __GO_X_AXIS_TICKS_LABELS__, jni_string_vector, (void **) &labels);
			for (i=0; i<10;i++)
			{
				printf("labels :%s \n",labels[i]);
			}
    ...
    //            sprintf(stringVector[i], "%.3f", dYGrads[i]);
				//stringVector[iSize] = '\0';
				//printf("stringVector : %s \n",stringVector[i]);
				//if (stringVector[iSize] == '\0')
				//{
				//	printf("ok \n");
				//}
				//printf("%d \n", i);
