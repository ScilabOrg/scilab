 ...
 * Its graphic and font context are initialized.
...
ConstructLegend (sciPointObj * pparentsubwin, char **text, long long tabofhandles[], int nblegends)
{
    ...
    tmp = (int*) getGraphicObjectProperty(pparentsubwin->UID, __GO_HAS_LEGEND_CHILD__, jni_bool);
    legendPresent = *tmp;
...
ConstructCompoundSeq (int number)
{
    ...
    tmp = (int*) getGraphicObjectProperty(psubwin->UID, __GO_CHILDREN_COUNT__, jni_int);
    numberChildren = *tmp;

    children = (char**) getGraphicObjectProperty(psubwin->UID, __GO_CHILDREN__, jni_string_vector);
    ...
    tmp = (int*) getGraphicObjectProperty(parentFigure, __GO_VISIBLE__, jni_bool);
    visible = *tmp;
    setGraphicObjectProperty(pobj->UID, __GO_VISIBLE__, &visible, jni_bool, 1);
