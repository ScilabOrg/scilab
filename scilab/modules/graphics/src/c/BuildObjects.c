 ...
 */
sciPointObj *ConstructSubWin(sciPointObj * pparentfigure)
{
    ...
    if (sciGetEntityType(pparentfigure) == SCI_FIGURE)
    {
        ...
        for (i = 0; i < (ppsubwin->axes).nxgrads; i++)
            ppsubwin->axes.xgrads[i] = ppaxesmdl->axes.xgrads[i];
        for (i = 0; i < (ppsubwin->axes).nygrads; i++)
            ppsubwin->axes.ygrads[i] = ppaxesmdl->axes.ygrads[i];
        for (i = 0; i < (ppsubwin->axes).nzgrads; i++)
            ppsubwin->axes.zgrads[i] = ppaxesmdl->axes.zgrads[i];
        ...
        CopyUserGrads(ppaxesmdl->axes.u_xgrads, ppsubwin->axes.u_xgrads, ppsubwin->axes.u_nxgrads);
        (ppsubwin->axes).u_ygrads = AllocUserGrads(ppsubwin->axes.u_ygrads, ppaxesmdl->axes.u_nygrads);
        CopyUserGrads(ppaxesmdl->axes.u_ygrads, ppsubwin->axes.u_ygrads, ppsubwin->axes.u_nygrads);
        (ppsubwin->axes).u_zgrads = AllocUserGrads(ppsubwin->axes.u_zgrads, ppaxesmdl->axes.u_nzgrads);
        CopyUserGrads(ppaxesmdl->axes.u_zgrads, ppsubwin->axes.u_zgrads, ppsubwin->axes.u_nzgrads);
        (ppsubwin->axes).u_xlabels = AllocAndSetUserLabelsFromMdl(ppsubwin->axes.u_xlabels, ppaxesmdl->axes.u_xlabels, ppsubwin->axes.u_nxgrads);

        (ppsubwin->axes).u_ylabels = AllocAndSetUserLabelsFromMdl(ppsubwin->axes.u_ylabels, ppaxesmdl->axes.u_ylabels, ppsubwin->axes.u_nygrads);

        (ppsubwin->axes).u_zlabels = AllocAndSetUserLabelsFromMdl(ppsubwin->axes.u_zlabels, ppaxesmdl->axes.u_zlabels, ppsubwin->axes.u_nzgrads);
        ...
        sciSetStrings(ppsubwin->mon_title, sciGetText(pSUBWIN_FEATURE(paxesmdl)->mon_title));
        ...
        sciSetStrings(ppsubwin->mon_x_label, sciGetText(pSUBWIN_FEATURE(paxesmdl)->mon_x_label));
        ...
        sciSetStrings(ppsubwin->mon_y_label, sciGetText(pSUBWIN_FEATURE(paxesmdl)->mon_y_label));
        ...
        sciSetStrings(ppsubwin->mon_z_label, sciGetText(pSUBWIN_FEATURE(paxesmdl)->mon_z_label));

        /* labels auto_position modes */
        pLABEL_FEATURE(ppsubwin->mon_x_label)->auto_position = pLABEL_FEATURE(ppaxesmdl->mon_x_label)->auto_position;

        pLABEL_FEATURE(ppsubwin->mon_y_label)->auto_position = pLABEL_FEATURE(ppaxesmdl->mon_y_label)->auto_position;

        pLABEL_FEATURE(ppsubwin->mon_z_label)->auto_position = pLABEL_FEATURE(ppaxesmdl->mon_z_label)->auto_position;

        pLABEL_FEATURE(ppsubwin->mon_title)->auto_position = pLABEL_FEATURE(ppaxesmdl->mon_title)->auto_position;

        /* labels auto_rotation modes */
        pLABEL_FEATURE(ppsubwin->mon_x_label)->auto_rotation = pLABEL_FEATURE(ppaxesmdl->mon_x_label)->auto_rotation;

        pLABEL_FEATURE(ppsubwin->mon_y_label)->auto_rotation = pLABEL_FEATURE(ppaxesmdl->mon_y_label)->auto_rotation;

        pLABEL_FEATURE(ppsubwin->mon_z_label)->auto_rotation = pLABEL_FEATURE(ppaxesmdl->mon_z_label)->auto_rotation;

        pLABEL_FEATURE(ppsubwin->mon_title)->auto_rotation = pLABEL_FEATURE(ppaxesmdl->mon_title)->auto_rotation;
 ...
 */
sciPointObj *ConstructLegend(sciPointObj * pparentsubwin, char **text, long long tabofhandles[], int nblegends)
{
    sciPointObj *pobj = (sciPointObj *) NULL;
    sciLegend *ppLegend;
    ...
    if (sciGetEntityType(pparentsubwin) == SCI_SUBWIN)
    {
        ...
        if ((ppLegend->tabofhandles = MALLOC(nblegends * sizeof(long long))) == NULL)
        {
...
sciPointObj *ConstructSurface(sciPointObj * pparentsubwin, sciTypeOf3D typeof3d,
                              ...
                              int *isfac, int *m1, int *n1, int *m2, int *n2, int *m3, int *n3, int *m3n, int *n3n)
{
    sciPointObj *pobj = (sciPointObj *) NULL;
    ...
    if (sciGetEntityType(pparentsubwin) == SCI_SUBWIN)
    {
      ...
      /*-------Replaced by: --------*/
        ...
        if (izc != 0 && nc > 0)
        {                       /* Allocation of good size depending on flagcolor for nc (see above) */
 ...
 */
sciPointObj *ConstructGrayplot(sciPointObj * pparentsubwin, double *pvecx, double *pvecy, double *pvecz, int n1, int n2, int type)
{
    sciPointObj *pobj = (sciPointObj *) NULL;
 ...
 * @see sciSetCurrentObj
 */
sciPointObj *ConstructSegs(sciPointObj * pparentsubwin, int type,
 ...
 */
sciPointObj *ConstructCompound(long *handelsvalue, int number)  /* Conflicting types with definition */
{
 ...
 * constructes an Compound of with the last n entities created in the current subwindow
 ...
 null->s1->s2->...->sn->null
*/
sciPointObj *ConstructCompoundSeq(int number)
{
    sciPointObj *pobj;
    int i;
 ...
 * @return  : pointer sciPointObj if ok , NULL if not
 */
sciPointObj *ConstructLabel(sciPointObj * pparentsubwin, char *text, int type)
{
    sciPointObj *pobj = NULL;
    ...
    if (sciGetEntityType(pparentsubwin) == SCI_SUBWIN)
    {
        ...
        ppLabel->text = allocateText(pparentsubwin, &emptyString, 1, 1,
                                     0.0, 0.0, TRUE, NULL, FALSE, &defaultColor, &defaultColor, FALSE, FALSE, FALSE, ALIGN_LEFT);
