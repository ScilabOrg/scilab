 ...
 */
int sciAddCallback(sciPointObj * pthis, char *code, int len, int mevent)
{
 ...
 */
int sciDelCallback(sciPointObj * pthis)
{
    switch (sciGetEntityType(pthis))
    {
    case SCI_SUBWIN:
    ...
    case SCI_SURFACE:
        ...
        case SCI_FAC3D:
            ...
            if (displacementSize == 3)
            {
                ...
                {
                    if ((pSURFACE_FEATURE(pobj)->pvecz = MALLOC(n * sizeof(double))) == NULL)
                        return -1;
                    for (i = 0; i < n; i++)
