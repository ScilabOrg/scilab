               ...
               const double initialRect[4], double endRect[4], int * usedButton)
{
    ...
    if (initialRect != NULL)
    {
        ...
        getGraphicObjectProperty(pSubwin, __GO_PARENT__, jni_string, &pstParentFigure);
        pixelRubberBox(pstParentFigure, isClick, initialPixelRect, endPixelRect, usedButton);
    }
    else
    {
        getGraphicObjectProperty(pSubwin, __GO_PARENT__, jni_string, &pstParentFigure);
        pixelRubberBox(pstParentFigure, isClick, NULL, endPixelRect, usedButton);
    }
