...
* @return  point to char[] if OK or NULL if not
*/
char * sciGetTextRenderer( sciPointObj * pobj )
{
    switch (sciGetEntityType (pobj))
    {
    case SCI_TEXT:
        return pTEXT_FEATURE (pobj)->renderer;
        break;
    case SCI_LEGEND:
        return pLEGEND_FEATURE (pobj)->text.renderer;
        break;
    case SCI_LABEL: /* F.Leray 28.05.04 */
        return sciGetTextRenderer( pLABEL_FEATURE (pobj)->text ) ;
        break;
    case SCI_SUBWIN:
       return pSUBWIN_FEATURE (pobj)->renderer;
       break;
    case SCI_AXES:
    case SCI_FIGURE:
    case SCI_UIMENU:
