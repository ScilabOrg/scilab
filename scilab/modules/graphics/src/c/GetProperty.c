...
int sciGetForegroundToDisplay(sciPointObj * pobj)
{
    ...
    if ((m - colorindex == -1) || (m - colorindex == -2))
    ...
    while (psonstmp != NULL)
    {
        ...
        if (sciGetEntityType(curObj) == SCI_SUBWIN)
        {
            ...
            if ((Abs(WRectTmp[0] - WRect[0]) < 1e-8)
                && (Abs(WRectTmp[1] - WRect[1]) < 1e-8) && (Abs(WRectTmp[2] - WRect[2]) < 1e-8) && (Abs(WRectTmp[3] - WRect[3]) < 1e-8))
            {
                /* subwin found */
                return curObj;
            }
    ...
    case SCI_SEGS:
        if (pSEGS_FEATURE(pthis)->ptype == 0)
        {
            *numrow = pSEGS_FEATURE(pthis)->Nbr1;
            ...
            if (pSEGS_FEATURE(pthis)->vz != NULL || pSUBWIN_FEATURE(sciGetParentSubwin(pthis))->is3d)
            {
                *numcol = 3;
            }
            else
...
sciPointObj *sciGetAxes(sciPointObj * pparentfigure, sciPointObj * psubwin)
{
    ...
    if (psonstmp != (sciSons *) NULL)
    {
        ...
        if (psonstmp->pnext == (sciSons *) NULL)
            return (sciPointObj *) NULL;
        else if (sciGetEntityType(psonstmp->pointobj) == SCI_SUBWIN)
...
{
    if (strcmp(marker, "arrow_size_factor") == 0)
    {
        return sci_matrix;
    }
    ...
    {
        return sci_matrix;
    }                           /* F.Leray 09.04.04 : Adding to support font_color user interface */
    ...
    else if (strcmp(marker, "textcolor") == 0)
    {
        return sci_matrix;
    }
    ...
    else if (strcmp(marker, "tics_textcolor") == 0)
    {
        return sci_matrix;
    }
    else if (strcmp(marker, "tics_textsize") == 0)
    {
        return sci_matrix;
    }
    ...
    else if (strcmp(marker, "sub_tics") == 0)
    {
        return sci_matrix;
    }
    else if (strcmp(marker, "sub_ticks") == 0)
    {
        return sci_matrix;
    }                           /* new writing F.Leray 12.10.04 to be consistent with x,y,z _ticks */
    ...
    else if (strcmp(marker, "colored") == 0)
    {
        ...
        if ((sciGetEntityType(pobj) == SCI_SURFACE) || (sciGetEntityType(pobj) == SCI_SEGS && pSEGS_FEATURE(pobj)->ptype == 1) ||   /* a champ */
    ...
    else if (strcmp(marker, "current_obj") == 0)
    {
        return sci_handles;
    }
    else if (strcmp(marker, "current_entity") == 0)
    {
        return sci_handles;
    }
    ...
    else if (strcmp(marker, "enable") == 0)
    {
        return sci_strings;
    }
    ...
    else if (strcmp(marker, "viewport") == 0)
    {
        return sci_matrix;
    }                           /*       01/07 */
    else if (strcmp(marker, "info_message") == 0)
    {
        return sci_strings;
    }                           /*       01/07 */
    else
...
int CheckForCompound(long *handelsvalue, int number)
{
    ...
    for (i = 0; i < number; i++)
    {
        ...
        case SCI_AXES:
            ...
            if ((sciGetEntityType(sciGetParent(sciGetPointerFromHandle(xtmp))) != SCI_SUBWIN)
...
BOOL sciGetLegendDefined(sciPointObj * pObj)
{
    ...
    if (sciisTextEmpty(ppSubWin->mon_x_label) && sciisTextEmpty(ppSubWin->mon_y_label) && sciisTextEmpty(ppSubWin->mon_z_label))
        return FALSE;
    else
        return TRUE;
...
{
    return sciGetEntityType(pObj) != SCI_LABEL && GetHandleVisibilityOnUimenu(pObj);
}
...
*/
void sciGetTextBoundingBox(sciPointObj * pObj, double corner1[3], double corner2[3], double corner3[3], double corner4[3])
{
    switch (sciGetEntityType(pObj))
    {
    case SCI_TEXT:
...
*/
void sciGetPixelBoundingBox(sciPointObj * pObj, int corner1[2], int corner2[2], int corner3[2], int corner4[2])
{
...
*/
void sciGet2dViewBoundingBox(sciPointObj * pObj, double corner1[2], double corner2[2], double corner3[2], double corner4[2])
{
...
BOOL sciIsAutomaticallyRedrawn(sciPointObj * pObj)
{
    ...
    return (entityType == SCI_UICONTROL)
        || (entityType == SCI_UIMENU) || (entityType == SCI_UICONTEXTMENU) || (entityType == SCI_WAITBAR) || (entityType == SCI_PROGRESSIONBAR);
}
