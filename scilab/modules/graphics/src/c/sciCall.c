 ...
 */
static void getDrect(const double vector[], int nbElements, double *dMin, double *dMax, double defaultMin, double defaultMax);
...
void Objrect(double *x, double *y, double *width, double *height, int *foreground, int *background, BOOL isfilled, BOOL isline, long *hdl)
{
    ...
    checkRedrawing();
    newObj = ConstructRectangle(psubwin, *x, *y, *height, *width, foreground, background, isfilled, isline);
...
void Objarc(double *angle1,
            double *angle2,
            double *x, double *y, double *width, double *height, int *foreground, int *background, BOOL isfilled, BOOL isline, long *hdl)
{
    sciPointObj *psubwin;
    sciPointObj *pobj;
    ...
    psubwin = sciGetCurrentSubWin();
    checkRedrawing();
    pobj = ConstructArc(psubwin, *x, *y, *height, *width, *angle1, *angle2, foreground, background, isfilled, isline);
...
void Objpoly(double *x, double *y, int n, int closed, int mark, long *hdl)
{
    ...
    if (mark <= 0)
    {
        ...
        pobj = ConstructPolyline(psubwin, x, y, PD0, closed, n, 1, NULL, NULL, &absmark, NULL, NULL, FALSE, FALSE, TRUE, FALSE);
    }
    else
    {
        pobj = ConstructPolyline(psubwin, x, y, PD0, closed, n, 1, &mark, NULL, NULL, NULL, NULL, TRUE, FALSE, FALSE, FALSE);
    }
    sciSetCurrentObj(pobj);
    *hdl = sciGetHandle(pobj);
}
...
void Objfpoly(double *x, double *y, int n, int *style, long *hdl, int shading)
{
    ...
    if (shading == 2)
    {
        ...
        pobj = ConstructPolyline(psubwin, x, y, PD0, closed, n, 1, NULL, style, NULL, NULL, NULL, FALSE, TRUE, FALSE, TRUE);
    }
    else
    {
        /* flat mode is "on" */
        ...
        {
            fillcolor = abs(*style);
            pobj = ConstructPolyline(psubwin, x, y, PD0, closed, n, 1, NULL, &fillcolor, NULL, NULL, NULL, FALSE, TRUE, FALSE, FALSE);
        }
        else if (*style == 0)
        {
            contourcolor = sciGetForeground(psubwin);
            pobj = ConstructPolyline(psubwin, x, y, PD0, closed, n, 1, &contourcolor, NULL, NULL, NULL, NULL, TRUE, FALSE, FALSE, FALSE);
        }
        else
        {                       /* *style > 0 */
            fillcolor = *style;
            contourcolor = sciGetForeground(psubwin);
            pobj = ConstructPolyline(psubwin, x, y, PD0, closed, n, 1, &contourcolor, &fillcolor, NULL, NULL, NULL, TRUE, TRUE, FALSE, FALSE);
        }
...
void Objsegs(int *style, int flag, int n1, double *x, double *y, double *z, double arsize)
{
    ...
    checkRedrawing();
    psubwin = sciGetCurrentSubWin();
    sciSetCurrentObj(ConstructSegs(psubwin, type, x, y, z, n1, n1, (z == NULL ? 0 : n1),    // x, y and z have the same size n1
...
void Objstring(char **fname,
               ...
               BOOL autoSize,
               double userSize[2],
               long *hdl, BOOL centerPos, int *foreground, int *background, BOOL isboxed, BOOL isline, BOOL isfilled, sciTextAlignment alignment)
{
    sciPointObj *psubwin = NULL;
    ...
    sciSetCurrentObj(ConstructText(psubwin,
                                   fname,
                                   nbRow, nbCol, x, y, autoSize, userSize, centerPos, foreground, background, isboxed, isline, isfilled, alignment));
    endFigureDataWriting(pFigure);
...
void Objplot2d(int ptype,
               char logflags[],
               double x[], double y[], int *n1, int *n2, int style[], char strflag[], char legend[], double brect[], int aaint[], BOOL flagNax)
{
 ...
 *  grayplot
 *-----------------------------------------------*/
void Objgrayplot(double x[], double y[], double z[], int *n1, int *n2, char strflag[], double brect[], int aaint[], BOOL flagNax)
{
 ...
 *  Matplot1
 *-----------------------------------------------*/
void Objmatplot1(double z[], int *n1, int *n2, double xrect[])
{
 ...
 *  plot3d
 *-----------------------------------------------*/
void Objplot3d(char *fname, int *isfac, int *izcol, double x[], double y[], double z[], double *zcol, int *m, int *n, double *theta, double *alpha, char *legend, int *iflag, double *ebox, int *m1,    /*Adding F.Leray 12.03.04 and 19.03.04 */
               int *n1, int *m2, int *n2, int *m3, int *n3, int *m3n, int *n3n)
     ...
     /* legends has not the same meaning than inside plot2dn (there, it is really the legends of the plotted curves) */
{
    ...
    if ((sciGetGraphicMode(psubwin)->autoscaling))
    {
        ...
        case 3:
        case 5:
        case 7:                /* Force psubwin->Srect=ebox */
            drect[0] = ebox[0]; /*xmin */
            drect[2] = ebox[2]; /*ymin */
            drect[1] = ebox[1]; /*xmax */
            drect[3] = ebox[3]; /*ymax */
        ...
        case 4:
        case 6:
        case 8:                /* Force psubwin->Srect to the x and y bounds */
            getDrect(x, (*m1) * (*n1), &drect[0], &drect[1], pSUBWIN_FEATURE(psubwin)->SRect[0], pSUBWIN_FEATURE(psubwin)->SRect[1]);

            getDrect(y, (*m2) * (*n2), &drect[2], &drect[3], pSUBWIN_FEATURE(psubwin)->SRect[2], pSUBWIN_FEATURE(psubwin)->SRect[3]);

            getDrect(z, (*m3) * (*n3), &drect[4], &drect[5], pSUBWIN_FEATURE(psubwin)->SRect[4], pSUBWIN_FEATURE(psubwin)->SRect[5]);
    ...
    if (iflag[1] != 0)
        pSUBWIN_FEATURE(psubwin)->isoview = (BOOL) (iflag[1] == 3 || iflag[1] == 4 || iflag[1] == 5 || iflag[1] == 6);
    ...
    {                           /*Distinction here between SCI_PARAM3D1 and others */
        ...
        pNewSurface = ConstructSurface(psubwin, typeof3d,
                                       x, y, z, zcol, *izcol, *m, *n, iflag, ebox, flagcolor, isfac, m1, n1, m2, n2, m3, n3, m3n, n3n);
    ...
    else
    {
        ...
        for (i = 0; i < *n; ++i)
        {
            ...
            {                   /* default case, nothing is given */
                ...
                sciSetCurrentObj(ConstructPolyline
                                 (sciGetCurrentSubWin(),
                                  &(x[*m * i]), &(y[*m * i]), &(z[*m * i]), 0, *m, 1, &curcolor, NULL, NULL, NULL, NULL, TRUE, FALSE, FALSE, FALSE));
            }
        ...
        if (*n > 1)
            sciSetCurrentObj(ConstructCompound(hdltab, *n));
        FREE(hdltab);
    }
...
void Objdrawaxis(char dir,
                 ...
                 int *nx,
                 double *y,
                 int *ny, char *val[], int subint, char *format, int font, int textcol, int ticscol, char flag, int seg, int nb_tics_labels)
{
    checkRedrawing();
    sciSetCurrentObj(ConstructAxes
                     ((sciPointObj *)
                      sciGetCurrentSubWin(), dir, tics, x, *nx, y, *ny, val, subint, format, font, textcol, ticscol, flag, seg, nb_tics_labels));
...
void Objnumb(char *fname, unsigned long fname_len, int n, int flag, double x, double y, double *angle, double *box)
{
...
void Objfec(double x[],
            ...
            int *n,
            int *m,
            char strflag[], char legend[], double brect[], int aaint[], double Zminmax[], int Colminmax[], int ColOut[], BOOL WithMesh, BOOL flagNax)
{
    C2F(fec) (x, y, noeud, fun, n, m, strflag, legend, brect, aaint, Zminmax, Colminmax, ColOut, WithMesh, flagNax, 4L, bsiz);
}
