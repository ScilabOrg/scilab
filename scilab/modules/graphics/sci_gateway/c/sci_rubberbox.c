...
static int returnRectAndButton(const int *_piJavaValues, int _iSelectedRectSize);
/*--------------------------------------------------------------------------*/
static int getInitialRectangle(double initRect[4])
{
    int rectNbRow = 0;
...
static int returnRectAndButton(const int *_piJavaValues, int _iSelectedRectSize)
{
    ...
    for ( i = 0; i < _iSelectedRectSize; i++)
    {
...
int sci_rubberbox(char * fname, unsigned long fname_len)
{
    ...
    {
        piJavaValues = javaDragRubberBox(pFigureUID, initialRect, initialRectSize);
    }
