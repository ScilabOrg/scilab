...
int sci_newaxes( char * fname,unsigned long fname_len )
{
  ...
  if ((subwinUID = ConstructSubWin (getCurrentFigure())) != NULL)
  {
    setCurrentObject(subwinUID);
    sciSetSelectedSubWin(subwinUID);
    setCurrentSubWin(subwinUID);

    CreateVar(Rhs+1,GRAPHICAL_HANDLE_DATATYPE,&numrow,&numcol,&outindex);

    *hstk(outindex) = getHandle(subwinUID);
