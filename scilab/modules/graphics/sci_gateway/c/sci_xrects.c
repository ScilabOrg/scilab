...
int sci_xrects(char *fname, unsigned long fname_len)
{
    ...
    for (i = 0; i < n1; ++i)
    {
      ...
      /** fil(i) = 0 rectangle i is drawn using the current line style (or color).**/
            ...
            Objrect(stk(l1 + (4 * i)), stk(l1 + (4 * i) + 1), stk(l1 + (4 * i) + 2), stk(l1 + (4 * i) + 3), &foreground, NULL, FALSE, TRUE, &hdl);
        }
        else
        {
            if (*istk(l2 + i) < 0)
        ...
        /** fil(i) < 0 rectangle i is drawn using the line style (or color) **/
                ...
                Objrect(stk(l1 + (4 * i)), stk(l1 + (4 * i) + 1), stk(l1 + (4 * i) + 2), stk(l1 + (4 * i) + 3), &tmp, NULL, FALSE, TRUE, &hdl);
            }
            else
            {
