...
function datatipsManagerMode(varargin)
...
//  This function is called by the datatips menu callbacks.  
  ...
  case 2 then
    ...
    error(msprintf(_("%s: too many input arguments"),'datatipsManagerMode'))
  end
  if fig.event_handler == '' then
    fig.event_handler = "datatipsEventhandler"
  end
  select action
  case 'on'
...
function datatipsEventhandler(win,x,y,ibut)
//The event handler which rules the datatips interactive edition 
  ...
  if or(ibut==[0 3]) then //select a point on a curve
    ...
    curve_handles=datatipsGetEntities(ax);
    ...
    if k<>[] then 
      ud=datatipsGetStruct(curve_handles(k))
    ...
    if dmin<10 then 
      ...
      ud=datatipsGetStruct(curve_handles(kmin))// the curve datatips data structure
      if typeof(ud)<>'datatips' then 
	datatipsInitStruct(curve_handles(kmin))
	ud=datatipsGetStruct(curve_handles(kmin))
      end
	...
	datatipCreate(curve_handles(kmin),l); //
      ...
      datatipsRemoveAll(curve_handles)
    end
  elseif or(ibut==[2 5]) then
    ...
    if k<>[] then 
       ...
       case 1 then 
	 datatipsRemoveAll(curve_handles(k))
       case 2 then
	 datatipRemove(curve_handles(k),l)
       case 3 then
	 datatipsSetDisplay(curve_handles(k))
       case 4 then
	 datatipsSetInterp(curve_handles(k),%T)
       case 5 then
	 datatipsSetInterp(curve_handles(k),%F)
       end
    end
  elseif or(ibut==[1 4]) then
    ...
    [k,l]=datatipLookfor(curve_handles,pt)
    if k<>[] then 
      ud=datatipsGetStruct(curve_handles(k))// the curve datatips data structure
      tip_handle=ud.tips(l);
...
//tip_index    :  the datatip index for the curve  
  for curve_index=1:size(curve_handles,'*')
    ud=datatipsGetStruct(curve_handles(curve_index))
    if typeof(ud)=='datatips' then
      tips=ud.tips
