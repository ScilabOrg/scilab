...
function ok=datatipsInitStruct(curve_handle,varargin)
...
//               (<property name), <property value>)  
  ...
  if typeof(ud)<>'datatips' then 
    if prod(size(ud))==0 then
      ud=tlist(['datatips','interpolate','replace','formatfunction', ...
		'tips'],%f,%t,datatipsDefaultDisplay,[])
