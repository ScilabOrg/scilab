...
function []=Sgrayplot(x,y,z, strf, rect, nax, zminmax, colminmax, mesh, colout)
    ...
    if ~(type(x)==1 & isreal(x) & type(y)==1 & isreal(y) & type(z)==1 & isreal(z)) then 
        error(msprintf(gettext("%s: Wrong type for input(s) argument(s): Arguments #%d, #%d and #%d must be real.\n"), "Sgrayplot",1,2,3));
    end
    nx = length(x); ny = length(y); [p,q] = size(z)
    if p ~= nx then
        error(msprintf(gettext("%s: Wrong size for input arguments #%d and #%d.\n"),"Sgrayplot", 1, 3));
    elseif q~=ny then
        error(msprintf(gettext("%s: Wrong size input arguments #%d and #%d.\n"),"Sgrayplot", 2, 3));
    end
