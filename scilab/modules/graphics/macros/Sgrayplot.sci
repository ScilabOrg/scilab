...
function []=Sgrayplot(x,y,z, strf, rect, nax, zminmax, colminmax, mesh, colout)
    ...
    if p ~= nx then
        error(msprintf(gettext("%s: Wrong size for input arguments #%d and #%d: The rows number of #%d argument must be equal to the size of #%d argument.\n"),"Sgrayplot", 1, 3, 3, 1));
    elseif q~=ny then
        error(msprintf(gettext("%s: Wrong size input arguments #%d and #%d: Same column dimensions expected.\n"),"Sgrayplot", 2, 3));
    end
