...
function opt1 = getColorIndex(ColorString)
    ...
    if (k == [] | (size(k,'*') > 1 & c1 <> 'b'))
        warning(msprintf(gettext("%s: Wrong value for input argument #%d: must be a color of the colormap.\n"),"getColorIndex", 1));
        return;
    end
    ...
    if (size(opt1,'*') > 1)  // unreachable case normally
        warning(msprintf(gettext("%s:  Wrong number of input argument(s): %d expected.\n"),"getColorIndex",1));
        return;
    end
