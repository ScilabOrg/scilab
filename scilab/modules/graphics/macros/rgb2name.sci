...
function r=rgb2name(r,g,b)
    ...
    if rhs==1 then
        if (type(r)<>1 & size(r,"*")<>3) then
            error(msprintf(gettext("%s: Wrong type for input argument #%d: vector of 3 integers expected.\n"),"rgb2name",1));
        end
        b=r(3); g=r(2); r=r(1);
    elseif rhs==3 then
        if (type(r)<>1 | type(g)<>1 | type(b)<>1 | size(r,"*")<>1 | size(g,"*")<>1 | size(b,"*")<>1) then
            error(msprintf(gettext("%s: Wrong size for input argument(s): integer(s) expected.\n"),"rgb2name"));
        end
    else
