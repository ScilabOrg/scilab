    ...
    case 'font_style'     // <=> fontangle
        ...
        if typeof(PropertyValue) == "string"
            ...
            if or(PropertyValue == fonts)
                ...
                warning(msprintf(_("%s: Wrong value for input argument #%d: at least one ""fonts"" expected.\n"),"setTitleLabelProperty",2));
                ResetFigureDDM(current_figure, cur_draw_mode);
                return;
            end
        elseif typeof(PropertyValue) == "constant" & or(PropertyValue==(0:9))
            ...
            warning(msprintf(_("%s: Wrong type for input argument #%d: real matrix or string matrix expected.\n"),"setTitleLabelProperty",2));
            ResetFigureDDM(current_figure, cur_draw_mode);
            return;    
        end
