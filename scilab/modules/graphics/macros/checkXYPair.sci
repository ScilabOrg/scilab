...
function [X,Y]=checkXYPair(typeOfPlot,x,y,current_figure,cur_draw_mode)
    ...
    if type(y)==13 // If y is a function
        ...
        if size(X,1)==1 & ~XScal, X=X', warning(_("Transposing row vector ''X'' to get column vector")), end; // If one of the vectors is a row, transpose it,
        if size(Y,1)==1 & ~YScal, Y=Y', warning(_("Transposing row vector ''Y'' to get column vector")), end; // but no need to transpose scalars.
