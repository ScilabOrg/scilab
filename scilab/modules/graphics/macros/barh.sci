...
function  barh(varargin)
    ...
    if size(argdb,"*")<> argdb($) then
        error(msprintf(gettext("%s: Wrong type for input arguments: x,y and width must be a constant.\n"),"barh"));
    end

    if size(argstr,"*") <> nv-argdb($) then
        error(msprintf(gettext("%s: Wrong type for input arguments: color and style must be a string.\n"),"barh"));
    end
