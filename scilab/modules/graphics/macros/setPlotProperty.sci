...
function [fail]=setPlotProperty(PropertyName,PropertyValue,Curves,current_figure,cur_draw_mode)
    ...
    case 'thickness'        // <=> LineWidth
        ...
        if (type(PropertyValue)==1)
            ...
            warning(msprintf(gettext("%s: Wrong type for input argument #%d: A constant expected.\n"),"setPlotProperty",2));
            ResetFigureDDM(current_figure, cur_draw_mode);
            return;
        end
    ...
    case 'markstyle'        // <=> Marker
        ...
        if (type(PropertyValue)==10)
            ...
            if (size(opt1,'*') > 1)
                warning(msprintf(gettext("%s: Wrong size for input argument #%d: Only one marker style expected.\n"),"setPlotproperty",2));
                ResetFigureDDM(current_figure, cur_draw_mode);
                return;
            end
