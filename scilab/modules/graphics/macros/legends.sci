...
function legends(leg, style, opt, with_box, font_size )
    ...
    elseif opt<1 | opt>6 then
        error(msprintf(gettext('%s: Wrong value for input argument %s: Must be in the interval [%d, %d]."),"legends", "opt", 1, 5)); // if opt is entered by a string by the user it is in the range
    end
    select opt
    case 1 then
