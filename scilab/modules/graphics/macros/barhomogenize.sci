    ...
    elseif size(varlist) == 3
        if type(varlist(1))==9  then
            ...
            elseif  size(varlist) > 3 then
                error(msprintf(gettext("%s: Wrong number of input argument(s): must be between %d and %d.\n"),"barhomogenize",0,3));
            end
        end
    end

    if ~isreal(WIDTH)
        error(msprintf(gettext("%s: Wrong type for input argument: width must be a real.\n"),"barhomogenize"));
    end
