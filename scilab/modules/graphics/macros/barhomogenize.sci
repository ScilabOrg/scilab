...
function  barhomogenize(varargin)
    ...
    elseif size(varlist) == 2 then  
        ...
        elseif or(varlist(1) == ["grouped","stacked"]) & (type(varlist(2)) == 1)
            ...
            error(msprintf(gettext("%s:Wrong type for input arguments #%d and #%d: A string and a scalar expected.\n"),"barhomogenize", 2, 3));
        end
    ...
    if ~isreal(WIDTH)
        error(msprintf(gettext("%s: Wrong type for input argument: A real expected for %s.\n"),"barhomogenize", "width"));
    end
