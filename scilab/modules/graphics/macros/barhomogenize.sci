...
function  barhomogenize(varargin)
    ...
    elseif size(varlist) == 2 then  
        ...
        elseif or(varlist(1) == ["grouped","stacked"]) & (type(varlist(2)) == 1)
            ...
            error(msprintf(gettext("%s:Wrong type for input arguments #%d and #%d: If the #%d argument is a handle or a string, the #%d argument must be a string or a scalar.\n"),"barhomogenize", 1, 2, 1, 2));
        end
