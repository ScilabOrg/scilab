...
if rhs==1 then
  if (type(h)<>9 | size(h,"*")<>1) then
    error(sprintf(gettext("%s: Wrong type for input argument #%d: Graphic handle expected.\n"), "twinkle", 1));
  end
  n=5;
elseif rhs==2 then
  if (type(h)<>9 | size(h,"*")<>1) then
  ...
  if (type(n)<>1 | size(n,"*")<>1) then
    error(sprintf(gettext("%s: Wrong type for input argument #%d: Scalar expected.\n"), "twinkle", 2));
  end	
else
