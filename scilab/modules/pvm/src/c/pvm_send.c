...
void C2F(scipvmsend) (int *taskId, int *p, int *pack, int *n, double *buff, int *msgtag, int *res)
{
    int info, bufid;
    double *ptr_double;
    int *ptr_int;
    ...
    (void)fprintf(stdout, "SEND: %d:%d:%d:%d:%d:%d:%p:%f:%f:%f:%f:%f:%f:%f\n",
                  *taskId, *p, *n, *msgtag, pack[0], pack[1], buff, buff[0], buff[1], buff[2], buff[3], buff[4], buff[5], buff[6]);
