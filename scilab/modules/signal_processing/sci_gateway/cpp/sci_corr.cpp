        ...
        else if (pS->getSize() == 1 && pS->get(0)[0] == L'u')
        {
            ...
            if ( iMode == 0)
            {
                ...
                if (in.size() == 4)
                {
                    ...
                    xu = pDblIn3->get();
                    xui = new double[pDblIn3->getSize()];
                    nbx =  pDblIn3->getSize();
                    ichaud = 1;
                }
                else
            ...
            else
            {
                ...
                {
                    double* xutempo = NULL;
                    double* xuitempo = NULL;
                    xu = new double[pDblIn3->getSize()];
                    xui = new double[pDblIn3->getSize()];

                    xutempo = pDblIn3->getImg();
                    xuitempo = pDblIn3->getReal();

                    memcpy(xu, xutempo, sizeof(double) * pDblIn3->getSize());
                    memcpy(xui, xuitempo, sizeof(double) * pDblIn3->getSize());
                }
                if (in.size() == 5)
                {
                    ...
                    nbx = pDblIn3->getSize();
                    w = new double[nbx];
                    memcpy(w, wtempo, sizeof(double) * pDblIn4->getSize());
                    ichaud = 1;
                    wi = new double[nbx];
                }
                else
            ...
            types::Double *pDblOut1 = new types::Double(1, mfft, true);
            pDblOut1->set(w);
            pDblOut1->setImg(wi);
            out.push_back(pDblOut1);
            ...
            if (_iRetCount == 2)
            {
                types::Double *pDblOut2 = new types::Double(1, mfft / 2);
