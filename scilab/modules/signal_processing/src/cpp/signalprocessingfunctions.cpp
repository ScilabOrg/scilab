...
void Signalprocessing::addSignalprocessingfunctions(Signalprocessingfunctions* _deFunction)
{
    types::ThreadId* pThread = ConfigVariable::getLastRunningThread();
    m_mapSignalprocessingfunctions[pThread->getThreadId()] = _deFunction;
}
...
{
    Signalprocessingfunctions* deFunction = NULL;
    deFunction = Signalprocessing::getSignalprocessingfunctions();
    ...
    deFunction->execFunctionDgetx(x, siz, iss);
}
...
void dgety_f(double* y, int* siz, int* iss)
{
    ...
    deFunction->execFunctionDgety(y, siz, iss);
}
