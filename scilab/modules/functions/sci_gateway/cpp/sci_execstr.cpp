...
Function::ReturnValue sci_execstr(types::typed_list &in, int _iRetCount, types::typed_list &out)
{
	...
	{//errcatch
        ...
        if(os_wcsicmp(pS->getString(0), L"errcatch") == 0)
        {
            bErrCatch = true;
        }
        else
	...
	if(in.size() == 3)
	{
        if(in[2]->getType() != InternalType::RealString || in[2]->getAsString()->getSize() != 1)
		{
            ScierrorW(999, _W("%ls: Wrong type for input argument #%d: A string expected.\n"), L"execstr", 3);
            return Function::Error;
        }

		if(os_wcsicmp(in[2]->getAsString()->getString(0), MUTE_FLAG) == 0)
        {
            bMute = true;
        }
        else if(os_wcsicmp(in[2]->getAsString()->getString(0), NO_MUTE_FLAG) == 0)
        {
            bMute = false;
        }
        else
    ...
    if(in[0]->getType() != InternalType::RealString || (in[0]->getAsString()->getRows() != 1 && in[0]->getAsString()->getCols() != 1))
	{
	...
	{
		iTotalLen += (int)wcslen(pS->getString(i));
	}
	...
	{
		wcscpy(pstCommand + iPos, pS->getString(i));
