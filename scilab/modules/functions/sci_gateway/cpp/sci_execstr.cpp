...
Function::ReturnValue sci_execstr(types::typed_list &in, int _iRetCount, types::typed_list &out)
{
	...
	if(in.size() > 1)
	{//errcatch
        if(in[1]->getType() != InternalType::RealString || in[1]->getAs<types::String>()->getSize() != 1)
        {
	...
	if(in.size() == 3)
	{
        if(in[2]->getType() != InternalType::RealString || in[2]->getAs<types::String>()->getSize() != 1)
		{
            ScierrorW(999, _W("%ls: Wrong type for input argument #%d: A string expected.\n"), L"execstr", 3);
            return Function::Error;
        }
    ...
    if(in[0]->getType() != InternalType::RealString || (in[0]->getAs<types::String>()->getRows() != 1 && in[0]->getAs<types::String>()->getCols() != 1))
	{
