...
Function::ReturnValue sci_exec(types::typed_list &in, int _iRetCount, types::typed_list &out)
{
	...
	{//errcatch or mode
		...
		else if(in[1]->getType() == InternalType::RealDouble && in[1]->getAs<Double>()->getSize() == 1)
        {//mode
    ...
    if(in[0]->getType() == InternalType::RealString && in[0]->getAsString()->getSize() == 1)
	{//1st argument is a path, parse file and execute it
		int iParsePathLen		= 0;
		String* pS = in[0]->getAsString();

        wchar_t* pstFile = pS->getString(0);
...
//    MessageBoxA(NULL, stPrompt, "", 0);
    ...
    wchar_t* pwstFile =  expandPathVariableW(in[0]->getAsString()->getString(0));
