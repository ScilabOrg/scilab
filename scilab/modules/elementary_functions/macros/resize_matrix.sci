...
function resMat = resize_matrix(mat, nbRow, nbCol, nbLay, typeOfMat)

    // Author Jean-Baptiste Silvy
    // Create a matrix of size nbRow x nbCol x nbLay whose elements (i,j,k) are mat(i,j,k)
    ...
    if rhs < 4 then
        nbLay = 1;
        minLay = 1;
    elseif rhs == 4 & typeof(nbLay) == "string" then
        typeOfMat = nbLay;
        nbLay = 1;
        minLay = 1;
        rhs = 5;
    ...
    if ((nbRow <= 0) | (nbCol <= 0) | (nbLay <= 0)) then
        resMat = [];
        return
    end
    ...
    elseif size(matSize, "c") == 3 then
        minLay = min(nbLay, matSize(3)); // Retrieve third dimension if input is a hypermatrix.
    else
        minLay = 1;
    end
    ...
    case "boolean" then
        ...
        if typeof(mat) == "boolean" then
            resMat(1:minRow, 1:minCol, 1:minLay) = (mat(1:minRow, 1:minCol, 1:minLay) == %t);
        else
            resMat(1:minRow, 1:minCol, 1:minLay) = (mat(1:minRow, 1:minCol, 1:minLay) == 1);
        end
        return
    case "int8" then
        mat = int8(mat);
    ...
    resMat(1:minRow, 1:minCol, 1:minLay) = mat(1:minRow, 1:minCol, 1:minLay);
