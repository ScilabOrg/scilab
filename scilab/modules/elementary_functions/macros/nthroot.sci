...
function y = nthroot(x,n)
    ...
    elseif (or (or(x(:)<0) & (n~=fix(n) | reste ==0))) then
        error(msprintf(gettext("%s: If x is negative, n must be odd integers\n"),"nthroot"));
        // If n ~=0 and n ~= %nan
    elseif (n~=0 & ~isnan(n)) then
        ...
        if (size(n,"*") == 1) then
            ...
            // If x = 0 and n is positive and n ~= %nan
            y(find(x==0 & (n>0 |isinf(n)))) = x(find(x==0 & (n>0 |isinf(n)))).^(1 ./n(find(x==0)));
            // If x is positive
            y(find(x>0)) = x(find(x>0)).^(1 ./n(find(x>0)));
            // If x is negative
            y(find(x<0)) = sign(x(find(x<0))).*(abs(x(find(x<0)))).^(1 ./n(find(x<0)));
        end
    end
