...
function y = nthroot(x,n)
    ...
    if (typeof(x) <> "constant" | typeof(n) <> "constant") then
    ...
    if (isnan(n) & or(x >= 0)) then
        y(find(x>=0)) = %nan.*x(find(x>=0));
    elseif (n==0 & (x>=0 | isnan(x))) then
        // if n = 0 and x = 1 or x = %nan then y = %nan
        y(find((x==1 | isnan(x)))) = %nan.*x(find((x==1 | isnan(x))));
        // if n = 0 and x>1 then y = %inf
        y(find(x>1)) = x(find(x>1)) .*%inf;
        // if n = 0 and x = %eps then y = 0
        y(find(x==%eps)) = x(find(x==%eps)) .*0;
