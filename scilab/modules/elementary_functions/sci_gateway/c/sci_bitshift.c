...
int sci_bitshift(char *fname, unsigned long fname_len)
{
    ...
    if ((m1 == m2 && n1 == n2) || (m1 == 1 && n1 == 1) || (m2 == 1 && n2 == 1)) // bitshift(mat, mat), bitshift(scal, mat) or bitshift(mat, scal)
    {
        for (i = 0; i < maxmn; ++i)
            if (iType1 == 1)
            {
                if (dbl2[MatOrScal2] >= 0)
                {
                    temp = ((int) dbl1[MatOrScal1]) << ((int) dbl2[MatOrScal2]); // Multiply c1[i] by 2^c2[i]
                }
                else
                {
                    temp = ((int) dbl1[MatOrScal1]) >> -((int) dbl2[MatOrScal2]); // Divide c1[i] by 2^-c2[i]
                }
                if (drop == 1) // 'Drop' mode
                ...
                switch (iPrec1)
                {
                    case SCI_INT8 :
                        if (c2[MatOrScal2] >= 0)
                        {
                            temp = c1[MatOrScal1] << c2[MatOrScal2]; // Multiply c1[i] by 2^c2[i]
                        }
                        else
                        {
                            temp = c1[MatOrScal1] >> -c2[MatOrScal2]; // Divide c1[i] by 2^-c2[i]
                        }
                        if (drop == 1) // 'Drop' mode
                    ...
                    case SCI_INT16 :
                        if (s2[MatOrScal2] >= 0)
                        {
                            temp = s1[MatOrScal1] << s2[MatOrScal2]; // Multiply c1[i] by 2^c2[i]
                        }
                        else
                        {
                            temp = s1[MatOrScal1] >> -s2[MatOrScal2]; // Divide c1[i] by 2^-c2[i]
                        }
                        if (drop == 1) // 'Drop' mode
                    ...
                    case SCI_INT32 :
                        if (i2[MatOrScal2] >= 0)
                        {
                            temp = i1[MatOrScal1] << i2[MatOrScal2]; // Multiply c1[i] by 2^c2[i]
                        }
                        else
                        {
                            temp = i1[MatOrScal1] >> -i2[MatOrScal2]; // Divide c1[i] by 2^-c2[i]
                        }
                        if (drop == 1) // 'Drop' mode
                            if (temp > 2147483647) // Maximum value of a 32 bits integer
                            {
                        ...
                        break;
                    case SCI_UINT8 :
                        utemp = uc1[MatOrScal1] << uc2[MatOrScal2]; // Multiply c1[i] by 2^c2[i]
                        ...
                        if (mn1 >= mn2)
                        {
                            uc1[i] = (unsigned char) utemp;
                        }
                        else
                        {
                            uc2[i] = (unsigned char) utemp;
                            res2 = 1;
                        }
                        break;
                    case SCI_UINT16 :
                        utemp = us1[MatOrScal1] << us2[MatOrScal2]; // Multiply c1[i] by 2^c2[i]
                        ...
                        if (mn1 >= mn2)
                        {
                            us1[i] = (unsigned short) utemp;
                        }
                        else
                        {
                            us2[i] = (unsigned short) utemp;
                            res2 = 1;
                        }
                        break;
                    case SCI_UINT32 :
                        utemp = ui1[MatOrScal1] << ui2[MatOrScal2]; // Multiply c1[i] by 2^c2[i]
                        ...
                        if (mn1 >= mn2)
                        {
                            ui1[i] = (unsigned short) utemp;
                        }
                        else
                        {
                            ui2[i] = (unsigned short) utemp;
                            res2 = 1;
                        }
                        break;
                    default:
