...
/***** get data *****/
    ...
    if(inputs[0]->isDouble() || inputs[0]->isInt())
    {
        wcsError = L"real";
        isDouble = true;
    }
    ...
    if(in.size() == 2)
    {
        if(in[1]->isString())
        {
            ...
            else if(wcsOrientation == L"m")
            {
                if(isSparse)
                {
                     iOrientation = (pGT->getRows() == 1) ? 2 : 1;
                }
                else
                {
...
//        }
    ...
    for(int i = 0; i < iCountElem; i++)
    {
        if(isDouble && (inputs[i]->isDouble() || inputs[i]->isInt()))
        {
        ...
        else if(isSparse && inputs[i]->isSparse())
        {
            ...
            if(piDimsArray[0] != pSparse->getRows() || piDimsArray[1] != pSparse->getCols())
            {
                ScierrorW(999, _W("%ls: Wrong size for input argument #%d: All arguments must have the same size.\n"), L"min", i+1);
                return types::Function::Error;
            }
...
/***** perform operation and set result *****/
    ...
    if(_iRetCount == 2)
    {
        ...
        else
        {
            ...
            {
                pSparseInd = new types::Sparse(iSizes[0], iSizes[1]);
            }
        }
    }
    ...
    else if(isSparse)
    {
        if(pDblIndex)
        {
            ...
            min(vectSparse[0], pDblIndex, pDblOut);
            out.push_back(pDblOut);
        }
        else // if pSparseInd
        {
            pSparseOut = new types::Sparse(iSizes[0], iSizes[1]);
            min(vectSparse, iOrientation, pSparseInd, pSparseOut);
