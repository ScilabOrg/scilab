...
types::Function::ReturnValue sci_sign(types::typed_list &in, int _iRetCount, types::typed_list &out)
{
    ...
    if (in[0]->isDouble())
    {
        ...
        if (pDblIn->isComplex())
        {
            ...
            double *outR = pOut->getReal();
            double *outImg = pOut->getImg() ;
            double *inR = pDblIn->getReal();
            double *inImg = pDblIn->getImg() ;
            for (i = 0 ; i < pOut-> getSize() ; i++)
            {
                double dblTemp = dpythags(inR[i], inImg[i]);
                if (dblTemp == 0)
                {
                    outR[i] = 0;
                    outImg[i] = 0;
                ...
                {
                    outR[i] =  inR[i] / dblTemp;
                    outImg[i] = inImg[i] / dblTemp;
                }
            }
        ...
        else
        {
            ...
            double * outt = pDblIn->getReal();
            double * in = pOut->getReal();

            for (i = 0 ; i < pOut-> getSize() ; i++)
            {
                outt[i] =  dsignsEx(in[i]);
            }
            out.push_back(pOut);
        }
    }
