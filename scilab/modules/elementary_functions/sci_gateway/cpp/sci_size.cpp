    ...
    default :
        // All types inherits of GenericType, so have this algorithm as default.
        {
            ...
            if(in.size() == 2)
            {
                ...
                if((iMode == 1 || iMode == 2) && in[0]->getAs<Double>()->get_dims() > 2)
                {
            ...
            int iDims   = in[0]->getAsGenericType()->get_dims();
            int* piDims = in[0]->getAsGenericType()->get_dims_array();
            
            std::cout << "dims : " << iDims << std::endl;
            std::cout << "piDims : " << in[0]->getAsGenericType() << std::endl;
            ...
            if(_iRetCount == 1)
            {
                ...
                case -1 : //lhs == 1
                    ...
                    break;
                case 0 : //"*"
                    pdbl[0] = in[0]->getAs<Double>()->size_get();
                    break;
                case 1 : //"r"
                    iColsOut = 1;
                    pdbl[0] = in[0]->getAs<Double>()->rows_get();
                    break;
                case 2 : //"c"
                    iColsOut = 1;
                    pdbl[0] = in[0]->getAs<Double>()->cols_get();
                    break;
                }
                out.push_back(pD);
            }
...
int getProcessMode(types::typed_list &in, int _iProcess, int _iRef)
{
    ...
    else if(in[1]->getType() == InternalType::RealDouble && in[1]->getAs<Double>()->isComplex() == false)
    {
        ...
        iMode = static_cast<int>(pD->get_real()[0]);
