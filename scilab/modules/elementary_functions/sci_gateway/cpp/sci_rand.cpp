...
types::Function::ReturnValue sci_rand(types::typed_list &in, int _iRetCount, types::typed_list &out)
{
    ...
    else if (in[0]->isGenericType())
    {
        ...
        if (iSizeIn == 1)
        {
            ...
            types::Double* pOut = new types::Double(in[0]->getAs<types::GenericType>()->getDims(), in[0]->getAs<types::GenericType>()->getDimsArray(), in[0]->getAs<types::GenericType>()->isComplex());;
