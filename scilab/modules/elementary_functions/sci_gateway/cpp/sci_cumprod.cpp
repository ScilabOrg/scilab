...
/***** get data *****/
    ...
    {
        pSparseIn = in[0]->getAs<types::Sparse>();
    }
    ...
    if(in.size() >= 2)
    {
        ...
        else if(in[1]->isString())
        {
            ...
            else if(wcscmp(wcsString, L"m") == 0)
            {
                if(pSparseIn)
                {
                     iOrientation = (pSparseIn->getRows() == 1) ? 2 : 1;
                }
                else
                {
                    int iDims = 0;
...
/***** perform operation *****/
    ...
    else if(pSparseIn)
    {
        if(iOrientation > 2)
        {
            pSparseOut = pSparseIn->clone()->getAs<types::Sparse>();
        }
        else
        {
            pSparseOut = new types::Sparse(pSparseIn->getRows(), pSparseIn->getCols(), pSparseIn->isComplex());
            cumprod(pSparseIn, iOrientation, pSparseOut);
        }

        if(iOuttype == 2)
        {
            pDblOut = new types::Double(pSparseOut->getRows(), pSparseOut->getCols(), pSparseOut->isComplex());
            pSparseOut->fill(*pDblOut);
        }
    }
    else if(pPolyIn)
    {
