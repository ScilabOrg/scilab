...
assert_checkequal(max(Yunf), 9.4279967527199310950436);
assert_checkequal(min(Yunf), -12.436144421661456505035);

// Example #4 (1D folded)
Yf = 1 + Y/10 + acos(cos(Y)); // Raw recovered Y [2.pi], + linear trend
[Yunf, K] = unwrap(Yf, "unfold");
assert_checkequal(size(Yunf), size(Y));
assert_checkequal(max(Yunf), 15.801371238872524926933);
assert_checkequal(min(Yunf), -6.1759877188984493301405);
