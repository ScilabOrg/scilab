...
typedef HRESULT(WINAPI * URLDownloadToFilePROC) (LPUNKNOWN pCaller, LPCTSTR szURL, LPCTSTR szFileName, DWORD dwReserved, LPBINDSTATUSCALLBACK lpfnCB);

static HRESULT dynlib_URLDownloadToFile(LPUNKNOWN pCaller, LPCTSTR szURL, LPCTSTR szFileName, DWORD dwReserved, LPBINDSTATUSCALLBACK lpfnCB);
...
typedef HINTERNET(WINAPI * InternetOpenUrlPROC) (HINTERNET hInternet,
                                                 LPCTSTR lpszUrl, LPCTSTR lpszHeaders, DWORD dwHeadersLength, DWORD dwFlags, DWORD_PTR dwContext);

static HINTERNET dynlib_InternetOpenUrl(HINTERNET hInternet,
                                        LPCTSTR lpszUrl, LPCTSTR lpszHeaders, DWORD dwHeadersLength, DWORD dwFlags, DWORD_PTR dwContext);
/*--------------------------------------------------------------------------*/
typedef HINTERNET(WINAPI * InternetOpenPROC) (LPCTSTR lpszAgent, DWORD dwAccessType, LPCTSTR lpszProxyName, LPCTSTR lpszProxyBypass, DWORD dwFlags);

static HINTERNET dynlib_InternetOpen(LPCTSTR lpszAgent, DWORD dwAccessType, LPCTSTR lpszProxyName, LPCTSTR lpszProxyBypass, DWORD dwFlags);
...
/*--------------------------------------------------------------------------*/
typedef BOOL(WINAPI * HttpQueryInfoPROC) (HINTERNET hRequest, DWORD dwInfoLevel, LPVOID lpvBuffer, LPDWORD lpdwBufferLength, LPDWORD lpdwIndex);

static BOOL dynlib_HttpQueryInfo(HINTERNET hRequest, DWORD dwInfoLevel, LPVOID lpvBuffer, LPDWORD lpdwBufferLength, LPDWORD lpdwIndex);

/*--------------------------------------------------------------------------*/
typedef BOOL(WINAPI * InternetReadFilePROC) (HINTERNET hFile, LPVOID lpBuffer, DWORD dwNumberOfBytesToRead, LPDWORD lpdwNumberOfBytesRead);

static BOOL dynlib_InternetReadFile(HINTERNET hFile, LPVOID lpBuffer, DWORD dwNumberOfBytesToRead, LPDWORD lpdwNumberOfBytesRead);
...
httpdownloadfile_error_code httpDownloadFile(char *szURL, char *szSaveFilePath)
{
    if (isValidURL(szURL))
    {
        ...
        if (!
            (hiDownload =
             dynlib_InternetOpenUrl(hiConnex, szURL, szHeader, lstrlen(szHeader),
        ...
        else
        {
            ...
            else
            {
                ...
                /* Get file size */
                if (!dynlib_HttpQueryInfo
                    (hiDownload, HTTP_QUERY_CONTENT_LENGTH | HTTP_QUERY_FLAG_NUMBER, (LPVOID) & dwBytesRequired, &dwSizeOfByReq, 0))
                {
                    dynlib_InternetCloseHandle(hiConnex);
...
/*--------------------------------------------------------------------------*/
HINTERNET dynlib_InternetOpenUrl(HINTERNET hInternet, LPCTSTR lpszUrl, LPCTSTR lpszHeaders, DWORD dwHeadersLength, DWORD dwFlags, DWORD_PTR dwContext)
{
    if (WinINETDll == NULL)
    ...
    if (WinINETDll)
    {
        ...
        {
            return (HINTERNET) (dllInternetOpenUrl) (hInternet, lpszUrl, lpszHeaders, dwHeadersLength, dwFlags, dwContext);
        }
    }
    return NULL;
}
...
/*--------------------------------------------------------------------------*/
HINTERNET dynlib_InternetOpen(LPCTSTR lpszAgent, DWORD dwAccessType, LPCTSTR lpszProxyName, LPCTSTR lpszProxyBypass, DWORD dwFlags)
{
    if (WinINETDll == NULL)
        WinINETDll = LoadLibrary("WININET.dll");
    if (WinINETDll)
    {
        ...
        if (dllInternetOpen)
        {
            return (HINTERNET) (dllInternetOpen) (lpszAgent, dwAccessType, lpszProxyName, lpszProxyBypass, dwFlags);
        }
    }
    return NULL;
}
...
BOOL dynlib_InternetCloseHandle(HINTERNET hInternet)
{
    ...
    if (WinINETDll)
    {
        InternetCloseHandlePROC dllInternetCloseHandle = (InternetCloseHandlePROC) GetProcAddress(WinINETDll, "InternetCloseHandle");
...
/*--------------------------------------------------------------------------*/
BOOL dynlib_HttpQueryInfo(HINTERNET hRequest, DWORD dwInfoLevel, LPVOID lpvBuffer, LPDWORD lpdwBufferLength, LPDWORD lpdwIndex)
{
    if (WinINETDll == NULL)
        WinINETDll = LoadLibrary("WININET.dll");
    if (WinINETDll)
    {
        HttpQueryInfoPROC dllHttpQueryInfo = (HttpQueryInfoPROC) GetProcAddress(WinINETDll, "HttpQueryInfoA");

        if (dllHttpQueryInfo)
        {
            return (BOOL) (dllHttpQueryInfo) (hRequest, dwInfoLevel, lpvBuffer, lpdwBufferLength, lpdwIndex);
        }
    }
    return FALSE;
}
...
/*--------------------------------------------------------------------------*/
BOOL dynlib_InternetReadFile(HINTERNET hFile, LPVOID lpBuffer, DWORD dwNumberOfBytesToRead, LPDWORD lpdwNumberOfBytesRead)
{
    if (WinINETDll == NULL)
        WinINETDll = LoadLibrary("WININET.dll");
    if (WinINETDll)
    {
        InternetReadFilePROC dllInternetReadFile = (InternetReadFilePROC) GetProcAddress(WinINETDll, "InternetReadFile");

        if (dllInternetReadFile)
        {
            return (BOOL) (dllInternetReadFile) (hFile, lpBuffer, dwNumberOfBytesToRead, lpdwNumberOfBytesRead);
        }
    }
    return FALSE;
}
