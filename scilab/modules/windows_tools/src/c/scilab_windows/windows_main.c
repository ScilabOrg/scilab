...
int Windows_Main(HINSTANCE hInstance, HINSTANCE hPrevInstance, PSTR szCmdLine, int iCmdShow)
{
    ...
    for (i = 1; i < my_argc; i++)
    {
        if ((_stricmp(my_argv[i], "-NW") == 0) || (_stricmp(my_argv[i], "-NWI") == 0) || (_stricmp(my_argv[i], "-TEXMACS") == 0)
            || (_stricmp(my_argv[i], "-NOGUI") == 0))
        {
            MessageBox(NULL, "Not with Windows Console", "Error", MB_ICONINFORMATION);
            exit(1);
        }

        if ((_stricmp(my_argv[i], "-VERSION") == 0) || (_stricmp(my_argv[i], "-VER") == 0))
        {
            disp_scilab_version();
            exit(1);
        }

        if ((_stricmp(my_argv[i], "-H") == 0) || (_stricmp(my_argv[i], "-?") == 0) || (_stricmp(my_argv[i], "-HELP") == 0))
        {
            char Msg[2048];
    ...
    if (argcount > 2)
    {
        if ((_stricmp(my_argv[1], "-X") == 0) || (_stricmp(my_argv[1], "-O") == 0) || (_stricmp(my_argv[1], "-P") == 0))
        {
            char *Commande = NULL;
            int CodeAction = -1;
            ...
            if (((IsAScicosFile(FileName) == TRUE) && (CodeAction == 1)) ||
                ((IsABinOrSavFile(FileName) == TRUE) && (CodeAction == 1)) || ((IsASciNotesFile(FileName) == TRUE)))
            {
                my_argc = -1;
