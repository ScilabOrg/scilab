...
if getos() == "Windows" then
    expectedDetailedErrorMsg = msprintf(_("Cannot create jar archive %s: %s (The system cannot find the file specified)"), jarDestPath, jarSrcPath);
else
    expectedDetailedErrorMsg = msprintf(_("Cannot create jar archive %s: %s (No such file or directory)"), jarDestPath, jarSrcPath);
end
expectedErrorMsg = msprintf(_("%s: An error occured: %s"), "jcreatejar", expectedDetailedErrorMsg);
assert_checkerror("jcreatejar(jarDestPath, jarSrcPath)", expectedErrorMsg);
