...
int ScilabJavaEnvironment::createJarArchive(char *jarFilePath, char **filePaths, int size)
{
    JavaVM *vm = getScilabJavaVM();
    try
    {
        return ScilabJarCreator::createJarArchive(vm, jarFilePath, filePaths, size);
    }
    catch (const GiwsException::JniException & e)
    {
