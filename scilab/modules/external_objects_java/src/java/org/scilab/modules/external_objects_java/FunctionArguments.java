...
public final class FunctionArguments {
    ...
    static {
        ...
        registerConverter(new Converter() {
            ...
            @Override
            public boolean canConvert(Class<?> from, Class<?> to) {
                return to.isArray() && from.isArray() && from.getComponentType().isArray()
                       && to.getComponentType() == from.getComponentType().getComponentType();
            }
        });
        ...
        // Converter to convert a DoubleBuffer to a double[] (or an other primitive type)
        registerConverter(new Converter() {
            @Override
            public Object convert(Object original, Class<?> to) {
                return ScilabJavaArray.toArray((Buffer) original);
            }

            @Override
            public boolean canConvert(Class<?> from, Class<?> to) {
                return to.isArray() && ScilabJavaArray.bufferMappings.get(from) == to.getComponentType();
            }
        });

        // Converter to org.scilab.modules.types.*
        registerConverter(new Converter() {
            @SuppressWarnings("unchecked")
            @Override
            public Object convert(Object original, Class<?> to) {
                return ScilabJavaType.convertTo(original, (Class<? extends ScilabType>) to);
            }

            @SuppressWarnings("unchecked")
            @Override
            public boolean canConvert(Class<?> from, Class<?> to) {
                return ScilabType.class.isAssignableFrom(to) && ScilabJavaType.canConvertTo(from, (Class<? extends ScilabType>) to);
            }
        });
    }
    ...
    /**
     * To find the "correct" method we proceed as follow: i) We calculate the distance between the Class of the arguments. For a Class B which derivates from A (A.isAssignableFrom(B) == true), the
     * distance between A and B is the number of derivations of A to obtain B. For example, if we have C extends B extends A, dist(C,A)=2. So the square of the distance between two Class array is
     * defined as the sum of the square of the distance between Class elements. In the particular case where a int.class is required and the argument is double.class, if the double value is an integer
     * the argument is converted into an int. In this case, dist(int.class, double.class) is equal to 2048. This value has been choosed to be "sure" that another method with double.class will be
     * considered better than the previous one. If the method has variable arguments foo(A a, B b, Object...), the Java reflection only shows that Class arguments are A.class, B.class and
     * Object[].class. So such a method will give a very high distance (more than 2^40). When a method is not correct, the distance will be Long.MIN_VALUE.
     *
     * ii) For each Method we calculate the distance and we try to find the method with the minimal distance. iii) If a method has variable arguments, then we transform the arguments and return the
     * new ones in the second element of the returned array.
     */
     ...
     *            the method name
     * @param functions
     *            the set of the valid functions in the class
     * @param argsClass
     *            the class of the arguments
     * @param args
     *            the arguments
     ...
     *
     * @param functions
     *            the set of the valid constructors in the class
     * @param argsClass
     *            the class of the arguments
     * @param args
     *            the arguments
     ...
     * @param B
     *            the class of the passed arguments
     * @param arr
     *            array of arguments (used to transform double in int)
     * @param bools
     *            references on boolean
     * @return the distance
     */
    ...
    /**
     * Calculate the distance between two classes. If B derivate from A, then the distance is the number of derivations.
     *
     * @param A
     *            a class
     * @param B
