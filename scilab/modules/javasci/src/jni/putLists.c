    ...
    SciErr sendList(JNIEnv * jenv, jobject list, int * parentList, int pos, char * varName, char listType)
    {
        ...
        for (; i < listLen - 1; i++)
        {
            ...
            case sci_matrix :
            {
                ...
                jobject data = (jobject)(*jenv)->GetObjectArrayElement(jenv, infos, 0);
                ...
                if (isReal)
                {
                    for (; j < nbRow; j++)
                    {
                        ...
                        jdouble* element = (jdouble*)(*jenv)->GetPrimitiveArrayCritical(jenv, oneDim, &isCopy1);
                ...
                else
                {
                    ...
                    for (; j < nbRow; j++)
                    {
                        ...
                        jboolean isCopy1 = JNI_FALSE;
                        jdouble* elementR = (jdouble*)(*jenv)->GetPrimitiveArrayCritical(jenv, oneDimR, &isCopy1);
                        isCopy1 = JNI_FALSE;
                        jdouble* elementI = (jdouble*)(*jenv)->GetPrimitiveArrayCritical(jenv, oneDimI, &isCopy1);
                        for (k = 0; k < nbCol; k++)
                        {
                ...
                else
                {
                    ...
                    for (; j < nbRow; j++)
                    {
                        ...
                        for (k = 0; k < nbCol; k++)
                        {
                            ...
                            jboolean isCopy1 = JNI_FALSE;
                            jdouble* element = (jdouble*)(*jenv)->GetPrimitiveArrayCritical(jenv, oneDimR, &isCopy1);
                            memcpy(real[k * nbRow + j], element, sizeof(double) * len);
            ...
            case sci_boolean :
            {
                ...
                for (; j < nbRow; j++)
                {
                    ...
                    {
                        nbCol = (*jenv)->GetArrayLength(jenv, oneDim);
                        b = (BOOL*)MALLOC(sizeof(BOOL) * nbRow * nbCol);
                    }
                    jboolean isCopy1 = JNI_FALSE;
                    jdouble* element = (jdouble*)(*jenv)->GetPrimitiveArrayCritical(jenv, oneDim, &isCopy1);
            ...
            case sci_sparse :
            {
                ...
                jboolean isCopy1 = JNI_FALSE;
                int* dims = (jint*)(*jenv)->GetPrimitiveArrayCritical(jenv, jdims, &isCopy1);
                isCopy1 = JNI_FALSE;
                int* nbItemRow = (int*)(*jenv)->GetPrimitiveArrayCritical(jenv, jnbItemRow, &isCopy1);
                isCopy1 = JNI_FALSE;
                int* colPos = (int*)(*jenv)->GetPrimitiveArrayCritical(jenv, jcolPos, &isCopy1);
                isCopy1 = JNI_FALSE;
                double* data = (double*)(*jenv)->GetPrimitiveArrayCritical(jenv, jdata, &isCopy1);
                isCopy1 = JNI_FALSE;
                double * imagData;
                if (!isReal)
                {
            ...
            case sci_ints :
            {
                ...
                case sci_int8 :
                {
                    ...
                    for (; j < nbRow; j++)
                    {
                        ...
                        jboolean isCopy1 = JNI_FALSE;
                        jbyte* element = (jbyte*)(*jenv)->GetPrimitiveArrayCritical(jenv, oneDim, &isCopy1);
                ...
                case sci_int16 :
                {
                    ...
                    for (; j < nbRow; j++)
                    {
                        ...
                        jboolean isCopy1 = JNI_FALSE;
                        jshort* element = (jshort*)(*jenv)->GetPrimitiveArrayCritical(jenv, oneDim, &isCopy1);
                ...
                case sci_int32 :
                {
                    ...
                    for (; j < nbRow; j++)
                    {
                        ...
                        jboolean isCopy1 = JNI_FALSE;
                        jint* element = (jint*)(*jenv)->GetPrimitiveArrayCritical(jenv, oneDim, &isCopy1);
