...
function %cpr = xcos_simulate(scs_m, needcompile)
    ...
    // First step: Search the %cpr tree for To_Workspace blocks, and extract the variable names.
    path = %cpr.sim;
    Names = [];
    increment = 1;
    for i=1:size(path.funs)
        increment2 = path.ipptr(i+1);
        if (increment2 - increment <> 0) then
            space = increment2 - increment;
            if (path.funs(i) == "tows_c") then
                varNameLen = space - 2; // Space minus buffer_size and var length
                varNameCode = path.ipar(increment+2:increment+2+varNameLen-1);
                varName = code2str(varNameCode);
                Names = [Names varName];
            end
