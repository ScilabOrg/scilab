...
function %cpr = xcos_simulate(scs_m, needcompile)
    ...
    function names = update_ToWorkspaceBlocks(tree_path)
        names = [];
        for i=1:size(tree_path)
            if (typeof(tree_path(i)) == "Block")
                if (tree_path(i).gui == "SUPER_f") // If a superblock is found, make a recursive call and append the result of it
                    names_rec = update_ToWorkspaceBlocks(tree_path(i).model.rpar.objs);
                    if ~isempty(names_rec) then
                        names = [names names_rec];
                    end
                end
                if (tree_path(i).gui) == "TOWS_c"
                    varName = tree_path(i).graphics.exprs(2);
    ...
    // At the end, Names contains the string names of the variables
    Names = update_ToWorkspaceBlocks(scs_m.objs);

    // Link the variable names to their values vectors,
    //and call '[names(1), names(2), ...] = resume(names(1), names(2), ...)' to save the variable into Scilab
    //pause
    if ~isempty(Names) then
        ierr = execstr(Names(1)+" = struct(''values'',"+Names(1)+"_val,''time'',"+Names(1)+"_valt)", "errcatch");
        if ierr <> 0 then
            str_err = split_lasterror(lasterror());
            message(["Simulation problem" ; "Unable to resume To Workspace Variable {"+Names(1)+"}:" ; str_err]);
        end
        Resume_line_args = Names(1);   // Will contain 'names(1), names(2), ...'
        for i=2:size(Names, 2)
            ierr = execstr(Names(i)+" = struct(''values'',"+Names(i)+"_val,''time'',"+Names(i)+"_valt)", "errcatch");
            if ierr <> 0 then
                str_err = split_lasterror(lasterror());
