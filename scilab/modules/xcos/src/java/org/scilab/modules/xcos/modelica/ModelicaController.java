	...
	private void updateInfo(Struct struct) {
		for (Object child : struct.getSubnodes().getTerminalOrStruct()) {
			if (child instanceof Terminal) {
				...
				incrementTerminalWeight(kind, weight);
	...
	private void setDefaultValues(final Struct struct) {
		for (final Object child : struct.getSubnodes().getTerminalOrStruct()) {
			if (child instanceof Terminal) {
				 ...
				 */
				TerminalAccessor.getData(TerminalAccessor.FIXED, terminal);
				TerminalAccessor.getData(TerminalAccessor.WEIGHT, terminal);
				TerminalAccessor.getData(TerminalAccessor.INITIAL, terminal);

				TerminalAccessor.getData(TerminalAccessor.MAX, terminal);
				TerminalAccessor.getData(TerminalAccessor.MIN, terminal);
	...
	private void updateWeight(final Struct struct, double derivative,
		...
		for (final Object child : struct.getSubnodes().getTerminalOrStruct()) {
			if (child instanceof Terminal) {
				...
				} else if (localVarName.contains(id) && !fixed) {
