...
public class BlockGraphicElement{
	...
	private static final Log commmonLogging = LogFactory.getLog(BlockGraphicElement.class);
	...
	private void fillOrigin(SimulinkBlock from, BasicBlock into) {
		...
		if (commmonLogging.isTraceEnabled()) {
			commmonLogging.trace(Integer.toString(position.length));
			for(int i = 0; i < position.length ; i++){
				commmonLogging.trace(position[i]);
			}
		}
