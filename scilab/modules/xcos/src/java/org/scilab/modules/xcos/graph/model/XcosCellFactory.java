...
import static org.scilab.modules.action_binding.InterpreterManagement.putCommandInScilabQueue;
...
public final class XcosCellFactory {
    ...
    public static void created(long uid, int kind) {
        lastCreated = new XcosCell(uid, Kind.values()[kind]);
        XcosCellFactory.class.notify();
    }
    private static XcosCell lastCreated = null;
    ...
    private static BasicBlock createBlock(final JavaController controller, BlockInterFunction func, String interfaceFunction) {
        ...
        try {
            putCommandInScilabQueue(
                "blk = " + buildCall(interfaceFunction, "define") +
                buildCall("xcosCellCreated", "blk".toCharArray()));

            XcosCellFactory.class.wait();

            if (lastCreated.getKind() == Kind.BLOCK) {
