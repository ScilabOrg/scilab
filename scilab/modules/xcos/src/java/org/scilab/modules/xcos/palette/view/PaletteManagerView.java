...
public class PaletteManagerView extends SwingScilabTab implements Tab {
    public static final String UUID = PaletteManager.MODEL_CLASS_PACKAGE;
    ...
    private PaletteManagerView(final PaletteManager controller) {
        super(XcosMessages.PALETTE_BROWSER + " - " + Xcos.TRADENAME, UUID);
    ...
    public static PaletteManagerView get() {
        return (PaletteManagerView) Xcos.getInstance().getTab(UUID);
    }
    ...
    public static void close(Void v) {
        PaletteManagerView tab = get();
        if (tab != null) {
    ...
    private Window createDefaultWindow() {
        ...
        win.setDims(new Size(600, 600));

        // set the default size
        win.setPosition(new Position(0, 0));
