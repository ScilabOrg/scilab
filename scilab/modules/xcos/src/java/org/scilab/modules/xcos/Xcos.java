			...
			SwingUtilities.invokeAndWait(new Runnable() {
				...
				public void run() {
					...
					while (block instanceof SuperBlock && !deque.isEmpty()) {
						((BasicBlock) block).getParentDiagram().warnCellByUID(((BasicBlock) block).getId(), XcosMessages.ERROR_UNABLE_TO_COMPILE_THIS_SUPER_BLOCK);
					...
					final XcosDiagram parent = ((BasicBlock) block).getParentDiagram();
					parent.warnCellByUID(((BasicBlock) block).getId(), message);
