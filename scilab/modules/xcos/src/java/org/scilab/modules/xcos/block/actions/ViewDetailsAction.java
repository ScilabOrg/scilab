...
public final class ViewDetailsAction extends VertexSelectionDependantAction {
	 ...
	 */
	private void viewDetails(BasicBlock block) {
		/*
		 * Export data
		 */
		File temp;
		try {
			temp = File.createTempFile(BasicBlock.INTERNAL_FILE_PREFIX, BasicBlock.INTERNAL_FILE_PREFIX, XcosConstants.TMPDIR);
			new H5RWHandler(temp).writeBlock(block);
