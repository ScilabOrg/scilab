				...
				public void actionPerformed(ActionEvent e) {
					...
					if (current != null && !current.isEmpty()) {
						try {
							...
							final File parent = getGraph().getSavedFile().getParentFile();
							chooser.setSelectedFile(new File(parent, current));
						}
					} else {
						chooser.setCurrentDirectory(getGraph().getSavedFile().getParentFile());
					}
				    ...
				    if (returnVal == javax.swing.JFileChooser.APPROVE_OPTION) {
			    		String parent;
						try {
							parent = getGraph().getSavedFile().getParentFile().getCanonicalPath() + File.separator;
							final String child = chooser.getSelectedFile().getCanonicalPath();
				    		
				    		final File relativeChild = new File(child.replace(parent, ""));
							imagePath.setText(relativeChild.getPath());
