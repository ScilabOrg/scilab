...
public class EditFormatAction extends DefaultAction {
	...
	private static EditFormatDialog createDialog(mxCell selectedCell, final XcosDiagram graph, 
		...
		if (selectedCell instanceof TextBlock) {
			final Object current = model.getValue(selectedCell);
			...
			final Object current = model.getValue(model.getCell(cellId));
			if (current == null) {
				text = "";
			} else {
				text = mxUtils.getBodyMarkup(current.toString(), false);
	...
	private static void updateFromDialog(EditFormatDialog dialog, Color borderColor, Color backgroundColor, 
		...
		if (cell instanceof TextBlock) {
			...
			graph.fireEvent(new mxEventObject(mxEvent.LABEL_CHANGED,
					"cell", cell, "value", text, "parent", cell.getParent()));
		} else {
			/*
