...
public class XcosGraphModel extends mxGraphModel {
    ...
    public void setBlockValues(final BasicBlock cell, final BasicBlock value) {
        ...
        // create a model transaction as we perform multiple modification there
        beginUpdate();
        try {
            // block update
            ...
            // cell update
            setStyle(cell, value.getStyle());

            // children update
            updateChildren(cell, value);
        } finally {
            endUpdate();
        }
    }
     ...
     * @param value
     *            the new block values
     */
    private final void updateChildren(final BasicBlock cell, final BasicBlock value) {
        /*
        ...
        Map < Class <? extends mxICell > , Deque<mxICell >> annotatedOlds = cell.getTypedChildren(types);
        Map < Class <? extends mxICell > , Deque<mxICell >> annotatedNews = value.getTypedChildren(types);
        ...
        try {
            for (Class <? extends mxICell > klass : types) {
                ...
                while (!olds.isEmpty() && !news.isEmpty()) {
                    ...
                    remo
                    removeCells(new Object[] { previous }, false);
