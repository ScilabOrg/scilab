...
#define MSGRK_BAD_MODE_INTEG  "Illegal value for lmm. The legal values are RK_ADAMS and RK_BDF."
#define MSGRK_BAD_MODE_ITER  "Illegal value for iter. The legal values are RK_FUNCTIONAL and RK_NEWTON."
 ...
 *    DGMAX       iter == RK_NEWTON, |gamma/gammap-1| > DGMAX => call lsetup
...
typedef struct RKMemRec {
    ...
    int rk_mode_integ;   /* lmm = RK_ADAMS or RK_BDF          */
    int rk_mode_iter;    /* iter = RK_FUNCTIONAL or RK_NEWTON */
    ...
    N_Vector rk_zn[L_MAX];  /* Nordsieck array, of size N x (q+1).         */
    ...
    long int rk_lrw;         /* no. of R words in RKODE work vectors */
    long int rk_liw;         /* no. of integer words in RKODE work vectors  */
