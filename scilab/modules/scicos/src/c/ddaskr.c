...
int DDaskrInit (void * ddaskr_mem, DDASResFn Res, realtype t0, N_Vector yy0, N_Vector yp0)
{
    ...
    rwork[13] = 2 * UNIT_ROUNDOFF / 3;			// steptol
    rwork[14] = 0.01;						// epinit, swing factor in the Newton iteration convergence test.
...
int DDaskrCalcIC (void * ddaskr_mem, int icopt, realtype tout1)
{
    ...
    tdist = ABS(tout1 - tStart);
    troundoff = 2 * UNIT_ROUNDOFF * (ABS(tStart) + ABS(tout1));
    if (tdist < troundoff)
    {
    ...
    C2F(ddaskr) (res, nEq, &tStart, yVec, ypVec, &tout1, info, &relTol, &absTol, &iState, rwork, &lrw, iwork, &liw, &dummy1, &dummy2, jac, g_fun, &ng_fun, jroot);
