...
int DDaskrInit (void * ddaskr_mem, DDASResFn Res, realtype t0, N_Vector yy0, N_Vector yp0)
{
    ...
    /* Allocate the info[15] tab, used to store parameters, and set info[10] = 1 to get consistent values */
 ...
 * Sets iOpt to 1 for rwork->hmax to be taken in consideration by ddaskr().
 */
 ...
 * Sets iOpt to 1 for rwork->tcrit to be taken in consideration by ddaskr().
 */
...
int DDaskrSolve (void * ddaskr_mem, realtype tOut, realtype * tOld, N_Vector yOut, N_Vector ypOut, enum DDiTask_t itask)
{
    DDaskrMem ddas_mem;
    ...
    *tOld  = tStart;
    ...
    /* For continuation calls, avoiding recomputation of consistent values */
    info[10] = 0;
