...
int DDaskrInit (void * ddaskr_mem, DDASResFn Res, realtype t0, N_Vector yy0, N_Vector yp0, DDASJacPsolFn Jacpsol, DDASPsolFn Psol)
{
    ...
    rwork->steptol = 2 * UNIT_ROUNDOFF / 3; // steptol
...
{
    DDaskrMem ddas_mem;
    double rwork1, rwork2, rwork14, rwork15;
    int i, iwork32, iwork33, iwork34, iwork35, LID, * setid;
    ...
    *nEq    = NV_LENGTH_S(yy0);
    ...
    /* Reinitialize rwork and iwork, leave default parameters unchanged */
    rwork1  = rwork->tcrit;
    rwork2  = rwork->hmax;
    rwork14 = rwork->steptol;
    rwork15 = rwork->epinit;
    iwork32 = iwork[31];
    iwork33 = iwork[32];
    iwork34 = iwork[33];
    iwork35 = iwork[34];
    setid   = calloc(*nEq, sizeof(int));
    memcpy(setid, iwork + LID, *nEq); /* copy the "SetId" segment of iwork into setid */
    memset(rwork, 0, lrw);
    memset(iwork, 0, liw);
    rwork->tcrit   = rwork1;
    rwork->hmax    = rwork2;
    rwork->steptol = rwork14;
    rwork->epinit  = rwork15;
    iwork[31] = iwork32;
    iwork[32] = iwork33;
    iwork[33] = iwork34;
    iwork[34] = iwork35;
    memcpy(iwork + LID, setid, *nEq); /* copy setid back into iwork */
    free(setid);
...
int DDaskrCalcIC (void * ddaskr_mem, int icopt, realtype tout1)
{
    ...
    /* DDaskrSetId() must have been called prior to that,
       to ensure that ddaskr will consider the info[13] flag */
    if (info[13] == 0)
    {
        info[13] = 1;
    }
