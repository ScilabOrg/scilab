...
void * LSodarCreate (CVRhsFnLSodar f, int * neq, N_Vector y, int itol, realtype reltol, realtype abstol, int itask, CVRootFnLSodar grblk, int ng, int * jroot);
...
// Solving the problem
int LSodar (void * cvode_mem, realtype tOut, N_Vector yVec, realtype * tOld, int itask, int * neq, int * jroot);
...
static void LSFreeVectors (LSodarMem cvode_mem);

// Allocating the CVODE vectors ewt, acor, tempv, ftemp, *zn[0], ..., zn[maxord], lrw and liw */
booleantype lsodarAllocVectors (LSodarMem cvode_mem, N_Vector tmpl);
 ...
 */
int lsodarRootFind (LSodarMem cvode_mem);
