...
void lsodarCreate (CVRhsFnlsodar f, int *neq, realtype *yVec, realtype t0PlusEps, realtype t0, int itol, realtype reltol, realtype abstol, int itask, int istate, int iopt, realtype *rwork, int lrw, int *iwork, int liw, int Jacobians, int jtype, CVRootFn grblk, int ng, int *jroot);

// Specifying the maximum step size
int lsodarSetMaxStep (void *cvode_mem, realtype hmax);

// Allocating the problem
int lsodarMalloc (void* rk_mem, CVRhsFn f, realtype t0, N_Vector y0);

// Reinitializing the problem
int lsodarReInit (void* rk_mem, CVRhsFn f, realtype t0, N_Vector y0);

// Initializing the root-finding problem
int lsodarRootInit (void* rk_mem, int n_root_functions, CVRootFn g, void *gdata);

// Solving the problem
void lsodarSolve (CVRhsFnlsodar f, int *neq, realtype *yVec, realtype t, realtype told, int itol, realtype reltol, realtype abstol, int itask, int istate, int iopt, realtype *rwork, int lrw, int *iwork, int liw, int Jacobians, int jtype, CVRootFn grblk, int ng, int *jroot);

// Freeing the problem memory allocated by lsodarMalloc
void lsodarFree (void** rkutta_mem);

// Freeing the lsodar vectors allocated in lsodarAllocVectors
void lsodarFreeVectors (CVodeMem rk_mem);

// Allocating the CVODE vectors ewt, acor, tempv, ftemp, *zn[0], ..., zn[maxord], lrw and liw */
booleantype lsodarAllocVectors (CVodeMem rk_mem, N_Vector tmpl);

// Specifying the time beyond which the integration is not to proceed
int lsodarSetStopTime (CVodeMem rkutta_mem, realtype itstop);

// Error handling function
void lsodarProcessError (CVodeMem rk_mem, int error_code, const char *module, const char *fname, const char *msgfmt, ...);
