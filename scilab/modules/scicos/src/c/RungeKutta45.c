...
int RKMalloc (void* cvode_mem, CVRhsFn f, realtype t0, N_Vector y0, int itol, realtype reltol, void *abstol)
{
    ...
    // abstol must be of type Scalar (CV_SS = CV_ScalarScalar)
    if (itol != CV_SS)
    {
...
int RK (void* cvode_mem, realtype t, N_Vector yout, realtype *told, int pass_tF_or_back)
{
    ...
    {
        realtype test_div = interval_size / hmax;
...
booleantype RKAllocVectors (CVodeMem cv_mem, N_Vector tmpl, int tol)
{
    int i, j;
...
int RKSetStopTime (CVodeMem cvode_mem, realtype ttstop)
{
    if (cvode_mem == NULL)
    {
