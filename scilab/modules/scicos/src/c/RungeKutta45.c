...
//printf("tn = %E, h = %E, n = %ld, tn+h = %E, t = %E\n", tn, h, n_points, tn+h, t);
  ...
  for (i=0; i<=n_points-2; ++i) {			// Fixed number of points, so a 'for' is possible, to bypass 'ABS(tn+(i*h) - t)' rounding error
    ...
    N_VLinearSum_Serial (1., K[0], 2., K[1], K[1]);	// K2 = K1 + 2K2,
    N_VLinearSum_Serial (2., K[2], 1., K[3], K[3]);	// K4 = 2K3 + K4,
    N_VLinearSum_Serial (1., K[1], 1., K[3], K[3]);	// K4 = K1 + K4, so K4 = K1 + 2K2 + 2K3 + K4 (initial vectors),
    N_VLinearSum_Serial (1., yout, h/6, K[3], yout);	// yout = Yn+1 = Yn + K4*h/6
...
{
  N_VAbs (ycur, tempv);
  N_VLinearSum(reltol, tempv, 1., Vabstol, tempv);
  if (N_VMin(tempv) <= 0.) return (-1);
  N_VInv(tempv, weight);
  return (0);
}
