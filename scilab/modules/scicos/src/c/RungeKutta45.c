...
#define CV_ADD_VEC              -49
#define CV_MUL_VEC              -50
#define MSGCV_BAD_LENGTH "Impossible operation : invalid vector lengths"
...
int RKMalloc (void* rkutta_mem, CVRhsFn f, realtype t0, N_Vector y0, int itol, realtype reltol, void *abstol)
{
...
int RKReInit (void* rkutta_mem, CVRhsFn f, realtype t0, N_Vector y0, int itol, realtype reltol, void *abstol)
{
...
int RKRootInit (void* rkutta_mem, int nrtfn, CVRootFn g, void *gdata)
{
  ...
  /* If RKRootInit() was called with nrtfn == 0, then set cv_nrtfn to
...
int RK (void* rkutta_mem, realtype t, N_Vector yout, realtype *told, int pass_tF_or_back)
{
...
printf("debut RK\n");
  ...
  retval = f(tn, zn[0], zn[1], f_data);		 // Calling f at (t0,y0), setting zn[1] = y'(t0)
  ...
  if (interval_size <= Sabstol) { // "Small" interval, h is the size of it
    ...
    else { // Add a point and h < Sabstol to fit the interval
      n = floor_test+2;
      h = interval_size/(n-1);
    }
  }
  printf("apres decoupage\n");
...
printf("tF = %E, tn = %E, h = %E, n = %ld, tn+h = %E, t = %E\n", t, tn, h, n, tn+h, t);
  ...
  // Looping on time
  i = 0;
  while (ABS(tn+(i*h) - t) > TINY) { printf("while\n"); // As long as tn if strictly inferior to t
	...
	// Incrementing i => taking a step
	i++;
   printf("incremented. tn+(i*h) = %E\n", tn + (i*h));
	
  } // End of time loop
  
  // Success : start time (*told) := end time (t)
  *told = tn+(i*h);
  printf("succes rk");
  return (CV_SUCCESS);
}
