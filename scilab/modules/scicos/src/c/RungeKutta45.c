...
int RK (void *rkutta_mem, R t, N_Vector yout, R *told, int pass_tF_or_back)
{
  ...
  long int nSolCallsloc, n;
  int retval, hflag, kflag, istate, ier, irfndp;
  int ewtsetOK;
  R troundoff, rh, nrm, interval_size;
  ...
  if (interval_size <= Sabstol) { // "Small" interval, h is the size of it
	...
	interval_size *= Sabstol;
    int size_floor;
    size_floor = floor(interval_size);
    if (interval_size-size_floor == ZERO) { // Testing if we can cut the interval into Sabstol-long parts.
	  n = (int) interval_size+1;
      h = interval_size/n;
    }
    else { // Add a point and h < Sabstol to fit the interval
      n = floor(interval_size+2);
      h = interval_size/n;
    }
  }
  ...
  N_Vector k[4]; // [k1, k2, k3, k4]
  
  // Looping on time
  while (tn + h < t) {
    retval = f(tn, y, k[0], f_data);		 // k[0] = k1
    retval = N_VAdd(k[0], y, y);			 // y += k1
    retval = f(tn + h/2, y, k[1], f_data);	 // k[1] = k2 = f(tn+h/2, y),
    retval = N_VAdd(k[1], y, y);			 // y += k2
    retval = f(tn + h/2, y, k[2], f_data);  // k[2] = k3 = f(tn+h/2, y),
    retval = N_VAdd(k[2], y, y);			 // y += k3
    retval = f(tn + h, y, k[3], f_data);	 // k[3] = k4 = f(tn+h, y),
    ...
    retval = N_VMulScal(2, k[1]);			// k2 *= 2
    retval = N_VMulScal(2, k[2]);			// k3 *= 2
    ...
    retval = N_VMulScal(h/6, k[3]);		// k4 *= h/6
    retval = N_VAdd(y, k[3], yout);		// yout = y + k4
    
    if (retval) { // Something went wrong in the operations
  ...
  // Rest to see later
int useless(RKMem rk_mem) {  
  int ier, retval, hflag, task, nSolCallsloc, ewtsetOK, kflag, istate;
  R t, rh, troundoff, irfndp, *told, nrm;
  N_Vector yout;
  if (nSolCalls == 0) {
    ...
    if (h == ZERO) {
      // hflag = RKHin (rk_mem, t); // Commenting to compile
      if (hflag != RK_SUCCESS) {
   ...
   *    - checking if we passed t (NORMAL mode)
  ...
  if (nSolCalls > 0) {
    ...
    /// In RK_NORMAL mode, testing if t was reached
...
static int RKStep (RKMem rk_mem)
{
  ...
  // if ((nSolCalls > 0) && (hprime != h)) RKAdjustParams(rk_mem); // Commenting to compile
  
  // Looping point for attempts to take a step
  for(;;) {

    //RKPredict (rk_mem); // Commenting to compile
    RKSet (rk_mem);
  ...
  //RKCompleteStep (rk_mem); // Commenting to compile
  //RKPrepareNextStep (rk_mem, dsm); // Commenting to compile

  /* If Stablilty Limit Detection is turned on, calling stability limit
     detection routine for possible order reduction */
  // if (sldeton) RKBDFStab(rk_mem); // Commenting to compile
  // etamax = (nSolCalls <= SMALL_NST) ? ETAMX2 : ETAMX3; // Commenting to compile
 ...
 */
static int RKNls(RKMem rk_mem, int nflag)
{
  int flag = RK_SUCCESS;
  /*
  switch(iter) {
