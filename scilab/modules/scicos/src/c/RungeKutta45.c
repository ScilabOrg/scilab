...
#define CV_ADD_VEC       -49  // N_VAdd() error
#define CV_MUL_VEC       -50  // N_VMulScal() error
#define MSGCV_BAD_LENGTH "Impossible operation : invalid vector lengths"  // N_Vector operation error
...
int RK (void* cvode_mem, realtype t, N_Vector yout, realtype *told, int pass_tF_or_back)
{
  ...
  if (retval < 0) { // Problem evaluating f
  ...
  // ZC
  if (nrtfn > 0) { // At least one Zero-Crossing block,
	  
    retval = RKRcheck3(cv_mem);  // Looking for zero crossings
    if (retval == INITROOT) {
...
//printf("tF = %E, tn = %E, h = %E, n = %ld, tn+h = %E, t = %E\n", t, tn, h, n, tn+h, t);
  
  // Looping on time
  i = 0;  // i-th step
  
  //while (ABS(tn+(i*h) - t) > TINY) {		// As long as tn if strictly inferior to t
  for (i=0;i<=n_points-2;++i) {			// Fixed number of points, so a 'for' is possible, to bypass 'ABS(tn+(i*h) - t)' rounding error
    ...
    retval = N_VMulScal (h/2, K[0], Kprime[0]);	// K1 *= h/2,
    retval = N_VAdd (Kprime[0], yout, y);			// y += K1,
    retval = f (tn + h/2, y, K[1], f_data);		// K[1] = K2 = f(Tn+h/2, Yn+ (h/2)*K1),
    retval = N_VMulScal (h/2, K[1], Kprime[1]);	// K2 *= h/2,
    retval = N_VAdd (Kprime[1], yout, y);			// y += K2,
    retval = f (tn + h/2, y, K[2], f_data);		// K[2] = K3 = f(Tn+h/2, Yn + (h/2)*K2)),
    retval = N_VMulScal (h, K[2], Kprime[2]);		// K3 *= h,
    retval = N_VAdd (Kprime[2], yout, y);			// y += K3,
    retval = f (tn + h, y, K[3], f_data);			// K[3] = K4 = f(Tn+h, Yn + h*K3)
    
    // Checking for errors thanks to retval
	if (retval == -49) { // Error in the additions
	  RKProcessError (cv_mem, CV_ADD_VEC, "Runge-Kutta", "N_VAdd", MSGCV_BAD_LENGTH, tn);
      return (CV_ADD_VEC);
	}
	if (retval == -50) { // Error in the scalings
	  RKProcessError (cv_mem, CV_MUL_VEC, "Runge-Kutta", "N_VAdd", MSGCV_BAD_LENGTH);
      return (CV_MUL_VEC);
	}
    ...
    retval = N_VMulScal (2., K[1], K[1]);		// K2 *= 2,
    retval = N_VMulScal (2., K[2], K[2]);		// K3 *= 2,
    retval = N_VAdd (K[0], K[1], K[1]);		// K2 += K1,
    retval = N_VAdd (K[2], K[3], K[3]);		// K4 += K3,
    retval = N_VAdd (K[1], K[3], K[3]);		// K4 += K2, so K4 = K1 + 2K2 + 2K3 + K4 (initial vectors),
    retval = N_VMulScal (h/6, K[3], K[3]);		// K4 *= h/6,
    retval = N_VAdd (yout, K[3], yout);		// yout = Yn+1 = Yn + K4
    
    // Checking for errors. Ideally, every "retval = ..." should be followed by an error test, but we will see that later. Right now, lisibility primes.
    if (retval == -49) { // Error in the additions
	  RKProcessError (cv_mem, CV_ADD_VEC, "Runge-Kutta", "N_VAdd", MSGCV_BAD_LENGTH);
      return (CV_ADD_VEC);
	}
	if (retval == -50) { // Error in the scalings
	  RKProcessError (cv_mem, CV_MUL_VEC, "Runge-Kutta", "N_VAdd", MSGCV_BAD_LENGTH);
	...
	// Incrementing i => taking a step (if using while loop)
	//i++;
  } // End of time loop
 ...
 */
static int RKRcheck3 (CVodeMem cv_mem)
{
  /* SUNDIALS EXTENSION */
  if (is_sundials_with_extension())
	return RKRcheck3Ext (cv_mem);
  else
	return RKRcheck3Std (cv_mem);
}

static int RKRcheck3Ext (CVodeMem cv_mem)
{
	int i, retval, ier;
	// Setting thi = tn or tout, whichever comes first; setting y = y(thi)
	...
	if (taskc == CV_NORMAL) {
		if ((toutc - tn)*h >= ZERO) {
		  ...
		  thi = toutc;
		  (void) RKGetDky (cv_mem, thi, 0, y);
		}
	}

	// Setting ghi = g(thi) and calling RKRootfind to search (tlo,thi) for roots
...
static int RKRcheck3Std (CVodeMem cv_mem)
{
  int i, retval, ier;
...
static int RKRootfind (CVodeMem cv_mem)
{
  /* SUNDIALS EXTENSION */
  if (is_sundials_with_extension())
	return RKRootfindExt (cv_mem);
  else
	return RKRootfindStd (cv_mem);
}

/* Solving for a root of g(t) between tlo and thi, if one exists.
...
static int RKRootfindStd (CVodeMem cv_mem)
{
  ...
  /* If no sign change was found, resetting trout and grout.
   * Then either returning CV_SUCCESS if no zero was found, or setting iroots and returning RTFOUND */
  if (!sgnchg) { // No sign change
    trout = thi;
    for (i = 0; i < nrtfn; ++i) grout[i] = ghi[i];
    if (!zroot) return (CV_SUCCESS); // No Zero
    for (i = 0; i < nrtfn; ++i) {
      iroots[i] = 0;
  ...
  for(;;) {           // Looping point
    ...
    if (sgnchg) {
      ...
      continue;  // Returning to looping point
    }

    if (zroot) {
      // No sign change in (tlo,tmid), but g = 0 at tmid; returning root tmid
      thi = tmid;
      for (i = 0; i < nrtfn; ++i) ghi[i] = grout[i];
      break;
    }
...
// N_Vector operation : Res = X + Y
int N_VAdd (N_Vector X, N_Vector Y, N_Vector Res)
{
  long int i, M, N, O;
  realtype *xd, *yd, *resd;
  xd = yd = resd = NULL;
  ...
  N  = NV_LENGTH_S(Y);
  O  = NV_LENGTH_S(Res);
  if ((M != N) || (M != O)) return (CV_ADD_VEC);
  
  xd = NV_DATA_S(X);
  yd = NV_DATA_S(Y);
  resd = NV_DATA_S(Res);

  for (i=0; i<N; ++i) resd[i] = xd[i]+yd[i];

  return (CV_SUCCESS);
}

// N_Vector operation : Res = a*X
int N_VMulScal (realtype a, N_Vector X, N_Vector Res)
{
  long int i, N, M;
  realtype *xd, *resd;
  xd = resd = NULL;
  M = NV_LENGTH_S(X);
  N = NV_LENGTH_S(Res);
  if (M != N) return (CV_MUL_VEC);
  
  xd = NV_DATA_S(X);
  resd = NV_DATA_S(Res);

  for (i=0; i<N; ++i) resd[i] = a*xd[i];
