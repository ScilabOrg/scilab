...
static void simblkddaskr(realtype *tOld, realtype *actual, realtype *actualP, realtype *res, int *flag, realtype *dummy1, int *dummy2);
static void grblkddaskr(int *nequations, realtype *tOld, realtype *actual, int *ngc, realtype *res, realtype *dummy1, int *dummy2);
...
static void Jacobiansddaskr(realtype *t, realtype *y, realtype *yp,
                     realtype *pd, realtype cj, realtype *dummy1, int *dummy2);
...
static void cossimdaskr(double *told)
{
    ...
    if (*neq > 0)
    {
        ...
        {
            *ierr = 200 + (-flag);
            if (*neq > 0)if (solver == IDA_BDF_Newton) IDAFree(&dae_mem);
            if (*neq > 0)N_VDestroy_Serial(IDx);
            if (*neq > 0)N_VDestroy_Serial(yp);
            if (*neq > 0)N_VDestroy_Serial(yy);
            if (ng != 0) FREE(jroot);
...
static void jacpsol(realtype *res, int *ires, int *nequations, realtype *tOld, realtype *actual, realtype *actualP,
                  ...
                  int *ier, realtype *dummy1, int *dummy2)
{
    ...
    /* Work arrays used to store actual and actualP in order to
       evaluate res(*tOld, actual + small_increment, actualP + small_increment).
       savr already contains res(*tOld, actual, actualP). */
    ysave  = calloc(*nequations, sizeof(realtype));
    ypsave = calloc(*nequations, sizeof(realtype));
                  ...
                  int *iwp, realtype *b, realtype *eplin, int *ier, realtype *dummy1, int *dummy2)
{
    ...
    int i, nColB = 1, ipiv[*nequations];

    C2F(dgesv) (nequations, &nColB, wp, nequations, ipiv, b, nequations, ier);
