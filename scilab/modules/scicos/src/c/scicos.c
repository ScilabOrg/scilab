...
static void cossim(double *told)
{
    ...
    int flag = 0, flagr = 0;
    int cnt = 0;
    realtype *yVec; int istate = 1;
    yVec = NULL;
    int lrw = 20 + 16* (*neq) + 3*ng;
    int liw = 20 + (*neq);
    realtype *rwork = NULL; int *iwork = NULL;
    ...
    if (*neq > 0) /* Unfortunately CVODE does not work with NEQ==0 */
    {
        ...
        switch (C2F(cmsolver).solver)
        {
			case 0:
                ...
                yVec = NV_DATA_S(y);
                lsodarCreate(simblklsodar, neq, yVec, 0., 0.1, CV_SS, reltol, abstol, CV_NORMAL_TSTOP, istate, 0, rwork, lrw, iwork, liw, 0, 2, grblk, ng, jroot);
                istate = 2;
            case 1:
...
L30:
                ...
                if (Discrete_Jump == 0) /* if there was a dzero, its event should be activated*/
                {
                    phase = 2;
                    if (C2F(cmsolver).solver == 0) {
                      lsodarSolve(simblklsodar, neq, yVec, *told, t, CV_SS, reltol, abstol, CV_NORMAL_TSTOP, istate, 0, rwork, lrw, iwork, liw, 0, 2, grblk, ng, jroot);
                      *told = t;
				  }
                    else
...
/*--------------------------------------------------------------------------*/
/* grblklsodar */
//static int grblklsodar(int *neq, realtype t, realtype * y, int ngc, realtype * g0)
//{
//    double tx = 0., *x = NULL;
//    int jj = 0, nantest = 0;
//
