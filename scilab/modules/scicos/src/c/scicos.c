                ...
                double *simpar, int *flag__, int *ierr_out)
{
    ...
    *ierr = 0;
    ...
    else if (*flag__ == 2)     /*run*/
    {
        ...
        /*     integration */
        if (C2F(cmsolver).solver >= 0 && C2F(cmsolver).solver <= 7)
        {
            /*  0 - LSODAR: Method: DYNAMIC, Nonlinear solver= DYNAMIC */
            /*  1 - CVODE: Method: BDF,   Nonlinear solver= NEWTON     */
            /*  2 - CVODE: Method: BDF,   Nonlinear solver= FUNCTIONAL */
            /*  3 - CVODE: Method: ADAMS, Nonlinear solver= NEWTON     */
            /*  4 - CVODE: Method: ADAMS, Nonlinear solver= FUNCTIONAL */
            /*  5 - DOPRI: Method: Dormand-Prince, Nonlinear solver=  */
            /*  6 - RK45: Method: Runge-Kutta, Nonlinear solver=  */
            /*  7 - ImpRK45: Method: Runge-Kutta, Nonlinear solver= FIXED-POINT */
            cossim(t0);
        }
...
static void cossim(double *told)
{
    ...
    int (* ODE) (void*, realtype, N_Vector, realtype*, enum iTask_t);
    int (* ODEInit) (void*, void*, realtype, N_Vector);
    int (* ODEReInit) (void*, realtype, N_Vector);
    int (* ODERootInit) (void*, int, void*);
    ...
    int (* ODESetErrHandlerFn) (void*, void*, void*);
    void (* simb), (* grb), (* errFunc);
    int isCVode = C2F(cmsolver).solver != 0; // Boolean
    simb = isCVode ? simblk : simblklsodar;
    grb = isCVode ? grblk : grblklsodar;
    errFunc = &SundialsErrHandler;
	ODE = isCVode ? &CVode : &LSodar;
	ODEInit = isCVode ? &CVodeInit : &LSodarInit;
	ODEReInit = isCVode ? &CVodeReInit : &LSodarReInit;
	ODERootInit = isCVode ? &CVodeRootInit : &LSodarRootInit;
    ODESetMaxStep = isCVode ? &CVodeSetMaxStep : &LSodarSetMaxStep;
	ODESetStopTime = isCVode ? &CVodeSetStopTime : &LSodarSetStopTime;
	ODEGetRootInfo = isCVode ? &CVodeGetRootInfo : &LSodarGetRootInfo;
    ODESStolerances = isCVode ? &CVodeSStolerances : &LSodarSStolerances;
	ODESetErrHandlerFn = isCVode ? &CVodeSetErrHandlerFn : &LSodarSetErrHandlerFn;
    jroot = NULL;
    if (ng != 0)
    {
    ...
    if (*neq > 0) /* Unfortunately CVODE does not work with NEQ==0 */
    {
        ...
        flag = ODESetErrHandlerFn(cvode_mem, errFunc, NULL);
        ...
        flag = ODEInit (cvode_mem, simb, T0, y);
        ...
        flag = ODESStolerances(cvode_mem, reltol, abstol);
        ...
        flag = ODERootInit(cvode_mem, ng, grb);
        ...
        if (hmax > 0)
        {
            flag = ODESetMaxStep(cvode_mem, (realtype) hmax);
...
L30:
                ...
                {
                    flag = ODESetStopTime(cvode_mem, (realtype)tstop);  /* Setting the stop time*/
                    ...
                    flag = ODEReInit(cvode_mem, (realtype)(*told), y);
                ...
                {
                    phase = 2;
                    flag = ODE(cvode_mem, t, y, told, CV_NORMAL);
                    if (*ierr != 0)
                    {
                        freeall;
                        return;
                ...
                if (flag == CV_ROOT_RETURN)
                {
                    ...
                    if (Discrete_Jump == 0)
                    {
                        flagr = ODEGetRootInfo(cvode_mem, jroot);
...
L30:
                ...
                if (hot == 0)  /* CIC calculation when hot==0 */
                {
                    ...
                    //   return;
                    // };
                    // flag=IDASetMaxNumItersIC(ida_mem, 10);/* (def=10) setting the maximum number of Newton iterations in any attempt to solve CIC */
                    // if (check_flag(&flag, "IDASetMaxNumItersIC", 1)) {
                    //   *ierr=200+(-flag);
                    //   freeallx;
                    //   return;
