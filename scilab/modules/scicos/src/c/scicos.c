...
static int Jacobians(long int Neq, realtype tt, N_Vector yy, N_Vector yp,
                     N_Vector resvec, realtype cj, void *jdata, DenseMat Jacque, N_Vector tempv1, N_Vector tempv2, N_Vector tempv3);
                 ...
                 int *critev_in, int *nblk1, int *ztyp, int *zcptr_in, int *subscr, int *nsubs, double *simpar, int *flag__, int *ierr_out)
{
    ...
    *ierr = 0;
    ...
    C2F(makescicosimport) (x, &nx, &xptr[1], &zcptr[1], z__, &nz, &zptr[1],
                           ...
                           &ordptr[1], &nordptr, &critev[1], iwa, Blocks, t0, tf, &Atol, &rtol, &ttol, &deltat, &hmax, xprop, xd);
    ...
    {                           /*run */
        ...
        {                       /*  CVODE: Method: BDF,   Nonlinear solver= NEWTON     */
            cossim(t0);
        }
...
static void cossim(double *told)
{
    ...
    if (*neq > 0)
    {                           /* Unfortunately CVODE does not work with NEQ==0 */
        ...
        {                       /* This is for compatibility */
            ...
            else
            {
                ...
                (*loc2z) (flag, &block->nevprt, t, block->xd, block->x, &block->nx,
                          ...
                          block->evout, &block->nevout, block->rpar, &block->nrpar,
                          block->ipar, &block->nipar, (double **)block->inptr,
                          block->insz, &block->nin, (double **)block->outptr, block->outsz, &block->nout, block->g, &block->ng);
            }
        ...
        for (i = 0; i < 10; i++)
        {
            ...
            for (j = 0; j < N; j++)
            {
                if (x[j] == 0)
                    ...
                    ysdata[j] += ratio / abs(x[j]);
                if (fsdata[j] == 0)
                    fsdata[j] = 1;
                else
                    fsdata[j] = 1 / abs(fsdata[j]);
                ysdata[j] /= ratio + 1;
            }
