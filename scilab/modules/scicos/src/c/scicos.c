...
#define freeallx				\
	...
	if (*neq>0) FREE(data);			\
	if (*neq>0) if (C2F(cmsolver).solver == 100) IDAFree(&ida_mem); else (DDaskrFree(&ida_mem)); 	\
                ...
                double *simpar, int *flag__, int *ierr_out)
{
    ...
    *ierr = 0;
    ...
    else if (*flag__ == 2)     /*run*/
    {
        ...
        else if (C2F(cmsolver).solver == 101)  /* DDASKR  : Method:       , Nonlinear solver=  */
        {
            cossimdaskr(t0);
        }
        else
...
static void cossimdaskr(double *told)
{
    ...
    if (*neq > 0)
    {
        ...
        if (C2F(cmsolver).solver == 101)
        {
            flag = DDaskrSetErrHandlerFn(ida_mem, SundialsErrHandler, NULL);
        }
		else
        ...
        if (C2F(cmsolver).solver == 101)
        {
            flag = DDaskrInit(ida_mem, simblkddaskr, T0, yy, yp);
        }
		else
        ...
        if (C2F(cmsolver).solver == 101)
        {
            flag = DDaskrSStolerances(ida_mem, reltol, abstol);
        }
		else
        ...
        if (C2F(cmsolver).solver == 101)
        {
            flag = DDaskrRootInit(ida_mem, ng, grblkddaskr);
        }
		else
        ...
        if (C2F(cmsolver).solver == 101)
        {
            //flag = DDaskrDlsSetDenseJacFn(ida_mem, Jacobiansddaskr);
        }
		else
        ...
        if (C2F(cmsolver).solver == 101)
        {
            flag = DDaskrSetUserData(ida_mem, data);
        }
		else
        ...
        if (hmax > 0)
        {
            if (C2F(cmsolver).solver == 101)
            {
                flag = DDaskrSetMaxStep(ida_mem, (realtype) hmax);
            }
		    else
        ...
        if (C2F(cmsolver).solver == 101)
        {
            //flag = DDaskrSetMaxNumJacsIC(ida_mem, maxnj);
        }
		else
        ...
        if (C2F(cmsolver).solver == 101)
        {
            //flag = DDaskrSetMaxNumItersIC(ida_mem, maxnit);
        }
		else
        ...
        if (C2F(cmsolver).solver == 101)
        {
            //flag = DDaskrSetMaxNumSteps(ida_mem, 2000);
        }
		else
...
L30:
                ...
                if (hot == 0)  /* CIC calculation when hot==0 */
                {
                    ...
                    if (C2F(cmsolver).solver == 101)
                    {
                        flag = DDaskrSetStopTime(ida_mem, (realtype)tstop);
                    }
		            else
                    ...
                    for (j = 0; j <= N_iters; j++)
                    {
                        ...
                        if (C2F(cmsolver).solver == 101)
                        {
                            flag = DDaskrReInit(ida_mem, (realtype)(*told), yy, yp);
                        }
		                else
                ...
                if (Discrete_Jump == 0) /* if there was a dzero, its event should be activated*/
                {
                    ...
                    if (C2F(cmsolver).solver == 101)
                    {
                        flagr = DDaskrSolve(ida_mem, t, told, yy, yp, DDAS_NORMAL);
                    }
                    else
                ...
                if (flagr == IDA_ROOT_RETURN)
                {
                    ...
                    if (Discrete_Jump == 0)
                    {
                        if (C2F(cmsolver).solver == 101)
                        {
                            flagr = DDaskrGetRootInfo(ida_mem, jroot);
                        }
                        else
