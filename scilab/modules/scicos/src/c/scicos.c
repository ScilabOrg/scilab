                ...
                double *simpar, int *flag__, int *ierr_out)
{
    ...
    *ierr = 0;
    ...
    else if (*flag__ == 2)     /*run*/
    {
        ...
        else if (C2F(cmsolver).solver == 4)   /*  CVODE: Method: Explicit Runge-Kutta, Nonlinear solver=  */
        {
            cossim(t0);
        }
        else if (C2F(cmsolver).solver == 5)   /*  CVODE: Method: Implicit Runge-Kutta, Nonlinear solver=  */
        {
            cossim(t0);
        }
