    ...
    if (*neq > 0) /* Unfortunately CVODE does not work with NEQ==0 */
    {
        ...
        switch (C2F(cmsolver).solver)
        {
			case 0:
                ...
                yVec = NV_DATA_S(y);
                lsodarCreate(simblk, neq, yVec, 0., 0., CV_SS, reltol, abstol, CV_NORMAL_TSTOP, 1, 0, NULL, 0, NULL, 0, Jacobians, 2, grblk, ng, jroot);
            case 1:
...
L30:
                ...
                if (Discrete_Jump == 0) /* if there was a dzero, its event should be activated*/
                {
                    phase = 2;
                    if (C2F(cmsolver).solver == 4)
                      flag = lsodarSolve(simblk, neq, yVec, t, *told, CV_SS, reltol, abstol, CV_NORMAL_TSTOP, 2, 0, NULL, 0, NULL, 0, Jacobians, 2, grblk, ng, jroot);
                    else
