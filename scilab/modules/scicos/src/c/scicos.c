...
static int simblkddasrt(realtype *tOld, realtype *actual, realtype *actualP, realtype *res, int *flag, int dummy1, int dummy2);
static int grblkddasrt(int *nequations, realtype *tOld, realtype *actual, int *ngroot, realtype *res, int *dummy1, int *dummy2);
...
static void cossimdaskr(double *told)
{
    ...
    if (*neq > 0)
    {
        ...
        if (C2F(cmsolver).solver == 101)
        {
            flag = DDasrtRootInit(ida_mem, ng, grblkdaskr);
        }
		else
...
L30:
                ...
                if (hot == 0)  /* CIC calculation when hot==0 */
                {
                    ...
                    for (j = 0; j <= N_iters; j++)
                    {
                        ...
                        if (C2F(cmsolver).solver == 101)
                        {
                            flag = DDasrtInit(ida_mem, simblkdaskr, T0, yy, yp);
                        }
		                else
                ...
                if (Discrete_Jump == 0) /* if there was a dzero, its event should be activated*/
                {
                    ...
                    if (C2F(cmsolver).solver == 101)
                    {
                        flagr = DDasrtSolve(ida_mem, t, told, yy, yp, IDA_NORMAL);
                    }
...
//    C2F(dcopy)(neq, actual, &c__1, res, &c__1);
    *ierr = 0;
    C2F(ierode).iero = 0;
    ...
    flag = 0; // To tell DDasrt that the function went well.
