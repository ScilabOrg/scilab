...
static void cossim(double *told)
{
    ...
    if (*neq > 0) /* Unfortunately CVODE does not work with NEQ==0 */
    {
        ...
        {
            case 0:
                cvode_mem = LSodarCreate(simblklsodar, neq, y, CV_SS, reltol, abstol, CV_NORMAL_TSTOP, grblklsodar, ng, jroot);/* Create the lsodar problem */
                break;
            case 1:
...
L30:
                ...
                if (Discrete_Jump == 0) /* if there was a dzero, its event should be activated*/
                {
                    phase = 2;
                    if (!C2F(cmsolver).solver) {
                      flag = LSodar(cvode_mem, t, y, told, CV_NORMAL_TSTOP, neq, jroot);
                    }
                    else
