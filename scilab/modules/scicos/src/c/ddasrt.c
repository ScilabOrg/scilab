...
extern void C2F(ddasrt) (DDASResFn res, int *neq, realtype *t, realtype *y, realtype *yp, realtype *tout, int * info, int *itol, realtype *reltol, realtype *abstol, enum DDiTask_t *itask, int *istate, int *iopt, struct DDrWork_t *rwork, int *lrw, int *iwork, int *liw,  int *jacobian, int *jacType, DDASRootFn grblk, int *ng, int *jroot);
...
void * DDasrtCreate (int * neq, int ng)
{
    ...
    if (ddasrt_mem == NULL)
    {
        DDASProcessError(NULL, 0, "DDASRT", "DDasrtCreate", MSGIDA_IDAMEM_FAIL);
        return (NULL);
    }

    /* Zero out ddas_mem */
    memset(ddasrt_mem, 0, sizeof(struct DDasrtMemRec));
    //zeroes out info[] ? need to be sure



    ddassrt_mem->maxord = MAXORD_DEFAULT;
    ...
    ddasrt_mem->iState     = 1;
    ddasrt_mem->info[10]   = 1; // If we don't use IDA's CalcIC & GetConsistentIC, then set yp to 0 too ?
...
# define jacType    ddas_mem->jacType
...
int DDasrtInit (void * ddasrt_mem, DDASResFn Res, realtype t0, N_Vector yy0, N_Vector yp0)
{
    ...
    if (ddasrt_mem == NULL)
    {
        DDASProcessError(NULL, IDA_MEM_NULL, "DDASRT", "DDasrtInit", MSGIDA_NO_MEM);
        return (IDA_MEM_NULL);
    }
    ...
    if (y == NULL)
    {
        DDASProcessError(ddas_mem, IDA_ILL_INPUT, "DDASRT", "DDasrtInit", MSGIDA_NULL_Y0);
        return (IDA_ILL_INPUT);
    }

    if (f == NULL)
    {
        DDASProcessError(ddas_mem, IDA_ILL_INPUT, "DDASRT", "DDasrtInit", MSGIDA_NULL_F);
        return (IDA_ILL_INPUT);
    }
...
int DDasrtReInit (void * ddasrt_mem, realtype tOld, N_Vector yy, N_Vector yyp)
{
    DDasrtMem ddas_mem;
    ...
    if (ddasrt_mem == NULL)
    {
        DDASProcessError(NULL, IDA_MEM_NULL, "DDASRT", "DDasrtReInit", MSGIDA_NO_MEM);
        return (IDA_MEM_NULL);
    }
    ddas_mem = (DDasrtMem) ddasrt_mem;
    ...
    if (y == NULL)
    {
        DDASProcessError(ddas_mem, IDA_ILL_INPUT, "DDASRT", "DDasrtReInit", MSGIDA_NULL_Y0);
        return (IDA_ILL_INPUT);
    }
...
int DDasrtSStolerances(void * ddasrt_mem, realtype reltol, realtype abstol)
{
    ...
    if (ddasrt_mem == NULL)
    {
        DDASProcessError(NULL, IDA_MEM_NULL, "DDasrt", "DDasrtSStolerances", MSGIDA_NO_MEM);
        return (IDA_MEM_NULL);
    }
    ddas_mem = (DDasrtMem) ddasrt_mem;
    ...
    if (reltol < 0.)
    {
        DDASProcessError(ddas_mem, IDA_ILL_INPUT, "DDASRT", "DDasrtSStolerances", MSGIDA_BAD_RELTOL);
        return (IDA_ILL_INPUT);
    }

    if (abstol < 0.)
    {
        DDASProcessError(ddas_mem, IDA_ILL_INPUT, "DDASRT", "DDasrtSStolerances", MSGIDA_BAD_ABSTOL);
        return (IDA_ILL_INPUT);
    }
...
int DDasrtRootInit (void * ddasrt_mem, int ng, DDASRootFn g)
{
    ...
    if (ddasrt_mem == NULL)
    {
        DDASProcessError(NULL, IDA_MEM_NULL, "DDASRT", "DDasrtRootInit", MSGIDA_NO_MEM);
        return (IDA_MEM_NULL);
    }
    ddas_mem = (DDasrtMem) ddasrt_mem;
    ...
    if (g == NULL)
    {
        DDASProcessError(ddas_mem, IDA_ILL_INPUT, "DDASRT", "DDasrtRootInit", MSGIDA_NULL_G);
        return (IDA_MEM_NULL);
    }
...
int DDasrtSetMaxStep (void * ddasrt_mem, realtype hMax)
{
    ...
    if (ddasrt_mem == NULL)
    {
        DDASProcessError(NULL, IDA_MEM_NULL, "DDASRT", "DDasrtSetMaxStep", MSGIDA_NO_MEM);
        return (IDA_MEM_NULL);
    }
    ddas_mem = (DDasrtMem) ddasrt_mem;
...
int DDasrtSetStopTime (void * ddasrt_mem, realtype tCrit)
{
    ...
    if (ddasrt_mem == NULL)
    {
        DDASProcessError(NULL, IDA_MEM_NULL, "DDASRT", "DDasrtSetStopTime", MSGIDA_NO_MEM);
        return (IDA_MEM_NULL);
    }
    ddas_mem = (DDasrtMem) ddasrt_mem;
...
int DDasrtSolve (void * ddasrt_mem, realtype tOut, realtype * tOld, N_Vector yOut, N_Vector ypOut, enum DDiTask_t itask)
{
    ...
    if (ddasrt_mem == NULL)
    {
        DDASProcessError(NULL, IDA_MEM_NULL, "DDASRT", "DDasrt", MSGIDA_NO_MEM);
        return (IDA_MEM_NULL);
    }
    ddas_mem = (DDasrtMem) ddasrt_mem;
    ...
    if (yOut == NULL)
    {
        DDASProcessError(ddas_mem, IDA_ILL_INPUT, "DDASRT", "DDasrt", MSGIDA_YOUT_NULL);
        return (IDA_ILL_INPUT);
    }

    if ((itask != DDAS_NORMAL) && (itask != DDAS_ONE_STEP) && (itask != DDAS_MESH_STEP) &&
            (itask != DDAS_NORMAL_TSTOP) && (itask != DDAS_ONE_STEP_TSTOP))
    {
        DDASProcessError(ddas_mem, IDA_ILL_INPUT, "DDASRT", "DDasrt", MSGIDA_BAD_ITASK);
        return (IDA_ILL_INPUT);
    }
    ...
    *nEq   = NV_LENGTH_S(yOut);
    ...
    (RES, NEQ, T, Y, YPRIME, TOUT, INFO, RTOL, ATOL, IDID, RWORK, LRW, IWORK, LIW, RPAR, IPAR, JAC, G, NG, JROOT)
    C2F(ddasrt) (res, nEq, &tStart, yVec, ypVec, &tEnd, &info, &relTol, &absTol, &iState, rwork, &lrw, iwork, &liw, 0., 0., &jac, g_fun, &ng_fun, jroot);
    ...
    *tOld  = tOut;
    ...
    switch (iState)
    {
        ...
        case -1:
            DDASProcessError(ddas_mem, IDA_TOO_MUCH_WORK, "DDASRT", "DDasrt", MSGIDA_MAX_STEPS, tStart);
            return (IDA_TOO_MUCH_WORK);
        case -2:
            DDASProcessError(ddas_mem, IDA_TOO_MUCH_ACC, "DDASRT", "DDasrt", MSGIDA_TOO_MUCH_ACC, tStart);
            return (IDA_TOO_MUCH_ACC);
        case -3:
            DDASProcessError(ddas_mem, IDA_ILL_INPUT, "DDASRT", "DDasrt", MSGIDA_BAD_INPUT, tStart);
            return (IDA_ILL_INPUT);
        case -4:
            DDASProcessError(ddas_mem, IDA_ERR_FAILURE, "DDASRT", "DDasrt", MSGIDA_ERR_FAIDDAS, tStart);
            return (IDA_ERR_FAILURE);
        case -5:
            DDASProcessError(ddas_mem, IDA_CONV_FAILURE, "DDASRT", "DDasrt", MSGIDA_CONV_FAIDDAS, tStart);
            return (IDA_CONV_FAILURE);
        case -6:
            DDASProcessError(ddas_mem, IDA_ILL_INPUT, "DDASRT", "DDasrt", MSGIDA_EWT_NOW_BAD, tStart);
            return (IDA_ILL_INPUT);
        default:
            return (IDA_SUCCESS);
    }
...
int DDasrtGetRootInfo (void * ddasrt_mem, int * rootsfound)
{
    ...
    if (ddasrt_mem == NULL)
    {
        DDASProcessError(NULL, IDA_MEM_NULL, "DDASRT", "DDasrtGetRootInfo", MSGIDA_NO_MEM);
        return (IDA_MEM_NULL);
    }
    ddas_mem = (DDasrtMem) ddasrt_mem;
...
int DDasrtSetErrHandlerFn (void * ddasrt_mem, DDASErrHandlerFn ehfun, void * eh_data)
{
    ...
    if (ddasrt_mem == NULL)
    {
        DDASProcessError(NULL, IDA_MEM_NULL, "DDASRT", "DDasrtSetErrHandlerFn", MSGIDA_NO_MEM);
        return(IDA_MEM_NULL);
    }
