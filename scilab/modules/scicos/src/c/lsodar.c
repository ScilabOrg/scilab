...
# define lrn   cv_mem->lrn
# define lrs   cv_mem->lrs
...
int LSodarMalloc (void * cvode_mem, CVRhsFnLSodar f, realtype t0, N_Vector y, int itol, realtype reltol, void * abstol)
{
    ...
    /* Allocate rwork and iwork and set them to zero */
    rwork = (rWork) malloc(sizeof(struct rWorkRec));
    rwork->g0 = calloc(ng_fun, sizeof(realtype));
    rwork->g1 = calloc(ng_fun, sizeof(realtype));
    rwork->gx = calloc(ng_fun, sizeof(realtype));
    if (lrn >= lrs)
    {
        rwork->yh = calloc(13 * (*nEq), sizeof(realtype));
        rwork->ewt  = calloc(*nEq, sizeof(realtype));
        rwork->savf = calloc(*nEq, sizeof(realtype));
        rwork->acor = calloc(*nEq, sizeof(realtype));
    }
    else
    {
        rwork->yh = calloc(6 * (*nEq), sizeof(realtype));
        rwork->wm = calloc(pow(*nEq, 2) + 2, sizeof(realtype));
        rwork->ewt  = calloc(*nEq, sizeof(realtype));
        rwork->savf = calloc(*nEq, sizeof(realtype));
        rwork->acor = calloc(*nEq, sizeof(realtype));
    }
    //rwork = calloc(lrw, sizeof(realtype));
...
int LSodarReInit (void * cvode_mem, CVRhsFnLSodar f, realtype tOld, N_Vector y, int itol, realtype reltol, void * abstol)
{
    ...
    *nEq = NV_LENGTH_S(y);
    ...
    /* Reinitialize rwork and iwork, leave rwork[0] and rwork[5] unchanged, containing tcrit and hmax */
    realtype rwork0 = rwork->tcrit;
    realtype rwork5 = rwork->hmax;
    bzero(rwork, lrw);
    bzero(iwork, liw);
    rwork->tcrit = rwork0;
    rwork->hmax = rwork5;
...
#define CV_NORMAL_TSTOP 4
#define CV_ONE_STEP_TSTOP 5
...
int LSodarGetRootInfo (void * cvode_mem, int * rootsfound)
{
    ...
    memcpy(rootsfound, jroot, ng_fun);
