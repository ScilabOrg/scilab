...
int LSodarSolve (void * cvode_mem, int *neq, N_Vector y, realtype t, realtype tout, int itask, int * jroot)
{
    ...
    C2F(lsodar) (cv_mem->func, neq, yVec, &t, &tout, &cv_mem->iTol, &cv_mem->relTol, &cv_mem->absTol, &itask, &cv_mem->iState, &cv_mem->iOpt, cv_mem->rWork, &cv_mem->Lrw, cv_mem->iWork, &cv_mem->Liw, &cv_mem->Jac, &cv_mem->Jactype, cv_mem->g_fun, &cv_mem->ng_fun, jroot);

    NV_DATA_S(y) = yVec;

    return (cv_mem->iState);
}
