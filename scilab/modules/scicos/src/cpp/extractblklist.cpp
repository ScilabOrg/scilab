...
bool sci2var(T* p, void** dest)
{
    ...
    if (p->isComplex())
    {
        ...
        *dest = (T::type*)MALLOC(sizeof(T::type) * size * 2);
        if (*dest == nullptr)
        {
            return false;
        }
    ...
    {
        *dest = (T::type*)MALLOC(sizeof(T::type) * size);
        if (*dest == nullptr)
        {
            return false;
        }
...
bool sci2var(types::InternalType* p, void** dest)
{
    *dest = nullptr;
    switch (p->getType())
    {
...
bool getString(types::InternalType* p, char** dest)
{
    *dest = nullptr;
...
bool getDoubleArray(types::InternalType* p, double** dest, int size)
{
    *dest = nullptr;
...
bool getDoubleArrayAsInt(types::InternalType* p, int** dest, int size)
{
    *dest = nullptr;
...
bool getDoubleAsInt(types::InternalType* p, int* dest)
{
    if (p == nullptr)
    {
        return false;
...
bool checkType(int type, types::InternalType* p)
{
    if (p == nullptr)
    {
        return false;
...
bool extractblklist(types::TList* t, scicos_block *Block)
{
    ...
    {
        types::Double* d = pIT->getAs<types::Double>();
        Block->scsptr = (voidg)(long long)d->get()[0];
    }
