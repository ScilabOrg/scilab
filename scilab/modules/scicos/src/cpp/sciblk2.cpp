...
#include "scicos.h" /* set_block_error() */
    ...
    /* Function defined in "core" module */
    extern void C2F(mklist)(int* /*nin*/);
}
             ...
             int ipar[], int* nipar, double* inptr[], int insz[], int* nin, double* outptr[], int outsz[], int* nout, void* scsptr)
{
    ...
    types::Double* Tvec = new types::Double(*ntvec, 1);
    memcpy(Tvec->get(), tvec, *ntvec * sizeof(double));
    in[5] = Tvec;
    ...
    for (int i = 0; i < *nin; ++i)
    {
        types::Double* U = new types::Double(insz[i], insz[*nin + i]);
        memcpy(U->get(), inptr[i], insz[i] * insz[*nin + i] * sizeof(double));
        Nin->append(U);
    }
    in[8] = Nin;
    ...
    ***********************/
    ...
    switch (*flag)
    {
        ...
        case 3 :
        {
            ...
            memcpy(t, Tout->get(), *ntvec * sizeof(double));
            break;
        }
