...
template<>
int inline decode(const double* const tab, const int tabSize, const int iDims, const int offset, types::Double* &res)
{
    int  iElements = 1;
    int* pDims = new int[iDims];
    for (int i = 0; i < iDims; ++i)
...
template<>
int inline decode(const double* const tab, const int tabSize, const int iDims, const int offset, types::String* &res)
{
    int  iElements = 1;
    int* pDims = new int[iDims];
    for (int i = 0; i < iDims; ++i)
...
inline bool vec2var(const double* const in, const int inSize, types::InternalType* &out)
{
    const int iType = in[0];
    const int iDims = in[1];

    int offset = 0;
    if (!readElement(in, iType, iDims, inSize, offset, out))
    {
        return false;
    }
