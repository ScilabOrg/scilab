    ...
    BaseAdapter<LinkAdapter, org_scilab_modules_scicos::model::Link>(adapter)
{
    ...
    // No model update is required here, so call setFrom() with 'false'.
...
types::InternalType* LinkAdapter::clone()
{
    ...
    std::shared_ptr<model::Link> adaptee = std::static_pointer_cast<model::Link>(controller.getObject(id));
    LinkAdapter* ret = new LinkAdapter(adaptee);

    // When cloning a LinkAdapter, clone its 'from' and 'to' information as well.
    // No model update is required here, so call setFrom() with 'false'.
    ret->setFrom(getFrom(), controller, false);
    ret->setTo(getTo(), controller, false);
    return ret;
}
