...
bool setLinkEnd(const ScicosID id, Controller& controller, const object_properties_t end, const link_t& v)
{
    ...
    {
        get_or_allocate_logger()->log(LOG_ERROR, _("Wrong value for element #%d of %s or %s: %d or %d expected.\n"), 3, "from", "to", 0, 1);
        return false;
    }
    ...
    {
        // Deleted Block
        get_or_allocate_logger()->log(LOG_ERROR, _("Trying to link to a deleted block.\n"));
        return false;
    }
