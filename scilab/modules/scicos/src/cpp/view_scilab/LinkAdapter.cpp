...
struct ct
{
    ...
    static types::InternalType* get(const LinkAdapter& adaptor, const Controller& controller)
    {
        ...
        std::vector<int> ct;
        controller.getObjectProperty(adaptee->id(), adaptee->kind(), CT, ct);

        double* data;
        types::Double* o = new types::Double(1, 2, &data);

        data[0] = static_cast<double>(ct[0]);
        data[1] = static_cast<double>(ct[1]);
        return o;
    }
    ...
    static bool set(LinkAdapter& adaptor, types::InternalType* v, Controller& controller)
    {
        ...
        controller.setObjectProperty(adaptee->id(), adaptee->kind(), CT, ct);
        return true;
    }
};
