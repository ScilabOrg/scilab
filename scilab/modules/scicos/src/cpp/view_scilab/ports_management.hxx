...
bool update_ports_property(const Adaptor& adaptor, const object_properties_t port_kind, Controller& controller, types::InternalType* v)
{
    ...
    if (!newPorts.empty())
    {
        while (!newPorts.empty())
        {
            ...
            {
                case INPUTS:
                    controller.setObjectProperty(id, PORT, PORT_KIND, (int) PORT_IN);
                    break;
                case OUTPUTS:
                    controller.setObjectProperty(id, PORT, PORT_KIND, (int) PORT_OUT);
                    break;
                case EVENT_INPUTS:
                    controller.setObjectProperty(id, PORT, PORT_KIND, (int) PORT_EIN);
                    break;
                case EVENT_OUTPUTS:
                    controller.setObjectProperty(id, PORT, PORT_KIND, (int) PORT_EOUT);
                    break;
                default:
                    // should never happen
