...
struct rpar
{
    ...
    static bool set(ModelAdapter& adaptor, types::InternalType* v, Controller& controller)
    {
        ...
        else if (v->getType() == types::InternalType::ScilabUserType)
        {
            ...
            DiagramAdapter* newDiagram = new DiagramAdapter(std::shared_ptr<model::Diagram>(newSubAdaptee));
            newDiagram->setBlocks(diagram->getBlocks());
            newDiagram->setLinks(diagram->getLinks());
            newDiagram->setTexts(diagram->getTexts());

            adaptor.setDiagram(newDiagram);
