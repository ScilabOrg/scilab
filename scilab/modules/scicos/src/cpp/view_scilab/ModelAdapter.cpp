...
#include "gw_scicos.hxx" /* var2vec and vec2var functions */
...
struct odstate
{
    ...
    static types::InternalType* get(const ModelAdapter& adaptor, const Controller& controller)
    {
        ...
        types::Double* vecIn = new types::Double(prop_content.size(), 1);
        memcpy(vecIn->get(), &prop_content[0], prop_content.size()*sizeof(double));

        types::typed_list tempIn  (1, vecIn);
        types::typed_list tempOut (1);
        if (sci_vec2var(tempIn, 1, tempOut) == types::Function::Error)
        {
            return 0;
        }
    ...
    static bool set(ModelAdapter& adaptor, types::InternalType* v, Controller& controller)
    {
        ...
        types::typed_list tempIn  (1, v->getAs<types::List>());
        types::typed_list tempOut (1);
        if (sci_var2vec(tempIn, 1, tempOut) == types::Function::Error)
        {
            return false;
        }

        types::Double* vec = tempOut[1]->getAs<types::Double>();
        std::vector<double> prop_content (vec->getSize());
        memcpy(&prop_content[0], vec->get(), vec->getSize() * sizeof(double));
