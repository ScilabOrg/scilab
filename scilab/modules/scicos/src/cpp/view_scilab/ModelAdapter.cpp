...
struct odstate
{
    ...
    static bool set(ModelAdapter& adaptor, types::InternalType* v, Controller& controller)
    {
        ...
        types::Double* res;
        if (!var2vec(v->getAs<types::List>(), res))
        {
            return false;
        }
        std::vector<double> prop_content (res->getSize());
        memcpy(&prop_content[0], res->get(), res->getSize() * sizeof(double));
