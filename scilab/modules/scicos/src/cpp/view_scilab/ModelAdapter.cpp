...
types::InternalType* getField(const ModelAdapter& adaptor, const Controller& controller, const object_properties_t FIELD)
{
    model::Block* adaptee = adaptor.getAdaptee();

    std::vector<int> field;
    controller.getObjectProperty(adaptee->id(), adaptee->kind(), FIELD, field);
    ...
    for (int i = 0; i < field[0]; ++i) // 'o' must have exactly 'field[0]' elements
    {
        ...
        switch (field[fieldIndex])
        {
            ...
            {
                m = field[fieldIndex + 1];
                n = field[fieldIndex + 2];
                int isComplex = field[fieldIndex + 3];
                ...
                if (isComplex == 0)
                {
                    ...
                    memcpy(real, &field[fieldIndex + 3 + 1], m * n * sizeof(double));
                }
                else
                {
                    ...
                    memcpy(real, &field[fieldIndex + 3 + 1], m * n * sizeof(double));
                    memcpy(imag, &field[fieldIndex + 3 + 1 + 2 * m * n], m * n * sizeof(double));
                }
                o->set(i, pDouble);
                break;
            }
            case types::InternalType::ScilabInt8:
            {
                ...
                memcpy(data, &field[fieldIndex + 3], m * n * sizeof(char));
                o->set(i, pInt8);
                break;
            }
            case types::InternalType::ScilabUInt8:
            {
                ...
                memcpy(data, &field[fieldIndex + 3], m * n * sizeof(unsigned char));
                o->set(i, pUInt8);
                break;
            }
            case types::InternalType::ScilabInt16:
            {
                ...
                memcpy(data, &field[fieldIndex + 3], m * n * sizeof(short int));
                o->set(i, pInt16);
                break;
            }
            case types::InternalType::ScilabUInt16:
            {
                ...
                memcpy(data, &field[fieldIndex + 3], m * n * sizeof(unsigned short int));
                o->set(i, pUInt16);
                break;
            }
            case types::InternalType::ScilabInt32:
            {
                ...
                memcpy(data, &field[fieldIndex + 3], m * n * sizeof(int));
                o->set(i, pInt32);
                break;
            }
            case types::InternalType::ScilabUInt32:
            {
                ...
                memcpy(data, &field[fieldIndex + 3], m * n * sizeof(unsigned int));
                o->set(i, pUInt32);
                break;
            }
            case types::InternalType::ScilabString:
            {
                ...
                for (int j = 0; j < m * n; ++j)
                {
                    int strLen = field[fieldIndex + 3 + numberOfIntNeeded];
                    wchar_t* str = new wchar_t[strLen + 1];
                    memcpy(str, &field[fieldIndex + 3 + numberOfIntNeeded + 1], strLen * sizeof(wchar_t));
                    str[strLen] = '\0';
                    pString->set(j, str);
                    delete str;
...
bool setField(ModelAdapter& adaptor, Controller& controller, const object_properties_t FIELD, types::InternalType* v)
{
    ...
    for (int i = 0; i < list->getSize(); ++i)
    {
        ...
        // Save the matrix dimensions in 'field' and increment the field to match the next list element
