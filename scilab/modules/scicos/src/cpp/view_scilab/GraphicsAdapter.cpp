...
struct exprs
{
    ...
    static types::InternalType* get(const GraphicsAdapter& adaptor, const Controller& controller)
    {
        ...
        else if (exprs[0] == "CBLOCK" || exprs[0] == "SUPER")
        {
    ...
    static bool set(GraphicsAdapter& adaptor, types::InternalType* v, Controller& controller)
    {
        ...
        std::vector<std::string> exprs (1); // First element will contain "CBLOCK", "SUPER", "MBLOCK" or "OTHER"
        ...
        else if (v->getType() == types::InternalType::ScilabList)
        {
            ...
            // Leave 'exprs[0]' empty for the moment, because we don't know yet if we are dealing with a SuperBlock or a "CBLOCK"
            char* c_str; // Buffer
