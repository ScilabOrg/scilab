...
struct exprs
{
    ...
    static types::InternalType* get(const GraphicsAdapter& adaptor, const Controller& controller)
    {
        ...
        else if (exprs[0] == "List_CBLOCK" || exprs[0] == "List_SUPER")
        {
    ...
    static bool set(GraphicsAdapter& adaptor, types::InternalType* v, Controller& controller)
    {
        ...
        std::vector<std::string> exprs (1); // First element will contain "List_CBLOCK", "List_SUPER", "List_MBLOCK" or "String"
        ...
        else if (v->getType() == types::InternalType::ScilabList)
        {
            ...
            // Leave 'exprs[0]' empty for the moment, because we don't know yet if we are dealing with a SuperBlock or a "List_CBLOCK"
            char* c_str; // Buffer
            ...
            {
                exprs[0] = "List_CBLOCK"; // List coming from a C block
