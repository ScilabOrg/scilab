...
struct objs
{
    ...
    static bool set(DiagramAdapter& adaptor, types::InternalType* v, Controller& controller)
    {
        ...
        for (int i = 0; i < list->getSize(); ++i)
        {
            ...
            {
                if (fieldNames->getSize() != 5) // !"Text" "graphics" "model" "void" "gui"!
                {
                    return false;
                }
                id = controller.createObject(ANNOTATION);
                TextAdapter* localAdaptor = new TextAdapter(static_cast<model::Annotation*>(controller.getObject(id)));

                for (int j = 1; j < modelElement->getSize(); ++j)
                {
                    std::wstring name = fieldNames->get(j);
                    types::InternalType* fieldValue = modelElement->getAs<types::InternalType>();
            ...
            {
                if (fieldNames->getSize() != 5) // !"Block" "graphics" "model" "gui" "doc"!
                {
                    return false;
                }
            ...
            {
                if (fieldNames->getSize() != 8) // !"Link" "xx" "yy" "id" "thick" "ct" "from" "to"!
                {
                    return false;
                }
                id = controller.createObject(LINK);
                LinkAdapter* localAdaptor = new LinkAdapter(static_cast<model::Link*>(controller.getObject(id)));
...
DiagramAdapter::DiagramAdapter(const DiagramAdapter& o) :
    BaseAdapter<DiagramAdapter, org_scilab_modules_scicos::model::Diagram>(o), contrib_content(o.contrib_content->clone()) {}
