    ...
    doc_content(nullptr)
{
    ...
    {
        setDiagram(new DiagramAdapter(*adapter.getDiagram()));

        /*
         * Set the children and reference all of them
         */
        Controller controller;
        std::vector<ScicosID> childDiagram;
        controller.getObjectProperty(getDiagram()->getAdaptee()->id(), DIAGRAM, CHILDREN, childDiagram);
        for (const ScicosID id : childDiagram)
        {
            controller.referenceObject(id);
        }
        controller.setObjectProperty(getAdaptee()->id(), BLOCK, CHILDREN, childDiagram);
    }

    setDocContent(adapter.getDocContent()->clone());
}
