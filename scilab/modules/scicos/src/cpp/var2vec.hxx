...
const std::string var2vecName = "var2vec";

bool var2vec(types::InternalType* in, types::Double* &out);
...
inline size_t required_length(const V* /*it*/, T* v)
{
...
template <typename T>
inline void encode(T* input, types::Double* &ret)
{
...
inline void encodeList(T* input, types::Double* &ret)
{
    ...
    for (int i = 0; i < iElements; ++i)
    {
        memcpy(ret->get() + 2 + offset, listElements[i]->get(), listElements[i]->getRows() * sizeof(double));
        offset += listElements[i]->getRows();
    }
    // An empty list input will return [22; 0]
}

inline bool var2vec(types::InternalType* in, types::Double* &out)
{
    switch (in->getType())
    {
        ...
        case types::InternalType::ScilabList   :
            encodeList(in->getAs<types::List>(), out);
            break;
        case types::InternalType::ScilabTList  :
            encodeList(in->getAs<types::TList>(), out);
            break;
        case types::InternalType::ScilabMList  :
            encodeList(in->getAs<types::MList>(), out);
            break;
