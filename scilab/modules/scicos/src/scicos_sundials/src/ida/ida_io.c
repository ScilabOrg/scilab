...
int IDASetConstraints(void *ida_mem, N_Vector constraints)
{
    ...
    if (constraints->ops->nvdiv == NULL ||
        ...
        constraints->ops->nvcompare == NULL || constraints->ops->nvconstrmask == NULL || constraints->ops->nvminquotient == NULL)
    {
...
int IDAGetIntegratorStats(void *ida_mem, long int *nsteps, long int *nrevals,
                          ...
                          int *klast, int *kcur, realtype * hinused, realtype * hlast, realtype * hcur, realtype * tcur)
{
    IDAMem IDA_mem;
