...
static int IDATestError(IDAMem IDA_mem, realtype ck, realtype * err_k, realtype * err_km1);
...
static int IDAStopTest1(IDAMem IDA_mem, realtype tout, realtype * tret, N_Vector yret, N_Vector ypret, int itask);
static int IDAStopTest2(IDAMem IDA_mem, realtype tout, realtype * tret, N_Vector yret, N_Vector ypret, int itask);
...
int IDAMalloc(void *ida_mem, IDAResFn res, realtype t0, N_Vector yy0, N_Vector yp0, int itol, realtype rtol, void *atol)
{
    IDAMem IDA_mem;
...
int IDAReInit(void *ida_mem, IDAResFn res, realtype t0, N_Vector yy0, N_Vector yp0, int itol, realtype rtol, void *atol)
{
    IDAMem IDA_mem;
...
static booleantype IDACheckNvector(N_Vector tmpl)
{
    if ((tmpl->ops->nvclone == NULL) ||
        ...
        (tmpl->ops->nvabs == NULL) ||
        (tmpl->ops->nvinv == NULL) || (tmpl->ops->nvaddconst == NULL) || (tmpl->ops->nvwrmsnorm == NULL) || (tmpl->ops->nvmin == NULL))
        return (FALSE);
    else
        return (TRUE);
}
    ...
    /*-----------------------
      ...
      -----------------------*/
        ...
        if (nflag != IDA_SUCCESS)
        {
            ...
            kflag = IDAHandleNFlag(IDA_mem, nflag, err_k, err_km1, &ncfn, &ncf, &netf, &nef);
...
static int IDAHandleNFlag(IDAMem IDA_mem, int nflag, realtype err_k, realtype err_km1, long int *ncfnPtr, int *ncfPtr, long int *netfPtr, int *nefPtr)
{
    realtype err_knew;
...
void IDAProcessError(IDAMem IDA_mem, int error_code, const char *module, const char *fname, const char *msgfmt, ...)
{
    va_list ap;
    char msg[256];
...
void IDAErrHandler(int error_code, const char *module, const char *function, char *msg, void *data)
{
    IDAMem IDA_mem;
    char err_type[10];
