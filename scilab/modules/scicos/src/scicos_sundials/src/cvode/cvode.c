...
static int CVStepRK(CVodeMem cv_mem)
{
	...
	if (retval < 0) {
      CVProcessError(cv_mem, CV_RHSFUNC_FAIL, "Runge-Kutta", "RK", MSGCV_RHSFUNC_FAILED, tn);
      return(CV_RHSFUNC_FAIL);
    }
	if (retval > 0) {
	  CVProcessError(cv_mem, CV_FIRST_RHSFUNC_ERR, "Runge-Kutta", "RK", MSGCV_RHSFUNC_FIRST);
      return(CV_FIRST_RHSFUNC_ERR);
	}
