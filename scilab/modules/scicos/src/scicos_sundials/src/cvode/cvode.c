...
void *CVodeCreate(int lmm, int iter)
{
  ...
  if ((lmm != CV_ADAMS) && (lmm != CV_BDF) && (lmm != CV_RK)) { /* Integration mode : ADAMS, BDF or RK */
          ...
          realtype *tret, int itask)
{
  ...
  if (nst > 0) {
    ...
    if (lmm==CV_RK) mxstep = CVHinRK(cv_mem, tout, tret);
...
static int CVStepRK(CVodeMem cv_mem)
{
    ...
    /* Update the Nordsieck history array to take the new values */
    ...
    return(CV_SUCCESS);
	  ...
	  int i, j;
	  N_Vector K[3];	// K = [K1, K2, K3], Runge-Kutta coefficients
	  ...
	  while (nb_iter <= maxiter) {  // Same operations as above, but with K[i] updated and store result in y to compare with tempv
	    ...
	    if (difference < 0.000001) {  // Converged
		  ...
		  //printf("succes\n");
		  for (i=0; i<=2; ++i) N_VDestroy(K[i]);	// Destroy the K[i].
	      return (CV_SUCCESS);
	    }
	  ...
	  // maxiter attained, we consider that the algorithm diverged
	  return (CONV_FAIL);
    }
