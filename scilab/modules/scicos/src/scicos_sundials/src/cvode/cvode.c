...
static int CVHinDoPri(CVodeMem cv_mem, realtype tout, realtype *tret);
          ...
          realtype *tret, int itask)
{
  ...
  if (nst == 0) {
    ...
    /* Set initial h (from H0 or CVHin). 
       If Dormand-Price is selected, then we choose to set h to Sabstol. */
	...
	if ((lmm == CV_ADAMS) || (lmm == CV_BDF)) {
	  ...
	  mxstep = CVHinDoPri(cv_mem, tout, tret);
 	}
  ...
  if (nst > 0) {
    ...
    if (lmm==CV_DOPRI) mxstep = CVHinDoPri(cv_mem, tout, tret);
...
static int CVStepDoPri(CVodeMem cv_mem)
{
  ...
  N_VScale(d1, zn[2], y); 									/* y = d1*K1 + 0*K2, */

  /* K3 */
  N_VLinearSum_Serial (a31*h, zn[2], 1., zn[0], ftemp);	/* ftemp = Yn + K1*a31*h, */
  N_VLinearSum_Serial (a32*h, zn[3], 1., ftemp, acor);		/* ftemp += K2*a32*h, */
  retval = f(tn + c3*h, acor, zn[4], f_data);				/* zn[4] = K3 = f(Tn+c3*h, Yn + a31*h*K1 + a32*h*K2), */
  ...
  N_VLinearSum_Serial (a42*h, zn[3], 1., ftemp, acor); 	/* ftemp += K2*a42*h, */
  N_VLinearSum_Serial (a43*h, zn[4], 1., acor, ftemp); 	/* ftemp += K3*a43*h, */
  ...
  N_VLinearSum_Serial (a52*h, zn[3], 1., ftemp, acor); 	/* ftemp += K2*a52*h, */
  N_VLinearSum_Serial (a53*h, zn[4], 1., acor, ftemp); 	/* ftemp += K3*a53*h, */
  N_VLinearSum_Serial (a54*h, zn[5], 1., ftemp, acor); 	/* ftemp += K4*a54*h, */
  retval = f(tn + c5*h, acor, zn[6], f_data);				/* zn[6] = K5 = f(Tn+c5*h, Yn + a51*h*K1 + a52*h*K2 + a53*h*K3 + a54*h*K4), */
  ...
  N_VLinearSum_Serial (a62*h, zn[3], 1., ftemp, acor); 	/* ftemp += K2*a62*h, */
  N_VLinearSum_Serial (a63*h, zn[4], 1., acor, ftemp); 	/* ftemp += K3*a63*h, */
  N_VLinearSum_Serial (a64*h, zn[5], 1., ftemp, acor); 	/* ftemp += K3*a64*h, */
  N_VLinearSum_Serial (a65*h, zn[6], 1., acor, ftemp); 	/* ftemp += K3*a65*h, */
  ...
  N_VLinearSum_Serial (a73*h, zn[4], 1., ftemp, acor); 	/* ftemp += K3*a73*h, */
  N_VLinearSum_Serial (a74*h, zn[5], 1., acor, ftemp); 	/* ftemp += K3*a74*h, */
  N_VLinearSum_Serial (a75*h, zn[6], 1., ftemp, acor); 	/* ftemp += K3*a75*h, */
  N_VLinearSum_Serial (a76*h, zn[7], 1., acor, ftemp); 	/* ftemp += K3*a76*h, */
