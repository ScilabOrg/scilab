      ...
      if (readGlobal <> [])
          for i = 1:size(readGlobal, '*')
              ...
              for (j = 1:readGlobalTimeSize(i))
                  mputl(['    global_'+readGlobal(i)+'_Time['+string(j-1)+'] = '+string(evstr(readGlobal(i)+'.time('+string(j)+')'))+';'
                      ], fd)
              end
              // Fead data
              for (j = 1:readGlobalTimeSize(i))
                  for (k = 1:readGlobalSize(i))
                  mputl(['    global_'+readGlobal(i)+'['+string(j-1)+']['+string(k-1)+'] = '+string(evstr(readGlobal(i)+'.values('+string(j)+', '+string(k)+')'))+';'
                      ], fd)
