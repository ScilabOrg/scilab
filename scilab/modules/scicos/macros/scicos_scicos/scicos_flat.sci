  ...
  if x(1)=='Block' then
	...
	if size(vec,1)<>size(loc_mat,1) then
	  ...
	  if (ksup==0)|flgcdgen<>-1  then
	    ...
	    hilite_path(path, "There is another local GOTO in this diagram with the same tag ''"+loc_mat($,3)+"''",%t);
	  end
	  ok=%f;return
	end
      else
	...
	for kk=ki
      ...
      // Some errors have been detected, return.
      if typeof(scs_m.objs(kc)) <> "Block" then
          messagebox(gettext("Please fix errors on your diagram."))
          ok = %f;
          return;
      end
