...
function Info = scicos_simulate(scs_m, Info, updated_vars, flag, Ignb)
    ...
    if ierr == 0 then
        ...
        if (tf - t) < tolerances(3) then
            needstart = %t
            [alreadyran, %cpr, Resume_line, TOWS_vals, Names] = do_terminate1(scs_m, %cpr)
            for i=1:size(Names, "c")  // Do not count the first element of TOWS_vals, which is ([] []) for type convenience
                execstr(Names(i)+" = TOWS_vals("+string(i+1)+");");
            end
        else
            %tcur = t
        end
...
function [alreadyran, %cpr, Resume_line, TOWS_vals, Names] = do_terminate1(scs_m, %cpr)
    ...
    if alreadyran then
        ...
        if ~isempty(Names) then
            ...
            execstr("TOWS_vals = [TOWS_vals "+Names(1)+"];");
            if ierr <> 0 then
            ...
            for i=2:size(Names, "c")
                ...
                execstr("TOWS_vals = [TOWS_vals "+Names(i)+"];");
