...
for i=2:4  // abstol = 5*10^-i, precision
 ...
 for j=1:2     // n, order
  for k=1:2
   ...
   scs_m.props.tol(1) = 5*10^(-i); scs_m.props.tol(6) = 3;					// abstol + solver
   xcos_simulate(scs_m, 4);	// DoPri
...
disp(maxi)
   ...
   if (maxi > 10^-(i+2)) then disp ("Failed maxi"); break; end
   if (mea > 10^-(i+2)) then disp("Failed mean"); break; end
   if (stdeviation > 10^-(i+2)) then disp("Failed deviation"); break; end
