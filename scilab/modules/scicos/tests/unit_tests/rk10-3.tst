...
//  or test_run('scicos', 'rk10-3', ['no_check_error_output']);
...
for j=1:10	 // n, order
 ...
 scs_m.props.tol(1) = 0.001; scs_m.props.tol(6) = 3;					// abstol + solver
 ...
 assert_checktrue(maxi <= 10^-5);
 assert_checktrue(mea <= 10^-5);
 assert_checktrue(stdeviation <= 10^-5);
