...
int sci_scicosim(char *fname, unsigned long fname_len)
{
    ...
    else
    {
        ...
        for (j = 0; j < nlnk; j++)  /*for each link */
        {
            ...
            case 8:
                ...
                default:
                    Scierror(888, _("%s : error. Type %d of int scalar matrix not yet supported for outtb.\n"), fname, subheader[3]);
                    FREE(outtbptr);
                    FREE(outtbtyp);
                    FREE(outtbsz);
                    FREE(opar);
	...
	************************/
    ...
    C2F(scicos) (l_state_x, l_sim_xptr, l_state_z,
                 ...
                 &m_oord, l_sim_zord, &m_zord, l_sim_critev, &nblk, l_sim_ztyp, l_sim_zcptr, l_sim_subscr, &m_subscr, simpar, &flag, &ierr);
	...
	*************************************/
    if (ierr > 0)
    {
        ...
        case 24:
            strcpy(C2F(cha1).buf,
                   _
                   ("You have changed a parameter in your model, but the model has been compiled to use an XML file containing initial values and parameters. So you should either recompile your Scicos diagram or [re]launch the initialization interface to regenerate the XML file  with new parameters."));
            break;
        ...
        case 26:
            strcpy(C2F(cha1).buf,
                   _
                   ("The number of parameter provided by Scicos blocks is different from what expected by the code generated by the Modelica compiler. You might have relaxed a parameter using FIXED property (i.e., fixed=false) in a Modelica model. This will be corrected in the next version"));
            break;
        ...
        case 202:
            strcpy(C2F(cha1).buf,
                   _
                   ("IDA_ILL_INPUT: One of the input arguments was illegal. This errors message may be returned if the linear solver function initialization (called by the user after calling IDACreate) failed to set the linear solver-specific lsolve field in ida_mem."));
            break;
        ...
        case 206:
            strcpy(C2F(cha1).buf,
                   _("IDA_ERR_FAIL: Error test failures occurred too many times (=MXETF = 10) during one internal step or occurred with |h|=h_min"));
            break;

        case 207:
            strcpy(C2F(cha1).buf,
                   _
                   ("IDA_CONV_FAIL: IDACalcIC->Failed to get convergence of the Newton iterations. or IDA_solve->Convergence test failures occurred too many times (= MXNCF = 10) during one internal step. "));
            break;
        ...
        case 237:
            strcpy(C2F(cha1).buf,
                   _
                   ("IDACalc cannot find the initial condition at this point. If you are using a Modelica model, you can try with the initialization GUI to try with other nonlinear solvers."));
            break;
        case 238:
        ...
        case 303:
            strcpy(C2F(cha1).buf,
                   _
                   ("CV_ERR_FAILURE: Error test failures occurred too many times (=MXETF = 7) during one internal step or occurred with |h|=h_min "));
            break;
        ...
        case 309:
            strcpy(C2F(cha1).buf,
                   _("CV_FIRST_RHSFUNC_ERR: The right-hand side function (user supplied ODE) had a recoverable error at the first call"));
            break;

        case 310:
            strcpy(C2F(cha1).buf,
                   _
                   ("CV_REPTD_RHSFUNC_ERR: Convergence tests occurred too many times due to repeated recoverable errors in the right-hand side function (user supplied ODE). This error may be raised due to repeated  recoverable errors during the estimation of an initial step size."));
            break;

        case 311:
            strcpy(C2F(cha1).buf,
                   _
                   ("CV_UNREC_RHSFUNC_ERR: The right-hand side function (user supplied ODE) had a recoverable error, but no recovery was possible."));
            break;
        case 312:
        ...
        case 324:
            strcpy(C2F(cha1).buf,
                   _
                   ("CV_BAD_K: k (the order of the derivative of y to be computed) is not in the range 0, 1, ..., qu, where qu is the order last used"));
            break;
        case 325:
        ...
        case 403:
            strcpy(C2F(cha1).buf,
                   _
                   ("KIN_NO_MALLOC: Additional system memory has not yet been  allocated for vector storage (forgot to call the  KINMalloc routine)"));
            break;
        case 404:
            strcpy(C2F(cha1).buf,
                   _
                   ("KIN_MEM_FAIL: An error occurred during memory allocation (either insufficient system resources are available or the vector kernel has not yet been initialized "));
            break;
        case 405:
            strcpy(C2F(cha1).buf,
                   _
        ...
        case 407:
            strcpy(C2F(cha1).buf,
                   _
                   ("KIN_MXNEWT_5X_EXCEEDE: Five consecutive steps have been taken that satisfy the following inequality:  ||uscale*p||_L2 > 0.99*mxnewtstep where p denotes the current step and mxnewtstep is a real scalar upper bound on the scaled step length such a failure may mean ||fscale*func(u)||_L2 asymptotes from above to a finite value, or the real scalar mxnewtstep is too small "));
            break;
        case 408:
            strcpy(C2F(cha1).buf,
                   _
                   ("KIN_LINESEARCH_BCFAI: The line search algorithm (implemented in KINLineSearch) was unable to satisfy the beta-condition for MXNBCF + 1 nonlinear iterations (not necessarily consecutive), which may indicate the algorithm is making  poor progress "));
            break;
        case 409:
            strcpy(C2F(cha1).buf,
                   _
        ...
        case 411:
            strcpy(C2F(cha1).buf,
                   _("KIN_LSETUP_FAIL: The user-supplied routine pset (used to compute the preconditioner) encountered an unrecoverable error ."));
            break;
        case 412:
            strcpy(C2F(cha1).buf,
                   _
                   ("KIN_LSOLVE_FAIL: Either the user-supplied routine psolve (used to solve the preconditioned linear system) encountered an unrecoverable error, or the linear solver routine (lsolve) encountered an error condition."));
            break;
        case 413:
        ...
        case 414:
            strcpy(C2F(cha1).buf,
                   _
                   ("KIN_FIRST_SYSFUNC_ERR: The system function failed at the first call. Please verify your model. There might be an error in the computing function, for example a function may be called with illegal inputs."));
            break;
        case 415:
            strcpy(C2F(cha1).buf,
                   _
                   ("KIN_REPTD_SYSFUNC_ERR: The system function failed repeatedly. Please verify your model. There might be an error in the computing function, for example a function may be called with illegal inputs"));
            break;
        case 416:
            strcpy(C2F(cha1).buf,
                   _
                   ("KIN_NAN_ERR: The residual function returns NAN. Please verify your model: some functions might be called with illegal inputs."));
            break;
