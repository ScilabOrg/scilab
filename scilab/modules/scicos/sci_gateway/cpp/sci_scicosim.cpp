...
types::Function::ReturnValue sci_scicosim(types::typed_list &in, int _iRetCount, types::typed_list &out)
{
    ...
    ****************/
    ...
    {
        if (il_state_oz_input->get(i)->isDouble() == false)
        {
            Scierror(999, _("%s: Wrong type for element #%d of element #%d of argument #%d : A matrix expected.\n"), funname.data(), i + 1, 4, 1);
            il_state->DecreaseRef();
            il_state->killMe();
            il_state_oz->DecreaseRef();
            il_state_oz->killMe();
            return types::Function::Error;
        }
        types::Double* l_state_oz_input = il_state_oz_input->get(i)->getAs<types::Double>();
    ...
    {
        if (il_state_outtb_input->get(i)->isDouble() == false)
        {
            Scierror(999, _("%s: Wrong type for element #%d of element #%d of argument #%d : A matrix expected.\n"), funname.data(), i + 1, 9, 1);
            il_state->DecreaseRef();
            il_state->killMe();
            il_state_outtb->DecreaseRef();
            il_state_outtb->killMe();
            return types::Function::Error;
        }
        types::Double* l_state_outtb_input = il_state_outtb_input->get(i)->getAs<types::Double>();
    ...
    *******************************/
    ...
    if (nlnk > 0)
    {
        ...
        if ((outtbsz = new (std::nothrow) int[nlnk]) == nullptr)
        {
            Scierror(999, _("%s: Memory allocation error.\n"), funname.data());
            il_state->DecreaseRef();
            il_state->killMe();
