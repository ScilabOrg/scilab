...
*
*/
const std::string name = "model2blk";

void freeSubBlock(void* _ptr)
{
    if (_ptr)
    {
        FREE(_ptr);
...
void freeBlock(scicos_block* block)
{
...
types::Function::ReturnValue sci_model2blk(types::typed_list &in, int _iRetCount, types::typed_list &out)
{
    ...
    types::InternalType::ScilabType typfsim = types::InternalType::ScilabNull;
    ...
    else if (il_sim->isString())
    {
        ...
        if (f != NULL)
        {
            ...
            Block.funpt = static_cast<voidg>(f);
        }
    ...
    if (Block.nin > 0)
    {
        ...
        d = pIT->getAs<types::Double>();
        int sizeIntyp = d->getSize();
        double* vIntype = d->get();
    ...
    if (sizeOut > 0)
    {
        ...
        /* alloc outsz */
        if ((Block.outsz = (int *)MALLOC(sizeOut * 3 * sizeof(int))) == NULL)
        {
            Scierror(888, _("%s : Allocation error.\n"), name.data());
            freeBlock(&Block);
            return types::Function::Error;
        ...
        /* alloc outptr */
        if ((Block.outptr = (void **)MALLOC(sizeOut * sizeof(void *))) == NULL)
        {
            Scierror(888, _("%s : Allocation error.\n"), name.data());
            freeBlock(&Block);
            return types::Function::Error;
        ...
        pIT = m->getField(L"intyp");
        d = pIT->getAs<types::Double>();
        int sizeOuttyp = d->getSize();
        double* vOuttype = d->get();
    ...
    if (Block.noz > 0)
    {
        ...
        for (int i = 0; i < Block.noz; ++i)
        {
            ...
            if (pIT->isDouble())
            {
                ...
                {
                    Block.oztyp[i] = 10;
                    if ((Block.ozptr[i] = (double *)MALLOC(size * sizeof(double))) == NULL)
                    {
                        freeBlock(&Block);
                        Scierror(888, _("%s : Allocation error.\n"), name.data());
                        return types::Function::Error;
                ...
                {
                    Block.oztyp[i] = 11;
                    if ((Block.ozptr[i] = (double *)MALLOC(2 * size * sizeof(double))) == NULL)
                    {
                        freeBlock(&Block);
                        Scierror(888, _("%s : Allocation error.\n"), name.data());
                        return types::Function::Error;
    ...
    if (Block.nrpar > 0)
    {
        if ((Block.ipar = (int *)MALLOC(Block.nrpar * sizeof(int))) == NULL)
        {
            freeBlock(&Block);
            Scierror(888, _("%s : Allocation error.\n"), name.data());
            return types::Function::Error;
    ...
    if (Block.nopar > 0)
    {
        ...
        for (int i = 0; i < Block.nopar; ++i)
        {
            ...
            if (pIT->isDouble())
            {
                ...
                {
                    Block.opartyp[i] = 10;
                    if ((Block.oparptr[i] = (double *)MALLOC(size * sizeof(double))) == NULL)
                    {
                        freeBlock(&Block);
                        Scierror(888, _("%s : Allocation error.\n"), name.data());
                        return types::Function::Error;
                ...
                {
                    Block.opartyp[i] = 11;
                    if ((Block.oparptr[i] = (double *)MALLOC(2 * size * sizeof(double))) == NULL)
                    {
                        freeBlock(&Block);
                        Scierror(888, _("%s : Allocation error.\n"), name.data());
                        return types::Function::Error;
