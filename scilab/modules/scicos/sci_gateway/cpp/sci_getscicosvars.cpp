    ...
    *******************/
    ...
    for (int j = 0; j < m1 * n1; j++)
    {
        ...
        *****************************************************************/
        ...
        {
            ok = CopyVarFromTList(element, get_il_state(), 2);
        }
        else if (strcmp(field, "z") == 0)     /* retrieve discrete state */
        {
            ok = CopyVarFromTList(element, get_il_state(), 3);
        }
        else if (strcmp(field, "oz") == 0)    /* retrieve object discrete state */
        {
            ok = CopyVarFromTList(element, get_il_state(), 4);
        }
        else if (strcmp(field, "outtb") == 0) /* retrieve outtb */
        {
            ok = CopyVarFromTList(element, get_il_state(), 9);
        }
        else if (strcmp(field, "tevts") == 0) /* retrieve tevts */
        {
            ok = CopyVarFromTList(element, get_il_state(), 6);
        }
        ...
        *****************************************************************/
        ...
        {
            ok = CopyVarFromTList(element, get_il_state(), 15);
        }
            ...
            if (ok)
            {
                ...
                for (int k = 0; k < nblk; ++k)
                {
                    ...
                    */
                    ...
                    /* Call createblklist() */
                    // FIXME: wait for createblklist() to be available in C++
