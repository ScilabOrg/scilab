...
template<typename T>
static int fill_int(const double* const tab, const int tabSize, const int iDims, T* &res)
{
    int  iElements = 1;
    ...
    if (tabSize < numberOfDoubleNeeded + 2 + iDims)
    {
        ...
        res->killMe();
        return -(numberOfDoubleNeeded + 2 + iDims);;
    }
...
static int fill_double(const double* const tab, const int tabSize, const int iDims, types::Double* &res)
{
    int  iElements = 1;
    int* pDims = new int[iDims];
    for (int i = 0; i < iDims; ++i)
...
static int fill_string(const double* const tab, const int tabSize, const int iDims, types::String* &res)
{
    int  iElements = 1;
    int* pDims = new int[iDims];
    for (int i = 0; i < iDims; ++i)
...
template<typename T>
static bool fill_list(const double* const tab, const int listSize, const int iDims, T* &res)
{
    res = new T();
    int offset = 0;
    for (int i = 0; i < iDims; ++i)
    {
        switch (static_cast<int>(*(tab + offset)))
        {
            case types::InternalType::ScilabDouble :
            {
                ...
                int offset_cur = fill_double(tab + 2 + offset, listSize, *(tab + 1 + offset), pDouble);
                if (offset_cur < 0)
                {
                    Scierror(999, _("%s: Wrong size for input argument #%d: At least %dx%d expected.\n"), funname.c_str(), 1, 2 + offset - offset_cur, 1);
                    res->killMe();
                    return false;
                }
                offset += offset_cur;
            ...
            {
                types::Int8* pInt8;
                int offset_cur = fill_int(tab + 2 + offset, listSize, *(tab + 1 + offset), pInt8);
                if (offset_cur < 0)
                {
                    Scierror(999, _("%s: Wrong size for input argument #%d: At least %dx%d expected.\n"), funname.c_str(), 1, 2 + offset - offset_cur, 1);
                    res->killMe();
            ...
            {
                types::UInt8* pUInt8;
                int offset_cur = fill_int(tab + 2 + offset, listSize, *(tab + 1 + offset), pUInt8);
                if (offset_cur < 0)
                {
                    Scierror(999, _("%s: Wrong size for input argument #%d: At least %dx%d expected.\n"), funname.c_str(), 1, 2 + offset - offset_cur, 1);
                    res->killMe();
            ...
            {
                types::Int16* pInt16;
                int offset_cur = fill_int(tab + 2 + offset, listSize, *(tab + 1 + offset), pInt16);
                if (offset_cur < 0)
                {
                    Scierror(999, _("%s: Wrong size for input argument #%d: At least %dx%d expected.\n"), funname.c_str(), 1, 2 + offset - offset_cur, 1);
                    res->killMe();
            ...
            case types::InternalType::ScilabUInt16 :
            {
                ...
                int offset_cur = fill_int(tab + 2 + offset, listSize, *(tab + 1 + offset), pUInt16);
                if (offset_cur < 0)
                {
                    Scierror(999, _("%s: Wrong size for input argument #%d: At least %dx%d expected.\n"), funname.c_str(), 1, 2 + offset - offset_cur, 1);
                    res->killMe();
            ...
            {
                types::Int32* pInt32;
                int offset_cur = fill_int(tab + 2 + offset, listSize, *(tab + 1 + offset), pInt32);
                if (offset_cur < 0)
                {
                    Scierror(999, _("%s: Wrong size for input argument #%d: At least %dx%d expected.\n"), funname.c_str(), 1, 2 + offset - offset_cur, 1);
                    res->killMe();
            ...
            case types::InternalType::ScilabUInt32 :
            {
                ...
                int offset_cur = fill_int(tab + 2 + offset, listSize, *(tab + 1 + offset), pUInt32);
                if (offset_cur < 0)
                {
                    Scierror(999, _("%s: Wrong size for input argument #%d: At least %dx%d expected.\n"), funname.c_str(), 1, 2 + offset - offset_cur, 1);
                    res->killMe();
            ...
            {
                types::Int64* pInt64;
                int offset_cur = fill_int(tab + 2 + offset, listSize, *(tab + 1 + offset), pInt64);
                if (offset_cur < 0)
                {
                    Scierror(999, _("%s: Wrong size for input argument #%d: At least %dx%d expected.\n"), funname.c_str(), 1, 2 + offset - offset_cur, 1);
                    res->killMe();
            ...
            case types::InternalType::ScilabUInt64 :
            {
                ...
                int offset_cur = fill_int(tab + 2 + offset, listSize, *(tab + 1 + offset), pUInt64);
                if (offset_cur < 0)
                {
                    Scierror(999, _("%s: Wrong size for input argument #%d: At least %dx%d expected.\n"), funname.c_str(), 1, 2 + offset - offset_cur, 1);
                    res->killMe();
            ...
            {
                types::Bool* pBool;
                int offset_cur = fill_int(tab + 2 + offset, listSize, *(tab + 1 + offset), pBool);
                if (offset_cur < 0)
                {
                    Scierror(999, _("%s: Wrong size for input argument #%d: At least %dx%d expected.\n"), funname.c_str(), 1, 2 + offset - offset_cur, 1);
                    res->killMe();
            ...
            case types::InternalType::ScilabString :
            {
                ...
                int offset_cur = fill_string(tab + 2 + offset, listSize, *(tab + 1 + offset), pString);
                if (offset_cur < 0)
                {
                    Scierror(999, _("%s: Wrong size for input argument #%d: At least %dx%d expected.\n"), funname.c_str(), 1, 2 + offset - offset_cur, 1);
                    res->killMe();
...
types::Function::ReturnValue sci_vec2var(types::typed_list &in, int _iRetCount, types::typed_list &out)
{
    ...
    switch (iType)
    {
        case types::InternalType::ScilabDouble :
        {
            ...
            types::Double* pDouble;
            int resSize = fill_double(input->get() + 2, inputRows, iDims, pDouble);
            if (resSize < 0)
            {
                Scierror(999, _("%s: Wrong size for input argument #%d: At least %dx%d expected.\n"), funname.c_str(), 1, -resSize, 1);
                return types::Function::Error;
            }
        ...
        case types::InternalType::ScilabInt8   :
        {
            ...
            types::Int8* pInt8;
            int resSize = fill_int(input->get() + 2, inputRows, iDims, pInt8);
            if (resSize < 0)
            {
                Scierror(999, _("%s: Wrong size for input argument #%d: At least %dx%d expected.\n"), funname.c_str(), 1, -resSize, 1);
                return types::Function::Error;
        ...
        case types::InternalType::ScilabUInt8  :
        {
            ...
            types::UInt8* pUInt8;
            int resSize = fill_int(input->get() + 2, inputRows, iDims, pUInt8);
            if (resSize < 0)
            {
                Scierror(999, _("%s: Wrong size for input argument #%d: At least %dx%d expected.\n"), funname.c_str(), 1, -resSize, 1);
                return types::Function::Error;
        ...
        case types::InternalType::ScilabInt16  :
        {
            ...
            types::Int16* pInt16;
            int resSize = fill_int(input->get() + 2, inputRows, iDims, pInt16);
            if (resSize < 0)
            {
                Scierror(999, _("%s: Wrong size for input argument #%d: At least %dx%d expected.\n"), funname.c_str(), 1, -resSize, 1);
                return types::Function::Error;
        ...
        case types::InternalType::ScilabUInt16 :
        {
            ...
            types::UInt16* pUInt16;
            int resSize = fill_int(input->get() + 2, inputRows, iDims, pUInt16);
            if (resSize < 0)
            {
                Scierror(999, _("%s: Wrong size for input argument #%d: At least %dx%d expected.\n"), funname.c_str(), 1, -resSize, 1);
                return types::Function::Error;
        ...
        case types::InternalType::ScilabInt32  :
        {
            ...
            types::Int32* pInt32;
            int resSize = fill_int(input->get() + 2, inputRows, iDims, pInt32);
            if (resSize < 0)
            {
                Scierror(999, _("%s: Wrong size for input argument #%d: At least %dx%d expected.\n"), funname.c_str(), 1, -resSize, 1);
                return types::Function::Error;
        ...
        case types::InternalType::ScilabUInt32 :
        {
            ...
            types::UInt32* pUInt32;
            int resSize = fill_int(input->get() + 2, inputRows, iDims, pUInt32);
            if (resSize < 0)
            {
                Scierror(999, _("%s: Wrong size for input argument #%d: At least %dx%d expected.\n"), funname.c_str(), 1, -resSize, 1);
                return types::Function::Error;
        ...
        case types::InternalType::ScilabInt64  :
        {
            ...
            types::Int64* pInt64;
            int resSize = fill_int(input->get() + 2, inputRows, iDims, pInt64);
            if (resSize < 0)
            {
                Scierror(999, _("%s: Wrong size for input argument #%d: At least %dx%d expected.\n"), funname.c_str(), 1, -resSize, 1);
                return types::Function::Error;
        ...
        case types::InternalType::ScilabUInt64 :
        {
            ...
            types::UInt64* pUInt64;
            int resSize = fill_int(input->get() + 2, inputRows, iDims, pUInt64);
            if (resSize < 0)
            {
                Scierror(999, _("%s: Wrong size for input argument #%d: At least %dx%d expected.\n"), funname.c_str(), 1, -resSize, 1);
                return types::Function::Error;
        ...
        case types::InternalType::ScilabBool   :
        {
            ...
            types::Bool* pBool;
            int resSize = fill_int(input->get() + 2, inputRows, iDims, pBool);
            if (resSize < 0)
            {
                Scierror(999, _("%s: Wrong size for input argument #%d: At least %dx%d expected.\n"), funname.c_str(), 1, -resSize, 1);
                return types::Function::Error;
        ...
        case types::InternalType::ScilabString :
        {
            ...
            int resSize = fill_string(input->get() + 2, inputRows, iDims, pString);
            if (resSize < 0)
            {
                Scierror(999, _("%s: Wrong size for input argument #%d: At least %dx%d expected.\n"), funname.c_str(), 1, -resSize, 1);
                return types::Function::Error;
        ...
        {
            types::List* pList;
            if (!fill_list(input->get() + 2, inputRows - 2, iDims, pList))
            {
                return types::Function::Error;
            }
        ...
        {
            types::TList* pTList;
            if (!fill_list(input->get() + 2, inputRows - 2, iDims, pTList))
            {
                return types::Function::Error;
            }
        ...
        {
            types::MList* pMList;
            if (!fill_list(input->get() + 2, inputRows - 2, iDims, pMList))
            {
                return types::Function::Error;
            }
