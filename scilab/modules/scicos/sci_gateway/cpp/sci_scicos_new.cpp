...
template<class Adaptor, class Adaptee>
types::InternalType * alloc_and_set_as_list(types::String* type_name, types::typed_list &in)
{
    // check header
    Adaptor adaptor(0);
    for (int i = 1; i < (int)in.size(); i++)
...
types::Function::ReturnValue sci_scicos_new(types::typed_list &in, int _iRetCount, types::typed_list &out)
{
    ...
    switch (adapter_index)
    {
        ...
        case view_scilab::Adapters::CPR_ADAPTER:
            returnType = alloc_and_set_as_list<view_scilab::CprAdapter, model::Diagram>(type_name, in);
            if (returnType == 0)
            {
                return types::Function::Error;
            }
