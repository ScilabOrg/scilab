...
function gateway_filename = ilib_gen_gateway(name,tables)
    ...
    for itable = 1:L
        ...
        //generate header interface file
        ...
        gate(:);
        "";
        "#endif /* __" + convstr(tname, "u") + "_GW_H__ */"];

        gateway_filename(2) = path + tname + ".h";
        // first check if we have already a gateway
...
function [gate,names] = new_names(table)
    ...
    for i = 1:mt
        ...
        case "cmex" then
            ...
            gate(i) = "C_GATEWAY_PROTOTYPE(" + names(i) + ");";
        case "fmex" then
        ...
        case "cppsci"  then
            names(i) = table(i,2);
            gate(i) = "CPP_GATEWAY_PROTOTYPE(" + names(i) + ");";
        case "direct"  then
            names(i) = table(i,2);
            gate(i) = "C_GATEWAY_PROTOTYPE(" + names(i) + ");";
        else
