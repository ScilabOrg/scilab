...
function bOK = generateLoader(name, tables, libs, libname, flag, loadername)
  ...
  if getos() == 'Windows' then
    if win64() then
      ...
      mfprintf(fd, "  warning(_(""This toolbox requires a Windows x64 platform.""));\n");
    else
      mfprintf(fd, "if win64() then\n");
      mfprintf(fd, "  warning(_(""This toolbox requires a Windows x86 platform.""));\n");
    end
    mfprintf(fd, "  return\n");
    mfprintf(fd, "end\n");
    mfprintf(fd, "//\n");
  ...
  for i = 1:nl
    ...
    if isabspath then
      ...
      data = "link(%s_path + filesep() + ''%s'' + getdynlibext());\n";
      mfprintf(fd, data, shortlibname_path, libfile);
    end
  end
