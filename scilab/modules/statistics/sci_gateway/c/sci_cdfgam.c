...
int cdfgamI(char *fname, unsigned long l)
{
    ...
    if (strcmp(cstk(l1), "PQ") == 0)
    {
        ...
        CdfBase(fname, 3, 2, callpos, "PQ", _("X,Shape and Rate"), 1, C2F(cdfgam), cdfgamErr);
    }
    else if (strcmp(cstk(l1), "X") == 0)
    {
        static int callpos[5] = { 2, 3, 4, 0, 1 };
        CdfBase(fname, 4, 1, callpos, "X", _("Shape,Rate,P and Q"), 2, C2F(cdfgam), cdfgamErr);
    }
    else if (strcmp(cstk(l1), "Shape") == 0)
    {
        static int callpos[5] = { 1, 2, 3, 4, 0 };
        CdfBase(fname, 4, 1, callpos, "Shape", _("Rate,P,Q and X"), 3, C2F(cdfgam), cdfgamErr);
    }
    else if (strcmp(cstk(l1), "Rate") == 0)
    {
        static int callpos[5] = { 0, 1, 2, 3, 4 };
        CdfBase(fname, 4, 1, callpos, "Rate", _("P,Q,X and Shape"), 4, C2F(cdfgam), cdfgamErr);
    }
    else
    {
