...
/*--------------------------------------------------------------------------*/
static void cdftncErr(int status, double bound);
/*--------------------------------------------------------------------------*/
/*
*  hand written interface
*  Interface for cdftnc
*  T distribution
*/
/*--------------------------------------------------------------------------*/
int cdftncI(char* fname, unsigned long l)
{
    ...
    if ( strcmp(cstk(l1), "PQ") == 0)
    {
        ...
        CdfBase(fname, 3, 2, callpos, "PQ", _("T, Df and Pnonc"), 1, C2F(cdftnc),
                cdftncErr);
    }
    else if ( strcmp(cstk(l1), "T") == 0)
    {
        ...
        CdfBase(fname, 4, 1, callpos, "T", _("Df, Pnonc, P and Q"), 2, C2F(cdftnc),
                cdftncErr);
    }
    else if ( strcmp(cstk(l1), "Df") == 0)
    {
        static int callpos[5] = {1, 2, 3, 4, 0};
        CdfBase(fname, 4, 1, callpos, "Df", _("Pnonc, P, Q and T"), 3, C2F(cdftnc),
                cdftncrr);
    }
    else if ( strcmp(cstk(l1), "Pnonc") == 0)
    {
        static int callpos[5] = {0, 1, 2, 3, 4};
        CdfBase(fname, 4, 1, callpos, "Pnonc", _("P, Q, T and Df"), 3, C2F(cdftnc),
                cdftncrr);
    }
    else
    {
        Scierror(999, _("%s: Wrong value for input argument #%d: '%s', '%s' or '%s' expected.\n"), fname, 1, "PQ", "T", "Df", "Pnonc");
    }
    return 0;
}
/*--------------------------------------------------------------------------*/
