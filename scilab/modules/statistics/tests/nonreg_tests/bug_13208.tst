...
// The second reglin() calls nanreglin().
x = 0:10;
y = 20:30;
[a1, b1] = reglin(x, [x ; y]);
...
x = 1:3;
y = [1 %nan 3];
refMsg = msprintf(_("%s: Cannot output standard deviation when y is a matrix containing NaNs. Wrong number of output arguments: %d to %d expected.\n"),"reglin",1,2);
assert_checkerror("[a, b, sig] = reglin(x, [x ; y])", refMsg);

refMsg = msprintf(_("%s: Cannot output standard deviation when y is a matrix containing NaNs. Wrong number of output arguments: %d to %d expected.\n"),"nanreglin",1,2);
assert_checkerror("[a, b, sig] = nanreglin(x, [x ; y])", refMsg);
