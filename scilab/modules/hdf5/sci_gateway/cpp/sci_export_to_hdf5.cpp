...
static bool export_hypermat(int* pvCtx, int _iH5File, int *_piVar, char* _pstName)
{
    ...
    char* wstrHypermat[] = {"hypermat", "dims", "entries"};
    ...
    //export string ["hypermat" "dims" "entries"]
    int piStrDims[2] = {1, 3};
    //    pStr = new types::String(1, 3, wstrHypermat);
    //    getListItemAddress(pvCtx, _piVar, 1, &pStr);
    char* pstPathName = createPathName(pstGroupName, 0);
    iRet = writeStringMatrix(_iH5File, pstPathName, 2, piStrDims, wstrHypermat);
    if (iRet)
    {
        return false;
    }
