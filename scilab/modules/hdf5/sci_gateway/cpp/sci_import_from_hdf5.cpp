...
int sci_import_from_hdf5(char *fname,unsigned long fname_len)
{
    ...
    int iFile = -1;
    {
        char* drv  = (char*)MALLOC(sizeof(char) * ((int)strlen(pstVarName) + 1));
        char* dir  = (char*)MALLOC(sizeof(char) * ((int)strlen(pstVarName) + 1));
        char* name = (char*)MALLOC(sizeof(char) * ((int)strlen(pstVarName) + 1));
        char* ext  = (char*)MALLOC(sizeof(char) * ((int)strlen(pstVarName) + 1));
        char* path = (char*)MALLOC(sizeof(char) * ((int)strlen(pstVarName) + 1));
        char* filename = (char*)MALLOC(sizeof(char) * ((int)strlen(pstVarName) + 1));
        ...
        if (drv && dir && name && ext && path && filename)
        {
            ...
            splitpath(pstVarName, FALSE, drv, dir, name, ext);
