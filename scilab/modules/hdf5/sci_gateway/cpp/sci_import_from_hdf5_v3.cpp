...
static int import_handle_segs(int dataset)
{
    int segs = createGraphicObject(__GO_SEGS__);

    //import "standards" properties
    import_handle_generic(dataset, segs, SegsHandle::getPropertyList());

    closeList6(dataset);
    return segs;
}

static int import_handle_arc(int dataset)
{
    int arc = createGraphicObject(__GO_ARC__);

    //import "standards" properties
    import_handle_generic(dataset, arc, ArcHandle::getPropertyList());

    closeList6(dataset);
    return arc;
}

static int import_handle_rectangle(int dataset)
{
    int rect = createGraphicObject(__GO_RECTANGLE__);

    //import "standards" properties
    import_handle_generic(dataset, rect, RectangleHandle::getPropertyList());

    closeList6(dataset);
    return rect;
}

static int import_handle_compound(int dataset)
{
    int compound = createGraphicObject(__GO_COMPOUND__);

    //import "standards" properties
    import_handle_generic(dataset, compound, CompoundHandle::getPropertyList());
...
static int import_handle_datatip(int dataset, int parent)
{
    ...
    //data
    int index = getHandleInt(dataset, "data_index");
    double indexes[2];
    indexes[0] = index;
    indexes[1] = 0;
    ...
    //import "standards" properties
    import_handle_generic(dataset, datatip, DatatipHandle::getPropertyList());
...
static int import_handle_polyline(int dataset)
{
    int polyline = createGraphicObject(__GO_POLYLINE__);
    createDataObject(polyline, __GO_POLYLINE__);

    //import "standards" properties
    import_handle_generic(dataset, polyline, PolylineHandle::getPropertyList());
...
static int import_handle_surface(int dataset, int uid)
{
    //import "standards" properties
    import_handle_generic(dataset, uid, SurfaceHandle::getPropertyList());
    return uid;
}

static int import_handle_plot3d(int dataset)
{
    int plot = createGraphicObject(__GO_PLOT3D__);
    createDataObject(plot, __GO_PLOT3D__);
    import_handle_surface(dataset, plot);
...
static int import_handle_fac3d(int dataset)
{
    int fac = createGraphicObject(__GO_FAC3D__);
    createDataObject(fac, __GO_FAC3D__);

    import_handle_surface(dataset, fac);
...
static int import_handle_champ(int dataset)
{
    ...
    //import "standards" properties
    import_handle_generic(dataset, champ, ChampHandle::getPropertyList());
...
static int import_handle_axes(int dataset)
{
...
static int import_handle_figure(int dataset)
{
...
static int import_handle(int dataset)
{
    ...
    switch (type)
    {
        case __GO_FIGURE__:
        {
            return import_handle_figure(dataset);
        }
        case __GO_AXES__:
        {
            return import_handle_axes(dataset);
        }
        case __GO_CHAMP__:
        {
            return import_handle_champ(dataset);
        }
        case __GO_FAC3D__:
        {
            return import_handle_fac3d(dataset);
        }
        case __GO_PLOT3D__:
        {
            return import_handle_plot3d(dataset);
        }
        case __GO_COMPOUND__:
        {
            return import_handle_compound(dataset);
        }
        case __GO_POLYLINE__:
        {
            return import_handle_polyline(dataset);
        }
        case __GO_RECTANGLE__:
        {
            return import_handle_rectangle(dataset);
        }
        case __GO_ARC__:
        {
            return import_handle_arc(dataset);
        }
        case __GO_SEGS__:
        {
            return import_handle_segs(dataset);
        }
    }
    return -1;
}
