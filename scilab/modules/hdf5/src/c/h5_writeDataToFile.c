...
static herr_t addIntAttribute(int _iDatasetId, const char *_pstName, const int _iVal)
{
    ...
    attributeTypeId = H5Acreate(_iDatasetId, _pstName, H5T_NATIVE_INT, attributeSpace, H5P_DEFAULT, NULL);
...
static herr_t addAttribute(int _iDatasetId, const char *_pstName, const char *_pstValue)
{
    ...
    //Create the attribute and write it.
    attr = H5Acreate(_iDatasetId, _pstName, attributeTypeId, attributeSpace, H5P_DEFAULT, NULL);
    if (attr < 0)
    {
        return -1;
    }
...
int writeStringMatrix(int _iFile, char *_pstDatasetName, int _iDims, int* _piDims, char **data)
{
    ...
    FREE(piDims);
    dset = H5Dcreate(_iFile, _pstDatasetName, typeId, space, iCompress, NULL, NULL);
    if (dset < 0)
    {
        return -1;
    }
...
int writeVoid(int _iFile, char *_pstDatasetName)
{
    ...
    iDataset = H5Dcreate(_iFile, _pstDatasetName, H5T_NATIVE_INT8, iSpace, iCompress, NULL, NULL);
    if (iDataset < 0)
    {
        return -1;
    }
...
int writeDoubleMatrix(int _iFile, char *_pstDatasetName, int _iDims, int* _piDims, double *_pdblData)
{
    ...
    if (_iDims == 2 && piDims[0] == 0 && piDims[1] == 0)
    {
        ...
        dset = H5Dcreate(_iFile, _pstDatasetName, H5T_NATIVE_DOUBLE, space, iCompress, NULL, NULL);
        if (dset < 0)
        {
            return -1;
        }
...
int writeDoubleComplexMatrix(int _iFile, char *_pstDatasetName, int _iDims, int* _piDims, double *_pdblReal, double *_pdblImg)
{
    ...
    dset = H5Dcreate(_iFile, _pstDatasetName, compoundId, space, iCompress, NULL, NULL);
    if (dset < 0)
    {
        return -1;
    }
...
int writeBooleanMatrix(int _iFile, char *_pstDatasetName, int _iDims, int* _piDims, int *_piData)
{
    ...
    //Create the dataset and write the array data to it.
    iCompress = enableCompression(9, _iDims, piDims);
    iDataset = H5Dcreate(_iFile, _pstDatasetName, H5T_NATIVE_INT, iSpace, iCompress, NULL, NULL);
    if (iDataset < 0)
    {
        return -1;
    }
...
static int writeCommonPolyMatrix(int _iFile, char *_pstDatasetName, char *_pstVarName, int _iComplex, int _iDims, int* _piDims, int *_piNbCoef, double **_pdblReal, double **_pdblImg)
{
    ...
    group = H5Gcreate(_iFile, pstGroupName, H5P_DEFAULT, NULL, NULL);
    ...
    iCompress = enableCompression(9, _iDims, piDims);
    FREE(piDims);
    dset = H5Dcreate(_iFile, _pstDatasetName, H5T_STD_REF_OBJ, space, iCompress, NULL, NULL);
    if (dset < 0)
    {
        FREE(pData);
        return -1;
...
int writeInteger16Matrix(int _iFile, char *_pstDatasetName, int _iDims, int* _piDims, short *_psData)
{
    ...
    iCompress = enableCompression(9, _iDims, piDims);
    FREE(piDims);
    iDataset = H5Dcreate(_iFile, _pstDatasetName, H5T_NATIVE_INT16, iSpace, iCompress, NULL, NULL);
    if (iDataset < 0)
    {
        return -1;
    }
...
int writeInteger32Matrix(int _iFile, char *_pstDatasetName, int _iDims, int* _piDims, int *_piData)
{
    ...
    iCompress = enableCompression(9, _iDims, piDims);
    FREE(piDims);
    iDataset = H5Dcreate(_iFile, _pstDatasetName, H5T_NATIVE_INT32, iSpace, iCompress, NULL, NULL);
    if (iDataset < 0)
    {
        return -1;
    }
...
int writeInteger64Matrix(int _iFile, char *_pstDatasetName, int _iDims, int* _piDims, long long *_pllData)
{
    ...
    iCompress = enableCompression(9, _iDims, piDims);
    FREE(piDims);
    iDataset = H5Dcreate(_iFile, _pstDatasetName, H5T_NATIVE_INT64, iSpace, iCompress, NULL, NULL);
    if (iDataset < 0)
    {
        return -1;
    }
...
int writeUnsignedInteger8Matrix(int _iFile, char *_pstDatasetName, int _iDims, int* _piDims, unsigned char *_pucData)
{
    ...
    iCompress = enableCompression(9, _iDims, piDims);
    FREE(piDims);
    iDataset = H5Dcreate(_iFile, _pstDatasetName, H5T_NATIVE_UINT8, iSpace, iCompress, NULL, NULL);
    if (iDataset < 0)
    {
        return -1;
    }
...
int writeUnsignedInteger16Matrix(int _iFile, char *_pstDatasetName, int _iDims, int* _piDims, unsigned short *_pusData)
{
    ...
    iCompress = enableCompression(9, _iDims, piDims);
    FREE(piDims);
    iDataset = H5Dcreate(_iFile, _pstDatasetName, H5T_NATIVE_UINT16, iSpace, iCompress, NULL, NULL);
    if (iDataset < 0)
    {
        return -1;
    }
...
int writeUnsignedInteger32Matrix(int _iFile, char *_pstDatasetName, int _iDims, int* _piDims, unsigned int *_puiData)
{
    ...
    iCompress = enableCompression(9, _iDims, piDims);
    FREE(piDims);
    iDataset = H5Dcreate(_iFile, _pstDatasetName, H5T_NATIVE_UINT32, iSpace, iCompress, NULL, NULL);
    if (iDataset < 0)
    {
        return -1;
    }
...
int writeUnsignedInteger64Matrix(int _iFile, char *_pstDatasetName, int _iDims, int* _piDims, unsigned long long *_pullData)
{
    ...
    iCompress = enableCompression(9, _iDims, piDims);
    FREE(piDims);
    iDataset = H5Dcreate(_iFile, _pstDatasetName, H5T_NATIVE_UINT64, iSpace, iCompress, NULL, NULL);
    if (iDataset < 0)
    {
        return -1;
    }
...
void *openList(int _iFile, char *pstDatasetName, int _iNbItem)
{
    ...
    //First create a group to store all referenced objects.
    group = H5Gcreate(_iFile, pstDatasetName, H5P_DEFAULT, NULL, NULL);
    status = H5Gclose(group);
    if (status < 0)
    {
        return NULL;
...
int closeList(int _iFile, void *_pvList, char *_pstListName, int _iNbItem, int _iVarType)
{
    ...
    if (_iNbItem == 0)
    {
        ...
        //Create the dataset and write the array data to it.
        iCompress = enableCompression(9, 1, dims);
        dset = H5Dcreate(_iFile, _pstListName, H5T_STD_REF_OBJ, space, iCompress, NULL, NULL);
        if (dset < 0)
        {
            return -1;
        }
