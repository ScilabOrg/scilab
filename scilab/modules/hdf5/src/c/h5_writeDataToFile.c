...
int updateFileVersion(int _iFile)
{
    herr_t status;
    //try to read attribute
    int iHdf5Version = getHdf5FormatAttribute(_iFile);
    if(iHdf5Version != -1)
    {
        status = H5Adelete(_iFile, g_SCILAB_CLASS_HDF5_VERSION);
        if(status < 0)
        {
            return -1;
        }
    ...
    return addIntAttribute(_iFile, g_SCILAB_CLASS_HDF5_VERSION, HDF5_SCILAB_FILE_VERSION);
}
