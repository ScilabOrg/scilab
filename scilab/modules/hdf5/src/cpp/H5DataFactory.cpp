...
H5Data & H5DataFactory::getObjectData(H5Object & parent, const hsize_t totalSize, const hsize_t dataSize, const hid_t type, const hsize_t ndims, const hsize_t * dims, void * data, const hsize_t stride, const size_t offset, const bool dataOwner)
{
    switch (H5Tget_class(type))
    {
        ...
        case H5T_REFERENCE:
            // TODO: virer le false
            return *new H5ReferenceData(parent, false, totalSize, dataSize, ndims, dims, (char *)data, stride, offset, dataOwner);
        case H5T_ENUM:
        {
        ...
        case H5T_VLEN:
        {
            /*	    H5Data ** _data = new H5Data *[totalSize]();
            	    hsize_t * _dims = new hsize_t[1];
            	    hid_t super = H5Tget_super(type);
            	    hsize_t baseSize = H5Tget_size(super);
            	    if (H5Tget_class(super) == H5T_STRING && !H5Tis_variable_str(super))
            	    {
            	    ...
            	    {
            		hvl_t * x = 0;
            		hsize_t * _adims = new hsize_t[1];

            	    	if (stride == -1)
            		{
            		    x = reinterpret_cast<hvl_t *>(data) + i;
            		}
            		else
            		{
            		    x = reinterpret_cast<hvl_t *>(reinterpret_cast<char *>(data) + offset + i * stride);
            		}
            		...
            		try
            		{
            		    _data[i] = &H5DataFactory::getObjectData(parent, (hsize_t)x->len, baseSize, super, 1, _dims, 1, _adims, x->p, -1, 0, false);
            		}
            		catch (const H5Exception & e)
            		{
            		    delete[] _dims;
