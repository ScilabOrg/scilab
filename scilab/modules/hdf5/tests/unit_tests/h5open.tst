...
if getos() == "Windows" then
    msgerr = msprintf(gettext("%s: %s\n"), "h5open", msprintf(gettext("Cannot create the given hdf5 file: %s."), filesep()));
    msgerr($+1) = gettext("HDF5 description") + ": " + "unable to open file: name = ''\'', errno = 2, error message = ''No such file or directory'', flags = 15, o_flags = 502.";
else
    msgerr = msprintf(gettext("%s: %s\n"), "h5open", msprintf(gettext("Cannot append the file (not HDF5): %s."), filesep()));
