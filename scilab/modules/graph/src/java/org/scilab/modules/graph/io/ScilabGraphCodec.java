...
public class ScilabGraphCodec extends mxObjectCodec {
    ...
    public Object beforeEncode(mxCodec enc, Object obj, Node node) {
        ...
        if (graph.getAsComponent() != null) {
            ((Element) node).setAttribute(
                    BACKGROUND,
                    String.valueOf(graph.getAsComponent().getBackground()
                            .getRGB()));
        }
     ...
     *            the message
     */
    protected void trace(mxCodec enc, Node node, String msg) {
        node.appendChild(enc.getDocument().createComment(msg));
    }
