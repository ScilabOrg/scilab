...
public class SciNotes extends SwingScilabTab implements Tab {
    ...
    public static void scinotes(final String filePath, final String option) {
        ...
        if (opt.startsWith("action")) {
            int eq = opt.indexOf('=');
            if (eq != -1) {
                hasAction = true;
                if (eq != opt.length() - 1) {
                    opt = opt.substring(eq + 1);
                    try {
                        executeAction(filePath, opt);
                    } catch (IOException e) {
    ...
    public static void executeAction(String fileName, ActionOnDocument action) throws IOException {
        ...
        try {
            charset = DefaultEncodingAction.tryToGuessEncoding(new File(fileName));
     ...
     */
    public static void executeAction(String fileName, final String[] actionsName) throws IOException {
