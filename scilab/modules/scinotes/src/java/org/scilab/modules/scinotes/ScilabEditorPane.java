...
public class ScilabEditorPane extends JEditorPane implements Highlighter.HighlightPainter,
    ...
    /**
     * @param positions the positions to select
     */
    public void select(int[][] positions) {
        if (positions != null && positions.length != 0) {
            if (positions.length == 1) {
                select(positions[0][0], positions[0][1]);
            } else {
                ((SciNotesCaret) getCaret()).protectHighlights(true);
                setCaretPosition(positions[0][0]);
                ((SciNotesCaret) getCaret()).protectHighlights(false);
                ((SciNotesCaret) getCaret()).select(positions);
            }
        }
    }

    /**
     * @return the start of the selection in column mode
     */
    public int getColumnSelectionStart() {
     ...
     * @return the start of the selection in column mode
     */
    public int getColumnSelectionEnd() {
        int[][] positions = ((SciNotesCaret) getCaret()).getSelectedPositions();
        if (positions != null && positions.length != 0) {
