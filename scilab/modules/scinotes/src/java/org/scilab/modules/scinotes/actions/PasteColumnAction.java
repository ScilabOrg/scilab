...
public class PasteColumnAction extends DefaultAction {
    ...
    public void doAction() {
        ...
        pasteColumnString(doc, str, pos);
        getEditor().getTextPane().setCaretPosition(pos);
    }

    /**
     * @param doc the Document where to paste
     * @param str the string to paste
     * @param position the position in the Document
     * @return the positions to highlight
     */
    public static int[][] pasteColumnString(ScilabDocument doc, String str, int position) {
        Element root = doc.getDefaultRootElement();
        int[][] ret = null;
        if (str != null) {
            ...
            ret = new int[tokens.countTokens()][2];
            if (crs.length() > 0) {
                try {
