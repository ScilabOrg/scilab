...
<xsl:stylesheet version ="1.0"
  ...
  <xsl:template match="scinotes-preferences">
    <Title text="Editor">
	  ...
	  <Radiobutton value="{@external-cmd}" expected-value="false" listener="ActionListener" text="Scilab macro: " gridx="1" gridy="2" fill="none" weightx="0" anchor="west" enable="{$enable}">
	    <actionPerformed choose="external-cmd">
	      <xsl:call-template name="context"/>
	    </actionPerformed>
	  </Radiobutton>
      ...
      <Grid>
        ...
        <Panel gridx="1" gridy="2" gridwidth="3">
          <xsl:call-template name="BooleanCheckBox">
            ...
            <xsl:with-param name="text" select="'Add carriage return at end of file.'"/>
          </xsl:call-template>
        </Panel>
        <Label gridx="1" gridy="3" weightx="0" text="Number of recently open files to show: "/>
