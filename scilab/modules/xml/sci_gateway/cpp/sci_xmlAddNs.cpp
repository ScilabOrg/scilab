...
int sci_xmlAddNs(char * fname, unsigned long fname_len)
{
    ...
    if (Rhs < 2)
    {
        Scierror(999, "%s: Bad number of arguments: 2 or more expected\n", fname);
        return 0;
    }
    ...
    for (; i <= Rhs; i++)
    {
        ...
        if (!isXMLNs(addr))
        {
            Scierror(999, "%s: Wrong type for input argument %i: XML Namespace expected\n", fname, i);
            return 0;
        }
        ...
        if (!ns)
        {
            Scierror(999, "%s: XML Namespace does not exist\n", fname);
            return 0;
        }
