    ...
    case "set" then
        ...
        while %t do
            ...
            if prod(size(tmask1)) > 1 then
                block_parameter_error(msprintf(gettext("Wrong value for ''Time Record Selection'' parameter: %s."), strcat(string(tmask1(:))," ")),..
            ...
            elseif lunit > 0 & min(length(frmt),1) <> min(length(frmt1), 1) then
                block_parameter_error([gettext("Simulation running !!! You cannot switch"); gettext("between formatted and unformatted")], ..
            ...
            elseif fname1 == "" then
              block_parameter_error(msprintf(gettext("Wrong value for ''Input File Name'' parameter: %s"), fname1), gettext("You must provide a filename."));
            // Simple check for including of the format's string  in parenthesis
            elseif frmt1 ~= "" &  (part(frmt1, 1) ~= "(" |  part(frmt1, length(frmt1)) ~= ")")
                block_parameter_error(msprintf(gettext("Wrong format for ''Input Format'' parameter: %s."), frmt1), ..
            ...
            elseif min(outmask) < 1 then
                block_parameter_error(msprintf(gettext("Wrong value for ''Outputs Record Selection'' parameter: %s"), ..
