...
// Addition of some check on the input of parameters by the user
...
function [x,y,typ] = RFILE(job,arg1,arg2)
    ...
    case "set" then
        ...
        while %t do
            ...
            elseif tmask1 ~= [] & tmask1 < 1 then
                message( ...
                    "Time record selection index must be strictly positive.")

            elseif find(outmask < 1, 1) ~= [] then
                message("Outputs record selection indexes " + ...
                    "must be strictly positive.")
                print(%io(2), find(outmask < 1, 1) )

            elseif lunit > 0 &min(length(frmt), 1) <> min(length(frmt1), 1) then
                message([ ...
                    "You cannot swich from formatted to unformatted"; ...
    ...
    case "define" then
        ...
        outmask = 1
        ipar = [ ...
            length(fname);length(frmt);0; ...
            N;str2code(fname);str2code(frmt); ...
            tmask;outmask ...
            ];
