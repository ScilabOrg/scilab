...
function [ok,%ipar,%rpar,%nz]=compile_expr(%foo)
    ...
    function [rpar, nrpar, ipar, nz] = decypher_variable(var, rpar, nrpar, ipar, nz)
    ...
    function [rpar, nrpar, ipar, nz] = decypher_funcall(f, rpar, nrpar, ipar, nz)
        ...
        for %i=1:length(f("rhs"))
            ...
            select typeof(oper)
            case "operation"
                [rpar, nrpar, ipar, nz] = decypher_operation(oper, rpar, nrpar, ipar, nz);
            case "funcall"
                [rpar, nrpar, ipar, nz] = decypher_funcall(oper, rpar, nrpar, ipar, nz);
            case "variable"
                [rpar, nrpar, ipar, nz] = decypher_variable(oper, rpar, nrpar, ipar, nz);
            else // "cste"
        ...
        elseif %indOperator <> [] then
            ...
            %indOperator = find(oper("name")==%ZCR)
            if %indOperator <> [] then
    ...
    function [rpar, nrpar, ipar, nz] = decypher_operation(op, rpar, nrpar, ipar, nz)
    ...
    for %j=1:length(%tlst(%i))
        if typeof(%tlst(%i)(%j)) == "equal" then
            ...
            select typeof(expr)
            case "operation"
                [%rpar, %nrpar, %ipar, %nz] = decypher_operation(expr, %rpar, %nrpar, %ipar, %nz);
            case "funcall"
                [%rpar, %nrpar, %ipar, %nz] = decypher_funcall(expr, %rpar, %nrpar, %ipar, %nz);
            case "variable"
                [%rpar, %nrpar, %ipar, %nz] = decypher_variable(expr, %rpar, %nrpar, %ipar, %nz);
            else // "cste"
