...
function [x,y,typ]=pre_simulate_block(job,arg1,arg2)
    x=[];y=[];typ=[];
    select job
    case 'set' then
        x=arg1;
        ...
        while %t do
            ...
            if ~isdef(fun) then
                message([_("Error occurred in pre_xcos_simulate: Cancelling simulation.");_('Undefined function.')])
