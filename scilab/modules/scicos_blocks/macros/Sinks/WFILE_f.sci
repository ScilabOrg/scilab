...
case 'set' then
  ...
  while %t do
    ...
    if lunit > 0 & min(length(frmt),1) <> min(length(frmt1),1) then
      block_parameter_error([gettext("Simulation running !!! You cannot switch"); ..
        gettext("between formatted and unformatted when running")], gettext("End current simulation first."));
      ok=%f
    ...
    elseif fileparts(fname1) ~= "" then
      ...
      if ~isdir(pa) then
        block_parameter_error(gettext("Wrong value for ''Output File Name'' parameter."), ..
    ...
    elseif N < 2 then
      block_parameter_error(msprintf(gettext("Wrong value for ''Buffer Size'' parameter: %d."), N), ..
        gettext("Must be greater or equal than 2."));
      ok=%f
    elseif in <= 0 then
      block_parameter_error(msprintf(gettext("Wrong value for ''Input Size'' parameter: %d."), in), ..
