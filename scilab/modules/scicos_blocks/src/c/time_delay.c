...
SCICOS_BLOCKS_IMPEXP void time_delay(scicos_block * block, int flag)
{
    ...
    if (flag == 4)
    {
        ...
        if ((*block->work = scicos_malloc(sizeof(int) + sizeof(double) * block->ipar[0] * (1 + block->insz[0]))) == NULL)
        {
            set_block_error(-16);
            return;
        }
    ...
    else if (flag == 1)
    {
        ...
        if (del != 0.0)
        {
            ...
            {
                block->outptr[0][k - 1] = ((pw[j] - td) * pw[i + block->ipar[0] * k] + (td - pw[i]) * pw[j + block->ipar[0] * k]) / del;
            }
        }
        else
        {
