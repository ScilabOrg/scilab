...
struct
{
    int bot, top, idstk[60000] /* was [6][10000] */ , lstk[10000], leps, bbot, bot0, infstk[10000], gbot, gtop, isiz;
} vstk_;
...
struct
{
    int ids[24576] /* was [6][4096] */ , pstk[4096], rstk[4096], pt, niv, macr, paus, icall, krec;
} recu_;
...
struct
{
    int nbvars, iwhere[1024], nbrows[1024], nbcols[1024], itflag[1024], ntypes[1024], lad[1024], ladc[1024], lhsvar[1024];
} intersci_;
...
#define zstk ((doublecomplex *)&stack_1)
    ...
    if (flag == 2 && nevprt > 0)
    {
        ...
        if (ipar[2] > 0)
        {
            ...
            for (j = 1; j <= i__1; ++j)
            {
                ...
                {
                    i__2 = do_fio(&c__1, (char *)&z__[n + 2 + (i__ - 1) * n + j], (short)sizeof(double));
                    if (i__2 != 0)
                    {
                        goto L100;
                    }
        ...
        else
        {
            ...
            for (j = 1; j <= i__1; ++j)
            {
                ...
                for (i__ = 0; i__ <= i__3; ++i__)
                {
                    i__2 = do_uio(&c__1, (char *)&z__[n + 2 + (i__ - 1) * n + j], (short)sizeof(double));
                    if (i__2 != 0)
                    {
                        goto L100;
                    }
    ...
    else if (flag == 5)
    {
        ...
        if (k >= 1)
        {
            ...
            {
                /*     .        formatted write */
                cvstr_(&lfmt, &ipar[ipar[1] + 5], cha1_1.buf, &c__1, (short)4096);
                i__1 = k;
                for (j = 1; j <= i__1; ++j)
                {
                    ci__1.cierr = 0;
                    ...
                    {
                        do_fio(&c__1, (char *)&z__[n + 2 + (i__ - 1) * n + j], (short)sizeof(double));
                    }
                    e_wsfe();
                    /* L20: */
                }
            ...
            else
            {
                ...
                for (j = 1; j <= i__1; ++j)
                {
                    ...
                    for (i__ = 0; i__ <= i__2; ++i__)
                    {
                        do_uio(&c__1, (char *)&z__[n + 2 + (i__ - 1) * n + j], (short)sizeof(double));
                    }
                    e_wsue();
                    /* L21: */
                }
