...
static char *getAxe(char const* pFigureUID, scicos_block * block, int input)
{
    ...
    // fast path for an existing object
    if (sco->scope.cachedAxeUID[input] != NULL)
    {
     ...
     */
    sco->scope.cachedAxeUID[input] = os_strdup(pAxe);
...
static char *getPolyline(char *pAxeUID, scicos_block * block, int input, int row, BOOL history)
{
    ...
    if (polylinesUIDs == NULL || polylinesUIDs[input] == NULL)
    {
        return NULL;
    }

    // fast path for an existing object
    if (polylinesUIDs[input][row] != NULL)
    {
     ...
     * then cache with local storage
     */
    polylinesUIDs[input][row] = os_strdup(pPolyline);
