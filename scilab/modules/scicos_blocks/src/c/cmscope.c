...
static void appendData(scicos_block* block, int input, double t, double* data) {
	...
	if (sco != NULL) {
		...
		for (i=0; i<block->insz[input]; i++) {
			sco->internal.data[input][i][numberOfPoints] = data[i];
			C2F(dset)(&setLen, &data[i], &(sco->internal.data[input][i][numberOfPoints + 1]), &i__1);
		}

		sco->internal.time[numberOfPoints] = t;
		C2F(dset)(&setLen, &t, &(sco->internal.time[numberOfPoints + 1]), &i__1);
...
static BOOL pushData(scicos_block* block, int input, int row) {
	...
	result &= setGraphicObjectProperty(pPolylineUID, __GO_DATA_MODEL_X__,
				sco->internal.time, jni_double_vector, sco->internal.maxNumberOfPoints);
	result &= setGraphicObjectProperty(pPolylineUID, __GO_DATA_MODEL_Y__,
			data, jni_double_vector, sco->internal.maxNumberOfPoints);
...
static void setFigureSettings(char* pFigureUID, scicos_block* block) {
	...
	setGraphicObjectProperty(pFigureUID, __GO_POSITION__, &win_pos,
			jni_int_vector, 2);
	setGraphicObjectProperty(pFigureUID, __GO_SIZE__, &win_dim,
			jni_int_vector, 2);
};
...
static char* getPolyline(char* pAxeUID, scicos_block* block, int input, int row) {
	char* pPolyline;
	const static double d__0 = 0.0;
	const static int i__1 = 1;
	const static BOOL b__true = TRUE;
	...
	if (sco->scope.cachedPolylinesUIDs != NULL &&
			sco->scope.cachedPolylinesUIDs[input] != NULL &&
			sco->scope.cachedPolylinesUIDs[input][row] != NULL) {
	...
	if (pPolyline == NULL) {
		...
		if (pPolyline != NULL) {
			 ...
			 */
			{
				...
				setGraphicObjectProperty(pPolyline,
								__GO_DATA_MODEL_NUM_ELEMENTS_ARRAY__, polylineSize,
								jni_int_vector, 2);
			}
			...
			color = block->ipar[6 + block->nin + row];
			if (color > 0) {
				setGraphicObjectProperty(pPolyline, __GO_LINE_MODE__, &b__true, jni_bool, 1);
				setGraphicObjectProperty(pPolyline, __GO_POLYLINE_STYLE__, &i__1, jni_int, 1);
...
static BOOL setPolylinesBuffers(scicos_block* block, int input, int maxNumberOfPoints) {
	...
	for (i=0; i<block->insz[input]; i++) {
		...
		result &= setGraphicObjectProperty(pPolylineUID,
				__GO_DATA_MODEL_NUM_ELEMENTS_ARRAY__, polylineSize,
				jni_int_vector, 2);
	}
...
static BOOL setPolylinesBounds(scicos_block* block, int input, int periodCounter) {
	...
	BOOL result;
	double dataBounds[6];
	double period = block->rpar[1 + input];
	int nin = block->nin;

	dataBounds[0] = periodCounter * period;     			// xMin
	dataBounds[1] = (periodCounter+1) * period; 			// xMax
	dataBounds[2] = block->rpar[1 + nin + input];       	// yMin
	dataBounds[3] = block->rpar[1 + nin + nin + input];     // yMax
	dataBounds[4] = -1.0;                       			// zMin
	dataBounds[5] = 1.0;                        			// zMax

	pFigureUID = getFigure(block);
	pAxeUID = getAxe(pFigureUID, block, input);
	return setGraphicObjectProperty(pAxeUID, __GO_DATA_BOUNDS__, dataBounds,
			jni_double_vector, 4);
}
