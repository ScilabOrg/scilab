...
static void freeScoData(scicos_block * block)
{
    ...
    if (sco != NULL)
    {
        ...
        for (i = 0; i < block->nin; i++)
        {
            for (j = 0; j < block->insz[i]; j++)
            {
                releaseGraphicObjectProperty(__GO_PARENT__, jni_string, sco->scope.cachedPolylinesUIDs[i][j]);
            }
            releaseGraphicObjectProperty(__GO_PARENT__, jni_string, sco->scope.cachedAxeUID[i]);
        }
