 ...
 */
static void setBuffersCoordinates(double* coordinates, const int numberOfPoints,
                                  ...
                                  const int bufferPoints, const double t, const double value)
{
    ...
    for (setLen = numberOfPoints; setLen < bufferPoints; setLen++)
    {
        coordinates[2 * bufferPoints + setLen] = 0.0;
    }
}
...
static void appendData(scicos_block * block, int input, double t, double *data)
{
    ...
    if (sco != NULL)
    {
        for (i = 0; i < block->insz[input]; i++)
        {
            ...
            setBuffersCoordinates(sco->internal.bufferCoordinates[input][i], sco->internal.numberOfPoints[input], block->ipar[2], t, value);
        }
