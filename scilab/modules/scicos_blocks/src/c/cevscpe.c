...
SCICOS_BLOCKS_IMPEXP void cevscpe(scicos_block * block, scicos_flag flag)
{
    ...
    case StateUpdate:
        ...
        appendData(block, block->nevprt, t);

        result = pushData(block, block->nevprt);
        if (result == FALSE)
        {
...
static sco_data *getScoData(scicos_block * block)
{
    ...
    int maxNumberOfPoints = DEFAULT_MAX_NUMBER_OF_POINTS;
    ...
    if (sco == NULL)
    {
        ...
        sco->internal.maxNumberOfPoints = (int *)CALLOC(nclk, sizeof(int));
        if (sco->internal.numberOfPoints == NULL)
        {
            goto error_handler_maxNumberOfPoints;
        }
        C2F(dset) (&nclk, &maxNumberOfPoints, sco->internal.maxNumberOfPoints, &i__1);
        ...
        for (i = 0; i < nclk; i += 2)
        {
             ...
             * Alloc base pointer
             */
            sco->internal.data[i] = (double *)CALLOC(3 * maxNumberOfPoints, sizeof(double));
             ...
             */
            sco->internal.data[i + 1] = (double *)CALLOC(3 * maxNumberOfPoints, sizeof(double));
            if (sco->internal.data[i + 1] == NULL)
            {
...
static sco_data *reallocScoData(scicos_block * block, int input, int numberOfPoints)
{
    ...
    setLen = maxNumberOfPoints - numberOfPoints;
    ...
    C2F(dset) (&setLen, &ptr[maxNumberOfPoints - 3], &(ptr[maxNumberOfPoints]), &i__3);
    C2F(dset) (&setLen, &ptr[maxNumberOfPoints - 2], &(ptr[maxNumberOfPoints]), &i__3);
    C2F(dset) (&setLen, &ptr[maxNumberOfPoints - 1], &(ptr[maxNumberOfPoints]), &i__3);
