...
/*--------------------------------------------------------------------------*/
void scoInitOfWindow(ScopeMemory * pScopeMemory, int dimension, int win_id, int *win_pos, int *win_dim, double *xmin, double *xmax, double *ymin,
        ...
        if (force_update[i] == 1)   //** scope need a redraw 
        {
            ...
            for (j = 0; j < scoGetNumberOfCurvesBySubwin(pScopeMemory, i); j++)
            {
                ...
                case SCI_POLYLINE:
                    ...
                    C2F(dcopy) (&NbrPtsToCopy, pPOLYLINE_FEATURE(pShortDraw)->pvx + inc, &c__1, pPOLYLINE_FEATURE(pLongDraw)->pvx + NbrPtsLong,
                                &c__1);
                    C2F(dcopy) (&NbrPtsToCopy, pPOLYLINE_FEATURE(pShortDraw)->pvy + inc, &c__1, pPOLYLINE_FEATURE(pLongDraw)->pvy + NbrPtsLong,
                                &c__1);
...
scoGraphicalObject scoCreatePlot3d(scoGraphicalObject pAxes, int size_x, int size_y)
{
    ...
    pPlot3d =
        ConstructSurface(pAxes, SCI_PLOT3D, pvecx, pvecy, pvecz, NULL, 0, size_x, size_y, flag, ebox, colorflag, &isfac, &m1, &n1, &m2, &n2, &m3, &n3,
                         &m3n, &n3n);
