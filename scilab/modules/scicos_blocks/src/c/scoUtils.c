...
char* findChildWithKindAt(char* parent, const char* type, int position) {
	...
	getGraphicObjectProperty(parent, __GO_CHILDREN_COUNT__, jni_int,
			(void**) &pChildrenCount);
	getGraphicObjectProperty(parent, __GO_CHILDREN__, jni_string_vector,
			(void**) &children);
...
BOOL setLabel(char* pAxeUID, char *_pstName, char* label) {
	char* pLabelUID;
	int dimensions[2];
