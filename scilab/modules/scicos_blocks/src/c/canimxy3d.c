...
static void appendData(scicos_block * block, double *x, double *y, double *z)
{
    ...
    if (sco != NULL)
    {
        ...
        for (i = 0; i < block->insz[0]; i++)
        {
            ...
            C2F(dset) (&setLen, &z[i], &(sco->internal.data[1][i][numberOfPoints + 1]), &i__1);
        }
...
static char *getPolyline(char *pAxeUID, scicos_block * block, int row)
{
    ...
    if (pPolyline == NULL)
    {
        ...
        if (pPolyline != NULL)
        {
            ...
            lineSize = block->ipar[3 + block->ipar[1] + row];
            if (color > 0)
            {
                ...
                setGraphicObjectProperty(pPolyline, __GO_LINE_THICKNESS__, &lineSize, jni_int, 1);
            }
            else
            {
                color = -color;
                ...
                setGraphicObjectProperty(pPolyline, __GO_MARK_SIZE__, &lineSize, jni_int, 1);
            }
        }
    }
