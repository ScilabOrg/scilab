                ...
                switch (ut)
                {
                    case SCSREAL_N    :
                        ...
                        else
                        {
                            ...
                            //allocMatrixOfDouble(NULL, 1, nRows, ptr_i[8], &Matrix);
                            if ((Matrix = (double *) scicos_malloc(nRows * ptr_i[8] * sizeof(double))) == NULL)
                            {
                                set_block_error(-16);
                                scicos_free(ptr->workt);
                                scicos_free(ptr);
                            ...
                            for (i = 0; i < ptr_i[8]*nRows; ++i)
                            {
                                Matrix[i] = ptr_d[i];
                            }
                            sciErr = createNamedMatrixOfDouble(NULL, varName, nRows, ptr_i[8], Matrix); /* pass Matrix to Scilab */
                            if (sciErr.iErr)
                            {
                                scicos_free(ptr->workt);
                                scicos_free(ptr);
