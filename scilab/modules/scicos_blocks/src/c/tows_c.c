            ...
            if ((ptr->cnt != 0) || (ptr->loop != 0))
            {
                ...
                switch (ut)
                {
                    case SCSREAL_N    :
                        ...
                        {
                            ptr_d = (SCSREAL_COP *) & (ptr_i[10]);
                            Matrix = NULL; /* Matrix is to be a copy of ptr_d (later, will be [ptr_d; workt->ptr_d]) */
                            //allocMatrixOfDouble(NULL, 1, buf_size, ptr_i[8], &Matrix);
                            if ((Matrix = (double *) scicos_malloc(buf_size * ptr_i[8] * sizeof(double))) == NULL)
                            {
                                set_block_error(-16);
                                scicos_free(ptr->workt);
                                scicos_free(ptr);
                            ...
                            for (i = 0; i < ptr_i[8]*buf_size; ++i)
                            {
                                Matrix[i] = ptr_d[i];
                            }
                            sciErr = createNamedMatrixOfDouble(NULL, varName, buf_size, ptr_i[8], Matrix); /* pass Matrix to Scilab */
                            if (sciErr.iErr)
                            {
                                scicos_free(ptr->workt);
                                scicos_free(ptr);
                            ...
                            scicos_free(Matrix);
                            printf("createMat\n"); // Info print to certify program came through here
