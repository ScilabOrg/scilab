...
*/
{
    ...
    char * varName;
    double * Matrix = NULL; /* Matrix is to be a copy of the work array, to pass to scilab */
    ...
    else if (flag == 5)
    {
        ...
        if (ptr != NULL)
        {
            ...
            */
            ...
            {
                /* write data */
                if ((Matrix = (double *) scicos_malloc(nRows * nCols * nCols2 * sizeof(double))) == NULL)
                {
                    set_block_error(-16);
                    scicos_free(ptr->workt);
                    scicos_free(ptr);
                ...
                switch (ut)
                {
                    case SCSREAL_N    :
                        ...
                        else
                        {
                            ...
                            sciErr = createNamedMatrixOfDouble(NULL, varName, nRows, nCols, Matrix);
                            if (sciErr.iErr)
                            {
                                scicos_free(ptr->workt);
                                scicos_free(ptr);
            ...
            if ((ptr->cnt != 0) || (ptr->loop != 0))
            {
                ...
                /* Reusing variable Matrix to hold time values */
                for (i = 0; i < nRows * 1; ++i)
                {
                    Matrix[i] = ptr_d[i];
                }
                /* pass Matrix to Scilab */
                sciErr = createNamedMatrixOfDouble(NULL, varName, nRows, 1, Matrix);
                if (sciErr.iErr)
                {
                    scicos_free(ptr->workt);
                    scicos_free(ptr);
