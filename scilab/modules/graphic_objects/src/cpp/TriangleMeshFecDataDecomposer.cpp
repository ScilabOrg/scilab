...
void TriangleMeshFecDataDecomposer::fillTextureCoordinates(char* id, float* buffer, int bufferLength)
{
    ...
    for (i = 0; i < numVertices; i++)
    {
        if (useOutsideColors == 1)
        {
            ...
            {
                buffer[bufferOffset++] = minColorIndex / (float) colormapSize;
            }
            else if (values[i] > maxValue)
            {
                buffer[bufferOffset++] = maxColorIndex / (float) colormapSize;
            }
            else
            {
                buffer[bufferOffset++] = ColorComputer::getIndex(values[i], minValue, valueRange, 0.5, minColorIndex, maxColorIndex) / (float) colormapSize;
            }
        }
        else
        {
