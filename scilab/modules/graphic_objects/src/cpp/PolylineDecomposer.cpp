...
void PolylineDecomposer::fillTextureCoordinates(char* id, float* buffer, int bufferLength)
{
    ...
    for (int i = 0; i < nPoints; i++)
    {
        // TO DO: add the function equivalent to getDirectColor (to take into account the white color)
        // ColorComputer::getDirectColor((double) interpColorVector[i] - 1.0, colormap, colormapSize, &buffer[bufferOffset]);

        double index = (ColorComputer::getClampedDirectIndex((double) interpColorVector[i] - 1.0, colormapSize) + 2.0 + COLOR_TEXTURE_OFFSET) / (double) (colormapSize + 2);
