...
Fac3DColorComputer::Fac3DColorComputer(double* colors, int numColors, int colorFlag, int dataMapping, int numGons, int numVerticesPerGon, int colormapSize)
{
    this->colors = colors;
    ...
    if (dataMapping == 0)
    {
        ...
        if (colorRange < DecompositionUtils::getMinDoubleValue())
        {
            ...
            /* Set the actually used color range to the colormap's bounds */
            usedMinColorValue = 0.0;
            colorRange = (double) colormapSize - 1.0;
        }
...
double Fac3DColorComputer::getFacetColor(int facetIndex, int vertexIndex)
{
    ...
    /*
     * Special case for scaled colors: return 0.5 if color range invalid.
...
int Fac3DColorComputer::isFacetColorValid(int facetIndex)
{
    ...
    /* Special case, no colors therefore facet always valid */
...
void Fac3DColorComputer::computeFacetMinMax(int facetIndex, double* currentMin, double* currentMax)
{
    ...
    for (int i = 0; i < numColorsPerFacet; i++)
    {
        ...
        /* To do: update min and max according to data_mapping (depending on infinite values and nan) */
        if (value < *currentMin)
        {
            *currentMin = value;
        }
