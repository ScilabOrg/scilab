...
void fillVertices(char* id, float* buffer, int bufferLength, int elementsSize, int coordinateMask, double* scale, double* translation)
{
    ...
    if (strcmp(type, __GO_POLYLINE__) == 0)
    {
        fillPolylineVertices(id, buffer, bufferLength, elementsSize, coordinateMask, scale, translation);
    }
}
...
void fillPolylineVertices(char* id, float* buffer, int bufferLength, int elementsSize, int coordinateMask, double* scale, double* translation)
{
    ...
    if (polylineStyle == 1)
    {
        polylineLinearDecomposition(id, buffer, bufferLength, elementsSize, coordinateMask, scale, translation, t, nPoints, xshift, yshift, zshift);
    }
    else if (polylineStyle == 2)
    {
        polylineStairDecomposition(id, buffer, bufferLength, elementsSize, coordinateMask, scale, translation, t, nPoints, xshift, yshift, zshift);
    }
    else if (polylineStyle == 3)
    {
        polylineVerticalLinesDecomposition(id, buffer, bufferLength, elementsSize, coordinateMask, scale, translation, t, nPoints, xshift, yshift, zshift);
    }
    else if (polylineStyle == 4)
    {
        polylineLinearDecomposition(id, buffer, bufferLength, elementsSize, coordinateMask, scale, translation, t, nPoints, xshift, yshift, zshift);
    ...
    {
        polylineMatlabBarDecomposition(id, buffer, bufferLength, elementsSize, coordinateMask, scale, translation, t, nPoints, xshift, yshift, zshift);
    }
    ...
    double* coordinates, int nPoints, double* xshift, double* yshift, double* zshift)
{
    ...
    for (i = 0; i < nPoints; i++)
    {
        ...
        polylineGetAndWriteVertexToBuffer(buffer, v0, coordinates, componentIndices, nPoints, elementsSize,
    ...
    double* coordinates, int nPoints, double* xshift, double* yshift, double* zshift)
{
    ...
    for (i = 0; i < nPoints-1; i++)
    {
        ...
        polylineGetAndWriteVertexToBuffer(buffer, v0, coordinates, componentIndices, nPoints, elementsSize, xshift, yshift, zshift, coordinateMask, scale, translation);

        componentIndices[0] = i+1;
        componentIndices[1] = i;
        componentIndices[2] = i;

        polylineGetAndWriteVertexToBuffer(buffer, v1, coordinates, componentIndices, nPoints, elementsSize, xshift, yshift, zshift, coordinateMask, scale, translation);
    }
...
void polylineWriteBarVerticesToBuffer(float* buffer, int* offsets, int componentOffset, double* coordinates, double shift, int shiftUsed, double scale, double translation)
{
    if (shiftUsed)
    {
        coordinates[0] += shift;
    ...
    double* coordinates, int nPoints, double* xshift, double* yshift, double* zshift)
{
    ...
    for (i = 0; i < nPoints; i++)
    {
        ...
        if (coordinateMask & 0x01)
        {
            ...
            polylineWriteBarVerticesToBuffer(buffer, offsets, 0, coords, shift, shiftUsed[0], scale[0], translation[0]);
        }
        ...
        if (coordinateMask & 0x02)
        {
            ...
            polylineWriteBarVerticesToBuffer(buffer, offsets, 1, coords, shift, shiftUsed[1], scale[1], translation[1]);
        }
        ...
        if (coordinateMask & 0x04)
        {
            ...
            polylineWriteBarVerticesToBuffer(buffer, offsets, 2, coords, shift, shiftUsed[2], scale[2], translation[2]);
        }
...
int getIndicesSize(char* id)
{
    ...
    if (strcmp(type, __GO_POLYLINE__) == 0)
    {
        return getPolylineIndicesSize(id);
    }
...
void fillIndices(char* id, int* buffer, int bufferLength)
{
    ...
    if (strcmp(type, __GO_POLYLINE__) == 0)
    {
        fillPolylineIndices(id, buffer, bufferLength);
    }
}
...
void fillPolylineIndices(char* id, int* buffer, int bufferLength)
{
    ...
    if (polylineStyle == 1)
    {
        polylineLinearDecompositionIndices(id, buffer, bufferLength, nPoints, closed);
    }
    else if (polylineStyle == 2)
    {
        polylineStairDecompositionIndices(id, buffer, bufferLength, nPoints, closed);
    }
    else if (polylineStyle == 3)
    {
        polylineVerticalLinesDecompositionIndices(id, buffer, bufferLength, nPoints);
    }
    else if (polylineStyle == 4)
    {
        polylineLinearDecompositionIndices(id, buffer, bufferLength, nPoints, closed);
    ...
    {
        polylineMatlabBarDecompositionIndices(id, buffer, bufferLength, nPoints);
    }
