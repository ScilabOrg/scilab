...
public class Arc extends ClippableContouredObject {
	...
	public Object getPropertyFast2(Object property) {
		Object returnedProp = null;
		//System.out.format("property.toString: %s\n", property.toString());
		
		switch((ArcProperty2) property) {
		case UPPERLEFTPOINT:
			returnedProp = getUpperLeftPoint();
			break;
		case WIDTH:
	...
	public void setPropertyFastHack(Object property, Object value) {
			...
			if (property == ArcProperty2.UPPERLEFTPOINT) {
				setUpperLeftPoint((Double[]) value);
			} else if (property == ArcProperty2.WIDTH) {
				setWidth((Double) value);
			} else if (property == ArcProperty2.HEIGHT) {
				setHeight((Double) value);
			} else if (property == ArcProperty2.STARTANGLE) {
				setStartAngle((Double) value);
			} else if (property == ArcProperty2.ENDANGLE) {
				setEndAngle((Double) value);
			} else if (property == ArcProperty2.ARCDRAWINGMETHOD) {
				setArcDrawingMethod((ArcDrawingMethod) value);
			} else {
				super.setPropertyFast(property, value);
			}
	...
	public void setPropertyFast2(Object property, Object value) {
			if (property == ArcProperty2.UPPERLEFTPOINT) {
				...
				setArcDrawingMethod((ArcDrawingMethod) value);
			} else if (property == ArcProperty2.CLIPPROPERTY) {
				this.setClipProperty((ClippableProperty) value);
			} else if (property == ArcProperty2.CLIPSTATE) {
				this.setClipState((ClipStateType) value);
			} else if (property == ArcProperty2.CLIPBOX) {
				this.setClipBox((Double[]) value);
			} else if (property == ArcProperty2.LINE) {
				this.setLine((Line) value);
			} else if (property == ArcProperty2.FILLMODE) {
				this.setFillMode((Boolean) value);
			} else if (property == ArcProperty2.BACKGROUND) {
				this.setBackground((Integer) value);
			} else if (property == ArcProperty2.MARK) {
				this.setMark((Mark) value);
			} else if (property == ArcProperty2.PARENT) {
				this.setParent((GraphicObject) value);
			} else if (property == ArcProperty2.CHILDREN) {
				// TBD
		//		this.setChildren(setChildren((ArrayList<GraphicObject>) value);
			} else if (property == ArcProperty2.VISIBLE) {
				this.setVisible((Boolean) value);
			} else if (property == ArcProperty2.USERDATA) {
