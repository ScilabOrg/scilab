...
import org.scilab.forge.scirenderer.core.maths.Rotation;
import org.scilab.forge.scirenderer.core.maths.Scale;
...
public class DrawerVisitor implements IVisitor {
    ...
    public void visit(Axes axes) {
        if (axes.getVisible()) {
            ...
            ITransformation transformation = new Rotation(DEFAULT_ALPHA - alpha, 1.0, 0.0, 0.0);
            transformation.times(new Rotation(DEFAULT_THETA - theta, 0.0, 0.0, 1.0));

            double ratio = canvas.getZoneControl().getFinalAspectRatio();
            // only if iso ?
            if (ratio < 1.0) {
                transformation.times(new Scale(ratio, 1, 1));
            } else {
                transformation.times(new Scale(1, 1/ratio, 1));
            }

            canvas.getTransformationsStack().push(transformation);

            canvas.draw(new FinalBox(1, 1, 1));
