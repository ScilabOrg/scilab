...
static BOOL gl2psSameColorThreshold(int n, GL2PSrgba rgba[], GL2PSrgba threshold)
{
    int i;
    ...
    for (i = 1; i < n; i++)
    {
        if (fabs(rgba[0][0] - rgba[i][0]) > threshold[0] ||
            fabs(rgba[0][1] - rgba[i][1]) > threshold[1] || fabs(rgba[0][2] - rgba[i][2]) > threshold[2])
            return FALSE;
    }
...
static float gl2psGetRGB(GL2PSimage * im, unsigned int x, unsigned int y, float *red, float *green, float *blue)
{
...
static int gl2psAddText(int type, const char *str, const char *fontname, short fontsize, int alignment, float angle)
{
    float pos[4];
    GL2PSprimitive *prim;
    BOOL valid;
...
static BOOL gl2psSupportedBlendMode(int sfactor, int dfactor)
{
    ...
    if ((sfactor == joglGL_SRC_ALPHA() && dfactor == joglGL_ONE_MINUS_SRC_ALPHA()) || (sfactor == joglGL_ONE() && dfactor == joglGL_ZERO()))
        return TRUE;
    return FALSE;
}
...
static void gl2psAssignTriangleProperties(GL2PStriangle * t)
{
    ...
    if (!GL2PS_ZERO(t->vertex[0].rgba[3] - t->vertex[1].rgba[3]) || !GL2PS_ZERO(t->vertex[1].rgba[3] - t->vertex[2].rgba[3]))
    {
        t->prop |= T_VAR_ALPHA;
    }
    else
...
static void gl2psFillTriangleFromPrimitive(GL2PStriangle * t, GL2PSprimitive * p, BOOL assignprops)
{
...
static void gl2psGetPlane(GL2PSprimitive * prim, GL2PSplane plane)
{
    ...
    case GL2PS_QUADRANGLE:
        ...
        if ((GL2PS_ZERO(v[0]) && GL2PS_ZERO(v[1]) && GL2PS_ZERO(v[2])) || (GL2PS_ZERO(w[0]) && GL2PS_ZERO(w[1]) && GL2PS_ZERO(w[2])))
        {
        ...
        else
        {
            ...
            plane[3] = -plane[0] * prim->verts[0].xyz[0] - plane[1] * prim->verts[0].xyz[1] - plane[2] * prim->verts[0].xyz[2];
        }
        break;
    case GL2PS_LINE:
        v[0] = prim->verts[1].xyz[0] - prim->verts[0].xyz[0];
...
static void gl2psCreateSplitPrimitive(GL2PSprimitive * parent, GL2PSplane plane, GL2PSprimitive * child, short numverts, short *index0, short *index1)
{
    short i;
...
static void gl2psAddIndex(short *index0, short *index1, short *nb, short i, short j)
{
    int k;
...
static int gl2psSplitPrimitive(GL2PSprimitive * prim, GL2PSplane plane, GL2PSprimitive ** front, GL2PSprimitive ** back)
{
...
static void gl2psTraverseBspTree(GL2PSbsptree * tree, GL2PSxyz eye, float epsilon,
                                 BOOL(*compare) (float f1, float f2), void (*action) (void *data), int inverse)
{
    float result;
            ...
            if (area > GL2PS_EPSILON)
            {
                dZdX =
                    ((prim->verts[2].xyz[1] - prim->verts[1].xyz[1]) *
                     ...
                     (prim->verts[1].xyz[1] - prim->verts[0].xyz[1]) * (prim->verts[2].xyz[2] - prim->verts[1].xyz[2])) / area;
                dZdY =
                    ((prim->verts[1].xyz[0] - prim->verts[0].xyz[0]) *
                     (prim->verts[2].xyz[2] - prim->verts[1].xyz[2]) -
                     (prim->verts[2].xyz[0] - prim->verts[1].xyz[0]) * (prim->verts[1].xyz[2] - prim->verts[0].xyz[2])) / area;
...
static GL2PSprimitive *gl2psCreateSplitPrimitive2D(GL2PSprimitive * parent, short numverts, GL2PSvertex * vertx)
{
    int i;
...
static void gl2psAddPolyPrimitive(short type, short numverts,
                                  GL2PSvertex * verts, int offset, unsigned short pattern, int factor, float width, char boundary)
{
    GL2PSprimitive *prim;
...
static void gl2psParseFeedbackBuffer(int used)
{
    ...
    while (used > 0)
    {
        ...
        else if (((int)*current == joglGL_BITMAP_TOKEN()) || ((int)*current == joglGL_DRAW_PIXEL_TOKEN())
                 || ((int)*current == joglGL_COPY_PIXEL_TOKEN()))
        {
            current++;
            used--;
            i = gl2psGetVertex(&vertices[0], current);
...
static void gl2psPrintPostScriptImagemap(float x, float y, int width, int height, const unsigned char *imagemap)
{
    int i, size;
...
static void gl2psPrintPostScriptHeader(void)
{
    ...
    gl2psPrintf("%%%%Title: %s\n"
                ...
                GL2PS_PATCH_VERSION, GL2PS_EXTRA_VERSION, GL2PS_COPYRIGHT, gl2ps->producer, ctime(&now));
    ...
    if (gl2ps->format == GL2PS_PS)
    {
        gl2psPrintf("%%%%Orientation: %s\n"
                    ...
                    (int)gl2ps->viewport[2], (gl2ps->options & GL2PS_LANDSCAPE) ? (int)gl2ps->viewport[2] : (int)gl2ps->viewport[3]);
    }
    ...
    gl2psPrintf("%%%%BeginProlog\n"
                ...
                (gl2ps->options & GL2PS_NO_PS3_SHADING) ? "false" : "true", gl2ps->threshold[0], gl2ps->threshold[1], gl2ps->threshold[2]);
    ...
    if (gl2ps->options & GL2PS_DRAW_BACKGROUND)
    {
        gl2psPrintf("%g %g %g C\n"
                    ...
                    (int)gl2ps->viewport[1], (int)gl2ps->viewport[2], (int)gl2ps->viewport[3], (int)gl2ps->viewport[0], (int)gl2ps->viewport[3]);
    }
}
...
static void gl2psParseStipplePattern(unsigned short pattern, int factor, int *nb, int array[10])
{
    int i, n;
...
static void gl2psPrintPostScriptPrimitive(void *data)
{
    ...
    case GL2PS_LINE:
        if (!gl2psSamePosition(gl2ps->lastvertex.xyz, prim->verts[0].xyz) ||
            ...
            gl2ps->lastlinewidth != prim->width || gl2ps->lastpattern != prim->pattern || gl2ps->lastfactor != prim->factor)
        {
    ...
    case GL2PS_TRIANGLE:
        if (!gl2psVertsSameColor(prim))
        {
            ...
            gl2psPrintf("%g %g %g %g %g %g %g %g %g %g %g %g %g %g %g ST\n",
                        ...
                        prim->verts[0].xyz[0], prim->verts[0].xyz[1], prim->verts[0].rgba[0], prim->verts[0].rgba[1], prim->verts[0].rgba[2]);
        }
        else
        {
            gl2psPrintPostScriptColor(prim->verts[0].rgba);
    ...
    case GL2PS_IMAGEMAP:
        if (prim->data.image->type != GL2PS_IMAGEMAP_WRITTEN)
        {
            ...
            gl2psPrintPostScriptImagemap(prim->data.image->pixels[0],
                                         ...
                                         prim->data.image->width, prim->data.image->height, (const unsigned char *)(&(prim->data.image->pixels[2])));
    ...
    case GL2PS_TEXT:
        ...
        gl2psPrintf("%g %g %d /%s ", prim->verts[0].xyz[0], prim->verts[0].xyz[1], prim->data.text->fontsize, prim->data.text->fontname);
...
static void gl2psPrintTeXHeader(void)
{
    ...
    fprintf(gl2ps->stream,
            ...
            name, (gl2ps->options & GL2PS_LANDSCAPE) ? "\\rotatebox{90}{" : "", (int)gl2ps->viewport[2], (int)gl2ps->viewport[3]);
}
...
static void gl2psPrintTeXPrimitive(void *data)
{
    ...
    case GL2PS_TEXT:
        ...
        fprintf(gl2ps->stream, "\\textcolor[rgb]{%g,%g,%g}{{%s}}",
                prim->verts[0].rgba[0], prim->verts[0].rgba[1], prim->verts[0].rgba[2], prim->data.text->str);
        if (prim->data.text->angle)
            fprintf(gl2ps->stream, "}");
        fprintf(gl2ps->stream, "}}\n");
        break;
...
static void gl2psPutPDFText(GL2PSstring * text, int cnt, float x, float y)
{
    ...
    else
    {
        ...
        gl2ps->streamlength +=
            gl2psPrintf("BT\n"
                        "/F%d %d Tf\n"
                        "%f %f %f %f %f %f Tm\n" "(%s) Tj\n" "ET\n", cnt, text->fontsize, cos_ang, -sin_ang, sin_ang, cos_ang, x, y, text->str);
    }
}
...
static void gl2psPDFgroupObjectInit(GL2PSpdfgroup * gro)
{
    ...
    gro->fontno = gro->gsno = gro->imno = gro->maskshno = gro->shno
        = gro->trgroupno = gro->fontobjno = gro->imobjno = gro->shobjno = gro->maskshobjno = gro->gsobjno = gro->trgroupobjno = -1;
}
    ...
    for (i = 0; i < gl2psListNbr(gl2ps->pdfprimlist); ++i)
    {
        ...
        case GL2PS_LINE:
            if (lasttype != p->type || lastwidth != p->width ||
                lastpattern != p->pattern || lastfactor != p->factor || !gl2psSameColor(p->verts[0].rgba, lastrgba))
            {
                gl2psPDFgroupObjectInit(&gro);
                gro.ptrlist = gl2psListCreate(1, 2, sizeof(GL2PSprimitive *));
                gl2psListAdd(gro.ptrlist, &p);
        ...
        case GL2PS_TRIANGLE:
            ...
            lastTriangleWasNotSimpleWithSameColor =
                !(tmpt.prop & T_CONST_COLOR && tmpt.prop & T_ALPHA_1) || !gl2psSameColor(tmpt.vertex[0].rgba, lastt.vertex[0].rgba);
                ...
                for (j = 0; j <= lastel; ++j)
                {
                    ...
                    gl2ps->streamlength
                        += gl2psPrintf("%f %f m\n"
                                       ...
                                       t.vertex[1].xyz[0], t.vertex[1].xyz[1], t.vertex[2].xyz[0], t.vertex[2].xyz[1]);
                }
            }
...
static int gl2psPrintPDFInfo(void)
{
    ...
    offs = fprintf(gl2ps->stream,
                   ...
                   "/Producer (%s)\n",
                   gl2ps->title, GL2PS_MAJOR_VERSION, GL2PS_MINOR_VERSION,
                   GL2PS_PATCH_VERSION, GL2PS_EXTRA_VERSION, GL2PS_COPYRIGHT, gl2ps->producer);
...
static int gl2psPrintPDFShaderStreamDataCoord(GL2PSvertex * vertex,
                                              size_t(*action) (unsigned long data, size_t size), float dx, float dy, float xmin, float ymin)
{
    int offs = 0;
    unsigned long imap;
    float diff;
...
static int gl2psPrintPDFShaderStreamData(GL2PStriangle * triangle,
                                         float dx, float dy, float xmin, float ymin, size_t(*action) (unsigned long data, size_t size), int gray)
{
    int i, offs = 0;
    GL2PSvertex v;
...
static void gl2psPDFRectHull(float *xmin, float *xmax, float *ymin, float *ymax, GL2PStriangle * triangles, int cnt)
{
    int i, j;
...
static int gl2psPrintPDFShader(int obj, GL2PStriangle * triangles, int size, int gray)
{
    ...
    if (gl2ps->options & GL2PS_COMPRESS)
    {
        ...
        for (i = 0; i < size; ++i)
            gl2psPrintPDFShaderStreamData(&triangles[i], xmax - xmin, ymax - ymin, xmin, ymin, gl2psWriteBigEndianCompress, gray);
    ...
    if (!done)
    {
        ...
        offs += fprintf(gl2ps->stream, "/Length %d " ">>\n" "stream\n", vertexbytes * 3 * size);
        for (i = 0; i < size; ++i)
            offs += gl2psPrintPDFShaderStreamData(&triangles[i], xmax - xmin, ymax - ymin, xmin, ymin, gl2psWriteBigEndian, gray);
    }
...
static void gl2psPrintSVGHeader(void)
{
    ...
    gl2psPrintf("Creator: GL2PS %d.%d.%d%s, %s\n"
                "For: %s\n"
                "CreationDate: %s",
                GL2PS_MAJOR_VERSION, GL2PS_MINOR_VERSION, GL2PS_PATCH_VERSION, GL2PS_EXTRA_VERSION, GL2PS_COPYRIGHT, gl2ps->producer, ctime(&now));
    gl2psPrintf("</desc>\n");
    gl2psPrintf("<defs>\n");
    gl2psPrintf("</defs>\n");
    ...
    if (gl2ps->options & GL2PS_DRAW_BACKGROUND)
    {
        ...
        gl2psPrintf("<polygon fill=\"%s\" points=\"%d,%d %d,%d %d,%d %d,%d\"/>\n", col,
                    ...
                    (int)gl2ps->viewport[2], (int)gl2ps->viewport[3], (int)gl2ps->viewport[0], (int)gl2ps->viewport[3]);
    }
...
static void gl2psPrintSVGPixmap(float x, float y, GL2PSimage * pixmap)
{
    ...
    gl2psListDelete(png);
#else
    gl2psMsg(GL2PS_WARNING, "GL2PS has to be compiled with PNG support in " "order to embed images in SVG streams");
#endif
}
...
static void gl2psPrintSVGPrimitive(void *data)
{
    ...
    case GL2PS_LINE:
        ...
        if (newline)
        {
            ...
            gl2psPrintf("<polyline fill=\"none\" stroke=\"%s\" stroke-width=\"%g\" ", col, 0.5 * prim->width);
            if (rgba[0][3] < 1.0F)
    ...
    case GL2PS_TEXT:
        {
            ...
            gl2psPrintf("<text fill=\"%s\" style=\"stroke:none\" x=\"%g\" y=\"%g\" "
                        ...
                        fontFamily,
                        (isItalicFont ? "italic" : (isObliqueFont ? "oblique" : "normal")), (isBoldFont ? "bold" : "normal"), prim->data.text->str);
    ...
    case GL2PS_SPECIAL_TEXT:
        ...
        {
            gl2psSVGGetColorString(prim->verts[0].rgba, col);
            gl2psPrintf("<g fill=\"%s\" stroke=\"black\" transform=\"rotate(%g,%g,%g) translate(%g,%g)\">%s</g>\n", col, -(prim->data.text->angle),
                        xyz[0][0], xyz[0][1], xyz[0][0], xyz[0][1], prim->data.text->str);
        }
        break;
    default:
        break;
...
static void gl2psPrintSVGBeginViewport(int viewport[4])
{
    ...
    if (gl2ps->options & GL2PS_DRAW_BACKGROUND)
    {
        ...
        gl2psPrintf("<polygon fill=\"%s\" points=\"%d,%d %d,%d %d,%d %d,%d\"/>\n", col,
                    x, gl2ps->viewport[3] - y, x + w, gl2ps->viewport[3] - y, x + w, gl2ps->viewport[3] - (y + h), x, gl2ps->viewport[3] - (y + h));
    }
...
static void gl2psPrintPGFPrimitive(void *data)
{
    ...
    case GL2PS_POINT:
        ...
        fprintf(gl2ps->stream,
                ...
                "{\\pgfpoint{%fpt}{%fpt}}\n\\pgfusepath{fill}\n",
                prim->verts[0].xyz[0] - 0.5 * prim->width, prim->verts[0].xyz[1] - 0.5 * prim->width, prim->width, prim->width);
        break;
    case GL2PS_LINE:
        gl2psPrintPGFColor(prim->verts[0].rgba);
        if (gl2ps->lastlinewidth != prim->width)
...
static void gl2psPrintPGFBeginViewport(int viewport[4])
{
    ...
    if (gl2ps->options & GL2PS_DRAW_BACKGROUND)
    {
        ...
        fprintf(gl2ps->stream, "\\pgfpathrectangle{\\pgfpoint{%dpt}{%dpt}}" "{\\pgfpoint{%dpt}{%dpt}}\n" "\\pgfusepath{fill}\n", x, y, w, h);
    }
...
GL2PSDLL_API int gl2psTextOpt(const char *str, const char *fontname, short fontsize, int alignment, float angle)
{
...
GL2PSDLL_API int gl2psDrawPixels(int width, int height, int xorig, int yorig, int format, int type, const void *pixels)
{
    ...
    {
        gl2psMsg(GL2PS_ERROR, "gl2psDrawPixels only implemented for " "joglGL_RGB/joglGL_RGBA, joglGL_FLOAT pixels");
        return GL2PS_ERROR;
    }
