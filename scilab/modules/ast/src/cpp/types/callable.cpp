...
bool Callable::invoke(typed_list & in, optional_list & opt, int _iRetCount, typed_list & out, ast::ConstVisitor & execFunc, const ast::Exp & e)
{
    ...
    if (Ret == Callable::Error)
    {
        ...
        // error message thrown in exeception are displayed
        // in this case the error message is already displayed
        throw ast::ScilabError(L"", ConfigVariable::getLastErrorNumber(), e.getLocation());
    }
