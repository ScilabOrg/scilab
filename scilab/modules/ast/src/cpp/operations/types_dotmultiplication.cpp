...
InternalType* dotmul_M_M<Double, Polynom, Polynom>(Double* _pL, Polynom* _pR)
{
        ...
        if (isComplexL)
        {
            if (isComplexR)
            {
                for (int i = 0 ; i < iSize ; ++i)
                {
                    ...
                    dotmul(pdblRR, pdblRI, (long long)iSPSize, pdblLR[i], pdblLI[i], pdblOutR, pdblOutI);
                    pSP[i] = pSPOut;
                }
            }
            else
            {
                for (int i = 0 ; i < iSize ; ++i)
                {
                    ...
                    double* pdblOutR = pSPOut->get();
                    double* pdblOutI = pSPOut->getImg();
                    dotmul(pdblRR, (long long)iSPSize, pdblLR[i], pdblLI[i], pdblOutR, pdblOutI);
                    pSP[i] = pSPOut;
                }
            }
        }
        else
        {
            if (isComplexR)
            {
                for (int i = 0 ; i < iSize ; ++i)
                {
                    ...
                    double* pdblOutR = pSPOut->get();
                    double* pdblOutI = pSPOut->getImg();
                    dotmul(pdblRR, pdblRI, (long long)iSPSize, pdblLR[i], pdblOutR, pdblOutI);
                    pSP[i] = pSPOut;
                }
            }
            else
            {
                for (int i = 0 ; i < iSize ; ++i)
                {
                    ...
                    double* pdblOutR = pSPOut->get();
                    dotmul(pdblRR, (long long)iSPSize, pdblLR[i], pdblOutR);
                    pSP[i] = pSPOut;
                }
            }
        }
    ...
    if (isComplexL)
    {
        if (isComplexR)
        {
            for (int i = 0 ; i < iSize ; i++)
            {
                dotmul(pSPR[i]->get(), pSPR[i]->getImg(), (long long)pSPR[i]->getSize(), pdblR[i], pdblI[i], pSP[i]->get(), pSP[i]->getImg());
            }
        }
        else
        {
            for (int i = 0 ; i < iSize ; i++)
            {
                dotmul(pSPR[i]->get(), (long long)pSPR[i]->getSize(), pdblR[i], pdblI[i], pSP[i]->get(), pSP[i]->getImg());
            }
        }
    }
    else
    {
        if (isComplexR)
        {
            for (int i = 0 ; i < iSize ; i++)
            {
                dotmul(pSPR[i]->get(), pSPR[i]->getImg(), (long long)pSPR[i]->getSize(), pdblR[i], pSP[i]->get(), pSP[i]->getImg());
            }
        }
        else
        {
            ...
            for (int i = 0 ; i < iSize ; i++)
            {
                dotmul(pSPR[i]->get(), (long long)pSPR[i]->getSize(), pdblR[i], pSP[i]->get());
            }
        }
    }
