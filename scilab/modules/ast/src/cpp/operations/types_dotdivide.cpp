...
InternalType* dotdiv_M_M<Double, Sparse, Sparse>(Double* _pL, Sparse* _pR)
{
    ...
    if (_pL->isScalar())
    {
        ...
        if (_pL->isComplex())
        {
            ...
            return _pR->divideL(dbl);
        }
        else
        {
            return _pR->divideL(_pL->get(0));
        }
    }
...
InternalType* dotdiv_M_M<Sparse, Double, Sparse>(Sparse* _pL, Double* _pR)
{
    if (_pR->isScalar())
    {
        ...
        if (_pR->isComplex())
        {
            std::complex<double> dbl(_pR->get(0), _pR->getImg(0));
            return _pL->divideR(dbl);
        }
        else
        {
            return _pL->divideR(_pR->get(0));
        }
    }
    ...
    if (_pL->isScalar())
    {
        ...
        InternalType* pIT = GenericDotTimes(_pR, pD);
        delete pD;
        return pIT;
    }
