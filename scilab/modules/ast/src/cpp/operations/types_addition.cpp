...
template<> InternalType* add_M_M<Polynom, Polynom, Polynom>(Polynom* _pL, Polynom* _pR)
{
    ...
    if (_pL->isScalar())
    {
        ...
        for (int i = 0 ; i < _pR->getSize() ; i++)
        {
            ...
            for (int j = 0 ; j < Min(pRank1[0], pRank2[i]) + 1 ; j++)
            {
                pRR[j] = p1R[j] + p2R[j];
            }

            double *pTemp = (pRank1[0] > pRank2[i] ? p1R : p2R);
            for (int j = Min(pRank1[0], pRank2[i]) + 1 ; j < Max(pRank1[0], pRank2[i]) + 1 ; j++)
            {
                pRR[j] = pTemp[j];
            }
    ...
    if (_pR->isScalar())
    {
        ...
        for (int i = 0 ; i < _pL->getSize() ; i++)
        {
            ...
            for (int j = 0 ; j < Min(pRank1[i], pRank2[0]) + 1 ; j++)
            {
                pRR[j] = p1R[j] + p2R[j];
            }

            double *pTemp = (pRank1[i] > pRank2[0] ? p1R : p2R);
            for (int j = Min(pRank1[i], pRank2[0]) + 1 ; j < Max(pRank1[i], pRank2[0]) + 1 ; j++)
            {
                pRR[j] = pTemp[j];
            }
