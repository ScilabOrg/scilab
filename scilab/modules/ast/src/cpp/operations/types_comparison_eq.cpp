...
{
    //pending changes
    if ((_pL->getSize() != _pR->getSize()) && (_pL->getSize() != 1) && (_pR->getSize() != 1))
    {
        return new Bool(false);
    }
...
InternalType* compequal_MCR_MCR(T *_pL, U *_pR)
{
    ...
    {
        types::MacroFile* pL = _pL->getAs<types::MacroFile>();
        ret = *pL == *_pR;
    }
    else if (_pL->getType() == GenericType::ScilabMacro)
    {
        types::Macro* pL = _pL->getAs<types::Macro>();
        if (_pR->getType() == GenericType::ScilabMacroFile)
        {
            types::MacroFile* pR = _pR->getAs<types::MacroFile>();
            ret = *pR == *pL;
        }
        else
        {
