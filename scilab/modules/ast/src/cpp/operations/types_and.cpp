...
InternalType* GenericLogicalAnd(InternalType* _pL, InternalType* _pR)
{
    ...
    and_function and = pAndfunction[_pL->getId()][_pR->getId()];
    if ( and )
    {
        pResult = and (_pL, _pR);
        if (pResult)
...
InternalType* and_M_M(T *_pL, U *_pR)
{
    ...
    and (_pL->get(), (long long)_pL->getSize(), _pR->get(), pOut->get());
    return pOut;
}
...
InternalType* and_S_M(T *_pL, U *_pR)
{
    ...
    and (_pL->get(0), (size_t)_pR->getSize(), _pR->get(), pOut->get());
    return pOut;
}
...
InternalType* and_M_S(T *_pL, U *_pR)
{
    ...
    and (_pL->get(), (size_t)_pL->getSize(), _pR->get(0), pOut->get());
    return pOut;
}
...
{
    O* pOut = new O(_pL->getDims(), _pL->getDimsArray());
    and (_pL->get(0), _pR->get(0), pOut->get());
    return pOut;
}
