...
bool updateStructHistory(std::list<std::vector<std::pair<int, types::InternalType*> > >& structHistory, std::list<const ast::Exp*>& fieldHistory, std::list<types::typed_list*>& argHistory, types::InternalType* _pAssignValue, const int _iLoop)
{
    std::vector<std::pair<int, InternalType*> > vectParent = structHistory.front();
    std::list<std::vector<std::pair<int, InternalType*> > >::iterator iterStuctH = structHistory.begin();
    std::list<typed_list*>::iterator iterArgH = argHistory.begin();
    iterStuctH++;
    iterArgH++;

    // modify field wich contain pCurParent by a new struct
    int iGrandParent = vectParent[_iLoop].first;
    InternalType* pITGrandParent = (*iterStuctH)[iGrandParent].second;

    if (pITGrandParent->isStruct())
    {
        Struct* pStruct = pITGrandParent->getAs<Struct>();

        //if (*iterArgH)
        //{
        //    InternalType* pIT = pStruct->extractWithoutClone(*iterArgH);
        //    pStruct = pIT->getAs<Struct>();
        //}

        pStruct->get(_iLoop)->set(((SimpleVar*)fieldHistory.front())->name_get().name_get(), _pAssignValue);
        vectParent[_iLoop].second = _pAssignValue;
    }
    else if (pITGrandParent->isTList())
    {
        TList* pTL = pITGrandParent->getAs<TList>();
        pTL->set(((SimpleVar*)fieldHistory.front())->name_get().name_get(), _pAssignValue);
        vectParent[_iLoop].second = _pAssignValue;
    }
    else
    {
                      ...
                      bool _bFromCallExp)
{
    ...
    else if (pVar)
    {
            ...
            for (int iLoop = 0; iLoop < vectParent.size(); iLoop++)
            {
                ...
                if (pCurParent->isStruct())
                {
                    ...
                    if (_pAssignValue)
                    {
                        ...
                        if (_bFromCallExp)
                        {
                            pOut.push_back(std::pair<int, InternalType*>(iLoop, pStructParent));
                            continue;
                        }
                ...
                else if (pCurParent->isTList() || pCurParent->isMList())
                {
                    ...
                    if (_pAssignValue && _bFromCallExp)
                    {
                        pOut.push_back(std::pair<int, InternalType*>(iLoop, pTL));
                        continue;
                    }
        ...
        reinsertHistory.push_front(std::vector<std::pair<int, InternalType*> >(vect));
    ...
    else if (pCall)
    {
        ...
        // evaluate a.b
        if (getStructFromExp(&pCall->name_get(), structHistory, fieldHistory, argHistory, reinsertHistory, _pAssignValue, bFromCallExp) == false)
        {
            return false;
        }
        ...
        if (_pAssignValue)
        {
            for (int iLoop = 0; iLoop < vectParent.size(); iLoop++)
            {
                ...
                else if (pCurParent->isTList())
                {
                    ...
                    InternalType* pITChildren = pCurParent->getAs<TList>()->getField(((SimpleVar*)fieldHistory.front())->name_get().name_get());
                    ...
                    // insert b(x,y) in a
                    pCurParent->getAs<TList>()->set(((SimpleVar*)fieldHistory.front())->name_get().name_get(), pIT);
                }
                else
                {
                    std::wostringstream os;
        ...
        else
        {
            ...
            for (int iLoop = 0; iLoop < vectParent.size(); iLoop++)
            {
                InternalType* pCurrent = vectParent[iLoop].second;
                if (pCurrent->isStruct())
                {
                    ...
                    InternalType* pIT = pCurStr->extractWithoutClone(pCurrentArgs);
                    vectParent[iLoop].second = pIT;
                    structHistory.pop_front();
                    structHistory.push_front(vectParent);
                }
                else if (pCurrent->isTList() || pCurrent->isMList())
                {
                    std::vector < std::pair<int, InternalType*>
                    > pOut;
                    ...
                    if (pArgOut.size() > 1 || pCurrent->isMList())
                    {
                        ...
                        vectParent[iLoop].second = pExtract;
                        structHistory.pop_front();
                        structHistory.push_front(vectParent);
                        continue;
                    }
