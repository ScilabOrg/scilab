...
bool getFieldsFromExp(const ast::Exp* _pExp, std::list<ExpHistory*>& fields)
{
    ...
    {
        return false;//getFieldsFromExp(&pCell->name_get(), fields);
    }
    else
    {
        return false;
...
types::InternalType* evaluateFields(std::list<ExpHistory*>& fields, InternalType* _pAssignValue)
{
    ...
    workFields.push_back(new ExpHistory(NULL, pFirstField->getExp(), pFirstField->getArgs(), pFirstField->getLevel(), pIT));
    ...
    while (iterFields != fields.end())
    {
        ...
        if (pITCurrent->isStruct())
        {
            ...
            if (iSizeStuct < pEH->getSizeFromArgs())
            {
            ...
            if (pEH->getArgs())
            {
                ...
                workFields.push_front(new ExpHistory(pEH, pEH->getExp(), NULL, pEH->getLevel(), pIT));
            }
            else
            {
                ...
                for (int iList = 0; iList < pLOut->getSize(); iList++)
                {
                    ...
                    ExpHistory* pEHChield = new ExpHistory(pEH, (*iterFields)->getExp(), (*iterFields)->getArgs(), (*iterFields)->getLevel(), pIT);
            ...
            if (pArgs && (*pArgs)[0]->isString() == false)
            {
                if (pArgs->size() > 1 || pITCurrent->isMList())
                {
                    ...
                    if ((*iterFields)->getExp() == NULL)
                    {
                        ...
                        workFields.push_back(new ExpHistory(pEH, NULL, (*iterFields)->getArgs(), (*iterFields)->getLevel(), pExtract));
                    ...
                    else
                    {
                        ...
                        workFields.push_front(new ExpHistory(pEH, pEH->getExp(), NULL, pEH->getLevel(), pExtract));
                ...
                else
                {
                    ...
                    if ((*iterFields)->getExp() == NULL)
                    {
                        ...
                        {
                            ExpHistory* pEHExtract = new ExpHistory(pEH, NULL, (*iterFields)->getArgs(), (*iterFields)->getLevel(), *iterVect);
                    ...
                    else
                    {
                        ...
                        for (iterVect = vectpIT.begin(); iterVect != vectpIT.end(); iterVect++, iLoop++)
                        {
                            ExpHistory* pEHExtract = new ExpHistory(pEH, pEH->getExp(), NULL, pEH->getLevel(), *iterVect);
                            pEHExtract->setWhereReinsert((int)(pdblArgs[iLoop] - 1));
            ...
            else
            {
                ...
                if (pEH->getArgs())
                {
                    if ((*iterFields)->getExp() == NULL)
                    {
                        // a('x')(y) => a.b(y)
                        // extract a(x) and push_BACK to extract next level
                        pEHChield = new ExpHistory(pEH, NULL, (*iterFields)->getArgs(), (*iterFields)->getLevel(), pExtract);
                        workFields.push_back(pEHChield);
                    }
                    else
                    {
                        // a('x').b
                        // extract a(x) and push_FRONT to extract b from a(x)
                        pEHChield = new ExpHistory(pEH, pEH->getExp(), NULL, pEH->getLevel(), pExtract);
                ...
                {
                    // a.x
                    pEHChield = new ExpHistory(pEH, (*iterFields)->getExp(), (*iterFields)->getArgs(), (*iterFields)->getLevel(), pExtract);
                    workFields.push_back(pEHChield);
                }
        ...
        else if (pITCurrent->isList())
        {
            ...
            if (pEH->getParent() && pEH->getParent()->getLevel() == pEH->getLevel())
            {
                ...
                {
                    ExpHistory* pEHExtract = new ExpHistory(pEH, pEH->getExp(), NULL, pEH->getLevel(), pL->get(iLoop));
            ...
            else
            {
                ...
                if (pEH->getArgs())
                {
                    ...
                    else
                    {
                        ...
                        if ((*iterFields)->getExp() == NULL)
                        {
                            ...
                            {
                                ExpHistory* pEHExtract = new ExpHistory(pEH, NULL, (*iterFields)->getArgs(), (*iterFields)->getLevel(), pL->get((int)pdblArgs[iLoop] - 1));
                                pEHExtract->setWhereReinsert((int)(pdblArgs[iLoop] - 1));
                                workFields.push_back(pEHExtract);
                            }
                        }
                        else
                        {
                            ...
                            for (int iLoop = 0; iLoop < pDblArgs->getSize(); iLoop++)
                            {
                                ExpHistory* pEHExtract = new ExpHistory(pEH, pEH->getExp(), NULL, pEH->getLevel(), pL->get((int)pdblArgs[iLoop] - 1));
                                pEHExtract->setWhereReinsert((int)(pdblArgs[iLoop] - 1));
                                workFields.push_front(pEHExtract);
                            }
                        }
                ...
                else
                {
                    ...
                    workFields.push_back(new ExpHistory(pEH, (*iterFields)->getExp(), (*iterFields)->getArgs(), (*iterFields)->getLevel(), pExtract));
                    workFields.back()->setReinsertion();
                }
            }
        }
    ...
    while (workFields.empty() == false)
    {
        ...
        if (pEH->getArgs())
        {
            ...
            InternalType* pIT = insertionCall(*(pEH->getExp()), pEH->getArgs(), pEH->getCurrent(), _pAssignValue);
            if (pIT == NULL)
            {
                std::wostringstream os;
                os << _W("Error in insertion of Struct.");
                throw ScilabError(os.str(), 999, pEH->getExp()->location_get());
            }
        ...
        else
        {
            ...
            if (pParent->isStruct())
            {
                ...
                {
                    for (int iStruct = 0; iStruct < pStruct->getSize(); iStruct++)
                    {
                        pStruct->get(iStruct)->set(pEH->getExpAsString(), _pAssignValue);
                    }
                }
            }
            else if (pParent->isTList() || pParent->isMList())
            {
                ...
                if (_pAssignValue->isListDelete() || (pTL->exists(pEH->getExpAsString()) == false))
                {
                    ...
                    InternalType* pIT = insertionCall(*(pEH->getParent()->getExp()), &args, pEH->getParent()->getCurrent(), _pAssignValue);
                    if (pIT == NULL)
                    {
                        std::wostringstream os;
    ...
    while (evalFields.empty() == false)
    {
        ...
        if (pEH->reinsertMe())
        {
            ...
            InternalType* pIT = insertionCall(*(pEHParent->getExp()), pParentArgs, pEHParent->getCurrent(), pEH->getCurrent());
            if (pIT == NULL)
            {
                std::wostringstream os;
                os << _W("Error in insertion of Struct.");
