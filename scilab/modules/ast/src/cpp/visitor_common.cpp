...
bool updateStructHistory(const ast::Exp* _pExp, std::list<std::vector<std::pair<int, types::InternalType*>>>& structHistory, std::list<const ast::Exp*>& fieldHistory, std::list<types::typed_list*>& argHistory, types::InternalType* _pAssignValue, const int _iLoop)
{
...
bool getStructFromExp(const ast::Exp* _pExp, std::list<std::vector<std::pair<int, types::InternalType*>>>& structHistory, std::list<const ast::Exp*>& fieldHistory, std::list<types::typed_list*>& argHistory, types::InternalType* _pAssignValue, bool _bFromCallExp)
{
    const FieldExp* pField      = dynamic_cast<const FieldExp*>(_pExp);
    const SimpleVar* pVar       = dynamic_cast<const SimpleVar*>(_pExp);
    const CallExp* pCall        = dynamic_cast<const CallExp*>(_pExp);
        ...
        else
        {
            ...
            for (int iLoop = 0; iLoop < vectParent.size(); iLoop++)
            {
                ...
                if (pCurParent->isStruct())
                {
                    ...
                    // set in parent all elements cloned
                    // in case where pStructParent is a struct wich contain
                    // only some elements of parent. a(args)
                    if (bCloned && argHistory.front())
                    {
                        pCurParent->getAs<Struct>()->insertWithoutClone(argHistory.front(), pStructParent);
                    }
                }
                ...
                else
                {
                    ...
                    if (structHistory.size() == 1)
                    {
                        ...
                        // reevaluate a
                        if (getStructFromExp(pExp, structHistory, fieldHistory, argHistory, _pAssignValue, _bFromCallExp) == false)
                        {
                            return false;
                        }
                    ...
                    {
                        updateStructHistory(_pExp, structHistory, fieldHistory, argHistory, new Struct(1, 1), iLoop);
                        vectParent = structHistory.front();
                        iLoop--;
                    }
                }
            } // for
        ...
        else
        {
            for (int iLoop = 0; iLoop < vectParent.size(); iLoop++)
            {
                ...
                else if (pCurrent->isTList() || pCurrent->isMList())
                {
                    ...
                    {
                        //// call overload
                        //InternalType* pExtract = callOverload(*_pExp, L"e", &pArgOut, pTL, NULL);
                        //InternalType* pInsert  = callOverload(*_pExp, L"i", &pArgOut, pExtract, pTL);

                        //updateStructHistory(_pExp, structHistory, fieldHistory, argHistory, pInsert, iLoop);
                        ////vectParent = structHistory.front();
