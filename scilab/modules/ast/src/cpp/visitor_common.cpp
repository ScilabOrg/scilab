            ...
            if (pCurrent->isList())
            {
                ...
                for (int i = 0; i < pLParent->getSize(); i++)
                {
                    ...
                    if (pCurParent->isStruct())
                    {
                        ...
                        List* pLChildren = pCurParent->getAs<Struct>()->extractFieldWithoutClone(fieldHistory.front()->name_get());
                        ...
                        for (int j = 0; j < pLChildren->getSize(); j++)
                        {
                            ...
                            // insert b(x,y) in a
                            pCurParent->getAs<Struct>()->get(j)->set(fieldHistory.front()->name_get(), pIT);
                        }
                    }
                }
            }
                ...
                for (int i = 0; i < pL->getSize(); i++)
                {
                    if (pL->get(i)->isStruct())
                    {
                        ...
                        if (iSizeStuct < *piMaxDim)
                        {
                            ...
                            {
                                for (int iterStruct = iSizeStuct; iterStruct < pCurStr->getSize(); iterStruct++)
                                {
                                    pCurStr->get(iterStruct)->addField(pFields->get(iterField));
                                }
                            }
                        }
