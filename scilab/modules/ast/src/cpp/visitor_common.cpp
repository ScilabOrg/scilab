...
static bool callStructOverload(std::wstring& _wstField, InternalType* _pValue, TList* _pL, InternalType** _pITRet)
{
    if (_pValue)
    {
        //insertion
        if (_pL->exists(_wstField))
        {
            //standard insertion
            return _pL->set(_wstField, _pValue);
        }
        else
        {
            ...
            std::wstring stOverload = L"%" + _pValue->getShortTypeStr() +  L"_i_" + _pL->getShortTypeStr();
    ...
    else
    {
        ...
        else
        {
            ...
            Callable::ReturnValue Ret = Overload::call(L"%" + _pL->getShortTypeStr() + L"_e", in, 1, out, &exec);
...
bool updateStructFromHistory(std::list<types::InternalType*>& structHistory, std::list<std::wstring>& fieldHistory)
{
    ...
    for (itWhere++; itWhere != structHistory.end(); itWhat++, itWhere++, itField++)
    {
        TList* pT = (*itWhere)->getAs<TList>();
        callStructOverload(*itField, *itWhat, pT, NULL);
    }
...
bool getStructFromExp(const ast::Exp* _pExp, std::list<types::InternalType*>& structHistory, std::list<std::wstring>& fieldHistory, typed_list** _pArgs, types::InternalType* _pAssignValue)
{
    const FieldExp* pField      = dynamic_cast<const FieldExp*>(_pExp);
    const SimpleVar* pVar       = dynamic_cast<const SimpleVar*>(_pExp);
    const CallExp* pCall        = dynamic_cast<const CallExp*>(_pExp);
    ...
    if (pField)
    {
        ...
        //evaluate head "y"
        if (getStructFromExp(pField->head_get(), structHistory, fieldHistory, &pArgs, NULL))
        {
            pVar    = dynamic_cast<const SimpleVar*>(pField->tail_get());

            InternalType* pHead = structHistory.front(); //last item
            if (pHead->isStruct())
            {
            ...
            else if (pHead->isTList() || pHead->isMList())
            {
                ...
                if (callStructOverload(wstField, _pAssignValue, pT, &pITRet) == false)
                {
                    std::wostringstream os;
                    if (pT->isMList())
                    {
    ...
    else if (pCall)
    {
        ...
        if (structHistory.empty())
        {
            ...
            if (getStructFromExp(&pCall->name_get(), structHistory, fieldHistory, &pReturnedArgs, NULL) == false)
            {
                return false;
            }
