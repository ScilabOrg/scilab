...
void RunVisitorT<T>::visitprivate(const SeqExp  &e)
{
    ...
    for (auto exp : e.getExps())
    {
                ...
                if (pIT->isCallable()) //to manage call without ()
                {
                    ...
                    catch (ast::ScilabError & se)
                    {
                        ...
                        if (pCall->isMacro() || pCall->isMacroFile())
                        {
                            ...
                            se.SetErrorMessage(se.GetErrorMessage() + szError + os.str());
                        }
                        else
                        {
                            se.SetErrorMessage(se.GetErrorMessage() + os.str());
                        }
                    }
                }
        ...
        catch (ScilabMessage& sm)
        {
            ...
            //    if (getResult() != NULL &&
            //        (getResult()->isMacro() || getResult()->isMacroFile() /*||
            //        getResult()->getAs<Callable>()->getName() == L"exec" ||
            //        getResult()->getAs<Callable>()->getName() == L"execstr"*/))
            //    {
            //        wostringstream os;
            //        PrintVisitor printMe(os);
