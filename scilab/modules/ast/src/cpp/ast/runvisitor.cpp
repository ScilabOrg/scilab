...
void RunVisitorT<T>::visitprivate(const ListExp &e)
{
    ...
    if (e.hasExplicitStep())
    {
        ...
        in.push_back(piStep);
        in.push_back(piEnd);
        Ret = Overload::call(L"%" + piStart->getShortTypeStr() + L"_b_" + piStep->getShortTypeStr(), in, 1, out, this);
    }
    else
    {
        // 1:2
        ...
        in.push_back(piEnd);
        Ret = Overload::call(L"%" + piStart->getShortTypeStr() + L"_b_" + piEnd->getShortTypeStr(), in, 1, out, this);
    }
