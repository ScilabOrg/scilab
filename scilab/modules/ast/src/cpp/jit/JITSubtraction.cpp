...
JITScilabPtr JITSubtraction::SS(JITScilabPtr & L, const analysis::TIType & Ltype, JITScilabPtr & R, const analysis::TIType & Rtype, const analysis::TIType & Otype, JITVisitor & jit)
{
    ...
    else if (Ltype.isfloating() && Rtype.isfloating())
    {
        if (Ltype.type == analysis::TIType::DOUBLE)
        {
            ...
            if (Rtype.type == analysis::TIType::DOUBLE)
            {
                ...
                {
                    ret = builder.CreateSub(l, r);
                    return jit.getScalar(ret, analysis::TIType::INT64);
                }
                else
                {
                    Cast::castInt64(l, jit);
                    Cast::castInt64(r, jit);
                    ret = builder.CreateFSub(l, r);
                }
            }
            else
            {
                ...
                ret = builder.CreateFSub(l, r_reim.first);
                llvm::Value * neg = builder.CreateFNeg(r_reim.second);
                return jit.getScalar(ret, neg, Otype.type);
            }
        }
        else
        {
            if (Rtype.type == analysis::TIType::DOUBLE)
            {
                ...
                ret = builder.CreateFSub(l_reim.first, r);
            ...
            else
            {
                ...
                std::pair<llvm::Value *, llvm::Value *> r_reim = R->loadReIm(jit);
                llvm::Value * ret_re = builder.CreateFSub(l_reim.first, r_reim.first);
                llvm::Value * ret_im = builder.CreateFSub(l_reim.second, r_reim.second);
