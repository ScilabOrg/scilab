...
llvm::ExecutionEngine * JITVisitor::InitializeEngine(llvm::Module * module, llvm::TargetMachine ** target)
{
    ...
    eb.setEngineKind(llvm::EngineKind::JIT).setMCJITMemoryManager(std::move(std::unique_ptr<llvm::RTDyldMemoryManager> {new MemoryManager()})).setRelocationModel(llvm::Reloc::Default/*PIC_*//*DynamicNoPIC*//*Static*/).setCodeModel(llvm::CodeModel::JITDefault/*Small*/).setTargetOptions(opt).setErrorStr(&err);
#else
