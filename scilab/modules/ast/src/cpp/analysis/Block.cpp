...
namespace analysis
{
    ...
    Block::Block(DataManager * _dm) : dm(_dm), parent(nullptr), exp(nullptr), gvn(&dm->getDefaultGVN()), id(0) { }

    Block::~Block()
    {
        std::for_each(blocks.begin(), blocks.end(), [](Block * b){ delete b; });
    }
    ...
    Info & Block::putSymsInScope(const symbol::Symbol & sym)
    {
        ...
        Block * block = getDefBlock(sym, it);
        if (!block)
        {
            dm->root->setDefaultData(sym);
            block = dm->root;
        }
        ...
        return symMap.find(sym)->second;
    }

    Block * Block::getDefBlock(const symbol::Symbol & sym, std::map<symbol::Symbol, Info>::iterator & it)
    {
        it = symMap.find(sym);
        if (it != symMap.end())
        {
        ...
        else if (parent)
        {
            ...
            return parent->getDefBlock(sym, it);
        }
    ...
    {
        TIType ty(TIType::MACRO);
        return addDefine(dec->getSymbol(), ty, dec);
    }
    ...
    std::vector<TIType> Block::addCall(AnalysisVisitor & visitor, const unsigned int lhs, const symbol::Symbol & sym, std::vector<TIType> & in, ast::CallExp * callexp)
    {
        ...
        {
            bool exists;
            type = DataManager::getSymInScilabContext(getGVN(), sym, exists, pIT);
        } //function y=add(a,b); y=a+b;end;jit("c=12;d=13;e=add(c,d)")
        ...
        case TIType::FUNCTION :
        {
            if (lhs > 0)
            {
                out[0] = Checkers::check(getGVN(), sym.getName(), in);
            }
            break;
        }
        case TIType::MACRO :
        {
            ...
            else
            {
                ...
                {
                    bool exists;
                    TIType _type = DataManager::getSymInScilabContext(getGVN(), sym, exists, pIT);
                    ExistingMacroDef macrodef(*static_cast<types::Macro *>(pIT));
                    visitor.getPMC().getOutTypes(visitor, &macrodef, in, out);
                }
            }
    ...
    {
        unsigned int n = block.blocks.size();
        out << L"Table " << block.id << L" (" << n << ((n >= 2) ? L" children):" : L" child):") << std::endl;
...
//#ifdef DEBUG_DATAMANAGER
        ...
        //}
        std::for_each(block.blocks.begin(), block.blocks.end(), [&](Block * b){ out << *b << std::endl; });
//#endif
