...
EXTERN_AST bool getStructFromExp(const ast::Exp* _pExp, std::list<std::vector<std::pair<int, types::InternalType*>>>& structHistory, std::list<const ast::Exp*>& fieldHistory, std::list<types::typed_list*>& argHistory, types::InternalType* _pAssignValue, bool _bFromCallExp);
EXTERN_AST bool updateStructFromHistory(std::list<types::InternalType*>& structHistory, std::list<std::wstring>& fieldHistory, std::list<types::typed_list*>& argHistory);
