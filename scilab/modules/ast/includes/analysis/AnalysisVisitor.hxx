...
public:
    ...
    typedef std::map<symbol::Symbol, std::shared_ptr<CallAnalyzer>> MapSymCall;
...
private:
    ...
    void visit(ast::SimpleVar & e)
    {
        ...
        double val;
        if (info.getDblValue(val))
        {
            res.getConstant().set(val);
        }
        else if (GVN::Value * gvnValue = info.getGVNValue())
        {
            res.getConstant().set(gvnValue);
        }
    ...
    void visit(ast::CallExp & e, const unsigned int lhs)
    {
        if (e.getName().isSimpleVar())
        {
            ...
            MapSymCall::iterator it = symscall.find(sym);
    ...
    void visit(ast::AssignExp & e)
    {
        ...
        if (e.getLeftExp().isSimpleVar())
        {
            ...
            else
            {
                ...
                double value;
                if (asDouble(e.getRightExp(), value) || RR.getDblValue(value))
                {
