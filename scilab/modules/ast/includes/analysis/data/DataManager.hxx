...
namespace analysis
{
	...
	inline void addGlobal(const symbol::Symbol & sym)
        ...
        inline void registerData(Data * _data, int line, char * file)
            {
		if (_data)
		{
/*		    bool ok = true;
...
*/
	...
	inline Block * getCurrent() { return current; }
	...
	inline TIType getType(const symbol::Symbol & sym)
	    {
		Block * parent = getCurrent();
		std::map<symbol::Symbol, Info>::iterator it;
		Block * block = parent->getDefBlock(sym, it);
        ...
        inline Info & getInfo(const symbol::Symbol & sym)
            {
                std::map<symbol::Symbol, Info>::iterator it;
		Block * block = current->getDefBlock(sym, it);
		if (block)
		{
		    return it->second;
		}
