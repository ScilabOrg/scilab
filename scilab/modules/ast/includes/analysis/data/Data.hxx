...
namespace analysis
{
    ...
    struct Data
    {
        ...
        Data(const bool _known, const symbol::Symbol & sym) : known(_known), valid(true) { sharedSyms.emplace(sym); }
        Data(const Data & d) : known(d.known), valid(true), sharedSyms(d.sharedSyms) {  }

        inline void clear() { sharedSyms.clear(); }
        ...
        inline bool hasOneOwner() { return valid && known && (sharedSyms.size() == 1); }
        inline bool isDeletable() { return valid && known && sharedSyms.empty(); }
