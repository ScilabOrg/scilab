...
struct Info
{
    ...
    Info(Data * _data = nullptr) : R(false), W(false), O(false), isint(false), local(Local::TRUE), cleared(false), exists(true), data(_data), exp(nullptr) { }
    ...
    inline bool getDblValue(double & val) const
    {
        if (constant.getKind() == ConstantValue::DOUBLE)
        {
            val = constant.get<double>();
            return true;
        }
        return false;
    }
    ...
    {
        if (constant.getKind() == ConstantValue::GVNVAL)
        {
            return constant.get<GVN::Value *>();
        }
        return nullptr;
    }
    ...
    friend std::wostream & operator<<(std::wostream & out, const Info & info)
    {
        out << L"Type: " << info.type << L" - RWO:"
            ...
            << L" - local:" << (info.local == Local::TRUE ? L"T" : (info.local == Local::FALSE ? L"F" : L"U"))
