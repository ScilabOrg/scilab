...
namespace jit
{
    namespace BaseFunctions
    {
        ...
        inline void powM(double * X, const int x_r, uint64_t N)
        {
            ...
            double * T1 = new double[size];
            const uint64_t l2N = __builtin_ctzll(N);
            if ((l2N & 1) && (N == (1 << l2N)))
            {
                // N is a power of 4
                while (N >>= 2)
        ...
        template<typename T, typename U, typename V = typename ScilabType<T, U>::type>
        inline V modulo(T x, U y) { return x % y; }

        inline double modulo(double x, double y) { return std::fmod(x, y); }

        template<typename T, typename U = T>
        inline U opposite(T x) { return -x; }

        template<typename T, typename U = T>
        inline U pow2(T x) { return x * x; }

        template<typename T, typename U = T>
        inline U powm1(T x) { return 1 / x; }

        template<typename T>
        inline int asbool(T x) { return x != 0; }

        template<typename T>
        inline int boolneg(T x) { return !x; }

        template<typename T>
        inline bool asBool(T x) { return x != 0; }

        template<typename T>
        inline bool Boolneg(T x) { return !x; }

        template<typename T, typename U, typename V = typename ScilabType<T, U>::type>
        inline V binand(T x, U y) { return x & y; }

        template<typename T, typename U, typename V = typename ScilabType<T, U>::type>
        inline V binor(T x, U y) { return x | y; }

        template<typename T, typename U, typename V = typename ScilabType<T, U>::type>
        inline V binxor(T x, U y) { return x ^ y; }

        template<typename T, typename U = T>
        inline U binneg(T x) { return ~x; }

        template<typename T, typename U, typename V = bool>
        inline V booland(T x, U y) { return x && y; }

        template<typename T, typename U, typename V = bool>
        inline V boolor(T x, U y) { return x || y; }

        template<typename T, typename U, typename V = bool>
        inline V boolxor(T x, U y) { return !x != !y; }

        template<typename T, typename U = bool>
        inline U boolneg(T x) { return !x; }

        template<typename T>
        inline bool isStrictNegative(T x) { return x < 0; }

        template<typename T>
        inline bool isStrictPositive(T x) { return x > 0; }

        template<typename T>
        inline bool isAbsGreaterThan1(T x) { return std::fabs(x) > 1; }

        template<typename T>
        inline bool getFalse(T x) { return false; }

        template<typename T>
        inline bool getTrue(T x) { return true; }
