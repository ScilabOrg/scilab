...
void visitprivate(const AssignExp  &e)
{
    ...
    try
    {
        ...
        if (pCall)
        {
            ...
            {
                //manage a.b(1) = x
                //this part is only for complex type ( struct/list/mlist/tlist)
                ...
                if (getStructFromExp(pCall, structHistory, fieldHistory, &pArgs, pITR))
                {
                    //after insertion of right part, we have to insert child in parent.
                    updateStructFromHistory(structHistory, fieldHistory);
            ...
            if (pOut != NULL)
            {
                if (bNew)
                {
                    if (pVar == NULL && pIT->isHandle() == false)
                    {
                        ...
                        std::list<InternalType*> structHistory;
                        std::list<std::wstring> fieldHistory;
                        if (getStructFromExp(&pCall->name_get(), structHistory, fieldHistory, NULL, pOut))
                        {
                            //after insertion of right part, we have to insert child in parent.
                            updateStructFromHistory(structHistory, fieldHistory);
        ...
        if (pField)
        {
            ...
            //try to find struct or handle
            {
                ...
                std::list<InternalType*> structHistory;
                std::list<std::wstring> fieldHistory;
                if (getStructFromExp(pField, structHistory, fieldHistory, NULL, pIT))
                {
                    //after insertion of right part, we have to insert child in parent.
                    updateStructFromHistory(structHistory, fieldHistory);
                }
