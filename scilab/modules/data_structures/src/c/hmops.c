...
int C2F(intehm)()
{
    ...
    if ( ! get_hmat(Rhs, &H) )
    {
        Scierror(999, _("%s: Wrong type for input argument(s): hypermatrix expected.\n"),"hmops");
        return 0;
    }
    ...
    {
        Scierror(999, _("%s: Incompatible hypermatrix extraction.\n"),"hmops");
        return 0;
    }
    ...
    for ( i = 1 ; i <= nb_index_vectors ; i++ )
    {
        ...
        {
            Scierror(999, _("%s: Bad (%d th) index in hypermatrix extraction. "),"hmops", i);
            return 0;
        }
