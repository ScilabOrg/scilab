...
function M = hypermat(dims, v)
    ...
    //
    // If v is a matrix or an hypermatrix whose dimensions dims_v = [n1, n2, ... , nv] are
    // exactly dims[1:length(dim_v)], M is created with V and padded with 0
    // This allows to cast a matrix or hypermatrix v in the new dimensions dims
    ...
    is_hypermat = (typeof(v) == "hypermat")
    if is_hypermat | (size(v, "r") ~= 1 & size(v, "c") ~ = 1)
    // v is not a vector
        dims_v = size(v);
        if length(dims_v) > length(dims)
            error(msprintf(_("%s: Wrong size for input argument #%d: Must be the the values of input argument #%d across at least %d dimensions.\n"), "hypermat", 2, 1, 2));
        end

        // Express v as a vector
        if is_hypermat
            v = (v.entries)
        else
            v = matrix(v, size(v, '*'), 1)
        ...
        // Pad with 0
        padding = zeros(double(prod(dims)) - size(v, '*'), 1)
        v = [v; padding]
    end
