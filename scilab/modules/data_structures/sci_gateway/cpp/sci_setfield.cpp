    ...
    else if (pIndex->isDouble())
    {
        ...
        for (int j = 0; j < pSt->getSize(); ++j)
        {
            ...
            for (int i = 0; i < pDlIndex->getSize(); ++i)
            {
                ...
                {
                    types::GenericType* pGTData = pData->getAs<types::GenericType>();
                    types::Double* pDblData = NULL;
                    types::Int32* pIntData = NULL;

                    if (pData->isDouble())
                    {
                        pDblData = pData->getAs<types::Double>();
                    }
                    if (pData->isInt())
                    {
                        pIntData = pData->getAs<types::Int32>();
                    }


                    if (pGTData->getSize() < 2)
                    {
                        Scierror(999, _("%s: Wrong size for input argument #%d: Vector expected.\n"), "setfield", 2);
                        return types::Function::Error;
                    }

                    int* piDims = new int[pGTData->getSize()];

                    if (pData->isDouble())
                    {
                        for (int i = 0; i < pGTData->getSize(); ++i)
                        {
                            piDims[i] = (int)pDblData->get(i);
                        }

                        pSt->resize(piDims, pGTData->getSize());
                    }
                    if (pData->isInt())
                    {
                        pSt->resize(pIntData->get(), pGTData->getSize());
                    }
