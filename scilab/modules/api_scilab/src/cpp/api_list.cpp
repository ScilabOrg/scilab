...
void pushListAddress(int _iRhsPos, int* _piAddr, int _iLast)
{
...
void pushNamedListAddress(std::string _stNamedList, int* _piAddr, int _iLast)
{
...
void popListAddress(int _iRhsPos)
{
...
void popNamedListAddress(std::string _stNamedList)
{
...
int* getLastListAddress(int _iRhsPos)
{
    StackList::iterator it = stackListPosition.find(_iRhsPos);
...
int* getLastNamedListAddress(std::string _stNamedList)
{
    StackNamedList::iterator it = stackNamedListPosition.find(_stNamedList);
...
//get address list
void getListAdressses(int _iRhsPos, int** _piAddresses)
{
    StackList::iterator it = stackListPosition.find(_iRhsPos);
    if(it == stackListPosition.end() || it->second->empty() || _piAddresses == NULL || *_piAddresses == NULL)
    {
        return;
    }
...
void getNamedListAdressses(std::string _stName, int** _piAddresses)
{
    StackNamedList::iterator it = stackNamedListPosition.find(_stName);
    if(it == stackNamedListPosition.end() || it->second->empty() || _piAddresses == NULL || *_piAddresses == NULL)
    {
        return;
    }
...
int getDepthNamedList(std::string _stNamedList)
{
    StackNamedList::iterator it = stackNamedListPosition.find(_stNamedList);
    if(it == stackNamedListPosition.end() || it->second->empty())
    {
...
void cleanStackListAddress(int _iRhsPos)
{
    StackList::iterator it = stackListPosition.find(_iRhsPos);
    if(it != stackListPosition.end())
    {
...
void cleanStackListAddress(std::string _stNamedList)
{
    StackNamedList::iterator it = stackNamedListPosition.find(_stNamedList);
    if(it != stackNamedListPosition.end())
    {
...
SciErr createCommonListInNamedList(void* _pvCtx, const char* _pstName, int* _piParent, int _iItemPos, int _iListType, int _iNbItem, int** _piAddress)
{
    ...
    sciErr = createCommonListInList(_pvCtx, Top, _pstName, _piParent, _iItemPos, _iListType, _iNbItem, _piAddress, 1);
    if(sciErr.iErr)
    {
    ...
    else
    {
        ...
        pushNamedListAddress(_pstName, *_piAddress, _iItemPos == _piParent[1]);
    }
...
SciErr createVoidInNamedList(void* _pvCtx, const char* _pstName, int* /*_piParent*/, int _iItemPos)
{
    ...
    int* piChildAddr    = NULL;
    int* piOffset       = NULL;
    int* piParent       = getLastNamedListAddress(_pstName);
