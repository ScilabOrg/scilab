...
SciErr getNamedMatrixOfIntegerPrecision(void *_pvCtx, const char *_pstName, int *_piPrecision)
{
    ...
    if (sciErr.iErr)
    {
        addErrorMessage(&sciErr, API_ERROR_GET_NAMED_INT_PRECISION, _("%s: Unable to get precision of variable \"%s\""),
                        "getNamedMatrixOfIntegerPrecision", _pstName);
        return sciErr;
    }
    ...
    {
        addErrorMessage(&sciErr, API_ERROR_INVALID_TYPE, _("%s: Invalid argument type, %s excepted"), "getNamedMatrixOfIntegerPrecision",
                        _("int matrix"));
        return sciErr;
    }
...
static int getCommonScalarInteger(void *_pvCtx, int *_piAddress, int _iPrec, void **_pvData)
{
    ...
    if (sciErr.iErr)
    {
        addErrorMessage(&sciErr, API_ERROR_GET_SCALAR_INTEGER, _("%s: Unable to get argument #%d"), "getScalarInteger",
                        getRhsFromAddress(_pvCtx, _piAddress));
        printError(&sciErr, 0);
        return sciErr.iErr;
    }
...
int createNamedScalarInteger16(void *_pvCtx, const char *_pstName, short _sData)
{
    ...
    if (sciErr.iErr)
    {
        addErrorMessage(&sciErr, API_ERROR_CREATE_NAMED_SCALAR_INT, _("%s: Unable to create variable in Scilab memory"),
