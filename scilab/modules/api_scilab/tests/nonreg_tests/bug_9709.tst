// =============================================================================
// Scilab ( http://www.scilab.org/ ) - This file is part of Scilab
// Copyright (C) 2011 - DIGITEO - Allan CORNET
//
//  This file is distributed under the same license as the Scilab package.
// =============================================================================
//
// <-- ENGLISH IMPOSED -->
//
// <-- JVM NOT MANDATORY -->
//
// <-- Non-regression test for bug 9709 -->
//
// <-- Bugzilla URL -->
// http://bugzilla.scilab.org/9709
//
// <-- Short Description -->
// createNamedXXX did not return a error when varname is a variable protected by predef

BUGNAME = "bug_9709";
ilib_verbose(0);
if haveacompiler() then
  cd TMPDIR;
  mkdir(BUGNAME);
  cd(BUGNAME);
  DEST = TMPDIR + "/" + BUGNAME + "/" + BUGNAME + ".c";
  copyfile(SCI + "/modules/api_scilab/tests/nonreg_tests/" + BUGNAME + ".c", DEST);
  assert_checkequal(isfile(DEST), %T);
  ilib_build("lib" + BUGNAME,[BUGNAME,"sci_" + BUGNAME], BUGNAME + ".c", []);
  exec loader.sce;
  assert_checkequal(execstr("bug_9709()", "errcatch"), 999);
end

