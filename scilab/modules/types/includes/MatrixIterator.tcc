...
namespace {
    ...
    bool notZero( std::complex<double> c)
    {
...
{
    Accessor(Eigen::SparseMatrix<std::complex<double>, _Options, _Index> & s_, int r_, int c_): s(s_), r(r_), c(c_) {}
    ...
    {
//        std::cerr<<"inserting "<<v<<" @"<<r<<","<<c<<std::endl;
        if( v != 0.)
        {
...
private:
    Eigen::SparseMatrix<std::complex<double>, _Options, _Index>& s;
    int r, c;
};
 ...
 */
template<>
struct IteratorFromVar<Adjacency>:std::iterator<std::forward_iterator_tag, std::pair<int, int> >
{
