...
namespace types
{
    ...
    protected :
        ...
        T*                      m_pImgData;
                                ...
                                ArrayOf() : GenericType(), m_bComplex(false), m_piDims(NULL), m_iDims(0), m_pRealData(NULL), m_pImgData(NULL) {}
        virtual                 ~ArrayOf() {}
    ...
    public :
        ...
        bool                    set(int _iPos, T _pdata);
        bool                    set(int _iRows, int _iCols, T _data);
        bool                    set(T* _data);
        bool                    set(const T* _data);
        T*                      get() const;
        T                       get(int _iPos);
    ...
    template <typename T>
    int ArrayOf<T>::get_dims()
    {
        return m_iDims;
    }

    template <typename T>
    int* ArrayOf<T>::get_dims_array()
    {
        return m_piDims;
    }

    template <typename T>
	bool ArrayOf<T>::isComplex()
	{
		return m_bComplex;
	}

    template <typename T>
	void ArrayOf<T>::complex_set(bool _bComplex)
	{
		if(_bComplex == false)
		{
			if(isComplex() == true)
    ...
    template <typename T>
    void ArrayOf<T>::create(int* _piDims, int _iDims, T** _pRealData, T** _pImgData)
    {
        //try
        //{
            m_iSize     = 1;
    ...
    template <typename T>
	void ArrayOf<T>::delete_img(bool _bSetReal)
	{
		if(m_pImgData != NULL)
		{
			delete[] m_pImgData;
    ...
    template <typename T>
	void ArrayOf<T>::delete_all(bool _bSetReal)
	{
		delete_real();
    ...
    template <typename T>
    int ArrayOf<T>::get_index(int* _piIndexes)
    {
        int idx = 0;
        int iMult = 1;
        for(int i = 0 ; i < m_iDims ; i++)
    ...
    template <typename T>
    int ArrayOf<T>::get_index_with_dims(int* _piIndexes, int* _piDims, int _iDims)
    {
        int idx = 0;
        int iMult = 1;
    ...
    template <typename T>
    void ArrayOf<T>::get_indexes(int _iIndex, int* _piIndexes)
    {
        get_indexes_with_dims(_iIndex, _piIndexes, m_piDims, m_iDims);
    }

    template <typename T>
    void ArrayOf<T>::get_indexes_with_dims(int _iIndex, int* _piIndexes, int* _piDims, int _iDims)
    {
        int iMul = 1;
        for(int i = 0 ; i < _iDims ; i++)
        {
    ...
    template <typename T>
    bool ArrayOf<T>::set(int _iPos, T _data)
    {
    ...
    template <typename T>
    bool ArrayOf<T>::set(int _iRows, int _iCols, T _data)
    {
    ...
    template <typename T>
    bool ArrayOf<T>::set(const T* _pdata)
    {
        if(m_pRealData == NULL)
        {
            return false;
    ...
    template <typename T>
    T* ArrayOf<T>::get() const
    {
        return m_pRealData;
    }

    template <typename T>
    T ArrayOf<T>::get(int _iPos)
    {
        if(m_pRealData)
        {
    ...
    template <typename T>
    T ArrayOf<T>::get(int _iRows, int _iCols)
    {
        int piIndexes[2] = {_iRows, _iCols};
    ...
    template <typename T>
    bool ArrayOf<T>::set_img(int _iPos, T _data)
    {
    ...
    template <typename T>
    bool ArrayOf<T>::set_img(int _iRows, int _iCols, T _data)
    {
        int piIndexes[2] = {_iRows, _iCols};
    ...
    template <typename T>
    bool ArrayOf<T>::set_img(T* _pdata)
    {
        if(m_pRealData == NULL)
        {
            return false;
    ...
    template <typename T>
    bool ArrayOf<T>::set_img(const T* _pdata)
    {
        if(m_pRealData == NULL)
        {
            return false;
    ...
    template <typename T>
    T* ArrayOf<T>::get_img() const
    {
        return m_pImgData;
    }

    template <typename T>
    T ArrayOf<T>::get_img(int _iPos)
    {
        if(m_pImgData)
        {
    ...
    template <typename T>
    T ArrayOf<T>::get_img(int _iRows, int _iCols)
    {
        int piIndexes[2] = {_iRows, _iCols};
    ...
    template <typename T>
	ArrayOf<T>* ArrayOf<T>::extract(typed_list* _pArgs)
	{
		ArrayOf<T> *pOut    = NULL;
    ...
    InternalType* ArrayOf<T>::insert(typed_list* _pArgs, InternalType* _pSource)
    {
        ...
        else // _iDims < m_iDims
        {
            ...
            {
                Double* pDbl = pArg[i]->getAs<Double>();
                double* pIdx = pDbl->get();
            ...
            Double* pDbl = pArg[pArg.size() - 1]->getAs<Double>();
            double* pIdx = pDbl->get();
    ...
    template <typename T>
    bool ArrayOf<T>::resize(int _iRows, int _iCols)
    {
    ...
    template <typename T>
    InternalType* ArrayOf<T>::insert_new(typed_list* _pArgs, InternalType* _pSource)
	{
        typed_list pArg;
        ...
        {//manage : and $ in creation by insertion
            ...
            for(int i = 0 ; i < iDims ; i++)
            {
                ...
                {//undefine value
                    ...
                    Double* pNewArg = new Double(1, piMaxDim[i]);
                    for(int j = 0 ; j < piMaxDim[i] ; j++)
                    {
                        pNewArg->set(j, j+1);
                    }
    ...
    template <typename T>
	ArrayOf<T>* ArrayOf<T>::get_col_value(int _iPos)
	{
		ArrayOf<T>* pOut = NULL;
		if(_iPos < m_iCols)
		{
