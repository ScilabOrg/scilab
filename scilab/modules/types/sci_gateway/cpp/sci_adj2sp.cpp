...
Function::ReturnValue sci_adj2sp(typed_list &in, int nbRes, typed_list &out)
{
    ...
    {
        ScierrorW(999, _W("%s: Wrong number of input argument(s): %d to %d expected.\n"), L"adj2sp", 3, 4);
    ...
    if(in.size()==4)
    {
        ...
        {
            ScierrorW(999, _W("%ls: Wrong size for input arguments ''%s'': A vector of size %d expected.\n" ), L"adj2sp", 2);
            return Function::Error;
        }
        if((n > dims.real_get(0,0)) || (m > dims.real_get(0,1)))
        {
            ScierrorW(999, _W("%s: Incompatible input argument.\n"), L"adj2sp");
            return Function::Error;
        }
        n= dims.real_get(0,0);
        m= dims.real_get(0,1);
