...
Function::ReturnValue sci_inspectorGetItemCount(typed_list &in, int _iRetCount, typed_list &out)
{
    if(_iRetCount == 1)
    {
        int iItemCount = Inspector::getItemCount();
        int iUnreferencedItemCount = Inspector::getUnreferencedItemCount();
        Double* pOut = new Double(1, 2);
        pOut->set(0, iItemCount);
        pOut->set(1, iUnreferencedItemCount);
        out.push_back(pOut);
    }
    else
    {
