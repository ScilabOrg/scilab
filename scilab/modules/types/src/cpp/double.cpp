...
namespace types
{
	...
	bool Double::setReal(double *_pdblReal)
	{
		return set(_pdblReal);
	}

	bool Double::setReal(const double *_pdblReal)
	{
		return set(_pdblReal);
	}

    bool Double::setReal(int* _piReal)
    {
        bool ret = true;
	...
	bool Double::setReal(int _iPos, double _dblReal)
	{
		return set(_iPos, _dblReal);
	}

	bool Double::setReal(int _iRows, int _iCols, double _dblReal)
	{
	...
	bool Double::append(int _iRows, int _iCols, Double *_poSource)
	{
		...
		if(m_bComplex)
		{
			for(int iRow = 0 ; iRow < iRows ; iRow++)
			{
                ...
                int iOrigOffset = iCols * _poSource->getRows() + iRow;
                memcpy(m_pRealData + iDestOffset, _poSource->getReal() + iOrigOffset, iCols * sizeof(double));
                if(_poSource->isComplex())
                {
                    memcpy(m_pImgData + iDestOffset, _poSource->getImg() + iOrigOffset, iCols * sizeof(double));
                }
                else
                {
				...
				//{
				//	setReal(_iRows + iRow, _iCols + iCol, _poSource->getReal(iRow, iCols));
				//	setImg(_iRows + iRow, _iCols + iCol, _poSource->getImg(iRow, iCol));
				//}
			}
		}
		else
		{
            if(iRows != 1)
            {
                for(int iCol = 0 ; iCol < iCols ; iCol++)
                {
                    ...
                    int iOrigOffset = iCol * _poSource->getRows();
                    memcpy(m_pRealData + iDestOffset, _poSource->getReal() + iOrigOffset, iRows * sizeof(double));
                }
            }
            else
            {
                for(int iCol = 0 ; iCol < iCols ; iCol++)
                {
                    set(_iRows, _iCols + iCol, _poSource->getReal(0, iCol));
                }
            }
            ...
            //    for(int iCol = 0 ; iCol < iCols ; iCol++)
            //    {
            //        setReal(_iRows + iRow, _iCols + iCol, _poSource->getReal(iRow, iCol));
            //    }
            //}
		}
		return true;
