...
bool Double::subMatrixToString(wostringstream& ostr, int* _piDims, int _iDims)
{
    ...
    if (isIdentity())
    {
        ...
        {
            DoubleFormat df;
            getDoubleFormat(m_pRealData[0], 0, &df);
            addDoubleValue(&ostr, m_pRealData[0], 0, &df);
            ostr << endl;
        }
        else
        {
            //complex value
            DoubleFormat df;
            getDoubleFormat(m_pRealData[0], m_pImgData[0], &df);
            addDoubleValue(&ostr, m_pRealData[0], m_pImgData[0], &df);
            ostr << endl;
        }
        ostr << endl;
    }
    ...
    else if (isScalar())
    {
        ...
        {
            DoubleFormat df;
            getDoubleFormat((m_pRealData[iPos]), 0, &df);
            addSpaces(&ostr, SIZE_BETWEEN_TWO_VALUES);
            addDoubleValue(&ostr, (m_pRealData[iPos]), 0, &df);
            ostr << endl;
        }
        else
        {
            //complex value
            DoubleFormat df;
            getDoubleFormat(m_pRealData[iPos], m_pImgData[iPos], &df);
            addSpaces(&ostr, SIZE_BETWEEN_TWO_VALUES);
            addDoubleValue(&ostr, m_pRealData[iPos], m_pImgData[iPos], &df);
            ostr << endl;
        }
    }
    else if (getCols() == 1)
    ...
    else if (getRows() == 1)
    {
        ...
        if (isComplex() == false)
        {
            for (int i = m_iCols1PrintState ; i < getCols() ; i++)
            {
                ...
                addDoubleValue(&ostemp, m_pRealData[iPos], 0, &df);
            }
        ...
        else //complex case
        {
            for (int i = m_iCols1PrintState ; i < getCols() ; i++)
            {
                ...
                addSpaces(&ostemp, SIZE_BETWEEN_TWO_VALUES);
                addDoubleValue(&ostemp, m_pRealData[iPos], m_pImgData[iPos], &df);
            }
