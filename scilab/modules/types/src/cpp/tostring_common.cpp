...
void addDoubleValue(std::wostringstream *_postr, double _dblVal, DoubleFormat* _pDF)
{
    ...
    {
        os_swprintf(pwstSign, L"%-*ls", _pDF->iSignLen, _dblVal < 0 ? MINUS_STRING : PLUS_STRING);
    }
    else
    {
        if(_pDF->bPaddSign)
        {
            os_swprintf(pwstSign, L"%-*ls", _pDF->iSignLen, _dblVal < 0 ? MINUS_STRING : NO_SIGN);
        }
        else
        {
            os_swprintf(pwstSign, L"%-*ls", _pDF->iSignLen, _dblVal < 0 ? MINUS_STRING : L"");
        }
    }
    ...
    {//do not print if _bPrintOne == false && _dblVal == 1
        if(ISNAN(_dblVal))
        {//NaN
            os_swprintf(pwstOutput, L"%ls%*s", pwstSign, _pDF->iPrec, L"Nan");
        }
        else if(!finite(_dblVal))
        {//Inf
            os_swprintf(pwstOutput, L"%ls%*s", pwstSign, _pDF->iPrec, L"Inf");
        }
        else if(_pDF->bExp)
        {
            double dblAbs = fabs(_dblVal);
            ...
            if(_pDF->bPrintPoint)
            {
                os_swprintf(pwstFormat, L"%ls%%#.0f%%0%d.0fD%%+d", pwstSign, _pDF->iPrec);
            }
            else
            {
                os_swprintf(pwstFormat, L"%ls%%.0f%%0%d.0fD%%+d", pwstSign, _pDF->iPrec);
            }
            os_swprintf(pwstOutput, pwstFormat, dblEnt, dblDec, (int)dblTemp);
        }
        else
        {
            if(_pDF->bPrintPoint)
            {
                os_swprintf(pwstFormat, L"%ls%%#-%d.%df", pwstSign, _pDF->iWidth - 1, _pDF->iPrec);
            }
            else
            {
                os_swprintf(pwstFormat, L"%ls%%-%d.%df", pwstSign, _pDF->iWidth - 2, _pDF->iPrec); //-2 no point needed
            }
            os_swprintf(pwstOutput, pwstFormat, abs(_dblVal));
        }
    ...
    {
        os_swprintf(pwstOutput, L"%ls", pwstSign);
        *_postr << pwstOutput;
    }
}
