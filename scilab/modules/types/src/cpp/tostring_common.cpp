...
static void getFormatInternal(double _dblVal, int *_piWidth, int *_piPrec, /*in/out*/bool* _pbExp)
{
    ...
    *_pbExp |= getFormatMode() == 0;
    ...
    if (dblEnt == 0)
    {
        ...
        if (abs(iNbDigit) >= (iPrecNeeded - EXPOSANT_SIZE) || *_pbExp)
        {
            //why can be happen ?
            //exponant
            *_pbExp = true;
            int iExpLen = (int)log10((double)abs(iNbDigit)) + 1;
...
void getDoubleFormat(double _dblR, double _dblI, DoubleFormat * _pDF)
{
    ...
    if (_dblI == 0)
    {
        ...
        {
            if (_dblR == 1 && _pDF->bPrintOneR == false)
            {
    ...
    if (_dblValR != 0 || _dblValI == 0)
    {
        ...
        if ((_pDF->bPrintOneR == true) || (isEqual(fabs(_dblValR), 1)) == false)
        {
            ...
            else if (_pDF->bExpR)
            {
                ...
                double dblMantissa = exp10(log10(dblAbs) - dblTemp);
                dblDec = modf(dblMantissa, &dblEnt) * pow(10., _pDF->iPrecR);
