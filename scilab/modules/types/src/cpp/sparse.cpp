...
namespace
{
    ...
    {
        os<<title;
        copy_n(beg, n, std::ostream_iterator<typename std::iterator_traits<In>::value_type, char>(os, " "));
        os<< endl;
        return os;
    }
    ...
    template<typename T> string toString(T const& m)
    {
        ...
        for (int k=0; k <m.outerSize(); ++k)
            ...
            {
                ostr<<"(\t" << it.row()+1<< ",\t"<< it.col()+1<<")\t"<<  it.value()<<'\n';
            }
    ...
    template<typename T> bool setNonZero(T& s, typename Eigen::ei_traits<T>::Scalar v= 1.) {
...
namespace types
{
    ...
    Sparse::Sparse(RealSparse_t* realSp, CplxSparse_t* cplxSp):  m_matrixReal(realSp), m_matrixCplx(cplxSp)
    {
        if(realSp)
        {
            m_iCols= realSp->cols();
    ...
    void Sparse::create(int rows, int cols, Double CONST& src, DestIter o, std::size_t n)
    {
        ...
        {
            m_matrixReal= 0;
            m_matrixCplx=  new CplxSparse_t( rows, cols);
            m_matrixCplx->reserve(n);
            Finalizer<CplxSparse_t> f(*m_matrixCplx);
            copy_n(makeMatrixIterator<std::complex<double> >(src, rowIter), n, makeMatrixIterator<std::complex<double> >(*m_matrixCplx, o));
        }
        else
        {
            m_matrixReal= new RealSparse_t(rows, cols);
            m_matrixReal->reserve(n);
            m_matrixCplx=  0;
            Finalizer<RealSparse_t> f(*m_matrixReal);
            copy_n(makeMatrixIterator<double >(src, rowIter), n, makeMatrixIterator<double>(*m_matrixReal, o));
    ...
    void Sparse::fillDouble(Double& dest) CONST
    {
        if(isComplex())
        {
            copy_n(makeMatrixIterator<std::complex<double> >(*m_matrixCplx, rowIter), size_get()
                   ,makeMatrixIterator<std::complex<double> >(dest, rowIter));
        }
        else
        {
            copy_n( makeMatrixIterator<double>(*m_matrixReal, rowIter), size_get(), makeMatrixIterator<double >(dest, rowIter));
        }
    }
    /*----------------*/
    /*      val_set     */
    ...
    bool Sparse::val_set(int _iRows, int _iCols, double _dblReal, double _dblImg)
    {
        ...
        else
        {
            if(m_matrixReal)
            {
                if(_dblReal != 0.)
                {
//                    std::cerr<<"inserting "<<_dblReal<<"@("<<_iRows<<','<<_iCols<<'\n';
                    Finalizer<RealSparse_t> f(*m_matrixReal);
    ...
    Sparse* Sparse::extract(int _iSeqCount, int CONST* _piSeqCoord, int CONST* _piMaxDim, int CONST* _piDimSize, bool _bAsVector) CONST
    {
        ...
        std::cerr<<"extracted sparse:"<<pSp->rows_get()<<", "<<pSp->cols_get()<<"seqCount="<<_iSeqCount<<"maxDim="<<_piMaxDim[0] <<","<< _piMaxDim[1]<<std::endl;
    ...
    bool Sparse::doInsert(Src CONST& src, RealSp& realSp, CplxSp& cplxSp, int const* coords, bool asVector, Sz n)
    {
        ...
        {
            copy_n(makeMatrixIterator<double>(src, rowIter), n
                   , makeMatrixIterator<double>(realSp, std::make_pair(coords, asVector)));
        }
        else
        {
            toComplex();
            copy_n(makeMatrixIterator<std::complex<double> >(src, rowIter), n
                   , makeMatrixIterator<std::complex<double> >(cplxSp, std::make_pair(coords, asVector)));
        }
        return true;
    }
    ...
    Sparse* Sparse::add(Sparse const& o) const
    {
        ...
        {
            realSp= new RealSparse_t(*m_matrixReal + *(o.m_matrixReal));
        }
        else
        {
            cplxSp= isComplex()
                ?( o.isComplex()
                   ? new CplxSparse_t(*m_matrixCplx + *(o.m_matrixCplx))
                   : new CplxSparse_t(*m_matrixCplx + o.m_matrixReal->cast<std::complex<double> >()))
                : new CplxSparse_t(m_matrixReal->cast<std::complex<double> >() + *(o.m_matrixCplx));
        }
    ...
    {
        return new Sparse( m_matrixReal ? new RealSparse_t(m_matrixReal->transpose()) : 0
                           ,m_matrixCplx ? new CplxSparse_t(m_matrixCplx->transpose()) : 0);
    }
    struct BoolCast
    {
    ...
    { // result is never cplx
        return new Sparse( m_matrixReal
                           ? new RealSparse_t(m_matrixReal->cast<bool>().cast<double>())
                           : new RealSparse_t(m_matrixCplx->cast<BoolCast>().cast<double>())// /!\ TODO
                           , 0);
    }
    std::size_t Sparse::nonZeros() const
    {
        return  m_matrixReal ? m_matrixReal->nonZeros() : m_matrixCplx->nonZeros() ;
    }
    std::size_t Sparse::nonZeros(std::size_t r) const
    {
        return  m_matrixReal ? m_matrixReal->innerNonZeros(r) : m_matrixCplx->innerNonZeros(r) ;
    }
...
namespace
{
    ...
    {
        return m_matrixReal
            ? std::make_pair(sparseTransform(*m_matrixReal, outReal, GetReal<RealSparse_t>()), outImag)
            : std::make_pair(sparseTransform(*m_matrixCplx, outReal, GetReal<CplxSparse_t>())
                             , sparseTransform(*m_matrixCplx, outImag, GetImag<CplxSparse_t>()));
    }

    double* Sparse::outputRowCol(double* out)const
    {
        return m_matrixReal
            ? sparseTransform(*m_matrixReal, sparseTransform(*m_matrixReal, out, GetRow<RealSparse_t>()), GetCol<RealSparse_t>())
            : sparseTransform(*m_matrixCplx, sparseTransform(*m_matrixCplx, out, GetRow<CplxSparse_t>()), GetCol<CplxSparse_t>());
    }
    double* Sparse::outputCols(double* out) const
    {
        return std::transform(out, isComplex()
                              ?  copy_n(m_matrixCplx->_innerIndexPtr(), nonZeros(), out)
                              : copy_n(m_matrixReal->_innerIndexPtr(), nonZeros(), out)
