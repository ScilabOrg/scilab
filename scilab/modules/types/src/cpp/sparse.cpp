...
namespace
{
    ...
    template<typename T> std::wstring emptyName() { return L" zero";}
    template<> std::wstring emptyName<bool>() { return L" False";}
    ...
    template<typename T> wstring toString(T const& m)
    {
        ...
        if(!m.nonZeros())
        {
            ostr<<emptyName<typename Eigen::internal::traits<T>::Scalar>();
        }
