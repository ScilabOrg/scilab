...
namespace types
{
	...
	Sparse::Sparse(types::Double *_obj)
	{
		...
		if ( _obj->isComplex() ) 
		{
			...
			m_iSize    = m_iCols * m_iRows;
			m_iSizeMax = m_iSize;
			m_pdblReal = new double[m_iSizeMax];
			m_pdblImg  = new double[m_iSizeMax];
			...
			for (int i = 0; i < _obj->rows_get(); i++)
			{
				for (int j = 0; j < _obj->cols_get(); j++)
				{
					...
					m_pdblReal[j*m_iRows + i] = _obj->real_get(i,j);
					m_pdblImg[j*m_iRows + i]  = _obj->img_get(i,j);			
	...
	void Sparse::CreateSparse(Double *_indices, Double *_vals, Double *_dims)
	{
		...
		if ( _vals->isComplex() ) 
		{
			...
			memset(m_pdblReal, 0x00, sizeof(double) * m_iSize);
			memset(m_pdblImg, 0x00, sizeof(double) * m_iSize);

			for (int i = 0; i < _indices->rows_get(); i++)
			{
				row = _indices->real_get(i,0)-1;
				col = _indices->real_get(i,1)-1;
				m_complexMatrix->insert(row,col) = _vals->img_get(i,0);
				m_pdblReal[col * maxRow + row] = real(m_complexMatrix->coeff(row,col));
				m_pdblImg[col * maxRow + row]  = imag(m_complexMatrix->coeff(row,col));
		...
		else
		{
			...
			for (int i = 0; i < _indices->rows_get(); i++)
			{
				row = _indices->real_get(i,0)-1;
				col = _indices->real_get(i,1)-1;
				m_matrix->insert(row,col) = _vals->real_get(i,0);
				m_pdblReal[col * maxRow + row] = m_matrix->coeff(row,col);
			}			
	...
	Sparse* Sparse::extract(int _iSeqCount, int* _piSeqCoord, int* _piMaxDim, int* _piDimSize, bool _bAsVector)
	{
		...
		double* pdblReal	= new double[m_iSizeMax];
		double* pdblImg		= new double[m_iSizeMax];
		...
		if(_bAsVector)
		{
			if(isComplex())
			{
				...
				pOut->real_set(pdblReal);
				pOut->img_set(pdblImg);
			}
			else
			{
				for(int i = 0 ; i < _iSeqCount ; i++)
		...
		if(_pdblReal != NULL)
		{
			...
			if (!isComplex()) {
				...
				for (int i = 0; i < m_iRows; i++)
				{
					...
					{
						m_pdblReal[j*m_iRows + i] = _pdblReal[j*m_iRows + i];
						m_matrix->insert(i,j) = _pdblReal[j*m_iRows + i];
					}
				}
			}
			else
	...
	bool Sparse::img_set(double *_pdblImg)
	{
		if(_pdblImg != NULL)
		{
			...
			for (int i = 0; i < m_iRows; i++)
			{
				for (int j = 0; j < m_iCols; j++)
				{
					m_pdblImg[j*m_iRows + i] = _pdblImg[j*m_iRows + i];
					m_complexMatrix->insert(i,j) = std::complex<double> (m_pdblReal[j*m_iRows + i], m_pdblImg[j*m_iRows + i]);
				}
			}
		}
		else
