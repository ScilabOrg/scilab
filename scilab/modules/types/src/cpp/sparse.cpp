...
namespace
{
    ...
    {
        os<<title;
        /* TODO: use to_stringcommon (with a kind of std::boolalpha for boolean output)
         */
    ...
    template<typename T, typename R=T>  struct Output
    {
        std::wstring operator()() const {return L" zero";}
        R operator()(T const&t) const {return t; }
    };
    template<> struct Output<bool>
    {
        std::wstring operator()() const {return L" False";}
        wchar_t operator()(bool b) const {return b ? L'T': L'F'; }
    };
    ...
    template<typename T> wstring toString(T const& m)
    {
        ...
        Output<typename Eigen::internal::traits<T>::Scalar> o;
        if(!m.nonZeros())
        {
            ostr<< o();
        }
        ...
        for (std::size_t k=0; k != m.outerSize(); ++k)
        {
            ...
            {
                ostr<<L"(\t" << it.row()+1<< L",\t"<< it.col()+1<<L")\t"<<  o(it.value())<<L'\n';
            }
        }
#if 0
...
// GenericType because we might return a Double* for scalar operand
    ...
    {
        std::cerr<<"in less than\n";
        SparseBool * res(cwiseOp<std::less>(*this, o));
        std::wcerr<<L" result is "<<res->toString(0,0)<<std::endl;
        return res;
    }
...
//    SparseBool* SparseBool::new
    ...
    std::wstring  SparseBool::toString(int _iPrecision, int _iLineLen) const
    {
        std::cerr<<"sparse bool to string\n";
        return ::toString(*matrixBool);
    }
