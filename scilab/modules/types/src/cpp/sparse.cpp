...
namespace types
{
    ...
    Sparse* Sparse::insert_new(int nbCoords, int CONST* coords, int CONST* maxCoords, SrcType CONST* src, bool asVector)
    {
        ...
        Sparse* pSp ( asVector ?
                      ((src->cols_get() == 1)
                       ? new Sparse(maxCoords[0], 1, cplx)
                       :( (src->rows_get() == 1)
                          ? new Sparse(1, maxCoords[0], cplx)
                          : 0))
                      :  new Sparse(maxCoords[0], maxCoords[1], cplx));
    ...
    {
        return src->getAsDouble()
            ? Sparse::insert_new(nbCoords, coords, maxCoords, src->getAsDouble(), asVector)
            : (src->getAsSparse()
               ? Sparse::insert_new(nbCoords, coords, maxCoords, src->getAsDouble(), asVector)
               : 0);
    }
    ...
    Sparse* Sparse::extract(int nbCoords, int CONST* coords, int CONST* maxCoords, int CONST* resSize, bool asVector) CONST
    {
        ...
        Sparse* pSp ( asVector ?
                      ((rows_get() == 1)
                       ?  new Sparse(1, resSize[0], cplx)
                       :new Sparse(resSize[0], 1, cplx))
                      :  new Sparse(resSize[0], resSize[1], cplx));
 ...
 extract std::make_pair(coords, asVector), rowIter
    ...
    { // result is never cplx
        return new Sparse( matrixReal
                           ...
                           : new RealSparse_t(matrixCplx->cast<BoolCast>().cast<double>())// /!\ TODO
                           , 0);
    }
