...
std::vector<InternalType*> Struct::extractFields(std::vector<std::wstring> _wstFields)
{
    ...
    for (int i = 0 ; i < _wstFields.size() ; i++)
    {
        ...
        else
        {
            if (getSize() == 1)
            {
                ResultList.push_back(get(0)->get(_wstFields[i]));
            }
            else
            {
                List* pL = new List();
                ...
                {
                    pL->append(get(j)->get(_wstFields[i]));
                }
...
std::vector<InternalType*> Struct::extractFields(typed_list* _pArgs)
{
    ...
    for (int i = 0 ; i < iSeqCount ; i++)
    {
        ...
        {
            //return elements
            ResultList.push_back(get(0)->getData()[iIndex - 3]);
        }
        else
        {
            ...
            for (int j = 0 ; j < getSize() ; j++)
            {
                ...
                pL->append(get(j)->getData()[iIndex - 3]);
            }
