...
int stackapi_addStackSizeError(SciErr* _psciErr, char* _pstCaller, int _iNeeded)
{
	char pstMsg1[bsiz];
	char pstMsg2[bsiz];
	char pstMsg3[bsiz];
	...
	return stackapi_addErrorMessage(_psciErr, 17, pstMsg1);
}
...
char* stackapi_getErrorMessage(SciErr _sciErr)
{
	if(_sciErr.iErr == 0)
	{
		return NULL;
