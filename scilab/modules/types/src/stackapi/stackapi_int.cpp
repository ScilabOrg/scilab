...
SciErr stackapi_createMatrixOfUnsigedInteger64(void* _pvCtx, int _iVar, int _iRows, int _iCols, const unsigned long long* _pullData)
{
	SciErr sciErr; sciErr.iErr = 0; sciErr.iMsgCount = 0;
	...
	sciErr = stackapi_allocMatrixOfUnsignedInteger64(_pvCtx, _iVar, _iRows, _iCols, &pullData);
	if(sciErr.iErr)
	{
		stackapi_addErrorMessage(&sciErr, STACKAPI_ERROR_CREATE_INT, _("%s: Unable to create variable in Scilab memory"), "stackapi_createMatrixOfUnsigedInteger64");
		return sciErr;
	}

	memcpy(pullData, _pullData, sizeof(unsigned long long) * iSize);
	return sciErr;
}
#endif
...
SciErr stackapi_createMatrixOfInteger64(void* _pvCtx, int _iVar, int _iRows, int _iCols, const long long* _pllData64)
{
	...
	sciErr = stackapi_allocMatrixOfInteger64(_pvCtx, _iVar, _iRows, _iCols, &pllData64);
	if(sciErr.iErr)
	{
...
SciErr stackapi_allocMatrixOfInteger64(void* _pvCtx, int _iVar, int _iRows, int _iCols, long long** _pllData64)
{
	...
	sciErr = stackapi_allocCommonMatrixOfInteger(_pvCtx, _iVar, piAddr, SCI_INT64, _iRows, _iCols, (void**)&_pllData64);
	if(sciErr.iErr)
	{
...
SciErr stackapi_allocMatrixOfUnsignedInteger64(void* _pvCtx, int _iVar, int _iRows, int _iCols, unsigned long long** _pullData64)
{
	...
	sciErr = stackapi_allocCommonMatrixOfInteger(_pvCtx, _iVar, piAddr, SCI_UINT64, _iRows, _iCols, (void**)&_pullData64);
	if(sciErr.iErr)
	{
