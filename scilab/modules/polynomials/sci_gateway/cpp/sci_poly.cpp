...
types::Function::ReturnValue sci_poly(types::typed_list &in, int _iRetCount, types::typed_list &out)
{
    ...
    wchar_t wstrName[5] = {0x20, 0x20, 0x20, 0x20, L'\0'}; // init with 4 blank
    ...
    wchar_t* wstr = pStrName->get(0);
    if (wcslen(wstr) > 4)
    {
        if (ConfigVariable::getWarningMode())
        {
            sciprint(_("%s: Warning: variable name must be lesser than 5 characters long.\n"), "poly");
        }
    }

    memcpy(wstrName, wstr, std::min((int)wcslen(wstr), 4) * sizeof(wchar_t));
