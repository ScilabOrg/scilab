...
types::Function::ReturnValue sci_pppdiv(types::typed_list &in, int _iRetCount, types::typed_list &out)
{
    ...
    for (int i = 0; i < 2; i++)
    {
        ...
        {
            std::wstring wstFuncName = L"%"  + in[i]->getShortTypeStr() + L"_coeff";
    ...
    {
        C2F(dpodiv)(pdblInR[0], pdblInR[1], &piRank[0], &piRank[1], &iErr);
    }
    else
    {
        ...
        C2F(wpodiv)(pdblInR[0], pdblInI[0], pdblInR[1], pdblInI[1], &piRank[0], &piRank[1], &iErr);
    }

    // after execution : pdblInR[0](1 -> piRank[1]) is the rest
    //                   pdblInR[0](piRank[0] - piRank[1] +1 -> end) is the coeff
    int iSizeRest      = piRank[1];
    int iSizeCoeff     = piRank[0] - piRank[1] + 1;
    double* pdblRestR  = pdblInR[0];
    double* pdblCoeffR = pdblInR[0] + piRank[1];
    ...
    else // return a types::Polynom
    {
        ...
        {
            double* pdblImg = NULL;
            pSP = new types::SinglePoly(&pdblReal, &pdblImg, iSizeCoeff + 1);
            memcpy(pdblImg, pdblCoeffI, (iSizeCoeff + 1)  * sizeof(double));
        }
        else
        {
            pSP = new types::SinglePoly(&pdblReal, NULL, iSizeCoeff + 1);
        }

        memcpy(pdblReal, pdblCoeffR, (iSizeCoeff + 1) * sizeof(double));
        pPolyOut->set(0, pSP);
    }
    ...
    if (_iRetCount == 2)
    {
        ...
        else // return a types::Polynom
        {
            ...
            {
                double* pdblImg = NULL;
                pSP = new types::SinglePoly(&pdblReal, &pdblImg, iSizeRest + 1);
                memcpy(pdblImg, pdblCoeffI, (iSizeRest + 1) * sizeof(double));
            }
            else
            {
                pSP = new types::SinglePoly(&pdblReal, NULL, iSizeRest + 1);
            }

            memcpy(pdblReal, pdblCoeffR, (iSizeRest + 1) * sizeof(double));
            pPolyOut->set(0, pSP);
