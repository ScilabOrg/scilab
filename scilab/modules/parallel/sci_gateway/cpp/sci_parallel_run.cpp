...
:      std::pair < char const *, dim_t > (name, dim)
    ...
    void fillUndefinedData(void *varData, scilabDesc_t resultVar, scilabDesc_t expectedVar)
    {
        if (!std::strcmp(resultVar.first, expectedVar.first))
        {
            if (!strcmp(resultVar.first, "constant"))
            {
                ...
                std::fill_n(static_cast < double *>(varData) + nbFilled, expectedVar.second.first * expectedVar.second.second - nbFilled,
    ...
    struct wrapper
    {
         ...
         */
          template < typename VarsIt > wrapper(VarsIt begin, VarsIt functionIt, VarsIt end, std::size_t functionLhs)
        {
...
private:
        ...
        template < typename ArgIt > void registerArgs(ArgIt it, ArgIt end)
        {
        ...
        template < typename VarPtrIt > void allocCompleteResults(VarPtrIt begin, VarPtrIt resBegin, VarPtrIt resEnd, std::size_t nbLhs)
        {
            ...
            if (resBegin != resEnd)
            {
                ...
                if (resBegin != resEnd)
                {
                    if (getVarType(*resBegin) == sci_matrix)
                    {
                        ...
                        default:
                            {
                                Scierror(999, _("%s: Wrong size of input argument #%d: Number of columns are incompatible "), currentFname,
                                         std::distance(begin, resBegin));
                            }
                        }
                    }
                }
         ...
         */
        template < bool byName > void macro(char const **args, char **res)
        {
            /* rhs models from  */
            int saveNbvars = Nbvars, saveTop = currentTop;

            for (std::vector < scilabDesc_t >::const_iterator it(rhsDesc.begin()); it != rhsDesc.end(); ++it, ++args)
            {
            ...
            // result r is now on first position on stack
            {
                ...
                for (std::vector < scilabDesc_t >::iterator it(lhsDesc.begin()); it != lhsDesc.end(); ++it, ++resPos, ++res)
                {
    ...
    bool getConfigParameters
        (int config_arg_pos, int &nb_workers, bool & shared_memory, bool & dynamic_scheduling, int &chunk_size, char const *&prologue,
         char const *&epilogue)
    {
        int log(0);
        int *addr = NULL;
    ...
    struct ConcurrencyState
    {
        ...
        {
            lock =
                static_cast < __threadSignalLock * >(mmap(0, sizeof(__threadSignalLock), PROT_READ | PROT_WRITE, MAP_SHARED | MAP_ANONYMOUS, -1, 0));
        ...
        {
            ScopedUpdater(ConcurrencyState & c, bool threads):lock(c.lock), countPtr(threads
                                                                                     ? &c.threadConcurrencyLevel : c.processConcurrencyLevelPtr)
            {
                __LockSignal(lock);
                ++(*countPtr);
                __UnLockSignal(lock);
...
int sci_parallel_run(char *fname, unsigned long fname_len)
{
    ...
    make_parallel_wrapper(w.argsDataBegin(), w.argsSizesBegin(), w.argsNbBegin(), w.nbRhs(), w.tasksNb(), w.resDataBegin(), w.resSizesBegin(), Lhs,
                          w.getHandle(), prologue, epilogue) (withThreads, nbWorkers, dynamicScheduling, chunkSize);
