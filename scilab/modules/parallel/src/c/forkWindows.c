...
{ SystemHandleInformation = 0x10 } SYSTEM_INFORMATION_CLASS;
...
typedef NTSTATUS(NTAPI * ZwCreateThread_t) (OUT PHANDLE ThreadHandle,
                                            ...
                                            OUT PCLIENT_ID ClientId, IN PCONTEXT ThreadContext, IN PUSER_STACK UserStack, IN BOOLEAN CreateSuspended);
/*--------------------------------------------------------------------------*/
...
static BOOL haveLoadedFunctionsForFork(void)
{
    ...
    if (ZwCreateProcess && ZwQuerySystemInformation && ZwQueryVirtualMemory &&
        ZwCreateThread && ZwGetContextThread && ZwResumeThread && ZwQueryInformationThread && ZwWriteVirtualMemory && ZwClose)
    {
        return TRUE;
    }
