...
namespace parallel
{
    ...
    ProcessesRunner::ProcessesRunner(types::typed_list& in, std::size_t nbRes, types::typed_list& out)
    ...
    void ProcessesRunner::run()
    {
        ...
        createSlices(std::back_inserter(fIn));
        std::vector<char> processed(n, 0);// not <boll> because we need pointers (check that the specialization oddities do not hurt us)
