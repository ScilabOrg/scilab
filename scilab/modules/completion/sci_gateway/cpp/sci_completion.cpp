...
/*--------------------------------------------------------------------------*/
static types::String* doCompletion(char* _pcSomechars, char** (*pvFunction)(char*, int*));
/*--------------------------------------------------------------------------*/
types::Function::ReturnValue sci_completion(types::typed_list &in, int _iRetCount, types::typed_list &out)
{
    ...
    if (_iRetCount > 6)
    {
        Scierror(77, _("%s: Wrong number of output argument(s): %d to %d expected."), "completion", 1, 6);
        return types::Function::Error;
    }
    ...
    else // if(in.size() == 2)
    {
        if (_iRetCount != 1)
        {
            Scierror(77, _("%s: Wrong number of output argument(s): %d expected."), "completion", 1);
            return types::Function::Error;
        }
