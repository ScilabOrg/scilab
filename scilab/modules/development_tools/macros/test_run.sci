...
// Remove empty line
// In scilab 5, the test is execute in a try/catch
// which remove empty line.
// In scilab 6, we can't execute the test in a try/catch
// because it will be parsed in first then executed
// so the diary will contain all script followed by the display
// of the execution.
dia(find(dia == '')) = [];
