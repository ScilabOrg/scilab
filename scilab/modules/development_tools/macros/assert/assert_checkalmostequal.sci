...
function [flag,errmsg] = assert_checkalmostequal ( varargin )
  ...
  if ( typeof(computed) <> "constant" ) then
    errmsg = sprintf ( gettext ( "%s: Wrong type for input argument #%d: Real matrix expected.") , "assert_checkalmostequal" , 1 )
    error(errmsg)
  end
  if ( typeof(expected) <> "constant" ) then
    errmsg = sprintf ( gettext ( "%s: Wrong type for input argument #%d: Real matrix expected.") , "assert_checkalmostequal" , 2 )
    error(errmsg)
  end
  if ( typeof(reltol) <> "constant" ) then
    errmsg = sprintf ( gettext ( "%s: Wrong type for input argument #%d: Real matrix expected.") , "assert_checkalmostequal" , 3 )
    error(errmsg)
  end  
  if ( typeof(abstol) <> "constant" ) then
    errmsg = sprintf ( gettext ( "%s: Wrong type for input argument #%d: Real matrix expected.") , "assert_checkalmostequal" , 4 )
    error(errmsg)
  end  
  if ( typeof(comptype) <> "string" ) then
    errmsg = sprintf ( gettext ( "%s: Wrong type for input argument #%d: A string expected.") , "assert_checkalmostequal" , 5 )
    error(errmsg)
  end  
  //
