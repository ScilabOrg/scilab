...
Function::ReturnValue sci_strcat(typed_list &in, int _iRetCount, typed_list &out)
{
    ...
    if(in.size() == 3)
    {
        wchar_t wcMode = in[2]->getAsString()->getString(0)[0];
        switch(wcMode)
        {
        case L'r' :
            iMode = 1;
    ...
    if(in.size() > 1)
    {
        if(in[1]->getAsString()->getSize() != 1)
        {
            ScierrorW(999, _W("%ls: Wrong type for input argument #%d: String expected.\n"), L"strcat", 2);
            return Function::Error;
        }

        pwstToInsert = in[1]->getAsString()->getString(0);
    }
    ...
    case 0 : //"*"
        {
            ...
            {
                iLen += (int)wcslen(pS->getString(i));
            }
            ...
            for(int i = 0 ; i < pS->getSize() ; i++)
            {
                ...
                wcscat(pwstOut + iOffset, pS->getString(i));
            }

            pOut->setString(0, pwstOut);
            FREE(pwstOut);
        }
        break;
    case 1 : //"r"
        {
            ...
            for(int i = 0 ; i < pS->getCols() ; i++)
            {
                ...
                {
                    iLen += (int)wcslen(pS->getString(j, i));
                }
                ...
                for(int j = 0 ; j < pS->getRows() ; j++)
                {
                    ...
                    wcscat(pwstOut + iOffset, pS->getString(j, i));
                }
                pOut->setString(0, i, pwstOut);
            }
            break;
        }
    case 2 : //"c"
        {
            ...
            for(int i = 0 ; i < pS->getRows() ; i++)
            {
                ...
                {
                    iLen += (int)wcslen(pS->getString(i, j));
                }
                ...
                for(int j = 0 ; j < pS->getCols() ; j++)
                {
                    ...
                    wcscat(pwstOut + iOffset, pS->getString(i, j));
                }
                pOut->setString(i, 0, pwstOut);
            }
            break;
        }
    }
