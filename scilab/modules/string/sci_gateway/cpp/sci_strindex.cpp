...
types::Function::ReturnValue sci_strindex(types::typed_list &in, int _iRetCount, types::typed_list &out)
{
    ...
    if(in.size() > 2)
    {
        ...
        if(in[2]->getAsString()->getString(0)[0] == WCHAR_R)
        {
            bRegExp = true;
        }
        else if(in[2]->getAsString()->getString(0)[0] == WCHAR_S)
        {
            bRegExp = false;
        }
        else
    ...
    if(in[1]->isString() == false || (in[1]->getAsString()->getRows() != 1 && in[1]->getAsString()->getCols() != 1))
    {
    ...
    wchar_t** pwstSearch = pS->getString();
    ...
    wchar_t* pwstData = in[0]->getAsString()->getString()[0];
    ...
    {
        pIndex->setReal(0, i, pstrResult[i].data);
    }
    ...
    if(_iRetCount == 2)
    {
        ...
        for(int i = 0 ; i < iValues ; i++)
        {
            pPos->setReal(0, i, pstrResult[i].position);
        }
        out.push_back(pPos);
    }
