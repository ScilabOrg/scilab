...
Function::ReturnValue sci_grep(typed_list &in, int _iRetCount, typed_list &out)
{
    ...
    if(in[0]->getType() == InternalType::RealDouble && in[0]->getAs<Double>()->getSize() == 0)
    {
    ...
    {
        pStr1[i] = wide_string_to_UTF8(pS1->getString(i));
    }

    char** pStr2 = (char**)MALLOC(sizeof(char*) * pS2->getSize());
    for(int i = 0 ; i < pS2->getSize() ; i++)
    {
        pStr2[i] = wide_string_to_UTF8(pS2->getString(i));
    }
