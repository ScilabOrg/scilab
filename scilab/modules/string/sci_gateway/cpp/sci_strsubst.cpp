...
Function::ReturnValue sci_strsubst(typed_list &in, int _iRetCount, typed_list &out)
{
    ...
    if(in.size() > 3)
    {
        if(in[3]->isString() == false && in[3]->getAsString()->getSize() != 1)
        {
            ScierrorW(999, _W("%ls: Wrong type for input argument #%d: A string expected.\n"), L"strsubst", 4);
            return Function::Error;
        }

        if(in[3]->getAsString()->getString(0)[0] == WCHAR_R)
        {
            bRegExp = true;
        }
        else if(in[3]->getAsString()->getString(0)[0] == WCHAR_S)
        {
            bRegExp = false;
        }
        else
    ...
    if(in[2]->isString() == false || in[2]->getAsString()->getSize() != 1)
    {
        ScierrorW(999, _W("%ls: Wrong type for input argument #%d: A string expected.\n"), L"strsubst", 3);
        return Function::Error;
    }

    wchar_t* pwstReplace = in[2]->getAsString()->getString()[0];

    if(in[1]->isString() == false || in[1]->getAsString()->getSize() != 1)
    {
        ScierrorW(999, _W("%ls: Wrong type for input argument #%d: A string expected.\n"), L"strsubst", 2);
        return Function::Error;
    }

    wchar_t* pwstSearch = in[1]->getAsString()->getString()[0];
    ...
    {
        int iErr = 0;
        pwstOutput = wcssubst_reg(pS->getString(), pS->getSize(), pwstSearch, pwstReplace, &iErr);
        if(iErr != NO_MATCH)
        {
    ...
    {
        pwstOutput = wcssubst(pS->getString(), pS->getSize(), pwstSearch, pwstReplace);
    }
