...
int sci_strsplit(char *fname, int* _piKey)
{
    ...
    if (isEmptyMatrix(_piKey, piAddressVarOne))
    {
        ...
        if (Lhs == 2)
        {
            ...
            // r1 eq []
            // r2 eq ''
            createSingleString(_piKey, Rhs + 2, "");
            LhsVar(2) = Rhs + 2;
        }
        PutLhsVar();
        return 0;
    ...
    if (Rhs > 2) /* Rhs eq. 3 */
    {
        ...
        if (!isDoubleType(_piKey, piAddressVarThree))
        {
            Scierror(999,_("%s: Wrong type for input argument #%d: A scalar expected.\n"), fname, 3);
            return 0;
        }

        if (getScalarDouble(_piKey, piAddressVarThree, &dValueThree) != 0)
        {
    ...
    if (Rhs > 1) /* Rhs >=  2 */
    {
        ...
        if (isDoubleType(_piKey, piAddressVarTwo))
        {
            ...
            if (isEmptyMatrix(_piKey, piAddressVarTwo))
            {
                Scierror(999,_("%s: Wrong value for input argument #%d.\n"), fname, 2);
                return 0;
            }

            if ( (isVector(_piKey, piAddressVarTwo)) ||
                (isRowVector(_piKey, piAddressVarTwo)) ||
                (isScalar(_piKey, piAddressVarTwo)) )
            {
        ...
        else if (isStringType(_piKey, piAddressVarTwo))
        {
            if (!isScalar(_piKey, piAddressVarTwo))
            {
                ...
                if (getAllocatedMatrixOfWideString(_piKey, piAddressVarTwo, &m2, &n2, &pStrsTwo) != 0)
                {
                    Scierror(999, _("%s: No more memory.\n"), fname);
                    return 0;
                }
