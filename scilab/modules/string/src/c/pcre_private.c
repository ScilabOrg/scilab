...
static int callout(pcre_callout_block * cb)
{
    ...
    return (cb->callout_number != callout_fail_id) ? 0 : (++callout_count >= callout_fail_count) ? 1 : 0;
}
...
static int check_match_limit(pcre * re, pcre_extra * extra, char *bptr, int len,
                             int start_offset, int options, int *use_offsets, int use_size_offsets, int flag, unsigned long int *limit, int errnumber)
{
    int count;
    int min = 0;
    int mid = 64;
    ...
    for (;;)
    {
        *limit = mid;
        ...
        count = pcre_exec(re, extra, (char *)bptr, len, start_offset, options, use_offsets, use_size_offsets);
        ...
        else if (count >= 0 || count == PCRE_ERROR_NOMATCH || count == PCRE_ERROR_PARTIAL)
        {
            if (mid == min + 1)
            {
                break;
            ...
            for (;; gmatched++) /* Loop for /g or /G */
            {
                ...
                if (find_match_limit)
                {
                    ...
                    (void)check_match_limit(re, extra, bptr, len, start_offset,
                                            ...
                                            PCRE_EXTRA_MATCH_LIMIT, &(extra->match_limit), PCRE_ERROR_MATCHLIMIT);

                    count = check_match_limit(re, extra, bptr, len, start_offset,
                                              options | g_notempty, use_offsets, use_size_offsets,
                                              PCRE_EXTRA_MATCH_LIMIT_RECURSION, &(extra->match_limit_recursion), PCRE_ERROR_RECURSIONLIMIT);
                }
                ...
                else if (callout_data_set)
                {
                    ...
                    count = pcre_exec(re, extra, (char *)bptr, len, start_offset, options | g_notempty, use_offsets, use_size_offsets);
