...
void stripblanks(char **InputStrings, char **OutputStrings, int InputStringsDim, BOOL withTAB)
{
    ...
    for (x = 0; x < InputStringsDim; x++)   /*Search for all the matrix */
    {
        ...
        for (i = (int)strlen(InputStrings[x]) - 1; i > 0; i--)
        {
            if (withTAB)
            {
                if ((InputStrings[x][i] == (char)BLANK_CHARACTER) || (InputStrings[x][i] == (char)TAB_CHARACTER))
                    continue;
                else
                {
                    lenghtInput_Two = i;
        ...
        {
            if ((lenghtInput_Two == 0) && ((InputStrings[x][0] == (char)BLANK_CHARACTER) || (withTAB && (InputStrings[x][0] == (char)TAB_CHARACTER))))
            {
