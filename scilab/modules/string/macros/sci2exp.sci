...
function t=h2exp(a,lmax) //Only for figure and uicontrol
    ...
    if a.type=="uicontrol"
       ...
       //If children is searched, an infinite loop is created (because of parent->children->parent...)
       //So, only children.type is computed
        ...
        if a.Userdata <>[] then
            ...
            elseif type(a.userdata) == 15 then
                if list(a.children) <> a.userdata then //to avoid infinite loop
                    f28_strg=list2exp(a.userdata,0)
                else
                    f28_strg="list(children)"
                end
    ...
    elseif a.type == "Figure" then
        ...
        if a.userdata <> [] then
            ...
            elseif type(a.userdata) == 15 then
                if list(a.children)<>a.userdata then //to avoid infinite loop
                    f47_strg=list2exp(a.userdata,0)
                else
                    f47_strg="list(children)";
                end
            elseif type(a.userdata) == 16 then
