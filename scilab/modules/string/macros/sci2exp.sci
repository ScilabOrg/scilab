...
function t=h2exp(a,lmax) //Only for figure and uicontrol
    ...
    if a.type=="uicontrol"
        f='uicontrol';
        flag=0;
        if a.parent<>[] & (a.parent.type=='uicontrol'| a.parent.type=='Figure') then
            f1_strg=h2exp(a.parent,0);
            x=f1+f1_strg+', ';
        else
            x=f1+''''+a.parent.type+''''+', ';
        end
       //If children is searched, an infinite loop is created
       //So, only children.type is computed
        if a.children <> [] then
            x=x+f2+''''+a.children.type+''''+', ';
        end
        ...
        if a.Userdata <>[] then
            ...
            elseif type(a.userdata) == 15 then
                f28_strg="list(";
                for i=1:length(a.userdata)
                    if a.children==a.userdata(i) //To avoid infinite loop
                        f28_strg=''''+f28_strg+"children"+")"+'''';
                        flag=1; //If one child is detected, shows list(children)
                    end
                end
                if flag==0 then f28_strg=list2exp(a.userdata); end
    ...
    elseif a.type == "Figure" then
        ...
        if a.userdata <> [] then
            ...
            elseif type(a.userdata) == 15 then
                ...
                for i=1:length(a.userdata)
                    if a.children==a.userdata(i) //To avoid infinite loop
                        f47_strg=''''+f47_strg+"children"+")"+'''';
                        flag=1; //If one child is detected, shows list(children)
                    end
                end
                if flag==0 then f47_strg=list2exp(a.userdata); end
            elseif type(a.userdata) == 16 then
