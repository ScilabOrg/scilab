...
import org.scilab.modules.commons.FileProperties;
...
public class FileNode {
    ...
    private final FileFilter FILTER = new FileFilter() {
		...
		FileProperties propr = new FileProperties(f);
		System.out.println(f+"::"+propr.isFile()+"::"+ propr.isDirectory()+"::"+"::"+ propr.canRead());
                return (propr.isFile() || propr.isDirectory()) && !f.isHidden() && propr.canRead() && (pat == null || pat.matcher(f.getName()).matches());
            }
        };
    ...
    protected static FileNode[] orderFiles(int order, File[] files) {
        ...
        case DATEORDER :
            Arrays.sort(files, new Comparator<File>() {
                    public int compare(File f1, File f2) {
			FileProperties p1 = new FileProperties(f1);
			FileProperties p2 = new FileProperties(f2);
                        int diff = new Long(p1.lastModified()).compareTo(p2.lastModified());
                        if (diff == 0) {
                            return alpha.compare(f1, f2);
                        } else {
                            return diff;
	...
	FileProperties propr = new FileProperties(file);
        return propr.lastModified() > lastModified
            || propr.canRead() != canRead
            || propr.canWrite() != canWrite
            || isEmpty != (propr.isDirectory() && CommonFileUtils.isEmptyDirectory(file.getAbsolutePath()) == 1);
    }
    ...
    private void init() {
	FileProperties propr = new FileProperties(file);
        this.lastModified = propr.lastModified(); //file.lastModified();
	this.canRead = propr.canRead();//file.canRead();
	this.canWrite = propr.canWrite();//file.canWrite();
	this.userHome = file.equals(userHomeDir);
        this.sci = file.equals(SCIDIR);
        this.isEmpty = propr.isDirectory() && CommonFileUtils.isEmptyDirectory(file.getAbsolutePath()) == 1;
        if (!isEmpty) {
