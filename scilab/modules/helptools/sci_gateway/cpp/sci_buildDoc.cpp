...
#define DEFAULTEXPORT "JH"
    ...
    int sci_buildDoc(char *fname, int* _piKey)
    {
        ...
        try
        {
            ...
            std::cout << "outputDirectory.c_str() = " << outputDirectory.c_str() << std::endl;
            std::cout << "language.c_str() = " << language.c_str() << std::endl;
            std::cout << "exportFormat.c_str() = " << exportFormat.c_str() << std::endl;
            std::cout << "masterXML.c_str() = "  << masterXML.c_str() << std::endl;
            std::cout << "styleSheet.c_str() = " << styleSheet.c_str() << std::endl;
            ...
            if (doc->setOutputDirectory((char *) outputDirectory.c_str()))
            {
                ...
                const char *pstOutputDir = outputDirectory.c_str();
                createMatrixOfString(_piKey, Rhs+1, 1, 1, &pstOutputDir);
            }
            else
            {
                ScierrorW(999,_W("%s: Could find or create the working directory %s.\n"), fname, outputDirectory.c_str());
                return FALSE;
            }
        }
