...
public class DocbookTagConverter extends DefaultHandler {
     ...
     */
    public DocbookTagConverter(String inName) throws IOException {
	this.in = inName.length() != 0 ? new FileInputStream(new File(inName)) : null;
	this.inName = inName;
    }
	...
	if (locator != null) {
	    throw new SAXException("Cannot parse " + inName + ":\n" + e.getMessage() + "\n" + sId + " at line " + locator.getLineNumber());
	} else {
	    throw new SAXException("Cannot parse " + inName + ":\n" + e.getMessage() + "\n" + sId);
	}
    }
