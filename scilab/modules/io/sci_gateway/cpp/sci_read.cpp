    ...
    if (iRows < 0)
    {
        if (iID != 5 /*stdout*/)
        {
            ...
            switch (itTypeOfData)
            {
                ...
                {
                    double** ppdData = new double*[4096];
                    ...
                    if (pstFormat == NULL)
                    {
                        while (error == 0)
                        {
                            ppdData[iRows] = new double[iCols];
                            C2F(read_double_line_file)(&iID, ppdData[iRows], &iCols, &error);
                            iRows++;
                        }
                    ...
                    else
                    {
                        ...
                        {
                            ppdData[iRows] = new double[iCols];
                            C2F(read_double_line_file_form)(&iID, pstFormat, ppdData[iRows], &iCols, &error);
                            iRows++;
                        }

                    }
                    iRows--;
                    delete ppdData[iRows];
                ...
                case types::InternalType::ScilabInt32:
                {
                    ...
                    while (error == 0)
                    {
                        ppiData[iRows] = new int[iCols];
                        C2F(read_int_line_file_form)(&iID, pstFormat, ppiData[iRows], &iCols, &error);
                        iRows++;
                    }


                    iRows--;
                    delete ppiData[iRows];
                ...
                case types::InternalType::ScilabString:
                {
                    ...
                    else
                    {
                        ...
                        char** ppC = new char*[4096];
                        for (iRows = 0; bEndWrite == false; iRows++)
                        {
                            ...
                            {
                                ppC[iRows] = new char[(siz + 1)];
                                ppC[iRows][0] = '\0';
                                strncat(ppC[iRows], pCt, siz);
                            }
                            FREE(pCt);

                        }

                        String* pS = new types::String((iRows - 1), iCols);
                        for (int i = (iRows - 2); i >= 0; i--)
                        {
                            pS->set(i, ppC[i]);
                            delete ppC[i];
                        }
        ...
        else//read from the console
        {
            ...
            switch (itTypeOfData)
            {
                case types::InternalType::ScilabDouble:
                {
                    ...
                    for (iRows = 0; (bEndWrite == false) && (error == 0); iRows++)
                    {
                        char* pCt = (char *)MALLOC(sizeof(char) * 4096);
                        int siz = 0;
                        ...
                        else
                        {
                            ...
                            while ((pch != NULL) && (error == 0) && (iColsTempo < iCols))
                            {
                                if (is_of_type<double>(pch))
                                {
                                    ...
                                    ppdData[iRows][iColsTempo] = atof(pch);
                                    iColsTempo++;
                                }
                                else
                                {
