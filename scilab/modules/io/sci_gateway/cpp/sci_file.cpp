    ...
    if(wcscmp(pSAction->get(0), L"open") == 0)
    {
        ...
        if(iErr)
        {
            if(_iRetCount == 1)
            {
                switch(iErr)
                {
                    case 65  : Scierror(iErr, _("%s: %d logical unit already used.\n"), "file", lunit); break;
                    case 66  : Scierror(iErr, _("%s: Too many files opened!\n"), "file"); break;
                    case 67  : Scierror(iErr, _("%s: Unknown file format.\n"), "file"); break;
                    case 240 : Scierror(iErr, _("%s: File \"%s\" already exists or directory write access denied.\n"), "file", pstFilename); break;
                    case 241 : Scierror(iErr, _("%s: File \"%s\" does not exist or read access denied.\n"), "file", pstFilename); break;
                    default  : Scierror(iErr, _("%s: Can not open File \"%s\"\n"), "file", pstFilename);
                }
            ...
            wcscmp(pSAction->get(0), L"last") == 0)
    {
        ...
        if(wcscmp(pSAction->get(0), L"close") == 0)
        {
            for(int i = 0; i < pDblFileUnit->getSize(); i++)
            {
                ...
                if(iErr)
                {
                    Scierror(999, _("%s: Cannot close file %s.\n"), "file", pdblUnit[i]);
                    return types::Function::Error;
                }
            }
        }
        else if(wcscmp(pSAction->get(0), L"rewind") == 0)
        {
            ...
            else if(pFile && pFile->getFileType() == 2)
            {
