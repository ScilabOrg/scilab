...
// SEG
//
function returnedSeg = extractSeg(h)
    returnedSeg = tlist([
            "ScilabSingleHandle", ...
            "type", ...
            "visible", ...
...
// TEXT
//
function returnedText = exctractText(h)
    returnedText = tlist([
            "ScilabSingleHandle", ...
            "type", ...
            "visible", ...
...
function returneduimenu = extractuimenu(h)
    ...
    for i = 1:size(fields, "*")
        if fields(i) == "children" then
            returneduimenu(fields(i)) = extractMatrixHandle(h(fields(i)));
        else
...
function returneduicontextmenu = extractuicontextmenu(h)
    ...
    for i = 1:size(fields, "*")
        if fields(i) == "children" then
            returneduicontextmenu(fields(i)) = extractMatrixHandle(h(fields(i)));
        else
