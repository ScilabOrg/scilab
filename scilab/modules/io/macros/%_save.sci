...
function [] = %_save(%__filename__, varargin)
    ...
    function result = isScicosObject(var)
        ...
        if type(var) == 128 then
            if typeof(var) == "diagram" | typeof(var) == "Block" | typeof(var) == "Link" then
                result = %t;
            end
        end
    endfunction
    ...
    function ml = toMList(o)
        f = getfield(1, o);
        ml = mlist(f);

        // Here, the cases need to be separated to dig in 'objs' and 'rpar' properties,
        // in order to do a deep recursive converion to mlist
    ...
    for %__i__ = size(varargin):-1:1
        ...
        elseif type(%__temp__) == 14 then //library, must not be save
            varargin(%__i__) = null();
        elseif isScicosObject(%__temp__) then
            execstr(varargin(%__i__) + " = toMList(%__temp__)");
        end
    end
    warning(oldMode);
