        ...
        if (isVarComplex(pvApiCtx, var_addr))
        {
            if (parent == NULL)
            {
                getAllocatedComplexSparseMatrix(pvApiCtx, var_addr, &dims[1], &dims[0],
                                                &scilabSparse.nel, &scilabSparse.mnel, &scilabSparse.icol, &scilabSparse.R, &scilabSparse.I);
            }
            else
            {
                _SciErr = getSparseMatrixInList(pvApiCtx, parent, item_position, &dims[1], &dims[0], &scilabSparse.nel, NULL, NULL, NULL);
        ...
        else
        {
            if (parent == NULL)
            {
                getAllocatedSparseMatrix(pvApiCtx, var_addr, &dims[1], &dims[0],
                                         &scilabSparse.nel, &scilabSparse.mnel, &scilabSparse.icol, &scilabSparse.R);
            }
            else
            {
                _SciErr = getSparseMatrixInList(pvApiCtx, parent, item_position, &dims[1], &dims[0], &scilabSparse.nel, NULL, NULL, NULL);
