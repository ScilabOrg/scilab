...
matvar_t *GetStructVariable(int iVar, const char *name, int matfile_version, char **fieldNames, int nbFields, int *parent, int item_position)
{
    ...
    MATIO_ERROR;                // YC on fait un test de type ??
    _SciErr = getVarDimension(pvApiCtx, var_addr, &nbRow, &nbFields);
    MATIO_ERROR;

    /* FIRST LIST ENTRY: fieldnames --> NO NEED TO BE READ */

    /* SECOND LIST ENTRY: dimensions */
    dimensionsVariable = GetMatlabVariable(iVar, "data",    /* Do not need to give the format because this variable is just temp */
                                           0, var_addr, 3);
    ...
    else
    {
        ...
        for (fieldIndex = 2; fieldIndex < nbFields; fieldIndex++)
        {
            _SciErr = getListInList(pvApiCtx, var_addr, fieldIndex + 1, &list_addr);
            MATIO_ERROR;
