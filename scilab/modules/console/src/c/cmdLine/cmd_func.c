/*
 ** cmd_func.c for reader 42sh in /home/mamode/share_blouno/zzledt/tc_prompt
 ** 
 ** Made by karim mamode
 ** Login   <mamode_k@epitech.net>
 ** 
 ** Started on  Fri May 20 17:25:34 2011 karim mamode
 ** Last update Sun May 22 16:29:49 2011 karim mamode
 */
 ...
 */
int			caseMetaKey(t_list_cmd **list_cmd)
{
  switch (getwchar())
    {
    case L'f': case L'F':
     return (next_word(list_cmd, META_F));
    case L'b': case L'B':
     return (previous_word(list_cmd, META_B));
    case L'[':
     return (caseDelOrArrowKey(list_cmd));
    case L'O':
     return (caseHomeOrEndKey(list_cmd));
    default:
     printf("\a");
     fflush(stdout);
     return (0);
