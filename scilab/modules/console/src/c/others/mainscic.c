...
*/
#define LINKER_ERROR_1 "Scilab startup function detected that the function proposed to the engine is the wrong one. Usually, it comes from a problem of linker in your distribution/OS.\n"
#define LINKER_ERROR_2 "If you do not know what it mean, please report a bug on http://bugzilla.scilab.org/. If you do, you probably know that you should change the link order in modules/Makefile.am\n"
    if (getScilabMode() != SCILAB_NWNI)
    {
        if (isItTheDisabledLib())
        {
            fprintf(stderr, LINKER_ERROR_1);
            fprintf(stderr, "Here, Scilab should have the defined libscijvm but instead, Scilab get libscijvm-disable.\n");
...
/* NWNI mode */
        ...
        {
            fprintf(stderr, LINKER_ERROR_1);
            fprintf(stderr, "Here, Scilab should have the defined libscijvm-disable but instead, Scilab get libscijvm.\n");
            fprintf(stderr, LINKER_ERROR_2);
            exit(1);
        }
    }
