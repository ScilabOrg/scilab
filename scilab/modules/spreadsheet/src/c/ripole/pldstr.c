...
char *PLD_strreplace_general(struct PLD_strreplace *replace_details)
{
    ...
    while ((segment_end != NULL) && (segment_ok == 0) && ((replace_details->preexist != NULL) || (replace_details->postexist != NULL)))
    {
        int pre_ok = 0;
        int post_ok = 0;
    ...
    while (segment_start != NULL)
    {
        ...
        if (size_required < 1)
        {
            LOGGER_log(_("%s:%d:PLD_strreplace_general: Allocated memory ran out while replacing '%s' with '%s'"), FL, replace_details->searchfor,
