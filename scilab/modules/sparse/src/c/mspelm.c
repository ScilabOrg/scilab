...
int C2F(intmsparse) (int *id)
{
    ...
    if (*istk(il) == 5)
    {
        ...
        *istk(ia) = 1;
        ...
        if (it == 0)
        {
            C2F(dspt) (&m, &n, stk(l), &nel, istk(ilr), istk(ia), stk(lat), istk(iat), istk(irc));
        }
        else
        {
            C2F(wspt) (&m, &n, stk(l), stk(l + nel), &nel, istk(ilr), istk(ia), stk(lat), stk(lat + nel), istk(iat), istk(irc));
        }
        *istk(il) = 7;
        I1 = ilr + n + 1 + nel;
        lr = I1 / 2 + 1;
        ...
        I1 = nel * (it + 1);
        C2F(unsfdcopy) (&I1, stk(lat), &c1, stk(lr), &c1);
...
int C2F(intmfull) (int *id)
{
    ...
    *istk(il) = 1;
    ...
    {
        wmspful(&m, &n, stk(ls), stk(ls + nel), &nel, istk(ils), stk(l), stk(l + m * n));
    }
