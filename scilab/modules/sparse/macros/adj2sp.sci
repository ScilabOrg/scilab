...
function A=adj2sp(varargin)
    ...
    if ( and(rhs <> [3 4]) ) then
        ...
        error ( msprintf(lstr,"adj2sp",3,4,rhs))
    end
    if ( and(lhs <> [0 1]) ) then
        lstr = gettext("%s: Wrong number of output arguments: %d to %d expected, but %d provided.\n")
        error ( msprintf(lstr,"adj2sp",0,1,lhs))
    end
    //
    // Get arguments
    xadj = varargin(1)
    ...
    if ( typeof(xadj) <> "constant" ) then
        ...
        error ( msprintf(lstr,"adj2sp",1,"constant",typeof(xadj)))
    end
    if ( typeof(iadj) <> "constant" ) then
        lstr = gettext("%s: Wrong type for input argument #%d: %s expected, but %s provided.\n")
        error ( msprintf(lstr,"adj2sp",2,"constant",typeof(iadj)))
    end
    if ( typeof(mn) <> "constant" ) then
        lstr = gettext("%s: Wrong type for input argument #%d: %s expected, but %s provided.\n")
        error ( msprintf(lstr,"adj2sp",3,"constant",typeof(mn)))
    end
    //
    ...
    if ( mn(1) < 0 ) then
        lstr = gettext("%s: Wrong content for input argument #%d: %s=%d, but a positive content is expected.\n")
    ...
    if ( int(mn(1)) <> mn(1) ) then
        lstr = gettext("%s: Wrong content for input argument #%d: %s=%s, but a floating point integer is expected.\n")
    ...
    if ( mn(1) <> max(iadj) ) then
        lstr = gettext("%s: Wrong content for input argument #%d: %s=%d, which does not match %s: %d.\n")
        error ( msprintf(lstr,"adj2sp",4,"mn(1)",mn(1),"max(iadj)",max(iadj)))
    end
    if ( mn(2) <> size(xadj,"*")-1 ) then
        lstr = gettext("%s: Wrong content for input argument #%d: %s=%d, which does not match size of %s: %d.\n")
