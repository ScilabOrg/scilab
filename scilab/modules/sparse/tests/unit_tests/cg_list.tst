...
[xcomputed, fail, err, iter, res]=cg(Alist,b,"pcg");
xexpected=ones(10,1);
...
[xcomputed, fail, err, iter, res]=cg(Alist,b,"cgs",maxIter=11);
...
// With the default 10 iterations, the algorithm performs well
[xcomputed, fail, err, iter, res]=cg(Alist,b,"bicg");
if norm(xcomputed-xexpected)>10**3*%eps then pause,end
if fail<>0 then pause,end
if iter<>10 then pause,end
if err > 10**3*%eps then pause,end
...
[xcomputed, fail, err, iter, res]=cg(Alist,b,"bicgstab");
