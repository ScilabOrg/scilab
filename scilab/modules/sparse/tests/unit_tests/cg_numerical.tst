...
[xcomputed, fail, err, iter, res]=cg(A,b,"pcg");
xexpected=ones(10,1);
...
tol=1.d-3;
[xcomputed, fail, err, iter, res]=cg(A,b,"pcg",tol);
if fail<>0 then pause,end
if iter>10 then pause,end
// With a tolerance of %eps but only 5 iterations allowed, the status is "fail"
tol=%eps;
maxIter = 5;
[xcomputed, fail, err, iter, res]=cg(A,b,"pcg",tol,maxIter);
...
[xcomputed, fail, err, iter, res]=cg(A,b,"cgs",maxIter=11);
...
tol=1.d-3;
[xcomputed, fail, err, iter, res]=cg(A,b,"cgs",tol);
if fail<>0 then pause,end
if iter>10 then pause,end
// With a tolerance of %eps but only 5 iterations allowed, the status is "fail"
tol=%eps;
maxIter = 5;
[xcomputed, fail, err, iter, res]=cg(A,b,"cgs",tol,maxIter);
if fail<>1 then pause,end
if iter<>maxIter then pause,end
...
// With the default 10 iterations, the algorithm performs well
[xcomputed, fail, err, iter, res]=cg(A,b,"bicg");
if norm(xcomputed-xexpected)>10**3*%eps then pause,end
if fail<>0 then pause,end
if iter<>10 then pause,end
if err > 10**3*%eps then pause,end
// With a tolerance of 1.e-3, there are 5 iterations and the status is "success"
tol=1.d-3;
[xcomputed, fail, err, iter, res]=cg(A,b,"bicg",tol);
if fail<>0 then pause,end
if iter>10 then pause,end
// With a tolerance of %eps but only 5 iterations allowed, the status is "fail"
tol=%eps;
maxIter = 5;
[xcomputed, fail, err, iter, res]=cg(A,b,"bicg",tol,maxIter);
if fail<>1 then pause,end
if iter<>maxIter then pause,end
...
[xcomputed, fail, err, iter, res]=cg(A,b,"bicgstab");
...
// With a tolerance of 1.e-3, there are 3 iterations and the status is "success"
tol=1.d-3;
[xcomputed, fail, err, iter, res]=cg(A,b,"bicgstab",tol);
if fail<>0 then pause,end
if iter>10 then pause,end
// With a tolerance of %eps but only 5 iterations allowed, the status is "fail"
tol=%eps;
maxIter = 5;
[xcomputed, fail, err, iter, res]=cg(A,b,"bicgstab",tol,maxIter);
if fail<>1 then pause,end
if iter<>maxIter then pause,end
