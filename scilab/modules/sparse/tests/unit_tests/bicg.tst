...
A = [10 1; 1 10];
b = [11; 11];
[xcomputed, flag, err, iter, res] = bicg(A, b);
xexpected = [1; 1];
...
b = [11; 11];
tol = 100*%eps;
[xcomputed, flag, err, iter, res] = bicg(A, b, tol);
xexpected = [1; 1];
assert_checkalmostequal ( xcomputed , xexpected , %eps);
...
tol = 100*%eps;
maxit = 10;
[xcomputed, flag, err, iter, res] = bicg(A, b, tol, maxit);
xexpected = [1;1];
assert_checkalmostequal ( xcomputed,xexpected,%eps);
...
maxit = 10;
M1 = [1,0;0,1];
[xcomputed, flag, err, iter, res] = bicg(A, b, tol, maxit, M1);
xexpected = [1;1];
assert_checkalmostequal ( xcomputed , xexpected , %eps);
...
M1=[1 0; 0 1];
M2=[1 0; 0 1];
[xcomputed, flag, err, iter, res] = bicg(A, b, tol, maxit, M1, M2);
xexpected = [1;1];
assert_checkalmostequal ( xcomputed , xexpected , %eps);
...
M2 = [1 0; 0 1];
x0 = [1; 1];
[xcomputed, flag, err, iter, res] = bicg(A, b, tol, maxit, M1, M2, x0);
xexpected = [1; 1];
assert_checkalmostequal ( xcomputed , xexpected , %eps);
...
b = [101; 11];
M = A;
[xcomputed, flag, err, iter, res] = bicg(A, b, tol, maxit,%M=M, maxIter=5, tol=%eps);
assert_checkequal ( flag , 0 );

// Test with non-positionnal input parameters so that good initialization generates 0 iterations
A = [100 1; 1 10];
b = [101; 11];
[xcomputed, flag, err, iter, res] = bicg(A, b, x0=[1;1]);
assert_checkequal ( iter , 0 );
...
b = ones(5, 1);
M = A;
[xcomputed, flag, err, iter, res] = bicg(A, b, %M=M, tol=%eps, maxIter=3);
expected = [
5
-120
630
...
M1 = C';
M2 = C;
[xcomputed, flag, err, iter, res] = bicg(A, b, %M=M1, %M2=M2, tol=%eps, maxIter=10);
expected = [
5
-120
630
...
b = ones(10, 1);
[x, fail, err, iter, res] = bicg(A, b, 1d-12, 15);
assert_checkequal ( flag ,  0 );
