...
types::Function::ReturnValue sci_luget(types::typed_list &in, int _iRetCount, types::typed_list &out)
{
    ...
    for (int i = 0; i < n1; i++)
    {
        ...
        {
            p->set(i, itemsRowP[j + tp] - 1, dblP[j + i * n1], true);
        }
        tp += itemsRowP[i];
    }
    tp = n1;
    for (int i = 0; i < n1; i++)
    {
        for (int j = 0; j < itemsRowL[i]; j++)
        {
            ...
            l->set(i, itemsRowL[j + tp] - 1, dblL[j + i * n1], true);
        }
        tp += itemsRowL[i];
    ...
    for (int i = 0; i < n1; i++)
    {
        ...
        {
            u->set(i, itemsRowU[j + tp] - 1, dblU[j + i * n1], true);
        }
        tp += itemsRowU[i];
    }
    tp = n1;
    for (int i = 0; i < n1; i++)
    {
        ...
        {
            q->set(i, itemsRowQ[j + tp] - 1, dblQ[j + i * n1], true);
        }
        tp += itemsRowQ[i];
    }
