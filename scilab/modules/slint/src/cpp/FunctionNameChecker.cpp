...
void FunctionNameChecker::preCheckNode(const ast::Exp & e, SLintContext & context, SLintResult & result)
{
    ...
    {
        result.report(context, e.getLocation(), L"Function name doesn\'t match the pattern: %s, %s", name, matcher.getPattern());
    }

    if (minimalLen > 0 && name.length() < minimalLen)
    {
        result.report(context, e.getLocation(), L"Function name length is lower than minimum: %d < %d", name.length(), minimalLen);
    }

    if (maximalLen > 0 && name.length() > maximalLen)
    {
        result.report(context, e.getLocation(), L"Function name length is greater than maximum: %d > %d", name.length(), minimalLen);
    }
}
...
const std::wstring FunctionNameChecker::getName() const
{
    return L"FunctionNameChecker";
}
}
