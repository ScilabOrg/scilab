    ...
    if (e.isCallExp())
    {
        ...
        if (ce.getName().isSimpleVar())
        {
            ...
            if (name == L"struct")
            {
                ...
                if (args.empty())
                {
                    result.report(context, e.getLocation(), L"All the fields in a struct should be defined at the struct definition.");
                }
                else if (args.size() % 2)
                {
                    result.report(context, e.getLocation(), L"Struct definition must contains an even number of arguments.");
                }
                else
                {
                    for (ast::exps_t::const_iterator i = args.begin(), end = args.end(); i != end; i += 2)
                    {
                        ...
                        if (arg.isStringExp())
                        {
                            ...
                            {
                                result.report(context, arg.getLocation(), L"Field name doesn\'t match the pattern: %s, %s", field, matcher.getPattern());
                            }

                            if (minimalLen > 0 && field.length() < minimalLen)
                            {
                                result.report(context, arg.getLocation(), L"Field name length is lower than minimum: %d < %d", field.length(), minimalLen);
                            }

                            if (maximalLen > 0 && field.length() > maximalLen)
                            {
                                result.report(context, arg.getLocation(), L"Field name length is greater than maximum: %d > %d", field.length(), minimalLen);
                            }
                            if (fields.find(field) != fields.end())
                            {
                                result.report(context, arg.getLocation(), L"Duplicated field definition: %s.", field);
                            }
                            else
                            {
                        ...
                        {
                            result.report(context, arg.getLocation(), L"Field should be defined by a constant string.");
                        }
                    }
                    ...
                    if (ce.getParent()->isAssignExp() && ce.getParent() != context.getLHSExp())
                    {
                        ...
                        {
                            result.report(context, e.getLocation(), L"Function struct has only one return argument.");
                        }
                    }
                    fields.clear();
                }
        ...
        if (fe.getHead()->isSimpleVar() && fe.getTail()->isSimpleVar())
        {
            ...
            if (i != vars.end())
            {
                ...
                if (sfields.find(fname) == sfields.end())
                {
                    ...
                    {
                        result.report(context, e.getLocation(), L"It should be better to define field %s at struct definition.", fname);
                    }
                    else
                    {
                        result.report(context, e.getLocation(), L"Read an undefined field: %s.", fname);
                    }
                }
            }
        }
...
const std::wstring StructChecker::getName() const
{
    return L"StructChecker";
}
}
