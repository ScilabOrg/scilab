...
void McCabeChecker::preCheckNode(const ast::Exp & e, SLintContext & context, SLintResult & result)
{
    ...
    if (cpx > max)
    {
        result.report(context, e.getLocation(), L"McCabe\'s complexity is %d and is greater than %d.", cpx, max);
    }
}
...
const std::wstring McCabeChecker::getName() const
{
    return L"McCabeChecker";
}
}
