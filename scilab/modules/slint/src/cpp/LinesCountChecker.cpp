...
void LinesCountChecker::preCheckNode(const ast::Exp & e, SLintContext & context, SLintResult & result)
{
    ...
    {
        const unsigned int count = context.countLines();
        if (count > max)
        {
            result.report(context, e.getLocation(), L"Maximum lines number exceeded: %d > %d.", count, max);
        }
    }
}
...
const std::wstring LinesCountChecker::getName() const
{
    return L"LinesCountChecker";
}
}
