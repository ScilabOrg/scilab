    ...
    if (left)
    {
        ...
        if (context.getPosition(left->getLocation(), pos))
        {
            if (pos.first < pos.second)
            {
                ...
                {
                    result.report(context, e.getLocation(), L"Operator %s should be surrounded by single spaces.", std::wstring(op, op + opSize));
                }
            }
        }
    }
...
const std::wstring SpacesAroundOpChecker::getName() const
{
    return L"SpacesAroundOpChecker";
}
