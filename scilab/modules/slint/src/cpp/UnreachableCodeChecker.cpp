...
void UnreachableCodeChecker::preCheckNode(const ast::Exp & e, SLintContext & context, SLintResult & result)
{
    ...
    for (const auto exp : exps)
    {
        if (returnExp)
        {
            if (!exp->isCommentExp())
            {
                result.report(context, returnExp->getLocation(), L"The code after the return statement is unreachable.");
                return;
            }
        }
...
const std::wstring UnreachableCodeChecker::getName() const
{
    return L"UnreachableCodeChecker";
}
}
