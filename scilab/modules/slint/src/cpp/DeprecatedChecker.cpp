...
void DeprecatedChecker::preCheckNode(const ast::Exp & e, SLintContext & context, SLintResult & result)
{
    ...
    if (ce.getName().isSimpleVar())
    {
        ...
        if (i != deprecated.end())
        {
            if (i->second.empty())
            {
                result.report(context, e.getLocation(), L"Deprecated function: %s.", name);
            }
            else
            {
                result.report(context, e.getLocation(), L"Deprecated function %s: use %s instead.", name, i->second);
            }
        }
        else
        {
...
const std::wstring DeprecatedChecker::getName() const
{
    return L"DeprecatedChecker";
}
...
void DeprecatedChecker::__Svd::preCheckNode(const ast::Exp & e, SLintContext & context, SLintResult & result)
{
    ...
    if (args.size() == 2)
    {
        ...
        {
            result.report(context, e.getLocation(), L"svd(..., 0) is deprecated.");
        }
    }
}
...
void DeprecatedChecker::__Mfprintf::preCheckNode(const ast::Exp & e, SLintContext & context, SLintResult & result)
{
    ...
    if (args.size() != 0)
    {
        ...
        {
            result.report(context, e.getLocation(), L"mfprintf(-1, ...) is deprecated.");
        }
    }
}
