...
void RedefinitionChecker::preCheckNode(const ast::Exp & e, SLintContext & context, SLintResult & result)
{
    ...
    if (context.isAssignedVar(var))
    {
        ...
        {
            result.report(context, e.getLocation(), L"Redefinition of Scilab\'s constant: %s.", name);
        }
        else
        {
            ...
            if (pIT)
            {
                if (pIT->isFunction())
                {
                    result.report(context, e.getLocation(), L"Redefinition of a built-in function: %s.", name);
                }
                else if (pIT->isMacroFile())
                {
                    result.report(context, e.getLocation(), L"Redefinition of a library function: %s.", name);
                }
                else if (pIT->isMacro())
                {
                    result.report(context, e.getLocation(), L"Redefinition of a macro: %s.", name);
                }
            }
        }
    }
...
const std::wstring RedefinitionChecker::getName() const
{
    return L"RedefinitionChecker";
}
}
