...
void FunctionArgsChecker::preCheckNode(const ast::Exp & e, SLintContext & context, SLintResult & result)
{
    ...
    if (!in_dup.empty())
    {
        ...
        result.report(context, e.getLocation(), L"Duplicated function arguments: %s.", wos.str());
    }
    ...
    if (!out_dup.empty())
    {
        ...
        result.report(context, e.getLocation(), L"Duplicated function output values: %s.", wos.str());
    }
    ...
    if (!inter.empty())
    {
        ...
        result.report(context, e.getLocation(), L"Function arguments used as output values: %s.", wos.str());
    }
}
...
const std::wstring FunctionArgsChecker::getName() const
{
    return L"FunctionArgsChecker";
}
}
