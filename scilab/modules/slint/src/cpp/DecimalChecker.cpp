    ...
    if (context.getPosition(de.getLocation(), out))
    {
        ...
        if (character != L'\0')
        {
            for (const wchar_t * c = code + out.first; c < code + out.second; ++c)
            {
                ...
                {
                    result.report(context, e.getLocation(), L"Bad decimal exponent: %s was expected and %s was found.", character, *c);
                }
            }
        }
        if (checkDot)
        {
            if (*(code + out.first) == L'.')
            {
                result.report(context, e.getLocation(), L"Decimal numbers musn\'t begin by a dot.");
            }
        }
    }
}
...
const std::wstring DecimalChecker::getName() const
{
    return L"DecimalChecker";
}
}
