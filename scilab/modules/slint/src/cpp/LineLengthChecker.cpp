...
void LineLengthChecker::preCheckNode(const ast::Exp & e, SLintContext & context, SLintResult & result)
{
    if (context.isFirstLevelFn())
    {
        ...
        if (!context.checkLineLength(max, out))
        {
            ...
            result.report(context, e.getLocation(), L"Maximum line length exceeded at lines: %s.", wos.str());
        }
    }
}
...
const std::wstring LineLengthChecker::getName() const
{
    return L"LineLengthChecker";
}
}
