...
void UselessArgChecker::postCheckNode(const ast::Exp & e, SLintContext & context, SLintResult & result)
{
    if (e.isFunctionDec())
    {
        ...
        {
            result.report(context, p.second, L"Function argument might be unused: %s.", p.first);
        }
        useless.pop();
    }
}

const std::wstring UselessArgChecker::getName() const
{
    return L"UselessArgChecker";
}
}
