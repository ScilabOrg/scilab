...
void VariableNameChecker::preCheckNode(const ast::Exp & e, SLintContext & context, SLintResult & result)
{
    ...
    if (!name.empty())
    {
        ...
        {
            result.report(context, e.getLocation(), L"Variable name doesn\'t match the pattern: %s, %s", name, matcher.getPattern());
        }

        if (minimalLen > 0 && name.length() < minimalLen)
        {
            result.report(context, e.getLocation(), L"Variable name length is lower than minimum: %d < %d", name.length(), minimalLen);
        }

        if (maximalLen > 0 && name.length() > maximalLen)
        {
            result.report(context, e.getLocation(), L"Variable name length is greater than maximum: %d > %d", name.length(), minimalLen);
        }
    }
}
...
const std::wstring VariableNameChecker::getName() const
{
    return L"VariableNameChecker";
}
}
