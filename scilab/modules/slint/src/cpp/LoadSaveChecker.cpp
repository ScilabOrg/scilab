    ...
    if (e.isCallExp())
    {
        ...
        if (ce.getName().isSimpleVar())
        {
            ...
            if (name == L"load" || name == L"save")
            {
                ...
                if (args.size() == 0)
                {
                    result.report(context, e.getLocation(), L"Function %s have at least one argument.", name);
                }
                else if (args.size() == 1)
                {
                    result.report(context, e.getLocation(), L"Function %s should be called variable name arguments.", name);
                }
                else
                {
                    for (ast::exps_t::const_iterator i = std::next(args.begin()), end = args.end(); i != end; ++i)
                    {
                        ...
                        if (!arg.isStringExp())
                        {
                            result.report(context, e.getLocation(), L"Function %s should be called with constant strings.", name);
                            break;
                        }
                    }
                }
...
const std::wstring LoadSaveChecker::getName() const
{
    return L"LoadSaveChecker";
}
}
