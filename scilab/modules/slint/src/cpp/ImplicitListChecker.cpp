...
void ImplicitListChecker::preCheckNode(const ast::Exp & e, SLintContext & context, SLintResult & result)
{
    ...
    if (le.getStart().isDoubleExp() && le.getStep().isDoubleExp() && le.getEnd().isDoubleExp())
    {
        ...
        {
            result.report(context, e.getLocation(), L"Invalid list, it contains NaN or Inf.");
        }
        else
        {
            if (start == end)
            {
                result.report(context, e.getLocation(), L"List has the same start and end.");
            }
            if (step == 0)
            {
                result.report(context, e.getLocation(), L"List has a null step.");
            }
            if ((start > end && step > 0) || (start < end && step < 0))
            {
                result.report(context, e.getLocation(), L"List is empty.");
            }
        }
    }
    else if (le.getStart().isListExp() || le.getStep().isListExp() || le.getEnd().isListExp())
    {
        result.report(context, e.getLocation(), L"Bad use of ':' operator.");
    }
}
...
const std::wstring ImplicitListChecker::getName() const
{
    return L"ImplicitListChecker";
}
}
