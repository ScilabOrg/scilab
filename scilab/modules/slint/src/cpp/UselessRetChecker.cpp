...
void UselessRetChecker::postCheckNode(const ast::Exp & e, SLintContext & context, SLintResult & result)
{
    if (e.isFunctionDec())
    {
        ...
        {
            result.report(context, p.second, L"Function returned value might be unused: %s.", p.first);
        }
        useless.pop();
    }
}

const std::wstring UselessRetChecker::getName() const
{
    return L"UselessRetChecker";
}
}
