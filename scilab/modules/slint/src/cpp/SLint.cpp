...
void SLint::setFiles(const std::vector<std::wstring> & files)
{
    ...
    {
        if (isdirW(file.c_str()))
        {
            collectInDirectory(file);
        }
        else if (hasSuffix(file, L".sci"))
        {
            scifiles.emplace_back(parseFile(file));
        }
    }
}
...
SciFilePtr SLint::parseFile(const std::wstring & filename)
{
    ...
    wchar_t * expandedPath = expandPathVariableW(const_cast<wchar_t *>(filename.c_str()));
    ...
    char * _path = wide_string_to_UTF8(expandedPath);
    std::wstring filepath(expandedPath);
    FREE(expandedPath);
    ...
    if (src.is_open())
    {
        ...
        {
            ThreadManagement::UnlockParser();
            return SciFilePtr(new SciFile(filepath, _buffer, parser.getTree()));
        }
    }
