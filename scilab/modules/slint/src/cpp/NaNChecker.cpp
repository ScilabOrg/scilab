    ...
    if (oper == ast::OpExp::lt || ast::OpExp::le || ast::OpExp::gt || ast::OpExp::ge || ast::OpExp::ne || ast::OpExp::eq)
    {
        ...
        if (hasNaN)
        {
            switch (oper)
            {
                ...
                case ast::OpExp::ge:
                {
                    result.report(context, e.getLocation(), L"Use %nan in a comparison expression will return false.");
                    break;
                }
                case ast::OpExp::ne:
                case ast::OpExp::eq:
                {
                    result.report(context, e.getLocation(), L"Use isnan() when comparing values with %nan.");
                    break;
                }
            }
        }
...
const std::wstring NaNChecker::getName() const
{
    return L"NaNChecker";
}
}
