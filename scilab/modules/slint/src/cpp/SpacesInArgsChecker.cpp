...
void SpacesInArgsChecker::preCheckNode(const ast::Exp & e, SLintContext & context, SLintResult & result)
{
    ...
    for (const auto arg : args)
    {
        ...
        if (context.getPosition(arg->getLocation(), pos))
        {
            ...
            if (next == L' ' || next == L'\t')
            {
                result.report(context, e.getLocation(), L"No space after function argument.");
            }
            if (pos.first >= 2)
            {
                ...
                if (first)
                {
                    if (prev == L' ' || prev == L'\t')
                    {
                        result.report(context, e.getLocation(), L"No space before first function argument.");
                    }
                }
                else
                {
                    ...
                    if (prev != L' ' || pprev != L',')
                    {
                        result.report(context, e.getLocation(), L"A function argument must be preceeded by a single space.");
                    }
                }
            }
            first = false;
...
const std::wstring SpacesInArgsChecker::getName() const
{
    return L"SpacesInArgsChecker";
}
}
