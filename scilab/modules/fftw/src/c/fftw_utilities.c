                      ...
                      unsigned flags, int isn)
{
  ...
  if ( (!(CheckGuruDims(&(Sci_Plan->gdim), gdim))) ||
       ...
       (Sci_Plan->plan_type != type)) { /* plan must be changed */
    ...
    switch (type)
      ...
      case R2R_PLAN:
        ...
        for (i=0;i<Sci_Plan->gdim.rank;i++) {
          kind[i]=FFTW_REDFT00;
          if  ((Sci_Plan->gdim.dims[i].n)%2 == 0) { /*even */
...
int check_1D_symmetry(double *Ar,double *Ai, int nA, int iA)
{
  ...
  double zero = 0.0;
 //Checking real part
  if ( nA % 2 == 0) { /* A length is even */
    for (i=1;i<nas2;i++) {
      if (Ar[iA*i] != Ar[iA*(nA-i)]) return 0;
    }
  }
  else { /* A length is odd */
    for (i=1;i<=nas2;i++) {
      if (Ar[iA*i] != Ar[iA*(nA-i)])  return 0;
    }
  }
...
/*--------------------------------------------------------------------------
 * "symmetrize" a vector A of length nA modifying the second half part of the vector
...
/*--------------------------------------------------------------------------
 * "symmetrize" a mA x nA array modifying the second half part of the columns
