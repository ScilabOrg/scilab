...
/*--------------------------------------------------------------------------*/
extern void C2F(dscal)(int *n,double *da,double *dx,int *incx);
...
int sci_fftw(char *fname,unsigned long fname_len)
{
  ...
  if (rhs<3) {/* fftw(A ,sign [,option])*/
    sci_fft_2args(fname, ndimsA, dimsA, Ar, Ai,isn,iopt);
  } 
  else if (rhs==3) {/* fftw(A ,sign ,sel [,option])*/
    sci_fft_3args(fname, ndimsA, dimsA, Ar, Ai,isn,iopt);
  }
  else if (rhs==4) {/* fftw(A ,sign ,dim,incr [option])*/
...
SciErr getVectorIntArg(void* _pvCtx, int _iVar, char *fname,int *pndims, int **pDim)
{
  ...
  if (isVarMatrixType(_pvCtx, piAddr) == 0) {
    addErrorMessage(&sciErr,API_ERROR_GET_INT,
                    _("%s: Wrong type for input argument #%d.\n"), fname, _iVar);
    return sciErr;
  }
...
int sci_fft_2args(char *fname, int ndimsA, int *dimsA, double *Ar,  double *Ai, int isn, int iopt)
{
  ...
  int  lA = 1, isrealA = (Ai==NULL), issymA = 1;
  ...
  if (iopt==0) { /* automatically selected algorithm*/
  ...
  if (isrealA) { /*A is real */
    if (issymA) { /*r2r =  isrealA &&  issymA*/
      ...
      if (isn== FFTW_BACKWARD) {  /*transform problem into a FORWARD fft*/
    ...
    if (issymA) { /*c2r =  ~isrealA &&  issymA*/
      sciErr = allocArrayOfDouble(pvApiCtx, Rhs + 1, ndimsA,dimsA, &ro);
      if(sciErr.iErr) {
        Scierror(999,_("%s: Cannot allocate more memory.\n"),fname);
        goto ERR;
      }
  ...
  p = GetFFTWPlan(type, &gdim, ri, ii, ro, io, cur_fftw_flags, isn);
  if (p == NULL) {
  ...
  if (scale != None) { 
    double ak = 1.0;
    for(i=0;i<ndims;i++) ak=ak*((double)(gdim.dims[i].n));
  ...
  case C2R_PLAN:
    break;
  case R2C_PLAN:
    if (issymA) { /*R2C has been used to solve an r2r problem*/
      if (complete_array(ro,NULL, gdim)==-1) {
        Scierror(999,_("%s: Cannot allocate more memory.\n"),fname);
        goto ERR;
      }
      ...
      if (isn== FFTW_BACKWARD) { /*conjugate result */
        double ak = -1.0;
...
int  sci_fft_3args(char *fname, int ndimsA, int *dimsA, double *Ar,  double *Ai, int isn, int iopt)
{
  ...
  if (gdim.howmany_rank>0) { /* it must be the case */
  ...
  /*
  printf("gdim.rank=%d,gdim.dims.n=[",gdim.rank);
  for (i=0;i<gdim.rank;i++) printf("%d ",gdim.dims[i].n);
  printf("],gdim.dims.is=[");
  for (i=0;i<gdim.rank;i++) printf("%d ",gdim.dims[i].is);
  printf("],gdim.dims.os=[");
  for (i=0;i<gdim.rank;i++) printf("%d ",gdim.dims[i].os);
  printf("]\n");
  printf("gdim.howmany_rank=%d,gdim.howmany_dims.n=[",gdim.howmany_rank);
  for (i=0;i<gdim.howmany_rank;i++) printf("%d ",gdim.howmany_dims[i].n);
  printf("],gdim.howmany_dims.is=[");
  for (i=0;i<gdim.howmany_rank;i++) printf("%d ",gdim.howmany_dims[i].is);
  printf("],gdim.howmany_dims.os=[");
  for (i=0;i<gdim.howmany_rank;i++) printf("%d ",gdim.howmany_dims[i].os);
  printf("]\n");
  */
  /******************** Select the  algorithm to be  used ****************************************/
  if (iopt==0) { /* automatically selected algorithm*/
