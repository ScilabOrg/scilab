...
static wchar_t* replace(wchar_t* _pwstInput, int* _piLines)
{
        ...
        if (_pwstInput[i] == '\\')
        {
            ...
            switch (_pwstInput[i + 1])
            {
                case 'n' :
                    ...
                    break;
                case 'r' :
                    if (iInputLen >= i + 3 && _pwstInput[i + 2] == '\\' && _pwstInput[i + 3] == 'n')
                    {
                        pwstFirstOutput[iPos++] = '\n';
                        (*_piLines)++;
                        i += 3;
...
wchar_t** scilab_sprintf(wchar_t* _pwstName, wchar_t* _pwstInput, typed_list &in, ArgumentPosition* _pArgs, int _iArgsCount, int* _piOutputRows, int* piNewLine)
{
    ...
    *piNewLine = 0;
    ...
    // replace \\n \\t... by \n \t...
    pwstFirstOutput = replace(_pwstInput, _piOutputRows);
