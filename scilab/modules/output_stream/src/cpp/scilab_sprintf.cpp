...
wchar_t** scilab_sprintf(wchar_t* _pwstName, wchar_t* _pwstInput, typed_list &in, ArgumentPosition* _pArgs, bool _bKeepEOL, int _iArgsCount, int* _piOutputRows, int* piNewLine)
{
    ...
    *piNewLine = 0;
    ...
    // split output string
    std::wstring wstr(pwstFirstOutput);
    ...
    while (pos != std::string::npos)
    {
        pwstOutput[i] = new wchar_t[pos + 1];
        wcsncpy(pwstOutput[i], wstr.c_str(), pos);
        pwstOutput[i][pos] = '\0';
    ...
    {
        pwstOutput[i] = os_wcsdup(wstr.c_str());
    }
