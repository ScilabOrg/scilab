...
static int AppendByFilenames(char *fname,
                             diary_filter filterMode, diary_prefix_time_format prefixMode, diary_prefix_time_filter prefixModeFilter, bool suspended);
...
static int sci_diary_one_rhs(char *fname)
{
    ...
    if (iType == sci_matrix)
    {
        ...
        // diary(0)
        // close all
        if ((IDs_size == 0) || ((IDs_size == 1) && ((int)IDs[0] == 0)))
        {
            diaryCloseAll();
            C2F(putlhsvar) ();
        }
...
static int sci_diary_two_rhs(char *fname)
{
    ...
    if (wcArgumentTwo)
    {
        ...
        else if ((wcscmp(wcArgumentTwo, DIARY_SECOND_ARG_PAUSE) == 0) || (wcscmp(wcArgumentTwo, DIARY_SECOND_ARG_OFF) == 0))
        {
            int iType = 0;
        ...
        else if ((wcscmp(wcArgumentTwo, DIARY_SECOND_ARG_RESUME) == 0) || (wcscmp(wcArgumentTwo, DIARY_SECOND_ARG_ON) == 0))
        {
            int iType = 0;
        ...
        else if (wcscmp(wcArgumentTwo, DIARY_SECOND_ARG_NEW) == 0)
        {
            ...
            {
                return NewByFilenames(fname, DIARY_FILTER_INPUT_AND_OUTPUT, PREFIX_TIME_FORMAT_UNIX_EPOCH, PREFIX_FILTER_NONE, false);
            }
            else
            {
                Scierror(999, _("%s: Wrong type for input argument #%d.\n"), fname, 1);
        ...
        else if (wcscmp(wcArgumentTwo, DIARY_SECOND_ARG_APPEND) == 0)
        {
            ...
            if (iType == sci_strings)
            {
                return AppendByFilenames(fname, DIARY_FILTER_INPUT_AND_OUTPUT, PREFIX_TIME_FORMAT_UNIX_EPOCH, PREFIX_FILTER_NONE, false);
            }
            else
            {
                Scierror(999, _("%s: Wrong type for input argument #%d.\n"), fname, 1);
