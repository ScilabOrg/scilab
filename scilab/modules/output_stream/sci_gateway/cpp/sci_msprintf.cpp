...
wchar_t** scilab_sprintf(wchar_t* _pwstName, wchar_t* _pwstInput, typed_list &in, ArgumentPosition* _pArgs, int _iArgsCount, int* _piOutputRows)
{
    ...
    //\n \n\r \r to string
    for(int i = 0 ; i < wcslen(pwstFirstOutput) ; i++)
    {
        if(pwstFirstOutput[i] == L'\r')
        {
            (*_piOutputRows)++;
    ...
    for(int i = 0 ; i < wcslen(pwstPtr) ; i++)
    {
        if(pwstPtr[i] == L'\r')
        {
            pwstOutput[iRows + 1] = (wchar_t*)MALLOC(sizeof(wchar_t) * wcslen(pwstPtr) - i);
            wcscpy(pwstPtr
            if(pwstPtr[i] == L'\n')
            {
                i++;
            }
            wcscpy(pwstOutput[iRows + 1], pwstPtr + i
        }
        else if(pwstPtr[i] == L'\n')
        {
        }
