...
extern int C2F(bicubicspline) (double *x, double *y, double *u, int *nx, int *ny, double *C, double *p, double *q, double *r, double *A_d,
                               double *A_sd, double *d, double *ll, double *qdu, double *u_temp, int *spline_type);
/*--------------------------------------------------------------------------*/
...
int intsplin2d(char *fname, unsigned long fname_len)
{
    ...
    {
        C2F(bicubicsubspline) (x, y, stk(lz), &nx, &ny, stk(lc), stk(lp), stk(lq), stk(lr), &spline_type);
    }
    ...
    else                        /*  spline */
    {
        ...
        C2F(bicubicspline) (x, y, stk(lz), &nx, &ny, stk(lc), stk(lp), stk(lq), stk(lr),
                            stk(lA_d), stk(lA_sd), stk(ld), stk(lll), stk(lqdu), stk(lutemp), &spline_type);
    }
