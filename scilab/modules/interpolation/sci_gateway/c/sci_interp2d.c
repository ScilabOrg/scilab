...
#include "Scierror.h"
/*--------------------------------------------------------------------------*/
extern int C2F(bicubicinterp) (double *x, double *y, double *C, int *nx, int *ny, double *x_eval, double *y_eval, double *z_eval, int *m,
                               int *outmode);
...
int intinterp2d(char *fname, unsigned long fname_len)
{
    ...
    if (mxp != myp || nxp != nyp || mx != 1 || my != 1 || nc != 1 || nx < 2 || ny < 2 || mc != 16 * (nx - 1) * (ny - 1))
    {
    ...
    if (Lhs == 1)
    {
         ...
         */
        C2F(bicubicinterp) (stk(lx), stk(ly), stk(lc), &nx, &ny, stk(lxp), stk(lyp), stk(lzp), &m, &outmode);
        LhsVar(1) = Rhs + 1;
    }
    else                        /* got also the derivatives */
    {
        ...
        if (Lhs <= 3)
        {
            C2F(bicubicinterpwithgrad) (stk(lx), stk(ly), stk(lc), &nx, &ny, stk(lxp), stk(lyp), stk(lzp), stk(ldzdxp), stk(ldzdyp), &m, &outmode);
            LhsVar(1) = Rhs + 1;
            LhsVar(2) = Rhs + 2;
            LhsVar(3) = Rhs + 3;
        }
        else                    /* got also 2d derivatives */
        {
            ...
            C2F(bicubicinterpwithgradandhes) (stk(lx), stk(ly), stk(lc), &nx, &ny, stk(lxp),
                                              ...
                                              stk(ld2zdx2p), stk(ld2zdxyp), stk(ld2zdy2p), &m, &outmode);
            LhsVar(1) = Rhs + 1;
            LhsVar(2) = Rhs + 2;
            LhsVar(3) = Rhs + 3;
            LhsVar(4) = Rhs + 4;
