...
// *** Perform operation. ***
    ...
    {
        double ret = C2F(db3val)(pDblXYZ[0]->get() + i, pDblXYZ[1]->get() + i, pDblXYZ[2]->get() + i, &der[0], &der[1], &der[2], pDblX->get(), pDblY->get(), pDblZ->get(), &sizeOfX, &sizeOfY, &sizeOfZ, &order[0], &order[1], &order[2], pDblCoef->get(), work);
        pDblDfp->set(i, ret);
    }
