...
element_list
  ...
  | import_clause SC annotation_option
      { [node (ImportClause ($1, $3))] }
  | extends_clause SC annotation_option
      { [node (ExtendsClause ($1, $3))] }
  | redeclare_option final_option dynamic_scope_option
    element_definition SC annotation_option
      { [node (ElementDefinition ($1, $2, $3, $4, $6))] }
  | element_list import_clause SC annotation_option
      { rhs_nodes 2 3 (ImportClause ($2, $4)) :: $1 }
  | element_list extends_clause SC annotation_option
...
equations
  : equation comment SC annotation_option
      { [node (Equation (rhs_nodes 1 3 $1, $2, $4))] }
  | equations equation comment SC annotation_option
...
algorithms
  : algorithm comment SC annotation_option
      { [node (Algorithm (rhs_nodes 1 3 $1, $2, $4))] }
  | algorithms algorithm comment SC annotation_option
...
arithmetic_expression
  ...
  | PLUS term PLUS arithmetic_expression2
      { let plus = rhs_nodes 1 2 (UnaryOperation (rhs_nodes 1 1 UnaryPlus, $2)) in
        node (BinaryOperation (rhs_nodes 3 3 Plus, plus, $4)) }
  | PLUS term MINUS arithmetic_expression2
      { let plus = rhs_nodes 1 2 (UnaryOperation (rhs_nodes 1 1 UnaryPlus, $2)) in
        node (BinaryOperation (rhs_nodes 3 3 Minus, plus, $4)) }
  | MINUS term
      { node (UnaryOperation (rhs_nodes 1 1 UnaryMinus, $2)) }
  | MINUS term PLUS arithmetic_expression2
      { let minus = rhs_nodes 1 2 (UnaryOperation (rhs_nodes 1 1 UnaryMinus, $2)) in
        node (BinaryOperation (rhs_nodes 3 3 Plus, minus, $4)) }
  | MINUS term MINUS arithmetic_expression2
      { let minus = rhs_nodes 1 2 (UnaryOperation (rhs_nodes 1 1 UnaryMinus, $2)) in
        node (BinaryOperation (rhs_nodes 3 3 Minus, minus, $4)) }
  ;

arithmetic_expression2
  : term
      { $1 }
  | arithmetic_expression2 PLUS term
      { node (BinaryOperation (rhs_nodes 2 2 Plus, $1, $3)) }
  | arithmetic_expression2 MINUS term
