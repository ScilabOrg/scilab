...
and expression_desc =
  ...
  | DynamicIdentifier of int (* number of nested classes to skip *) *
      string (* name to be looked for at instanciation time *)
  | False
...
and resolve_composition ctx ctx' comp = match comp.Syntax.nature with
  | Syntax.Composition (elts, other_elts, extern) ->
      {
        class_annotations = resolve_class_annotations ctx' elts other_elts;
...
and resolve_external_call ctx extern =
  let resolve_language ext = function
    | "C" -> C
    | "FORTRAN" -> FORTRAN
    | lang -> raise (CompilError
        {err_msg =
            [ "_ModelicaUnsupported";
              "_ExtCallToUnsupportedLang";
              lang ];
         err_info = [];
         err_ctx = {ctx with location = ext.Syntax.info}}) (*error*)
  and resolve_lhs = function
    | None -> None
    | Some expr -> Some (resolve_lhs_expression ctx expr) in
  let external_procedure_call lang extern_call =
    match extern_call.Syntax.nature with
      | Syntax.ExternalFunctionCall (lhs, id, exprs) ->
          let lang' = resolve_language extern_call lang
          and rhs' = resolve_lhs lhs
          and exprs' = List.map (resolve_expression ctx) exprs in
          ExternalProcedureCall (lang', rhs', id, exprs') in
  let resolve_external_call' extern' = match extern'.Syntax.nature with
    ...
    | Syntax.External (Some lang, Some extern_call, _, _) ->
        { nature = external_procedure_call lang extern_call;
          info = extern' }
...
and resolve_modification_equation ctx cpnt_type expr =
  ...
  match expr'.info.type_description with
    | Types.ComponentElement cpnt_type' ->
        begin match Types.compare_component_types cpnt_type cpnt_type' with
          | Types.SameType | Types.CoercingType -> expr'
          | Types.Supertype | Types.Subtype | Types.NotRelated ->
  ...
  match expr'.info.type_description with
    | Types.ComponentElement cpnt_type' ->
        begin
          ...
          | Types.SameType | Types.CoercingType | Types.Supertype
          ...
          | Types.SameType | Types.CoercingType | Types.Supertype ->
              ...
              raise (CompilError
                {err_msg = ["_VariabilityConflictsInAssignExpr"];
                 err_info = [("_ExprKind", "A := B");
                             ("_VariabilityOfA", var);
                             ("_VariabilityOfB", var')];
                 err_ctx = ctx}) (*error*)
          | Types.Subtype | Types.NotRelated ->
              let type_A = Types.string_of_component_type cpnt_type
              and type_B = Types.string_of_component_type cpnt_type' in
              raise (CompilError
                {err_msg = ["_TypeConflictsInAssignExpr"];
                 err_info = [("_ExprKind", "A := B");
                             ("_TypeOfA", type_A);
                             ("_TypeOfB", type_B)];
                 err_ctx = ctx}) (*error*)
...
and resolve_equal ctx equ expres expres' =
  let resolve_equal' cpnt_type expr cpnt_type' expr' =
    ...
    | _, _ when expression_of_variable expres ->
        equations ctx equ resolved_equation cpnt_type expr cpnt_type' expr'
    | _, _ ->
        raise (CompilError
          {err_msg = ["_LHSOfDiscreteEquMustBeAVar"];
           err_info = [];
           err_ctx = {ctx with location = expres.Syntax.info}}) (*error*) in
...
and equations ctx equ resolved_equation cpnt_type expr cpnt_type' expr' =
  let equivalent_types predef predef' =
    ...
    | (Types.SameType | Types.CoercingType), _
    | _, (Types.SameType | Types.CoercingType) -> true
    | _, _ -> false in
  ...
  and record_equations subs cl_type expr cl_type' expr' =
    ...
    let record_equations' expr expr' =
      ...
      let record_equation (id, elt_type) =
        let elt_type' = List.assoc id named_elts' in
    ...
    let matchable_types cpnt_type cpnt_type' =
      ...
      let rec matchable_types' cl_spec cl_spec' = match cl_spec, cl_spec' with
        ...
        | _, _ ->
            ...
            (type_compare = Types.CoercingType) in
            ...
            {err_msg = ["_ClassElemFoundInExpr"];
             err_info =
               [("_TypeOfThenBranche", "_ClassElement");
                ("_TypeOfElseBranche", "_ClassElement")];
             err_ctx = ctx}) (*error*) in
    match Types.compare_element_natures elt_nat elt_nat',
          Types.compare_element_natures elt_nat' elt_nat with
      | Types.SameType, _ | Types.CoercingType, _ |
        _, Types.CoercingType -> expres
      | _, _ ->
          display_error elt_nat elt_nat' in
  let alts = List.map resolve_alternative alts in
...
and resolve_integer ctx expr s =
  let nat = Integer (Int32.of_string s) in
...
and resolve_no_event ctx expr expr' =
  ...
  | Types.ComponentElement cpnt_type ->
      ...
      and flow = evaluate cpnt_type.Types.flow
      and var = Types.Continuous
      and inout = evaluate cpnt_type.Types.causality
      and cl_spec = evaluate cpnt_type.Types.base_class in
      let cpnt_type =
        component_element (lazy flow) (lazy var) (lazy inout) (lazy cl_spec) in
...
and resolve_range ctx expr start step stop =
  let range_dim start' step' stop' =
    match start'.nature, step'.nature, stop'.nature with
      ...
      | Integer i, Integer p, Integer j ->
          Int32.div (Int32.succ (Int32.sub j i)) p
      | _, _, _ ->
          raise (CompilError
  ...
  let integer_range var start' step' stop' =
    let nat = Range (start', step', stop')
    and dim = range_dim start' step' stop' in
    let elt_nat = Types.integer_array_type var (Types.ConstantDimension dim) in
...
and resolve_vector_elements ctx expr exprs =
  let component_type_of_expression expr expr' =
    match expr'.info.type_description with
      | Types.ComponentElement cpnt_type -> cpnt_type
      | Types.ClassElement _ | Types.ComponentTypeElement _ |
        Types.PredefinedTypeElement _ ->
  ...
  let type_of_elements cpnt_types =
    ...
    and update cl_spec cl_spec' = match cl_spec, cl_spec' with
      | Types.PredefinedType { Types.base_type = bt },
        Types.PredefinedType { Types.base_type = bt' } when bt <> bt' ->
          raise (CompilError
            {err_msg = ["_TypeConflictsInVectorExpr"];
             err_info =
               [("_MismatchingTypes",
                 ...
                 Types.string_of_class_specifier cl_spec')];
             err_ctx = ctx})
      | Types.PredefinedType _, Types.PredefinedType _ -> cl_spec
      | _ -> raise (CompilError
          {err_msg = ["_NotYetImplemented"; "_NonPredefTypeVectorExpr"];
           err_info =
             [("_ExpectedType", "_PredefType");
              ("_TypeFound", Types.string_of_class_specifier cl_spec)];
           err_ctx = ctx}) in
  ...
  let exprs' = List.map (resolve_expression ctx) exprs in
  let cpnt_types = List.map2 component_type_of_expression exprs exprs' in
  let var = List.fold_left max_variability Types.Constant cpnt_types in
...
and resolve_and ctx expr arg arg' =
  let resolve_and' cpnt_type cpnt_type' =
    ...
    let elt_nat =
      let cpnt_type =
        component_element (lazy false) (lazy var) (lazy inout) (lazy cl_spec) in
...
and element_field_type_nature ctx flow var inout cl_spec id =
  ...
  let find_predefined_local_identifier predef id =
    match predef.Types.base_type with
      | Types.BooleanType when id = "start" -> Types.boolean_type Types.Constant
      | Types.IntegerType when id = "start" || id = "nominal" ->
          Types.integer_type Types.Constant
      | Types.RealType when id = "start" || id = "nominal" ->
          Types.real_type Types.Continuous
      | Types.StringType when id = "start" -> Types.string_type Types.Constant
      | Types.EnumerationType enum_lits when id = "start" ->
          Types.enumeration_type Types.Constant enum_lits
