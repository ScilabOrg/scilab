...
let integer_array_type var dim =
  let cl_spec =
    ArrayType (dim, PredefinedType { base_type = IntegerType; attributes = [] })
  in
  let cpnt_type =
    {
      flow = lazy false;
...
let rec compare_class_types ct ct' = match Lazy.force ct.kind, Lazy.force ct'.kind with
  ...
  | ExpandableConnector, ExpandableConnector -> compare_expandable_connectors ct ct'
...
and compare_classes ct ct' =
  let rec compare_classes' type_cmp named_elts named_elts' = match named_elts' with
    | [] -> type_cmp
    | (s', _) :: _ when not (List.mem_assoc s' named_elts) -> NotRelated
    | (s', elt_type') :: named_elts' ->
        begin
          let type_cmp' =
            compare_elements (Lazy.force (List.assoc s' named_elts)) (Lazy.force elt_type') in
...
and compare_specifiers cs cs' = match cs, cs' with
  ...
  | ArrayType (dim, cs), ArrayType (dim', cs') when dim = dim' ->
...
and compare_predefined_types pt pt' = match pt.base_type, pt'.base_type with
  ...
  | RealType, IntegerType -> CoercingType (* use of type coercion *)
