...
function [g_r,g_t,go_on]=position()
  ...
  while ~fin
    sleep(1)
    if findobj('tag', 'sphereposition')==[] then
      go_on = %F;
      g_r = get(slider1,'Value');
      g_t = get(slider2,'Value'); 
      return;
...
function [g_V,g_Vdir,go_on]=speed()
  ...
  while ~fin
    sleep(1)
    if findobj('tag', 'spherespeed')==[] then
      go_on = %F;  
      g_V = get(slider1,'Value');
      g_Vdir = get(slider2,'Value');
      return;
...
function change_visibility()
  ...
  if eff then
    drawlater();
    curAxe=gca();
    e2=curAxe.children(1);
    e1=curAxe.children(2);
...
function draw_traj_interface(Y)
  ...
  while ~fin
    ...
    if findobj('tag', 'sphereviewtraj')==[] then //the frame is closed by user
      go_on = %F;
      return;
    end
    ...
    if ~m_start then
      ...
      if (step<num) then
        if (step == 0) then
            ...
            change_visibility();drawnow();show_pixmap();
        else
            drawlater();
