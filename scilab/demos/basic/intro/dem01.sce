...
s=sin(0:0.1:5*%pi);
ss=fft(s(1:128),-1);        //FFT   //simple plot

xbasc(); plot2d3("enn",1,abs(ss)');xtitle('Fast Fourier Transform');current_axe=gca(); current_axe.title.font_size=3;
...
xbasc();plot2d(instants',y');xtitle('Second order system analysis step response');current_axe=gca(); current_axe.title.font_size=3;
//             Delayed step response
deff('[in]=u(t)','if t<3 then in=0;else in=1;end');
y1=csim(u,instants,sl);plot2d(instants',y1');xtitle('Second order system analysis delayed step response');current_axe=gca(); current_axe.title.font_size=3;
clear u;
//             Impulse response;
yi=csim('imp',instants,sl);
xbasc();plot2d(instants',yi');xtitle('Second order system analysis impulse response');current_axe=gca(); current_axe.title.font_size=3;
yi1=csim('step',instants,s*sl);
plot2d(instants',yi1');xtitle('Second order system analysis impulse response');current_axe=gca(); current_axe.title.font_size=3;
...
u=ones(instants);
yyy=flts(u,sld);
xbasc(); plot(instants,yyy);xtitle('Second order system analysis discretized step response');current_axe=gca(); current_axe.title.font_size=3;
clear u;
...
yy=flts(u,sld);
xbasc(); plot(instants,yy);xtitle('Second order system analysis discretized impulse response');current_axe=gca(); current_axe.title.font_size=3;
clear u;
