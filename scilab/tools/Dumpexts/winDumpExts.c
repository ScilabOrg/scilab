...
void DumpSymbolTable(PIMAGE_SYMBOL pSymbolTable, FILE * fout, unsigned cSymbols)
{
    ...
    for (i = 0; i < cSymbols; i++)
    {
        ...
        fprintf(fout, " sect:%s aux:%X type:%02X st:%s\n",
                sectionName, pSymbolTable->NumberOfAuxSymbols, pSymbolTable->Type, GetSZStorageClass(pSymbolTable->StorageClass));
        /*
...
void DumpObjFile(PIMAGE_FILE_HEADER pImageFileHeader, FILE * fout, int full)
{
    ...
    PCOFFSymbolTable = (PIMAGE_SYMBOL) LongToPtr(((DWORD) PtrToLong(pImageFileHeader) + pImageFileHeader->PointerToSymbolTable));
