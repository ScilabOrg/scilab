...
createProfileDoubleAlloc() {
    ...
    NBCOL="$3"
    VAR="$4"
    OUT="/* Create the matrix as return of the function */\nsciErr = allocMatrixOfDouble(pvApiCtx, nbInputArgument(pvApiCtx) + $POSITION, $NBROW, $NBCOL, $VAR);\nfree($VAR); // Data have been copied into Scilab memory\nif (sciErr.iErr)\n{\n    free($VAR); // Make sure everything is cleanup in case of error\n    printError(\&sciErr, 0);\n    return 0;}"
}
...
for (( i=1; i <= 3; i++ ));do
#ORIG="if \(is(.*)Type\(pvApiCtx, TODO_ADDRESS_OF_$i\)\).*{(.*)sciErr = getVarAddressFromPosition\(pvApiCtx, $i, (.*)\);(.*)MYMARK$i"
#ORIG="if \(is(.*)Type\(pvApiCtx, TODO_ADDRESS_OF_$i\)\).*{(.*)sciErr = getVarAddressFromPosition\(pvApiCtx, $i, .*\);(.*)MYMARK$i"
#echo $ORIG
#perl -0777 -pe "s/$ORIG/\/* 2 *\/\2
#sciErr = getVarAddressFromPosition\(pvApiCtx, $i, \/* 3 *\/ \3);
#\/* 4 *\/\4\n
#if \( \/* 1 *\/is\1Type\(pvApiCtx,  \/* 3 *\/\3\)) {/sg"  $FILES > $FILES.TMP
