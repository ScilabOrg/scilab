...
ORIG="if \(is(.*)Type\(pvApiCtx, TODO_ADDRESS_OF_1\)\).*{(.*)sciErr = getVarAddressFromPosition\(pvApiCtx, 1, (.*)\);(.*)MYMARK"
perl -0777 -pe "s/$ORIG/\2sciErr = getVarAddressFromPosition\(pvApiCtx, 1, \3\);\4\nif \(is\1Type\(pvApiCtx, &piAddr\)) {/sg"  $FILES > $FILES.TMP
