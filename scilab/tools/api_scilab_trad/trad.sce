...
function status = trad(inputFile, backupFile)
    ...
    cmd = "astyle --pad-header -n --pad-oper --indent-col1-comments --indent-switches --style=linux --indent=spaces=4 -A1 -q ";
    ...
    if index == [] then
        ...
        if index <> [] then
            result = [result(1 : (index(1)+1)); "    SciErr sciErr;"; result((index(1)+2):$)];
        else
...
function result = sed(str, findExp, replaceExp)
    ...
    while index <> []
        idx = index(1);
        [start, end, match, captured] = regexp(result(idx), findExp);
...
function result = commonDouble(str, findExp, replaceExp)
    ...
    while index <> []
        ...
        if size(captured, "c") >= 5 then
            ...
            if captured(5) <> "" then
                result = sed(result, "/[i]?[^c]stk\(" + captured(5) + "(.*)\)/", "(" + captured(5) + "\1)");
            end
        end
...
function result = commonDoubleComplex(str, findExp, replaceExp)
    ...
    while index <> []
        ...
        if size(captured, "c") >= 6 then
            ...
            if captured(6) <> "" then
                result = sed(result, "/[i]?[^c]stk\(" + captured(6) + "(.*)\)/", "(" + captured(6) + "\1)");
            end
        end
        ...
        if size(captured, "c") >= 7 then
            ...
            if captured(7) <> "" then
                result = sed(result, "/[i]?[^c]stk\(" + captured(7) + "(.*)\)/", "(" + captured(7) + "\1)");
            end
        end
