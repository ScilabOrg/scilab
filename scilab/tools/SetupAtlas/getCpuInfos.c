...
wchar_t *getCpuVendor(void)
{
    ...
    if (result == ERROR_SUCCESS)
    {
        ...
        if (RegQueryValueExW(key, KeyVendorIdentifier, 0, &Type, (LPBYTE) LineResult, &length) == ERROR_SUCCESS)
        {
            if (Type == REG_SZ)
            {
...
wchar_t *getProcessorNameString(void)
{
    ...
    if (result == ERROR_SUCCESS)
    {
        ...
        if (RegQueryValueExW(key, KeyProcessorNameString, 0, &Type, (LPBYTE) LineResult, &length) == ERROR_SUCCESS)
        {
            if (Type == REG_SZ)
            {
...
wchar_t *getCpuIdentifier(void)
{
    ...
    if (result == ERROR_SUCCESS)
    {
        ...
        if (RegQueryValueExW(key, KeyCpuIdentifier, 0, &Type, (LPBYTE) LineResult, &length) == ERROR_SUCCESS)
        {
            if (Type == REG_SZ)
            {
...
int getCpuFamily(void)
{
    ...
    if (Identifier)
    {
        ...
        swscanf(Identifier, L"%s %s %s %s %s %s %s", wArch, wFamily, wFamilyValue, wModel, wModelValue, wStepping, wSteppingValue);
