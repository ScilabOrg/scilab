...
wchar_t *buildDestinationFilename(wchar_t * execpath)
{
    ...
    if (execpath)
    {
        int len = (int)(wcslen(FORMAT_DESTINATION_FILENAME) + wcslen(execpath) + wcslen(BLAS_FILENAME_DEST) + 1);

        DestinationFilename = (wchar_t *) calloc(len, sizeof(wchar_t));
        if (DestinationFilename)
        {
            wsprintfW(DestinationFilename, FORMAT_DESTINATION_FILENAME, execpath, BLAS_FILENAME_DEST);
        }
    }
...
wchar_t *buildSourceFilename(wchar_t * execpath, wchar_t * blasDllName)
{
    ...
    {
        int len = (int)(wcslen(FORMAT_SOURCE_FILENAME) + wcslen(execpath) + wcslen(ATLAS_DIRECTORY) + wcslen(blasDllName) + 1);

        SourceFilename = (wchar_t *) calloc(len, sizeof(wchar_t));
        if (SourceFilename)
        {
            wsprintfW(SourceFilename, FORMAT_SOURCE_FILENAME, execpath, ATLAS_DIRECTORY, blasDllName);
        }
    }
