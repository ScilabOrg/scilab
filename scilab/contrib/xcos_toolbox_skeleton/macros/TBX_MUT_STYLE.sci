...
function [x,y,typ]=TBX_MUT_STYLE(job,arg1,arg2)
  ...
  select job
   ...
   case 'set' then
    ...
    // no parameters yet, just change the style of the blocks and of the ports
    hexa = [string(1:9) "A" "B" "C" "D" "E" "F"];
    graphics.style = "BIGSOM_f;fillColor=#" + strcat(hexa(15 * rand(1,6) + 1));

    port_styles = ["rectangle" "ellipse" "doubleEllipse" "rhombus" "arrow" ..
                   "cylinder" "connector" "connector"];
    
    graphics.in_style = "ExplicitInputPort;shape=" + port_styles(size(port_styles, '*') * rand(model.in) + ones(model.in));
    graphics.out_style = "ExplicitOutputPort;shape=" + port_styles(size(port_styles, '*') * rand(model.in) + ones(model.in));
